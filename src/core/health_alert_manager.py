...
class HealthAlertManager:
    ...
    def create_alert(self, agent_id: str, severity: str, message: str, 
                    metric_type: str, current_value: float, threshold: float) -> HealthAlert:
    ...
    def clear_resolved_alerts(self):
        ...
        resolved_alerts = [alert_id for alert_id, alert in self.alerts.items() if alert.resolved]
        for alert_id in resolved_alerts:
            del self.alerts[alert_id]
        logger.info(f"Cleared {len(resolved_alerts)} resolved alerts")
    
    def get_alert_count(self, severity: Optional[str] = None, 
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            self.create_alert("agent_1", "critical", "Critical alert", "memory_usage", 95.0, 90.0)
            self.create_alert("agent_2", "warning", "Warning alert", "cpu_usage", 90.0, 85.0)
...
def main():
    ...
    if args.test:
        ...
        manager.create_alert("agent_001", "warning", "High response time", "response_time", 1500.0, 1000.0)
        manager.create_alert("agent_001", "critical", "Memory usage critical", "memory_usage", 95.0, 90.0)
        manager.create_alert("agent_002", "warning", "CPU usage high", "cpu_usage", 90.0, 85.0)
        ...
        for alert in manager.alerts.values():
            ...
            print(f"  {alert.alert_id}: {alert.severity.upper()} - {alert.message} ({status}, {ack_status})")
