...
def test_performance_monitor_basic():
    ...
    try:
        from services.performance_monitor import PerformanceMonitor, MetricData, MetricType
...
async def test_dashboard_backend():
    ...
    try:
        ...
        # Create components
        monitor = PerformanceMonitor()
        dashboard = DashboardBackend(monitor, host="localhost", port=0)  # Use port 0 for testing
...
def test_dashboard_frontend():
    ...
    try:
        from services.dashboard_frontend import (
            DashboardFrontend, DashboardWidget, ChartType, DashboardLayout
        )
...
async def test_alerting_system():
    ...
    try:
        from services.performance_alerting import (
            AlertingSystem, AlertRule, EmailAlertChannel
        )
        from services.performance_monitor import AlertCondition, AlertSeverity, PerformanceAlert
    ...
    try:
        ...
        if metric_names:
            ...
            if series and series.data_points:
                print(f"✅ Dashboard can access metrics: {len(series.data_points)} data points")
            else:
