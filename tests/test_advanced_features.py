    ...
    for component_id, health_data in all_health.items():
        if health_data:
            ...
            for metric_name, metric_data in health_data["metrics"].items():
                print(
                    f"     {metric_name}: {metric_data['value']:.2f}{metric_data['unit']} ({metric_data['status']})"
                )
...
def test_performance_profiling():
    ...
    for component_id, component_data in perf_stats.get("components", {}).items():
        ...
        for metric_name, metric_data in component_data.get("metrics", {}).items():
            if "current" in metric_data:
                print(
                    f"       {metric_name}: {metric_data['current']:.2f} (avg: {metric_data['average']:.2f})"
                )
    ...
    if bottlenecks:
        ...
        for bottleneck in bottlenecks[:3]:  # Show first 3
            print(
                f"     {bottleneck['component_id']}: {bottleneck['metric_name']} = {bottleneck['value']:.2f}"
            )
    ...
    if trends:
        ...
        for trend in trends[-3:]:  # Show last 3
            print(
                f"     {time.strftime('%H:%M', time.localtime(trend['timestamp']))}: "
...
def test_integration():
    ...
    class IntegratedService:
        ...
        def initialize(self):
            ...
            self.connection_pool = pool_manager.create_pool(
                f"{self.service_id}_pool", pool_config
            )
    ...
    for pool_name, stats in pool_stats.items():
        print(
            f"     {pool_name}: {stats['total_connections']} connections, "
    ...
    if "response_time" in perf_summary:
        print(
            f"     Response time: {perf_summary['response_time']['average']:.2f}ms average"
        )
