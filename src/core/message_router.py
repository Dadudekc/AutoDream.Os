...
class MessageRouter:
        ...
        try:
            ...
            for recipient_id in target_agents:
                if recipient_id != sender_id:  # Don't send to self
                    message_id = self.send_message(sender_id, recipient_id, message_type, content, priority)
                    if message_id:
    ...
    def get_pending_messages(self, recipient_id: str) -> List[Message]:
        ...
        for message in self.message_history.values():
            if (message.recipient_id == recipient_id and 
                message.status == MessageStatus.PENDING and
                not self._is_message_expired(message)):
                pending_messages.append(message)
        
        # Sort by priority and timestamp
        pending_messages.sort(key=lambda m: (self._get_priority_value(m.priority), m.timestamp))
                ...
                if not self.message_queue.empty():
                    ...
                    if success:
                        ...
                        if message.delivery_attempts < message.max_attempts:
                            ...
                            retry_priority = priority + 1000  # Lower priority for retries
                            self.message_queue.put((retry_priority, message))
                            self.logger.warning(f"Message {message.message_id} queued for retry")
                        else:
                            message.status = MessageStatus.FAILED
                            self._update_message_status(message)
                            self.logger.error(f"Message {message.message_id} delivery failed after {message.max_attempts} attempts")
    ...
    def _attempt_delivery(self, message: Message) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Delivery attempt failed for message {message.message_id}: {e}")
            return False
    ...
    def _default_delivery(self, message: Message) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Message {message.message_id} delivered to {message.recipient_id}")
            return True
    ...
    def get_routing_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            pending_messages = len([m for m in self.message_history.values() if m.status == MessageStatus.PENDING])
            delivered_messages = len([m for m in self.message_history.values() if m.status == MessageStatus.DELIVERED])
            failed_messages = len([m for m in self.message_history.values() if m.status == MessageStatus.FAILED])
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            message_id = self.send_message("Agent-1", "Agent-2", MessageType.STATUS_UPDATE, 
                                        {"status": "online"}, MessagePriority.NORMAL)
            if not message_id:
                return False
            
            # Test broadcast
            broadcast_ids = self.broadcast_message("Agent-1", MessageType.COORDINATION, 
                                                {"action": "sync"}, MessagePriority.NORMAL)
...
def run_smoke_test():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test message sending
            message_id = router.send_message("Agent-1", "Agent-2", MessageType.STATUS_UPDATE, 
                                          {"status": "online"}, MessagePriority.NORMAL)
            assert message_id
            
            # Test broadcast
            broadcast_ids = router.broadcast_message("Agent-1", MessageType.COORDINATION, 
                                                  {"action": "sync"}, MessagePriority.NORMAL)
...
def main():
    ...
    parser.add_argument("--send", nargs=4, metavar=("SENDER", "RECIPIENT", "TYPE", "CONTENT"), help="Send message")
    parser.add_argument("--broadcast", nargs=3, metavar=("SENDER", "TYPE", "CONTENT"), help="Broadcast message")
    ...
    if args.send:
        ...
        try:
            ...
            message_id = router.send_message(sender, recipient, message_type, {"data": content})
        ...
        try:
            message_type = MessageType(msg_type)
            message_ids = router.broadcast_message(sender, message_type, {"data": content})
