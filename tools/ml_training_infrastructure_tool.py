...
class MLTrainingInfrastructureTool:
    ...
    def monitor_training_job(self, job_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            monitoring_data = {
                ...
                "resources_allocated": [
                    {"type": r.resource_type.value, "quantity": r.quantity}
    ...
    def get_resource_status(self) -> dict[str, Any]:
        ...
        allocated_resources = {"cpu_cores": 0, "gpu_units": 0, "memory_gb": 0, "storage_gb": 0}
        ...
        return {
            ...
            "utilization_percentage": {
                resource_type: (
                    allocated_resources[resource_type] / self.resource_pool[resource_type]
                )
                * 100
                for resource_type in self.resource_pool
            },
        ...
        return {
            ...
            "queued_jobs": len(
                [
                    ...
                    and self.active_jobs[job.job_id].status == TrainingStatus.QUEUED
                ]
            ),
            "running_jobs": len(
                [
    ...
    def _allocate_resources(self, job: TrainingJob) -> bool:
        ...
        for resource in job.resources:
            if (
                ...
                and self.resource_pool["gpu_units"] < resource.quantity
            ):
                return False
            if (
                resource.resource_type == ResourceType.CPU
                and self.resource_pool["cpu_cores"] < resource.quantity
            ):
                return False
    ...
    def _get_job_by_id(self, job_id: str) -> TrainingJob | None:
        """Get job by ID."""
        ...
        if job_file.exists():
            with open(job_file) as f:
                ...
                job_data["environment"]["framework"] = FrameworkType(
                    job_data["environment"]["framework"]
                )
