...
class ExecutionCoordinator(BaseExecutionManager):
    ...
    def execute(
        ...
        try:
            ...
            if operation in ["create_task", "execute_task", "cancel_task", "list_tasks", "get_task_status"]:
                return self.task_manager.execute(context, operation, payload)
            elif operation in ["register_protocol", "execute_protocol", "list_protocols", "enable_protocol", "disable_protocol"]:
