...
class SocialMediaCommands(commands.Cog):
    ...
    async def social_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                embed.add_field(
                    name="Last Update", value=status.get("last_update", "Unknown"), inline=True
                )

            except ImportError:
                embed.add_field(
                    name="Integration Status", value="⚠️ Service Not Available", inline=False
                )
    ...
    async def post_update(
        self, interaction: discord.Interaction, message: str, platform: str = "all"
    ):
        ...
        try:
            ...
            embed = discord.Embed(
                title="❌ Error", description=f"Failed to post update: {str(e)}", color=0xFF0000
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def social_analytics(self, interaction: discord.Interaction, platform: str = "all"):
        ...
        try:
            ...
            embed = discord.Embed(
                title="❌ Error", description=f"Failed to get analytics: {str(e)}", color=0xFF0000
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
