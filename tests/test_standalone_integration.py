...
def test_integration_coordinator_standalone():
    ...
    try:
        ...
        coordinator_module = load_module_from_file("integration_coordinator", coordinator_path)
        ...
        print(f"   - Middleware Orchestrator: {coordinator.middleware_orchestrator is not None}")
...
def test_configuration():
    ...
    try:
        ...
        if os.path.exists(config_path):
            ...
            print(f"   - Message Queue: {config.get('integration_infrastructure', {}).get('message_queue', {}).get('enabled', 'N/A')}")
            print(f"   - Caching: {config.get('integration_infrastructure', {}).get('caching', {}).get('enabled', 'N/A')}")
            return True
        else:
...
def main():
    ...
    overall_success = all([
        api_success, middleware_success, coordinator_success,
        config_success, launcher_success, async_success
    ])
    
    if overall_success:
        print("\n🎉 All Tests Passed! Integration Infrastructure is working correctly.")
        print("\n📋 Next Steps:")
        print("   1. Start the infrastructure: python scripts/launch_integration_infrastructure.py")
        print("   2. Check health: python scripts/launch_integration_infrastructure.py --action health")
        print("   3. View status: python scripts/launch_integration_infrastructure.py --action status")
        return 0
    else:
