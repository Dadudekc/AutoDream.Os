...
class V2RefactoringAssistant:
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if len(node.bases) > 2:
                    metrics["deep_inheritance"].append(
                        {"name": node.name, "bases": len(node.bases), "line": node.lineno}
                    )
    ...
    def _generate_suggestions(self, line_count: int, metrics: dict[str, Any]) -> list[str]:
        ...
        if metrics.get("complex_functions"):
            for func in metrics["complex_functions"]:
                suggestions.append(
                    f"🧩 Complex function '{func['name']}': Break into smaller functions"
                )
        ...
        if metrics.get("deep_inheritance"):
            for cls in metrics["deep_inheritance"]:
                suggestions.append(
                    f"🌳 Deep inheritance '{cls['name']}': Simplify inheritance chain"
                )
    ...
    def generate_refactoring_report(self, output_file: str | None = None) -> str:
