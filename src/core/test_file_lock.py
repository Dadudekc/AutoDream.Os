...
from .file_lock import FileLockManager, LockConfig, LockTimeoutError
...
def test_basic_file_locking():
    ...
    print("✅ Basic file locking tests passed")
...
def test_concurrent_file_access():
    ...
    print("✅ Concurrent file access tests passed")


def test_lock_timeout():
    """Test lock timeout functionality."""
    print("🧪 Testing lock timeout...")

    with tempfile.TemporaryDirectory() as temp_dir:
        test_file = os.path.join(temp_dir, "timeout_test.txt")
        ...
        def hold_lock():
            ...
            try:
                ...
                print(f"❌ Hold lock thread error: {e}")
            finally:
                lock_released.set()
        ...
        try:
            ...
            print("✅ Lock timeout correctly triggered")
        except Exception as e:
    ...
    print("✅ Lock timeout tests passed")
...
def test_stale_lock_cleanup():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        test_file = os.path.join(temp_dir, "stale_test.txt")
        lock_manager = FileLockManager()
        ...
        assert os.path.exists(lock_file), "Stale lock file should exist"

        # Clean up stale locks
        cleaned = lock_manager.cleanup_stale_locks(temp_dir)
        assert cleaned == 1, f"Expected 1 stale lock cleaned, got {cleaned}"
        assert not os.path.exists(lock_file), "Stale lock file should be removed"

    print("✅ Stale lock cleanup tests passed")
...
def test_messaging_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        from ..services.messaging_delivery import MessagingDelivery
        ...
        expected_files = list(Path(inbox_dir).glob("CAPTAIN_MESSAGE_*.md"))
    ...
    print("✅ Messaging integration tests passed")


def run_all_tests():
    """Run all file locking tests."""
    print("🚀 Starting File Locking System Tests")
    print("=" * 50)
