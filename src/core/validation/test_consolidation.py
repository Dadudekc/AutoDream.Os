#!/usr/bin/env python3
"""
Validator System Consolidation Test

This module tests that all duplicate validator functionality has been successfully
consolidated into the unified validation framework.
"""
...
# Add the parent directory to the path to import the validation modules
...
class TestValidatorConsolidation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_workflow_data = {
            ...
            "name": "Test Workflow",
            "steps": [
                {"id": "step1", "name": "Start", "type": "start", "timeout": 60}
            ]
        }
...
"""
    ...
    def test_storage_validator_consolidation(self):
        ...
        integrity_result = storage_validator.validate_data_integrity(test_data, checksum)
        self.assertEqual(integrity_result.status.value, "passed")
        
        # Test with wrong checksum
        wrong_checksum = "wrong_checksum"
        integrity_result = storage_validator.validate_data_integrity(test_data, wrong_checksum)
    ...
    def test_onboarding_validator_consolidation(self):
        ...
        phase_response = onboarding_validator._wait_for_phase_response(session, "training", "msg123")
        ...
        completion_valid = onboarding_validator._validate_onboarding_completion(
            self.test_onboarding_data,
            {"test_agent": {"onboarding_phases": ["registration", "verification", "training"]}},
            None
        )
        self.assertIsInstance(completion_valid, bool)
        
        # Test performance metrics validation
        metrics_valid = onboarding_validator._validate_performance_metrics(self.test_onboarding_data)
    ...
    def test_config_validator_consolidation(self):
        ...
        section_results = config_validator.validate_config_sections(self.test_config_data)
    ...
    def test_workflow_validator_consolidation(self):
        ...
        execution_results = workflow_validator.validate_workflow_execution_state(execution_data)
    ...
    def test_contract_validator_consolidation(self):
        ...
        legacy_results = contract_validator.validate_contract_legacy(self.test_contract_data)
    ...
    def test_quality_validator_consolidation(self):
        ...
        legacy_results = quality_validator.validate_service_quality_legacy("test_service", self.test_quality_data)
        self.assertIsInstance(legacy_results, list)
    ...
    def test_security_validator_consolidation(self):
        ...
        legacy_results = security_validator.validate_security_policy_legacy(self.test_security_data)
        ...
        summary = security_validator.get_security_policy_summary(self.test_security_data)
        self.assertIsInstance(summary, dict)
        self.assertIn("unified_validation", summary)
        self.assertIn("legacy_validation", summary)
    ...
    def test_code_validator_consolidation(self):
        ...
        validation_result = code_validator.validate_code_with_legacy_fallback(self.test_code)
    ...
    def test_validation_manager_integration(self):
        """Test that ValidationManager can access all consolidated validators"""
        # Test that all validators are available
        self.assertIn("storage", self.validation_manager.validators)
        self.assertIn("onboarding", self.validation_manager.validators)
        self.assertIn("config", self.validation_manager.validators)
        self.assertIn("workflow", self.validation_manager.validators)
        self.assertIn("contract", self.validation_manager.validators)
        self.assertIn("quality", self.validation_manager.validators)
        self.assertIn("security", self.validation_manager.validators)
        self.assertIn("code", self.validation_manager.validators)
        
        # Test that we can run validation through the manager
        for validator_name, validator in self.validation_manager.validators.items():
    ...
    def test_consolidation_completeness(self):
        ...
        self.assertTrue(hasattr(onboarding_validator, '_validate_onboarding_completion'))
        ...
        workflow_validator = WorkflowValidator()
        self.assertTrue(hasattr(workflow_validator, 'validate_workflow_execution_state'))
