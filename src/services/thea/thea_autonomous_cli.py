...
class TheaAutonomousCommunication:
    ...
    def _store_communication(self, communication: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Add message ID
            communication["message_id"] = f"msg_{len(communications) + 1}_{datetime.now().strftime('%H%M%S')}"
    ...
    def get_session_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            session_communications = [comm for comm in history if comm.get("session_id") == self.session_id]
            
            return {
                "session_id": self.session_id,
                "total_messages": len(session_communications),
                "pending_responses": len([comm for comm in session_communications if comm.get("status") == "sent"]),
                "completed_communications": len([comm for comm in session_communications if comm.get("status") == "completed"]),
                "session_start": session_communications[0]["timestamp"] if session_communications else None,
                "last_activity": session_communications[-1]["timestamp"] if session_communications else None
            }
...
def status_command_handler(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if history:
            for comm in history:
                status_icon = "✅" if comm.get("status") == "completed" else "⏳" if comm.get("status") == "sent" else "❌"
                print(f"   {status_icon} {comm.get('message_id', 'N/A')}: {comm.get('message', '')[:50]}{'...' if len(comm.get('message', '')) > 50 else ''}")
