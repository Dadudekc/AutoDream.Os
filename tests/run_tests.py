...
class TestRunner:
    ...
    def run_tests(self, categories: List[str] = None, 
        ...
        for category in categories:
            if category in self.test_categories:
                ...
                category_results = self._run_category_tests(category, coverage, parallel, verbose)
    ...
    def _run_category_tests(self, category: str, coverage: bool, 
                           parallel: bool, verbose: bool) -> Dict[str, Any]:
    ...
    def _print_final_results(self) -> None:
        ...
        print(f"🎯 V2 Standards Compliance: {'✅ COMPLIANT' if self.results['v2_compliance'] else '❌ NON-COMPLIANT'}")
        print()
    ...
    def _audit_component_category(self, category_dir: Path, category: str) -> Dict[str, Any]:
    ...
    def _check_cli_compliance(self, file_path: Path) -> bool:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                return 'argparse' in content and ('def main(' in content or 'if __name__' in content)
        except Exception:
            return False
    ...
    def _update_audit_results(self, overall_results: Dict[str, Any], 
        ...
        overall_results['smoke_tests_compliant'] += category_results['smoke_tests_compliant']
    ...
    def _print_audit_summary(self, audit_results: Dict[str, Any]) -> None:
        ...
        print(f"📏 LOC Compliance: {audit_results['loc_compliant']}/{audit_results['components_audited']}")
        print(f"🏗️  OOP Design: {audit_results['oop_compliant']}/{audit_results['components_audited']}")
        print(f"🎯 Single Responsibility: {audit_results['srp_compliant']}/{audit_results['components_audited']}")
        print(f"🖥️  CLI Interface: {audit_results['cli_compliant']}/{audit_results['components_audited']}")
        print(f"🧪 Smoke Tests: {audit_results['smoke_tests_compliant']}/{audit_results['components_audited']}")
        print()
...
def main():
    ...
    parser.add_argument(
        "--categories", "-c",
        nargs="+",
        choices=["smoke", "unit", "integration", "v2_standards", "performance", "security"],
    ...
    try:
        ...
        if runner.results.get('failed_tests', 0) > 0 or runner.results.get('error_tests', 0) > 0:
            sys.exit(1)
        else:
            sys.exit(0)
