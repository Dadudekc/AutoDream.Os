...
class MetricsStorage:
    ...
    def get_series(self, metric_name: str, start_time: Optional[float] = None, 
                   end_time: Optional[float] = None) -> Optional[MetricSeries]:
...
class PerformanceMonitor:
    ...
    def get_metric_series(self, metric_name: str, start_time: Optional[float] = None,
                         end_time: Optional[float] = None) -> Optional[MetricSeries]:
        """Get metric series with optional time range."""
        return self.metrics_storage.get_series(metric_name, start_time, end_time)
    
    def aggregate_metrics(self, metric_name: str, aggregation: str,
    ...
    def check_alerts(self, metric_data: MetricData) -> List[PerformanceAlert]:
        ...
        for rule in self.alert_rules:
            ...
            # Check tags filter
            if rule.tags_filter:
                if not all(metric_data.tags.get(k) == v for k, v in rule.tags_filter.items()):
                    continue
            
            # Check cooldown
            cooldown_key = f"{rule.name}_{metric_data.metric_name}"
            if cooldown_key in self.alert_cooldowns:
                if time.time() - self.alert_cooldowns[cooldown_key] < rule.cooldown_period:
                    continue
    ...
    def _evaluate_alert_condition(self, rule: AlertRule, value: Union[float, int]) -> bool:
    ...
    async def _cleanup_loop(self):
        ...
        while self.running:
            try:
                ...
                self.cleanup_old_metrics(self.retention_period / 3600)  # Convert to hours
...
class MetricsCollector(ABC):
    ...
    def set_enabled(self, enabled: bool):
        ...
        logger.info(f"Metrics collector {self.__class__.__name__} {'enabled' if enabled else 'disabled'}")


# Export all classes for use in other modules
__all__ = [
    'MetricType', 'AlertSeverity', 'AlertCondition',
    'MetricData', 'MetricSeries', 'AlertRule', 'PerformanceAlert',
    'MetricsStorage', 'PerformanceMonitor', 'MetricsCollector'
]
