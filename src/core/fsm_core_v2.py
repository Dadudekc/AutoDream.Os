...
class FSMCoreV2:
    ...
    def __init__(self, workspace_manager, inbox_manager, fsm_data_path: str = "fsm_data"):
    ...
    def create_task(self, title: str, description: str, assigned_agent: str,
        ...
        try:
            ...
            # Send FSM update
            self._send_fsm_update(task_id, assigned_agent, TaskState.NEW, 
    ...
    def _send_fsm_update(self, task_id: str, agent_id: str, state: TaskState,
    ...
    def get_agent_tasks(self, agent_id: str, state: Optional[TaskState] = None) -> List[FSMTask]:
        """Get agent tasks."""
        tasks = []
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for state in TaskState:
                state_counts[state.value] = len([t for t in self.tasks.values() if t.state == state])
            
            priority_counts = {}
            for priority in TaskPriority:
                priority_counts[priority.value] = len([t for t in self.tasks.values() if t.priority == priority])
            ...
            return {
                ...
                "active_agents": len(set(t.assigned_agent for t in self.tasks.values())),
