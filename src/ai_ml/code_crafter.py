...
class CodeCrafter:
    ...
    def _validate_generation_request(self, request: CodeGenerationRequest) -> None:
        ...
        if request.framework and request.framework not in self.supported_frameworks.get(request.language.lower(), []):
            raise ValueError(f"Unsupported framework {request.framework} for {request.language}")
    
    def _generate_with_openai(self, request: CodeGenerationRequest) -> CodeGenerationResult:
    ...
    def _generate_with_anthropic(self, request: CodeGenerationRequest) -> CodeGenerationResult:
...
Format as JSON with: code, tests, documentation, explanation, estimated_complexity, dependencies, usage_examples"""
    ...
    def _parse_openai_response(self, response: Any, request: CodeGenerationRequest) -> CodeGenerationResult:
        ...
        try:
            ...
            return CodeGenerationResult(
                ...
                documentation=data.get("documentation") if request.include_docs else None,
    ...
    def _parse_anthropic_response(self, response: Any, request: CodeGenerationRequest) -> CodeGenerationResult:
    ...
    def _analyze_python_code(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
        """Analyze Python code"""
        try:
            ...
            # Check documentation
            docstring_count = len([n for n in ast.walk(tree) if isinstance(n, ast.Expr) and isinstance(n.value, ast.Str)])
            total_functions = len([n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)])
            analysis.documentation_coverage = (docstring_count / max(total_functions, 1)) * 100
    ...
    def _analyze_javascript_code(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
        """Analyze JavaScript/TypeScript code"""
        # Basic analysis for JavaScript
        lines = content.split('\n')
        analysis.complexity_score = len([line for line in lines if any(keyword in line for keyword in ['if', 'for', 'while', 'case'])])
    ...
    def _analyze_java_code(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
        ...
        lines = content.split('\n')
        analysis.complexity_score = len([line for line in lines if any(keyword in line for keyword in ['if', 'for', 'while', 'case', 'catch'])])
        analysis.maintainability_index = max(0, 100 - analysis.complexity_score * 2)
    ...
    def _analyze_generic_code(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
    ...
    def _enhance_analysis_with_ai(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
    ...
    def _enhance_with_openai(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
...
Format as JSON with: suggestions, security_issues, performance_issues"""
    ...
    def _enhance_with_anthropic(self, content: str, analysis: CodeAnalysis) -> CodeAnalysis:
...
Format as JSON with: suggestions, security_issues, performance_issues"""
    ...
    def refactor_code(self, file_path: Union[str, Path], suggestions: List[str]) -> str:
        ...
        try:
            ...
            if self.openai_key:
                return self._refactor_with_openai(content, suggestions, file_path.suffix)
            elif self.anthropic_key:
                return self._refactor_with_anthropic(content, suggestions, file_path.suffix)
            else:
    ...
    def _refactor_with_openai(self, content: str, suggestions: List[str], file_extension: str) -> str:
...
Return only the refactored code."""
    ...
    def _refactor_with_anthropic(self, content: str, suggestions: List[str], file_extension: str) -> str:
...
Return only the refactored code."""
    ...
    def generate_tests(self, file_path: Union[str, Path], framework: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            if self.openai_key:
                return self._generate_tests_with_openai(content, framework, file_path.suffix)
            elif self.anthropic_key:
                return self._generate_tests_with_anthropic(content, framework, file_path.suffix)
            else:
    ...
    def _generate_tests_with_openai(self, content: str, framework: str, file_extension: str) -> str:
...
Return only the test code."""
            ...
            response = openai.ChatCompletion.create(
                model="gpt-4",
                messages=[
                    {"role": "system", "content": "You are an expert testing specialist."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=3000,
                temperature=0.2
    ...
    def _generate_tests_with_anthropic(self, content: str, framework: str, file_extension: str) -> str:
