...
def demo_fsm_discord_integration():
    ...
    try:
        ...
        for title, description, agent, priority in test_tasks:
            task_id = bridge.create_task_via_discord(title, description, agent, priority)
        ...
        decision_scenarios = [
            ...
            ("Task priority allocation", ["round_robin", "load_balanced", "priority_based"]),
        ...
        test_commands = [
            ...
            ("!create_task", ["Demo Task", "Testing command processing", "agent-2"], "demo_user", "tasks")
        ]
        ...
        fsm_status = {
            "total_tasks": len(fsm_core.tasks),
            "status": fsm_core.status
        }
