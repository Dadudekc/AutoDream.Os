    ...
    def get_learning_status(self) -> Dict[str, Any]:
        ...
        with self.learning_lock:
            return {
                ...
                "last_learning_update": self.learning_data[-1].timestamp if self.learning_data else None
            }
        ...
        for metric_name, pattern in patterns.items():
            if isinstance(pattern, dict) and "trend" in pattern:
                if pattern["trend"] == "declining":
                    recommendations.append(f"Investigate declining {metric_name} performance")
