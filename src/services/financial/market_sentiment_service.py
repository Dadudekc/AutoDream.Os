...
class MarketSentimentService:
    ...
    def __init__(self, market_data_service=None, data_dir: str = "market_sentiment"):
        ...
        self.bullish_keywords = [
            "bullish", "positive", "growth", "recovery", "rally", "surge", "gain",
            "strong", "improve", "beat", "exceed", "upgrade", "buy", "outperform"
        ]
        
        self.bearish_keywords = [
            "bearish", "negative", "decline", "fall", "drop", "crash", "loss",
            "weak", "worsen", "miss", "downgrade", "sell", "underperform"
        ]
    ...
    def analyze_news_sentiment(self, articles: List[NewsArticle]) -> List[SentimentData]:
        ...
        try:
            ...
            for article in articles:
                ...
                title_score, title_confidence = self.analyze_text_sentiment(article.title)
                content_score, content_confidence = self.analyze_text_sentiment(article.content)
                
                # Combined sentiment score
                combined_score = (title_score * 0.4) + (content_score * 0.6)
                combined_confidence = (title_confidence * 0.3) + (content_confidence * 0.7)
    ...
    def calculate_news_impact(self, article: NewsArticle) -> float:
        ...
        try:
            ...
            # Source credibility
            credible_sources = ["reuters", "bloomberg", "cnbc", "wsj", "ft", "yahoo finance"]
            ...
            # Keywords presence
            keyword_count = sum(1 for keyword in self.bullish_keywords + self.bearish_keywords 
    ...
    def analyze_social_media_sentiment(self, social_data: List[Dict[str, Any]]) -> List[SentimentData]:
    ...
    def analyze_analyst_ratings(self, ratings_data: List[Dict[str, Any]]) -> List[SentimentData]:
        ...
        try:
            ...
            for rating in ratings_data:
                ...
                sentiment_data.append(SentimentData(
                    ...
                    timestamp=datetime.fromisoformat(rating.get("timestamp", datetime.now().isoformat())),
    ...
    def analyze_options_flow_sentiment(self, options_data: List[Dict[str, Any]]) -> List[SentimentData]:
        ...
        try:
            ...
            for option in options_data:
                ...
                sentiment_data.append(SentimentData(
                    ...
                    timestamp=datetime.fromisoformat(option.get("timestamp", datetime.now().isoformat())),
    ...
    def aggregate_sentiment(self, symbol: str, time_window: timedelta = timedelta(days=7)) -> SentimentAggregate:
    ...
    def get_sentiment_signals(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if aggregate.confidence > 0.8:
                if aggregate.overall_sentiment == SentimentType.BULLISH and aggregate.sentiment_score > 0.5:
                    signals.append({
                ...
                elif aggregate.overall_sentiment == SentimentType.BEARISH and aggregate.sentiment_score < -0.5:
                    signals.append({
            ...
            elif aggregate.sentiment_trend == "DETERIORATING" and aggregate.momentum < -0.2:
                signals.append({
    ...
    def add_sentiment_data(self, symbol: str, sentiment_data: List[SentimentData]) -> None:
        ...
        try:
            ...
            logger.info(f"Added {len(sentiment_data)} sentiment data points for {symbol}")
    ...
    def save_data(self):
        ...
        try:
            ...
            aggregates_data = {symbol: asdict(aggregate) 
            ...
            # Save news articles
            news_data = {symbol: [asdict(article) for article in articles]
        ...
        try:
            ...
            if self.aggregates_file.exists():
                ...
                for symbol, aggregate_dict in aggregates_data.items():
                    if "last_updated" in aggregate_dict:
                        aggregate_dict["last_updated"] = datetime.fromisoformat(aggregate_dict["last_updated"])
                    
                    aggregate_obj = SentimentAggregate(**aggregate_dict)
                    self.sentiment_aggregates[symbol] = aggregate_obj
                
                logger.info(f"Loaded sentiment aggregates for {len(aggregates_data)} symbols")
            ...
            if self.psychology_file.exists():
                ...
                if "last_updated" in psychology_dict:
                    psychology_dict["last_updated"] = datetime.fromisoformat(psychology_dict["last_updated"])
