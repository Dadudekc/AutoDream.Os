...
from .unified_analytics_engine import AnalyticsData, UnifiedAnalyticsEngine
...
class SwarmMetric:
    ...
    phase: ConsolidationPhase | None = None
...
class SwarmBusinessIntelligence:
    ...
    def update_consolidation_progress(
        ...
        try:
            completion_percentage = (
                (files_processed / files_target * 100) if files_target > 0 else 0
            )
            ...
            self.logger.info(
                f"Updated consolidation progress: {chunk_id} - {completion_percentage:.1f}%"
            )
    ...
    def generate_health_report(self) -> SwarmHealthReport:
        ...
        try:
            ...
            active_agents = len({m.agent_id for m in recent_metrics})
        ...
        try:
            ...
            if metrics_file.exists():
                with open(metrics_file) as f:
                    ...
                    self.metrics = [SwarmMetric(**metric_data) for metric_data in metrics_data]
