...
class BaseMiddlewareComponent(ABC):
    ...
    @abstractmethod
    async def process(self, data_packet: DataPacket, context: Dict[str, Any]) -> DataPacket:
        ...
        try:
            ...
            for condition, transformation in self.transformations.items():
                if self._matches_condition(data_packet, condition):
                    ...
                    data_packet.processing_history.append(f"{self.name}:{transformation}")
            
            # Update packet metadata
            data_packet.metadata['transformed'] = True
            data_packet.metadata['transformation_count'] = len(data_packet.processing_history)
                ...
                if field in data_packet.metadata:
                    ...
                    for rule, constraint in rules.items():
                        if not self._validate_field(value, rule, constraint):
                            error_msg = f"Validation failed for {field}: {rule} {constraint}"
...
class MiddlewareOrchestrator:
    ...
    def register_middleware(self, middleware: BaseMiddlewareComponent) -> None:
        ...
        if middleware.name in self.middleware_components:
            logger.warning(f"Middleware {middleware.name} already registered, overwriting")
        
        self.middleware_components[middleware.name] = middleware
        logger.info(f"Registered middleware: {middleware.name} ({middleware.middleware_type.value})")
    ...
    def create_chain(self, chain: MiddlewareChain) -> None:
        ...
        for middleware_name in chain.middleware_list:
            if middleware_name not in self.middleware_components:
                raise ValueError(f"Middleware '{middleware_name}' not found in chain '{chain.name}'")
        
        self.middleware_chains.append(chain)
        # Sort by priority
        self.middleware_chains.sort(key=lambda x: x.priority)
        
        logger.info(f"Created middleware chain: {chain.name} with {len(chain.middleware_list)} components")
    ...
    async def process_data_packet(self, data_packet: DataPacket, 
        ...
        try:
            ...
            if not chain or not chain.enabled:
                logger.warning(f"No appropriate chain found for packet {data_packet.id}")
                return data_packet
    ...
    def _select_appropriate_chain(self, data_packet: DataPacket) -> Optional[MiddlewareChain]:
    ...
    def _chain_matches_packet(self, chain: MiddlewareChain, data_packet: DataPacket) -> bool:
    ...
    async def _execute_chain(self, data_packet: DataPacket, chain: MiddlewareChain, 
                           context: Dict[str, Any]) -> DataPacket:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (self.total_processing_time / self.total_packets_processed 
                             if self.total_packets_processed > 0 else 0)
        ...
        return {
            ...
            'packets_per_second': self.total_packets_processed / uptime if uptime > 0 else 0,
...
async def main():
    ...
    orchestrator.register_middleware(DataTransformationMiddleware({
        'transformations': {
            'json': 'json_to_dict',
    ...
    orchestrator.register_middleware(ValidationMiddleware({
        'validation_rules': {
    ...
    orchestrator.register_middleware(RoutingMiddleware({
        'routing_rules': {
    ...
    standard_chain = MiddlewareChain(
        ...
        middleware_list=["DataTransformationMiddleware", "ValidationMiddleware", "RoutingMiddleware"],
        priority=2,
