...
class OnboardingSystemLauncher:
    ...
    def start_complete_onboarding(self, agent_id: str, target_role: str, workflow_type: str = "standard") -> Dict[str, Any]:
        """Start complete onboarding process for an agent"""
        if not IMPORT_SUCCESS:
            return {"error": "Services not available due to import issues"}
            
        try:
            self.logger.info(f"Starting complete onboarding for agent {agent_id} with role {target_role}")
            
            # Step 1: Start onboarding session
            onboarding_session = self.onboarding_service.start_onboarding(agent_id, AgentRole(target_role))
            if not onboarding_session:
                raise Exception("Failed to start onboarding session")
            
            # Step 2: Start orientation workflow
            workflow_id = self.workflow_service.start_orientation_workflow(agent_id, workflow_type)
            if not workflow_id:
            ...
            for module_id in training_modules:
                session_id = self.training_service.start_learning_session(agent_id, module_id)
                if session_id:
            ...
            self.logger.error(f"Failed to start complete onboarding for {agent_id}: {e}")
            return {
                "agent_id": agent_id,
                "status": "failed",
                "error": str(e),
    ...
    def get_agent_onboarding_status(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            agent_onboarding = [s for s in onboarding_sessions.values() if s.agent_id == agent_id]
            ...
            agent_training = [t for t in training_progress.values() if t["agent_id"] == agent_id]
            
            return {
                "agent_id": agent_id,
                "onboarding_sessions": len(agent_onboarding),
                "active_workflows": len([w for w in agent_workflows if w.status.value == "in_progress"]),
                "completed_workflows": len([w for w in agent_workflows if w.status.value == "completed"]),
    ...
    def run_demo_onboarding(self) -> Dict[str, Any]:
        ...
        if not IMPORT_SUCCESS:
            return {"demo_completed": False, "error": "Services not available due to import issues"}
            
        try:
            self.logger.info("Starting demo onboarding process")
            
            # Demo agent onboarding
            demo_result = self.start_complete_onboarding("demo-agent", "worker", "standard")
...
def main():
    ...
    parser.add_argument("--start", nargs=2, help="Start complete onboarding (agent_id,role)")
