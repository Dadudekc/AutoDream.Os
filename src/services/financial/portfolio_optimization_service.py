...
class PortfolioOptimizationService:
    ...
    def __init__(
        ...
        risk_manager=None,
        data_dir: str = "portfolio_optimization",
    ):
    ...
    def optimize_portfolio_sharpe(
        ...
        try:
            ...
            mean_returns, covariance_matrix = self.calculate_returns_and_covariance(
                symbols
            )
            ...
            def objective(weights):
                ...
                sharpe_ratio = (
                    portfolio_return - self.optimization_params["risk_free_rate"]
                ...
                constraints_list.append(
                    {
                        ...
                        - self.optimization_params["min_position_size"],
                    }
                )
                constraints_list.append(
                    {
                        "type": "ineq",
                        "fun": lambda x, i=i: self.optimization_params[
                            "max_position_size"
                        ]
                        - x[i],
                    }
                ...
                for constraint in constraints:
                    if constraint.constraint_type == "SECTOR_LIMIT":
                        sector_symbols = [
                            ...
                            if symbol
                            in self.sector_classifications.get(constraint.sector, [])
                        ]
                        if sector_symbols:
                            constraints_list.append(
                                {
                                    "type": "ineq",
                                    "fun": lambda x, sector_symbols=sector_symbols: constraint.max_sector_weight
                                    - np.sum(x[sector_symbols]),
                                }
                            )
            ...
            bounds = [
                (
                    self.optimization_params["min_position_size"],
                    self.optimization_params["max_position_size"],
                )
            ] * n_assets
            ...
            optimal_weights_dict = {
                symbol: weight for symbol, weight in zip(symbols, optimal_weights)
            }
            ...
            risk_metrics = {
                "var_95": self.calculate_value_at_risk(
                    optimal_weights, mean_returns, covariance_matrix, 0.95
                ),
                "cvar_95": self.calculate_conditional_var(
                    optimal_weights, mean_returns, covariance_matrix, 0.95
                ),
                "max_drawdown": self.calculate_max_drawdown(
                    optimal_weights, mean_returns, covariance_matrix
                ),
            ...
            self.optimization_results[
                f"sharpe_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ] = result_obj
    ...
    def optimize_portfolio_minimum_variance(
        self, symbols: List[str], constraints: List[OptimizationConstraint] = None
    ) -> OptimizationResult:
        ...
        try:
            ...
            # Store result
            self.optimization_results[
                f"minvar_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ] = result_obj
    ...
    def calculate_conditional_var(
        ...
        try:
            ...
            cvar = portfolio_return - (
                portfolio_volatility * stats.norm.pdf(z_score) / (1 - confidence_level)
            )
    ...
    def calculate_portfolio_beta(
        ...
        try:
            ...
            betas = []
            for i, symbol in enumerate(symbols):
                symbol_data = self.market_data_service.get_historical_data(
                    symbol, "1y", "1d"
                )
                if symbol_data and symbol_data.data is not None:
                    ...
                    aligned_returns = pd.concat(
                        [symbol_returns, market_returns], axis=1
                    ).dropna()
                    if len(aligned_returns) > 30:
                        ...
                        # Calculate beta
                        covariance = np.cov(
                            symbol_returns_aligned, market_returns_aligned
                        )[0, 1]
    ...
    def generate_rebalancing_signals(
        self, current_portfolio: Dict[str, float], target_weights: Dict[str, float]
        ...
        try:
            ...
            for symbol in all_symbols:
                ...
                if abs(weight_difference) > threshold:
                    if weight_difference > 0:
                        action = "BUY"
                        priority = (
                            "HIGH" if weight_difference > threshold * 2 else "MEDIUM"
                        )
                    else:
                        action = "SELL"
                        priority = (
                            "HIGH"
                            if abs(weight_difference) > threshold * 2
                            else "MEDIUM"
                        )
            ...
            signals.sort(
                key=lambda x: (x.priority == "HIGH", abs(x.weight_difference)),
                reverse=True,
            )
    ...
    def analyze_portfolio_allocation(
        ...
        try:
            ...
            for position in portfolio_positions:
                ...
                if self.market_data_service:
                    ...
                    if symbol in market_data:
                        ...
                        for (
                            cap_type,
                            threshold,
                        ) in self.market_cap_classifications.items():
                ...
                style = (
                    "BLEND"  # Would need fundamental data for accurate classification
                )
        ...
        try:
            ...
            if optimization_result.expected_volatility > 0.25:
                recommendations.append(
                    "HIGH_VOLATILITY - Consider reducing risk through diversification"
                )

            if optimization_result.risk_metrics.get("var_95", 0) < -0.1:
                recommendations.append(
                    "HIGH_DOWNSIDE_RISK - Consider defensive positioning"
                )

            if optimization_result.risk_metrics.get("beta", 1.0) > 1.2:
                recommendations.append(
                    "HIGH_BETA - Portfolio may be too aggressive for current market"
                )

            if optimization_result.risk_metrics.get("correlation", 0) > 0.6:
                recommendations.append(
                    "HIGH_CORRELATION - Consider adding uncorrelated assets"
                )

            # Return analysis recommendations
            if optimization_result.sharpe_ratio < 0.5:
                recommendations.append(
                    "LOW_SHARPE_RATIO - Risk-adjusted returns need improvement"
                )

            if optimization_result.expected_return < 0.05:
                recommendations.append(
                    "LOW_EXPECTED_RETURN - Consider higher-yielding assets"
                )
            ...
            if max_weight > 0.15:
                recommendations.append(
                    "CONCENTRATED_POSITIONS - Consider reducing largest positions"
                )
            ...
            for sector, weight in sector_weights.items():
                if weight > 0.4:
                    recommendations.append(
                        f"SECTOR_OVERWEIGHT - {sector} sector concentration too high"
                    )
            ...
            if self.signals_file.exists():
                ...
                for signal_dict in signals_data:
                    if "timestamp" in signal_dict:
                        signal_dict["timestamp"] = datetime.fromisoformat(
...
if __name__ == "__main__":
    ...
    print(
        f"Available optimization methods: {[method.value for method in OptimizationMethod]}"
    )
    print(
        f"Default rebalancing threshold: {pos.optimization_params['rebalancing_threshold']}"
    )
