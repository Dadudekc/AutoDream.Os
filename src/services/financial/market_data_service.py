...
class MarketDataService:
    ...
    def get_real_time_data(self, symbols: List[str], source: str = None) -> Dict[str, MarketData]:
        ...
        try:
            ...
            for symbol in symbols:
                ...
                if (symbol_upper in self.market_data_cache and 
                    time.time() - self.market_data_cache[symbol_upper].get("timestamp", 0) < self.cache_duration):
                    cached_data[symbol_upper] = self.market_data_cache[symbol_upper]["data"]
                else:
    ...
    def get_historical_data(self, symbol: str, period: str = "1y", interval: str = "1d") -> Optional[HistoricalData]:
    ...
    def get_market_sentiment(self, symbol: str, period: str = "1y") -> Optional[MarketSentiment]:
        ...
        try:
            ...
            latest_bb_upper = bollinger_upper.iloc[-1] if not bollinger_upper.empty else 0.0
            latest_bb_lower = bollinger_lower.iloc[-1] if not bollinger_lower.empty else 0.0
            latest_bb_middle = bollinger_middle.iloc[-1] if not bollinger_middle.empty else 0.0
    ...
    def get_portfolio_market_data(self, portfolio_symbols: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "real_time_data": {symbol: asdict(data) for symbol, data in real_time_data.items()},
    ...
    def _calculate_portfolio_market_metrics(self, market_data: Dict[str, MarketData]) -> Dict[str, Any]:
        """Calculate portfolio-level market metrics"""
        try:
            if not market_data:
                return {}
            
            total_market_cap = sum(data.market_cap for data in market_data.values() if data.market_cap)
            avg_pe_ratio = np.mean([data.pe_ratio for data in market_data.values() if data.pe_ratio])
            avg_dividend_yield = np.mean([data.dividend_yield for data in market_data.values() if data.dividend_yield])
    ...
    def get_market_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            major_indices = ["^GSPC", "^DJI", "^IXIC", "^RUT"]  # S&P 500, Dow, NASDAQ, Russell 2000
            ...
            return {
                "major_indices": {symbol: asdict(data) for symbol, data in indices_data.items()},
                "market_breadth": {
                    "advancing": advancing,
                    "declining": declining,
