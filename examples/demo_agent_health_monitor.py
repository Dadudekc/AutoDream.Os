...
class AgentHealthMonitorDemo:
    ...
    def __init__(self):
        ...
        self.metric_patterns = {
            "agent_001": {"response_time": 500, "memory_usage": 75, "cpu_usage": 60, "error_rate": 2},
            "agent_002": {"response_time": 1200, "memory_usage": 85, "cpu_usage": 90, "error_rate": 8},
            "agent_003": {"response_time": 300, "memory_usage": 45, "cpu_usage": 30, "error_rate": 1},
            "agent_004": {"response_time": 8000, "memory_usage": 95, "cpu_usage": 98, "error_rate": 25},
            "agent_005": {"response_time": 200, "memory_usage": 25, "cpu_usage": 20, "error_rate": 0}
        }
    ...
    def _test_health_metrics_collection(self):
        ...
        try:
            ...
            assert len(all_health) == len(self.demo_agents), "Not all agents have health data"
    ...
    def _test_alert_management(self):
        ...
        try:
            ...
            if alerts:
                ...
                acknowledged_alert = next((a for a in updated_alerts if a.alert_id == first_alert.alert_id), None)
                assert acknowledged_alert is not None, "Alert not found after acknowledgment"
            ...
            if alerts:
                ...
                # Verify resolution
                updated_alerts = self.health_monitor.get_health_alerts()
                resolved_alert = next((a for a in updated_alerts if a.alert_id == first_alert.alert_id), None)
    ...
    def _test_health_scoring(self):
        ...
        try:
            ...
            assert 'average_health_score' in summary, "Health summary missing average_health_score"
            assert 'status_distribution' in summary, "Health summary missing status_distribution"
            
            # Verify health scores are calculated
            for agent_id in self.demo_agents:
                health = self.health_monitor.get_agent_health(agent_id)
                assert health.health_score >= 0, f"Invalid health score for {agent_id}"
                assert health.health_score <= 100, f"Invalid health score for {agent_id}"
                
                # Check for recommendations
                assert hasattr(health, 'recommendations'), f"Health snapshot missing recommendations for {agent_id}"
                assert isinstance(health.recommendations, list), f"Recommendations not a list for {agent_id}"
    ...
    def _test_web_interface(self):
        ...
        try:
            ...
            expected_routes = ['/', '/api/health/summary', '/api/health/agents', '/api/health/alerts']
    ...
    def _test_performance_under_load(self):
        ...
        try:
            ...
            for agent_id in load_test_agents:
                health = self.health_monitor.get_agent_health(agent_id)
                assert health is not None, f"Load test agent {agent_id} missing health data"
    ...
    def _test_core_system_integration(self):
        ...
        try:
            ...
            assert HealthMetricType.RESPONSE_TIME in thresholds, "Custom threshold not set"
            
            custom_threshold = thresholds[HealthMetricType.RESPONSE_TIME]
            assert custom_threshold.warning_threshold == 800.0, "Warning threshold not set correctly"
            assert custom_threshold.critical_threshold == 3000.0, "Critical threshold not set correctly"
...
def main():
    ...
    parser.add_argument("--scenario", action="store_true", help="Run realistic demo scenario")
