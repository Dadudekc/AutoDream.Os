...
class ErrorRecoveryStrategy:
    ...
    def execute_recovery(self, error_info: ErrorInfo) -> bool:
        ...
        strategy = self.recovery_strategies.get(error_info.error_type, self.default_strategy)
...
class ErrorHandler:
    ...
    def create_retry_strategy(self, 
                             component_id: str,
                             max_retries: int = 3,
    ...
    def _attempt_recovery(self, error_info: ErrorInfo):
        ...
        try:
            ...
            if success:
                self.recovery_counts[error_info.error_type] = self.recovery_counts.get(error_info.error_type, 0) + 1
                self.logger.info(f"Automatic recovery successful for {error_info.error_id}")
            else:
                self.logger.warning(f"Automatic recovery failed for {error_info.error_id}")
        except Exception as e:
    ...
    def clear_errors(self, before_timestamp: Optional[float] = None):
        ...
        self.errors = [error for error in self.errors if error.timestamp >= before_timestamp]
    ...
    def is_component_healthy(self, component_id: str) -> bool:
        ...
        recent_errors = [error for error in self.errors 
                        if error.component_id == component_id and 
                        error.timestamp > time.time() - 300]  # Last 5 minutes
        ...
        def wrapper(*args, **kwargs) -> T:
            try:
                ...
                if circuit_breaker and component_id in error_handler.circuit_breakers:
                    return error_handler.circuit_breakers[component_id].call(func, *args, **kwargs)
                
                # Use retry strategy if enabled
                if retry and component_id in error_handler.retry_strategies:
                    return error_handler.retry_strategies[component_id].execute(func, *args, **kwargs)
