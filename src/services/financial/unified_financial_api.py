...
try:
    ...
    from .risk_management_service import RiskManager, RiskMetric, RiskAlert, PortfolioRiskProfile
    from .market_data_service import MarketDataService, MarketData, HistoricalData
    from .trading_intelligence_service import TradingIntelligenceService, TradingSignal, StrategyType
    from .options_trading_service import OptionsTradingService, OptionContract, OptionsChain
    from .financial_analytics_service import FinancialAnalyticsService, BacktestResult, PerformanceMetrics
    from .market_sentiment_service import MarketSentimentService, SentimentData, SentimentAggregate
    from .portfolio_optimization_service import PortfolioOptimizationService, OptimizationResult
except ImportError:
...
class UnifiedFinancialAPI:
    ...
    def register_agent(self, agent_id: str, agent_name: str, agent_type: str, 
                      required_services: List[str]) -> bool:
        """Register a new agent with the unified API"""
        try:
            # Validate required services
            available_services = [
                "portfolio_management", "risk_management", "market_data",
                "trading_intelligence", "options_trading", "financial_analytics",
                "market_sentiment", "portfolio_optimization"
            ]
    ...
    def get_all_agents_status(self) -> List[Dict[str, Any]]:
        ...
        try:
            return [self.get_agent_status(agent_id) for agent_id in self.registered_agents.keys()]
        except Exception as e:
            logger.error(f"Error getting all agents status: {e}")
            return []
    
    def request_service(self, source_agent: str, target_service: str, 
                       request_type: str, request_data: Dict[str, Any], 
        ...
        try:
            ...
            if target_service not in self.registered_agents[source_agent].required_services:
                raise ValueError(f"Service {target_service} not available for agent {source_agent}")
    ...
    def execute_service_request(self, request_id: str) -> CrossAgentResponse:
        ...
        try:
            ...
            if source_agent in self.performance_metrics:
                ...
                current_avg = self.performance_metrics[source_agent]["average_response_time"]
                total_successful = self.performance_metrics[source_agent]["successful_requests"]
                new_avg = ((current_avg * (total_successful - 1)) + response_time) / total_successful
                self.performance_metrics[source_agent]["average_response_time"] = new_avg
                self.performance_metrics[source_agent]["last_updated"] = datetime.now().isoformat()
    ...
    def _execute_service(self, target_service: str, request_type: str, 
    ...
    def _execute_portfolio_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
    ...
    def _execute_risk_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
    ...
    def _execute_market_data_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "get_real_time_data":
                ...
                return self.market_data_service.get_historical_data(symbol, period, interval)
            else:
    ...
    def _execute_trading_intelligence_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "generate_signals":
                ...
                raise ValueError(f"Unknown trading intelligence request type: {request_type}")
        except Exception as e:
            logger.error(f"Error executing trading intelligence service: {e}")
            raise
    
    def _execute_options_trading_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "get_options_chain":
                ...
                raise ValueError(f"Unknown options trading request type: {request_type}")
        except Exception as e:
            logger.error(f"Error executing options trading service: {e}")
            raise
    
    def _execute_financial_analytics_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "run_backtest":
                ...
                return self.financial_analytics.calculate_performance_metrics(**request_data)
            else:
                raise ValueError(f"Unknown financial analytics request type: {request_type}")
        except Exception as e:
            logger.error(f"Error executing financial analytics service: {e}")
            raise
    
    def _execute_market_sentiment_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "analyze_text_sentiment":
                ...
                raise ValueError(f"Unknown market sentiment request type: {request_type}")
        except Exception as e:
            logger.error(f"Error executing market sentiment service: {e}")
            raise
    
    def _execute_portfolio_optimization_service(self, request_type: str, request_data: Dict[str, Any]) -> Any:
        ...
        try:
            if request_type == "optimize_portfolio_sharpe":
                ...
                raise ValueError(f"Unknown portfolio optimization request type: {request_type}")
        except Exception as e:
    ...
    def get_system_health_metrics(self) -> SystemHealthMetrics:
        ...
        try:
            ...
            successful_requests = sum(metrics["successful_requests"] 
                                   for metrics in self.performance_metrics.values())
            failed_requests = sum(metrics["failed_requests"] 
                               for metrics in self.performance_metrics.values())
            
            # Calculate average response time
            response_times = [metrics["average_response_time"] 
                            for metrics in self.performance_metrics.values()
