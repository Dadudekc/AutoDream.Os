...
def test_strategies_initialization():
    ...
    try:
        ...
        assert (
            len(service.strategies) == 4
        ), f"Expected 4 strategies, got {len(service.strategies)}"
...
def test_capture_control():
    ...
    try:
        ...
        assert (
            service.status == "capturing"
        ), f"Expected 'capturing', got '{service.status}'"
...
def test_response_capture():
    ...
    try:
        ...
        ), f"Expected 'Agent-1', got '{response.agent}'"
...
def test_response_filtering():
    ...
    try:
        ...
        assert (
            len(agent1_responses) == 2
        ), f"Expected 2 responses from Agent-1, got {len(agent1_responses)}"
        print("✅ Agent filtering successful")

        # Test filtering by source
        source1_responses = service.get_responses_by_source("source1")
        assert (
            len(source1_responses) == 2
        ), f"Expected 2 responses from source1, got {len(source1_responses)}"
...
def test_service_status():
    ...
    try:
        ...
        assert (
            "strategies_enabled" in status
        ), "Status should have strategies_enabled field"
        ...
        ), "Capture should not be active initially"
        assert (
            status["strategies_enabled"] == 4
        ), f"Expected 4 strategies, got {status['strategies_enabled']}"
...
def run_all_smoke_tests():
    ...
    if passed == total:
        print(
            "🎉 All smoke tests passed! Response Capture Service is working correctly."
        )
        return True
    else:
