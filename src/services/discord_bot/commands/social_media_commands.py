...
class SocialMediaCommands(commands.Cog):
    ...
    @app_commands.command(
        name="social_stats", description="Get cross-platform community statistics"
    )
    async def social_stats(self, interaction: discord.Interaction):
        ...
        try:
            ...
            for platform, data in analytics["platforms"].items():
                platform_text += (
                    f"**{platform.title()}:** {data.get('engagement', 0):,} engagement\n"
                )

            embed.add_field(name="📱 Platforms", value=platform_text, inline=False)
            ...
            if analytics["insights"]:
                ...
                embed.add_field(name="💡 Key Insights", value=insights_text, inline=False)
    ...
    async def sentiment_analysis(self, interaction: discord.Interaction, text: str):
        ...
        try:
            ...
            embed.add_field(name="📝 Text Sample", value=sentiment["text"], inline=False)
    ...
    @app_commands.command(
        name="community_dashboard", description="View unified community dashboard"
    )
    async def community_dashboard(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="📊 Unified Community Dashboard", color=0x0099FF, timestamp=datetime.utcnow()
            )
            ...
            if enabled:
                ...
                embed.add_field(name="📱 Connected Platforms", value=platforms_text, inline=False)
            ...
            if "recommendations" in dashboard_data:
                ...
                if recommendations:
                    ...
                    embed.add_field(name="🎯 Recommendations", value=recs_text, inline=False)
    ...
    async def platform_status(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if service.enabled_platforms:
                ...
                embed.add_field(name="📱 Enabled Platforms", value=platforms_text, inline=False)
            else:
                embed.add_field(
                    name="⚠️ No Platforms",
