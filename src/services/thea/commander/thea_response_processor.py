...
class CommanderTheaResponseProcessor:
    ...
    def __init__(self, config: TheaConfig, browser_service: TheaBrowserService):
        ...
        self.response_patterns = {
            "confidence_levels": [
                "(\\d+)%\\s*confidence",
                "confidence[:\\s]*(\\d+)%",
                "success probability[:\\s]*(\\d+)%",
            ],
            "ratings": ["★{1,5}", "(\\d+\\.?\\d*)\\s*/\\s*5", "rating[:\\s]*(\\d+\\.?\\d*)"],
            "recommendations": [
                "recommend[^.]*?\\.([^.]*\\.)",
                "suggest[^.]*?\\.([^.]*\\.)",
                "should\\s+([^.]*\\.)",
            ...
            "swarm_references": ["agent-?\\d+", "swarm", "coordination", "multi-agent"],
        }

    def process_thea_response(
        self, screenshot_path: Path, context: dict[str, Any] = None
    ...
    def generate_commander_thea_analysis(
        self, message: str, response_result: ResponseAnalysisResult, context: dict[str, Any] = None
    ) -> str:
...
"""
    ...
    def _structure_response_content(self, text: str) -> dict[str, Any]:
        ...
        greeting_pattern = "^(.*?(?:Commander THEA|Hello|Excellent work|Locking into).*?\\.)"
    ...
    def _calculate_confidence_metrics(self, text: str) -> dict[str, float]:
        ...
        if metrics["overall_confidence"] > 0:
            ...
            metrics["implementation_confidence"] = metrics["overall_confidence"] * 0.9
    ...
    def _generate_enhanced_analysis(
        self, result: ResponseAnalysisResult, context: dict[str, Any] = None
    ) -> dict[str, Any]:
        ...
        analysis = {
            ...
            "swarm_relevance": "swarm" in result.raw_text.lower()
            or "agent" in result.raw_text.lower(),
