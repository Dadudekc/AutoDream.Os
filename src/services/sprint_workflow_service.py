...
class SprintWorkflowService:
    ...
    def plan_sprint_tasks(self, sprint_id: str, task_ids: List[str]) -> bool:
        ...
        try:
            if len(task_ids) > 10:
                self.logger.warning(f"Cannot plan more than 10 tasks, got {len(task_ids)}")
                return False
            ...
            self.logger.info(f"Planned {len(sprint.tasks)} tasks for sprint {sprint.name}")
            return True
        except Exception as e:
    ...
    def update_daily_progress(self, sprint_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            progress = {
                ...
                'completion_percentage': (completed / len(sprint.tasks)) * 100 if sprint.tasks else 0
            }
    ...
    def complete_sprint_workflow(self, sprint_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            retrospective = {
                ...
                'total_tasks': len(sprint.tasks),
                'completed_tasks': completed,
                'success_rate': (completed / len(sprint.tasks)) * 100 if sprint.tasks else 0,
