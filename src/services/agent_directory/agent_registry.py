...
class AgentRegistry:
    ...
    def get_active_agents(self) -> list[str]:
        ...
        return [
            agent_id for agent_id, agent in self.agents.items() if agent.get("status") == "ACTIVE"
        ]
    ...
    def _save_registry(self):
        ...
        try:
            registry_data = {"last_updated": datetime.now().isoformat(), "agents": self.agents}
...
def main():
    ...
    parser.add_argument(
        "--assign-role", nargs=2, metavar=("AGENT", "ROLE"), help="Assign role to agent"
    )
