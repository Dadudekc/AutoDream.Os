...
class AutonomousPerformanceMonitor:
    ...
    def __init__(self, system_id: str = "default-monitor"):
        ...
        self._metrics: Dict[MetricType, deque] = {
            metric: deque(maxlen=1000) for metric in MetricType
        }
    ...
    def _create_alert(self, metric: PerformanceMetric, threshold: float, severity: str):
        ...
        self.logger.warning(
            f"Performance alert: {alert.message} (value: {metric.value}, threshold: {threshold})"
        )

    def get_performance_summary(
        self, component: Optional[str] = None, agent_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def get_active_alerts(
        ...
        if severity:
            return [
                alert for alert in self._alerts.values() if alert.severity == severity
            ]
    ...
    def _perform_maintenance_tasks(self):
        ...
        old_alerts = [
            aid for aid, alert in self._alerts.items() if alert.timestamp < cutoff_time
        ]
...
def main():
    ...
    if args.test:
        ...
        monitor.record_metric(
            MetricType.RESPONSE_TIME, 1500, "test-component", "test-agent"
        )
        monitor.record_metric(
            MetricType.ERROR_RATE, 0.08, "test-component", "test-agent"
        )
