...
class HealthMonitorWebInterface:
    ...
    def __init__(self, health_monitor=None, config: Dict[str, Any] = None):
        ...
        self.app.config['SECRET_KEY'] = self.config.get('secret_key', 'health_monitor_secret')
        ...
        def get_thresholds():
            ...
            try:
                if self.health_monitor and hasattr(self.health_monitor, 'thresholds'):
                    thresholds = {}
                    for metric_type, threshold in self.health_monitor.thresholds.items():
            ...
            try:
                if self.health_monitor:
                    status = {
                        ...
                        'total_agents': len(self.health_monitor.health_data) if hasattr(self.health_monitor, 'health_data') else 0,
                        'active_alerts': len([a for a in self.health_monitor.alerts.values() if not a.resolved]) if hasattr(self.health_monitor, 'alerts') else 0,
    ...
    def _on_health_update(self, health_data, alerts):
        ...
        try:
            ...
            update_data = {
                ...
                'active_alerts': len([a for a in alerts if not a.resolved]) if alerts else 0
            }
