...
class HealthMonitor:
    ...
    def check_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if cpu_percent > self.health_thresholds['cpu_usage']:
                ...
                health_data['cpu_warning'] = f"CPU usage {cpu_percent}% exceeds threshold"
            
            if memory.percent > self.health_thresholds['memory_usage']:
                health_data['status'] = 'warning'
                health_data['memory_warning'] = f"Memory usage {memory.percent}% exceeds threshold"
            
            if disk.percent > self.health_thresholds['disk_usage']:
                health_data['status'] = 'critical'
                health_data['disk_warning'] = f"Disk usage {disk.percent}% exceeds threshold"
    ...
    def check_agent_health(self, agent_info: AgentInfo) -> Dict[str, Any]:
        ...
        try:
            ...
            if agent_info.performance_score < 0.5:
                ...
                health_data['recommendations'].append('Check agent workload and resources')
            ...
            self.logger.error(f"Agent health check error for {agent_info.agent_id}: {e}")
            return {
                'agent_id': agent_info.agent_id,
                'timestamp': time.time(),
                'status': 'error',
    ...
    def check_workspace_health(self, workspace_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if disk_usage.percent > self.health_thresholds['disk_usage']:
                ...
                health_data['warnings'].append(f'Disk usage {disk_usage.percent}% is high')
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        avg_cpu = sum(h.get('cpu_usage', 0) for h in recent_health) / len(recent_health) if recent_health else 0
        avg_memory = sum(h.get('memory_usage', 0) for h in recent_health) / len(recent_health) if recent_health else 0
