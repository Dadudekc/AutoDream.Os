...
class V2OnboardingLauncher:
    ...
    def start_agent_onboarding(self, agent_id: str) -> bool:
        ...
        try:
            ...
            if agent_id in self.onboarding_sessions:
                logger.warning(f"Agent {agent_id} already has an active onboarding session")
                return False
            ...
            if session_id:
                ...
                logger.info(f"✅ Onboarding started for {agent_id} with session {session_id}")
                return True
            else:
    ...
    def start_bulk_onboarding(self, agent_ids: List[str]) -> Dict[str, bool]:
        ...
        try:
            ...
            logger.info(f"✅ Bulk onboarding completed: {successful_count}/{len(agent_ids)} successful")
    ...
    def monitor_onboarding_progress(self, timeout: int = 300) -> Dict[str, Any]:
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                ...
                failed_sessions = [
                    agent_id for agent_id, status in progress_data.items()
...
def main():
    ...
    parser.add_argument("--full", action="store_true", help="Run full onboarding for all agents")
    ...
    parser.add_argument("--all-status", action="store_true", help="Get status for all agents")
    parser.add_argument("--cleanup", action="store_true", help="Clean up completed sessions")
