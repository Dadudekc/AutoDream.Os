...
class NLPPipeline:
    ...
    def generate_response(self, intent: Intent) -> Response:
        ...
        template = self.response_templates.get(
            intent.intent_type,
            {"text": "I understand. Let me help you with that.", "actions": ["process_request"]},
        )
        ...
        for action in response.actions:
            if action in self.command_handlers:
                try:
                    ...
                    action_results.append({"action": action, "status": "success", "result": result})
                except Exception as e:
                    action_results.append({"action": action, "status": "error", "error": str(e)})
...
if __name__ == "__main__":
    ...
    for test_input in test_inputs:
        ...
        print(
            f"Intent: {result['intent']['type']} (confidence: {result['intent']['confidence']:.2f})"
        )
