...
from ..config.browser_config import TheaConfig, SessionInfo, RateLimitStatus
        ...
        try:
            ...
            if rate_limit.requests_this_minute >= self.config.rate_limit_requests_per_minute:
                ...
                if time_since_last < 60:
                    return False, f"Rate limit exceeded: {rate_limit.requests_this_minute} requests per minute"
            
            # Check hour rate limit
            if rate_limit.requests_this_hour >= self.config.rate_limit_requests_per_hour:
                time_since_last = current_time - rate_limit.last_request_time
                if time_since_last < 3600:
                    return False, f"Rate limit exceeded: {rate_limit.requests_this_hour} requests per hour"
