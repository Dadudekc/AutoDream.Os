...
class HealthThresholdMonitoring:
    ...
    def check_health_status(
        ...
        try:
            ...
            if threshold_operations_count > 1000:
                health_status["warnings"].append(f"High number of threshold operations: {threshold_operations_count}")
                health_status["status"] = "warning"
            
            # Check validation operations
            if validation_operations_count > 500:
                health_status["warnings"].append(f"Large validation history: {validation_operations_count} records")
                if validation_operations_count > 1000:
                    health_status["status"] = "warning"
            
            # Check configuration changes
            if configuration_changes_count > 100:
                health_status["warnings"].append(f"High number of configuration changes: {configuration_changes_count}")
                health_status["status"] = "warning"
    ...
    def should_trigger_cleanup(
        ...
        return (
            threshold_operations_count > 2000 or
            validation_operations_count > 1500 or
            configuration_changes_count > 200
        )
