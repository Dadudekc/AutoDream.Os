...
class TechnicalAnalyzer:
    ...
    def calculate_technical_indicators(
        ...
        indicators["volume_ratio"] = (
            volumes[-1] / indicators["volume_sma"] if indicators["volume_sma"] > 0 else 1
        )
    ...
    def _calculate_bollinger_bands(self, prices: list[float], period: int = 20) -> dict[str, float]:
        ...
        return {"bb_upper": sma + (2 * std_dev), "bb_middle": sma, "bb_lower": sma - (2 * std_dev)}
            ...
            signals.append(
                {
                    ...
                    "signal": "sell",
                    "strength": "weak",
                    "reason": f"RSI approaching overbought at {rsi:.1f}",
                }
            )
        elif rsi < 40:
            signals.append(
            ...
            signals.append(
                {
                    ...
                    "signal": "buy",
                    "strength": "strong",
                    "reason": "Price above both SMAs (bullish trend)",
                }
            )
            ...
            signals.append(
                {
                    ...
                    "signal": "sell",
                    "strength": "strong",
                    "reason": "Price below both SMAs (bearish trend)",
                }
            )
    ...
    def display_technical_analysis(self, analysis: dict[str, Any]):
        ...
        for signal in signals_data["signals"]:
            ...
            print(
                f"  {strength_icon} {signal_icon} {signal['indicator']}: {signal['signal'].upper()} - {signal['reason']}"
            )
