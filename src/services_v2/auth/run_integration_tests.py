...
def run_basic_functionality_tests(auth_service: AuthService, logger: logging.Logger) -> dict:
    ...
    try:
        ...
        result = auth_service.authenticate_user_v2("admin", "secure_password_123", "127.0.0.1", "test_agent")
        ...
        if result.status.value == "SUCCESS":
            ...
            logger.error(f"❌ Basic Authentication: FAIL - Status: {result.status.value}")
    ...
    try:
        start_time = time.time()
        result = auth_service.authenticate_user_v2("admin", "wrong_password", "127.0.0.1", "test_agent")
        duration = time.time() - start_time
    ...
    try:
        ...
        if metrics and "total_attempts" in metrics:
            test_results["tests_passed"] += 1
            logger.info(f"✅ Performance Metrics: PASS - {metrics['total_attempts']} attempts")
        else:
            test_results["tests_failed"] += 1
    ...
    try:
        ...
        if security_status and "security_level" in security_status:
            test_results["tests_passed"] += 1
            logger.info(f"✅ Security Status: PASS - Level: {security_status['security_level']}")
        else:
            test_results["tests_failed"] += 1
        ...
        test_results["test_details"].append({
            ...
            "status": "PASS" if security_status and "security_level" in security_status else "FAIL",
            "duration": 0,
            "details": str(security_status) if security_status else "No security info"
        })
...
def run_performance_tests(auth_service: AuthService, logger: logging.Logger) -> dict:
    ...
    try:
        start_time = time.time()
        result = auth_service.authenticate_user_v2("admin", "secure_password_123", "127.0.0.1", "perf_test_agent")
        duration = time.time() - start_time
        ...
        for i in range(10):
            try:
                result = auth_service.authenticate_user_v2(
                    f"perf_user_{i}", "secure_password_123", f"192.168.1.{i}", "perf_test_agent"
                )
                if result.status.value == "SUCCESS":
                    successful_auths += 1
            except Exception:
    ...
    try:
        ...
        for i in range(20):
            try:
                result = auth_service.authenticate_user_v2(
                    f"error_user_{i}", "wrong_password", f"192.168.1.{i}", "error_test_agent"
                )
        ...
        if error_count < 20:  # Some errors are expected, but not all should be system errors
            test_results["tests_passed"] += 1
...
def run_comprehensive_integration_suite(logger: logging.Logger) -> dict:
    ...
    if not INTEGRATION_AVAILABLE:
        logger.warning("⚠️ Integration components not available, running basic tests only")
...
def run_basic_tests_only(logger: logging.Logger) -> dict:
    ...
    try:
        ...
        # Combine results
        total_tests = basic_results["tests_run"] + perf_results["tests_run"] + integration_results["tests_run"]
        total_passed = basic_results["tests_passed"] + perf_results["tests_passed"] + integration_results["tests_passed"]
        total_failed = basic_results["tests_failed"] + perf_results["tests_failed"] + integration_results["tests_failed"]
...
def generate_test_report(test_results: dict, logger: logging.Logger) -> dict:
    ...
    if success_rate < 90:
        report["recommendations"].append("Overall success rate below 90% - review system")
