...
class DiscoveryConfig:
    ...
    technology_patterns: Dict[str, List[str]] = field(
        default_factory=lambda: {
...
class IntelligentRepositoryScanner:
    ...
    def scan_repository(
        self, repository_path: str, deep_scan: bool = False
    ...
    def _detect_architecture_patterns(
        self, metadata: RepositoryMetadata, repo_path: Path
    ):
    ...
    def _analyze_performance(self, metadata: RepositoryMetadata, repo_path: Path):
        ...
        try:
            ...
            if file_sizes:
                ...
                performance_metrics["average_file_size"] = sum(file_sizes) / len(
                    file_sizes
                )
            ...
            performance_metrics["complexity_score"] = min(
                100, code_files * 2
            )  # Simple heuristic
    ...
    def _calculate_scores(self, metadata: RepositoryMetadata):
        ...
        try:
            ...
            logger.debug(
                f"Scores: health={metadata.health_score}, readiness={metadata.market_readiness}"
            )
        ...
        try:
            ...
            if metadata.health_score < 50:
                recommendations.append(
                    "Critical: Repository needs significant restructuring"
                )
                ...
                recommendations.append(
                    "Medium: Repository could benefit from minor improvements"
                )
            else:
            ...
            if "test-separation" not in metadata.architecture_patterns:
                recommendations.append(
                    "Add dedicated test directory and testing framework"
                )

            if "documentation-separation" not in metadata.architecture_patterns:
                recommendations.append(
                    "Create documentation directory with README and guides"
                )
            ...
            if metadata.security_analysis.get("security_score", 100) < 80:
                recommendations.append(
                    "Review and fix security issues identified in analysis"
                )

            # Technology stack recommendations
            if not metadata.technology_stack.framework:
                recommendations.append(
                    "Consider adding a framework for better structure and features"
                )

            if not metadata.technology_stack.database:
                recommendations.append(
                    "Evaluate if database integration would benefit the project"
                )
    ...
    def scan_multiple_repositories(
        self, repository_paths: List[str], max_workers: int = 4
    ...
    def get_discovery_summary(self) -> Dict[str, Any]:
        ...
        try:
            summary = {
                ...
                "total_size_mb": sum(
                    r.size_bytes for r in self.discovered_repositories.values()
                )
                / (1024 * 1024),
                "average_health_score": sum(
                    r.health_score for r in self.discovered_repositories.values()
                )
                / len(self.discovered_repositories)
                if self.discovered_repositories
                else 0,
                "average_market_readiness": sum(
                    r.market_readiness for r in self.discovered_repositories.values()
                )
                / len(self.discovered_repositories)
                if self.discovered_repositories
                else 0,
    ...
    try:
        if args.discover:
            ...
            if metadata:
                ...
                print(
                    f"  Architecture Patterns: {', '.join(metadata.architecture_patterns)}"
                )
            ...
            if repositories:
                ...
                print(
                    f"  Average Health Score: {summary['average_health_score']:.1f}/100"
                )
                print(
                    f"  Average Market Readiness: {summary['average_market_readiness']:.1f}/100"
                )
