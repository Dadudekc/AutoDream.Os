...
from src.services.api_manager import (
    APIManager, APIEndpoint, APIMethod, 
    LoggingMiddleware, RateLimitMiddleware, AuthenticationMiddleware
)
from src.services.middleware_orchestrator import (
    MiddlewareOrchestrator, MiddlewareChain, DataPacket,
    DataTransformationMiddleware, ValidationMiddleware, RoutingMiddleware
)
from src.services.service_registry import (
    ServiceRegistry, ServiceInfo, ServiceType, ServiceStatus,
    ServiceEndpoint, ServiceMetadata, HTTPHealthCheck, TCPHealthCheck
)
from src.services.integration_config_manager import (
    IntegrationConfigManager, ConfigSource, ConfigScope,
    TypeValidator, RangeValidator, RequiredValidator, PatternValidator
)
from src.services.integration_coordinator import (
    IntegrationCoordinator, IntegrationStatus, IntegrationMetrics
)
...
class TestIntegrationConfigManager:
    """Test suite for the Integration Configuration Manager component."""
    ...
    def config_manager(self, temp_config_dir):
        ...
        return IntegrationConfigManager(temp_config_dir)
    ...
    def test_config_manager_initialization(self, config_manager):
        ...
        assert not config_manager.watching_files
    
    def test_set_and_get_config_value(self, config_manager):
        ...
        value = config_manager.get_config_value('test.key')
        assert value == 'test_value'
        
        # Test with default value
        default_value = config_manager.get_config_value('nonexistent.key', 'default')
        assert default_value == 'default'
    
    def test_add_validation_rules(self, config_manager):
        """Test adding validation rules to configuration keys."""
        config_manager.add_validation_rules('test.port', {
            'type': 'integer',
        ...
        value = config_manager.get_config_value('test.port')
        assert value == 8000
    
    def test_get_config_section(self, config_manager):
        """Test retrieving configuration sections."""
        config_manager.set_config_value('api.host', 'localhost', ConfigSource.FILE, ConfigScope.GLOBAL)
        config_manager.set_config_value('api.port', 8000, ConfigSource.FILE, ConfigScope.GLOBAL)
        config_manager.set_config_value('database.host', 'db.local', ConfigSource.FILE, ConfigScope.GLOBAL)
        
        api_config = config_manager.get_config_section('api')
        assert api_config['host'] == 'localhost'
        assert api_config['port'] == 8000
        assert 'database.host' not in api_config
    
    def test_export_config(self, config_manager):
        """Test exporting configuration in different formats."""
        config_manager.set_config_value('test.key', 'test_value', ConfigSource.FILE, ConfigScope.GLOBAL)
        
        # Export as JSON
        json_config = config_manager.export_config('json')
        assert 'test.key' in json_config
        assert json.loads(json_config)['test.key']['value'] == 'test_value'
        
        # Export as YAML
        yaml_config = config_manager.export_config('yaml')
        assert 'test.key' in yaml_config
    
    def test_load_config_file(self, config_manager, temp_config_dir):
...
database:
  ...
  port: 5432
        ...
        config_manager.load_config_file(str(config_file))
        
        assert config_manager.get_config_value('api.host') == 'localhost'
        assert config_manager.get_config_value('api.port') == 8000
        assert config_manager.get_config_value('database.host') == 'db.local'
