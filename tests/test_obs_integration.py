...
class TestOBSVirtualCameraIntegration:
    ...
    def test_connect_to_obs_virtual_camera_fallback(self, obs_integration):
        ...
        with patch.object(obs_integration, '_connect_obs_websocket', return_value=False):
            with patch.object(obs_integration, '_setup_virtual_camera', return_value=True):
                result = obs_integration.connect_to_obs()
    ...
    def test_connect_to_obs_failure(self, obs_integration):
        ...
        with patch.object(obs_integration, '_connect_obs_websocket', return_value=False):
            with patch.object(obs_integration, '_setup_virtual_camera', return_value=False):
                result = obs_integration.connect_to_obs()
    ...
    def test_push_frame_to_obs_virtual_camera_success(self, obs_integration, mock_frame):
        """Test successful frame push via virtual camera"""
        obs_integration.is_connected = True
        obs_integration.obs_websocket = None
        obs_integration.virtual_camera = Mock()
        
        with patch.object(obs_integration, '_push_frame_virtual_camera', return_value=True):
            result = obs_integration.push_frame_to_obs(mock_frame)
    ...
    def test_output_directory_creation(self, obs_integration):
        ...
        assert obs_integration.config.get("output_directory") is not None or "temp_directory" in obs_integration.config
...
class TestOBSIntegrationPerformance:
    ...
    def test_frame_push_performance(self, performance_obs_integration):
        ...
        with patch.object(performance_obs_integration, '_push_frame_virtual_camera', return_value=True):
            for _ in range(100):
    ...
    def test_buffer_performance(self, performance_obs_integration):
        ...
        assert len(performance_obs_integration.frame_buffer) <= performance_obs_integration.config["buffer_size"]
...
class TestOBSIntegrationErrorHandling:
    ...
    def test_virtual_camera_error_handling(self, error_obs_integration):
        ...
        with patch('web.multimedia.obs_integration.virtual_camera') as mock_virtual_camera:
    ...
    def test_frame_push_error_handling(self, error_obs_integration, mock_frame):
        ...
        with patch.object(error_obs_integration, '_push_frame_virtual_camera', side_effect=Exception("Push error")):
    ...
    def test_streaming_error_handling(self, error_obs_integration):
        ...
        with patch.object(error_obs_integration, 'push_frame_to_obs', return_value=True):
