...
class OrphanCleanup:
    ...
    def confirm_removal(self) -> bool:
        ...
        print("Removing them will clean up the repository without affecting functionality.")
        print()
        
        response = input("Do you want to proceed with removal? (yes/no): ").lower().strip()
                ...
                for dir_name in dirs:
                    ...
                    try:
                        if not any(dir_path.iterdir()):  # Directory is empty
                            dir_path.rmdir()
                            print(f"  🗑️  Removed empty directory: {dir_path.relative_to(self.repo_root)}")
                    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean up orphaned files in Agent Cellphone V2")
