...
class Agent5Phase1BISupportAdvanced(Agent5Phase1BISupportCore):
    ...
    def _analyze_quality_metrics(self, active_alerts: List) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_alerts = sum(1 for alert in active_alerts if hasattr(alert, 'alert_level') and alert.alert_level == "CRITICAL")
            warning_alerts = sum(1 for alert in active_alerts if hasattr(alert, 'alert_level') and alert.alert_level == "WARNING")
