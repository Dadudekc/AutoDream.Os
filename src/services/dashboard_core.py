...
class DashboardCore:
    ...
    def validate_layout(self) -> bool:
        ...
        max_height = max(widget.height for widget in self.widgets) if self.widgets else 0
        
        if total_width > self.layout.columns:
            logger.warning(f"Total widget width ({total_width}) exceeds layout columns ({self.layout.columns})")
            return False
        
        if max_height > self.layout.rows:
            logger.warning(f"Widget height ({max_height}) exceeds layout rows ({self.layout.rows})")
            return False
        ...
        if 'widgets' in config:
            ...
            for widget_config in config['widgets']:
                try:
                    ...
                    logger.error(f"Failed to import widget {widget_config.get('widget_id', 'unknown')}: {e}")
