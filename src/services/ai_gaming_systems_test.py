...
class AIGamingSystemsTest:
    ...
    def test_message_queuing(self):
        ...
        try:
            ...
            print(f"✅ Queue status: Regular={status['regular_queue_size']}, High={status['high_priority_queue_size']}")
    ...
    def test_message_delivery(self):
        ...
        try:
            ...
            test_message = f"[TEST] AI Gaming Systems Test - {datetime.now().strftime('%H:%M:%S')}"
            message_id = self.mq.send_message(
                sender_agent="TEST_SYSTEM",
    ...
    def test_broadcast_system(self):
        ...
        try:
            ...
            broadcast_messages = [msg for msg in self.mq.message_history[-len(message_ids):] 
                               if msg.get('message', {}).get('sender_agent') == 'TEST_SYSTEM']
            
            successful_deliveries = sum(1 for msg in broadcast_messages 
                                     if 'sent' in msg.get('status', ''))
    ...
    def test_high_priority_system(self):
        ...
        try:
            ...
            high_priority_deliveries = [msg for msg in self.mq.message_history[-len(message_ids):] 
                                      if 'high_priority' in msg.get('status', '')]
            
            if len(high_priority_deliveries) == len(message_ids):
                print(f"✅ All {len(high_priority_deliveries)} high priority messages processed")
                self.test_results['high_priority_system'] = True
                return True
            else:
                print(f"❌ High priority processing incomplete: {len(high_priority_deliveries)}/{len(message_ids)}")
                self.test_results['high_priority_system'] = False
                return False
    ...
    def test_system_health(self):
        ...
        try:
            ...
            print(f"✅ Queue status: Regular={queue_status['regular_queue_size']}, High={queue_status['high_priority_queue_size']}")
