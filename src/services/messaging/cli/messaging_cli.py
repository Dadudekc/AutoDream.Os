...
class MessagingCLI:
    ...
    def create_parser(self) ->argparse.ArgumentParser:
        ...
        parser = argparse.ArgumentParser(description=
            'Unified Messaging CLI', formatter_class=argparse.
...
Examples:
  ...
  python -m messaging.cli history --agent Agent-1
        ...
        subparsers = parser.add_subparsers(dest='command', help=
            'Available commands')
        send_parser = subparsers.add_parser('send', help=
            'Send message to agent')
        send_parser.add_argument('--message', '-m', required=True, help=
            'Message content')
        send_parser.add_argument('--recipient', '-r', required=True, help=
            'Recipient agent')
        send_parser.add_argument('--priority', '-p', choices=['low',
            'normal', 'high', 'urgent'], default='normal', help=
            'Message priority')
        send_parser.add_argument('--type', '-t', choices=['direct',
            'broadcast', 'system'], default='direct', help='Message type')
        send_parser.add_argument('--sender', '-s', default='CLI', help=
            'Sender identifier')
        broadcast_parser = subparsers.add_parser('broadcast', help=
            'Broadcast message to all agents')
        broadcast_parser.add_argument('--message', '-m', required=True,
            help='Message content')
        broadcast_parser.add_argument('--priority', '-p', choices=['low',
            'normal', 'high', 'urgent'], default='normal', help=
            'Message priority')
        broadcast_parser.add_argument('--sender', '-s', default='CLI', help
            ='Sender identifier')
        history_parser = subparsers.add_parser('history', help=
            'Show message history')
        history_parser.add_argument('--agent', '-a', help=
            'Agent to show history for')
        ...
        start_parser.add_argument('--dry-run', action='store_true', help=
            'Run in dry-run mode (no actual clicking/pasting)')
        start_parser.add_argument('--agent', '-a', help=
            'Specific agent to onboard (default: all agents)')
        return parser
    ...
    def _handle_send_message(self, args) ->int:
        ...
        try:
            message = UnifiedMessage(content=args.message, recipient=args.
                recipient, sender=args.sender, message_type=
                UnifiedMessageType(args.type.upper()), priority=
                UnifiedMessagePriority(args.priority.upper()))
    ...
    def _handle_broadcast_message(self, args) ->int:
        ...
        try:
            results = self.messaging_service.broadcast_message(content=args
            ...
            if successful > 0:
                ...
                logger.info(f'❌ Failed to send broadcast to any agents')
                return 1
        except Exception as e:
    ...
    def _handle_start_onboarding(self, args) ->int:
        ...
        try:
            return self.messaging_service.start_agent_onboarding(dry_run=
                args.dry_run, specific_agent=args.agent)
        except Exception as e:
...
if __name__ == '__main__':
    ...
    logger.info(
        'Example: python -m src.services.messaging.consolidated_messaging_service'
        )
