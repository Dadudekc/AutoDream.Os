...
class OnboardingContextBuilder:
    ...
    def create_context_summary(self) -> str:
        ...
        if "system_health" in health:
            ...
            lines.append(f"  Active Agents: {sh.get('active_agents', 0)}/{sh.get('total_agents', 0)}")
            lines.append("")
        ...
        for agent in context["agent_statuses"]:
            ...
            lines.append(f"  {status_emoji} {agent['agent_id']}: {agent['status']} | {agent['role']}")
        lines.append("")
...
def main():
    ...
    parser.add_argument("--context-only", action="store_true", help="Only generate context, don't send message")
