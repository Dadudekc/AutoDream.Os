...
class DiscordConfig:
    ...
    def _get_intents(self) -> discord.Intents | None:
    ...
    def validate(self) -> list[str]:
        ...
        if self.token == "development_token_placeholder":
            issues.append(
                "DISCORD_BOT_TOKEN is set to development placeholder - update with real token for production"
            )
        elif not self.token:
        ...
        if self.guild_id == "development_guild_placeholder":
            issues.append(
                "DISCORD_GUILD_ID is set to development placeholder - update with real guild ID for production"
            )
...
class DiscordConnectionManager:
    ...
    async def create_bot(self) -> commands.Bot | None:
