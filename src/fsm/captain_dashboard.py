...
from .fsm_registry import read_agent_state, read_swarm_state, get_state_summary
...
class CaptainDashboard:
        ...
        for agent_id, data in report.items():
            if data["is_inactive"] or data["is_messaging_inactive"]:
                inactive_agents.append({
                    "agent_id": agent_id,
                    "status": data["status"],
                    "reason": data["inactivity_reason"] if data["is_inactive"] else data["messaging_reason"],
    ...
    def get_swarm_health_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "health_status": "HEALTHY" if health_percentage >= 75 else "DEGRADED" if health_percentage >= 50 else "CRITICAL"
        }
...
"""
        ...
        if inactive_agents:
            ...
            report += f"• No immediate actions required\n"
