...
class ProgressTrackingAdvanced:
    ...
    async def _initialize_web_interface(self):
        ...
        try:
            ...
            web_monitor_thread = threading.Thread(target=self._monitor_web_interface, daemon=True)
        ...
        try:
            while self.notification_queue:
                ...
                if len(self.notification_queue) > self.web_interface_config["max_notifications"]:
                    self.notification_queue = self.notification_queue[
                        -self.web_interface_config["max_notifications"] :
                    ]
    ...
    def queue_notification(self, event_type: str, data: dict[str, Any], priority: str = "normal"):
        ...
        try:
            notification = NotificationEvent(event_type=event_type, data=data, priority=priority)
    ...
    def update_progress_with_notifications(
        self,
        phase_name: str,
        phase: ProgressPhase,
        tasks_completed: int,
        ...
        try:
            ...
            success = self.core.update_progress(
                phase_name, phase, tasks_completed, tasks_total, quality_score, velocity, blockers
            )
