...
class SystemDuplicationAnalysis:
    ...
    def calculate_consolidation_metrics(self) -> dict[str, Any]:
        ...
        reduction_percentage = (
            (files_reduced / total_files_before * 100) if total_files_before > 0 else 0
        )
        ...
        pending_consolidations = len(
            [dup for dup in self.duplication_analysis if dup.status == ConsolidationStatus.PENDING]
        )
        total_effort = sum(dup.consolidation_effort for dup in self.duplication_analysis)
        remaining_effort = sum(
            dup.consolidation_effort
            for dup in self.duplication_analysis
                ...
                recommendations.append(
                    {
                        ...
                        "effort_hours": dup.consolidation_effort,
                        "ssot_benefit": "Eliminates SSOT violation"
                        if dup.ssot_violation
                        else "Improves maintainability",
                    }
                )
...
if __name__ == "__main__":
    ...
    for dup in plan["duplication_analysis"]:
        ...
        print(
            f"  {status_icon} {dup['type'].replace('_', ' ').title()}: {dup['files_before']} → {dup['files_after']} files ({dup['status']})"
        )

    print("\n📋 Consolidation Priorities:")
    for i, priority in enumerate(plan["consolidation_priorities"], 1):
        print(
            f"  {i}. {priority['duplication_type'].replace('_', ' ').title()}: {priority['priority_score']} points ({priority['impact_level']} impact)"
        )
