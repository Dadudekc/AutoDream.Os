...
class LoadBalancer:
    ...
    def get_next_server(self) -> BackendServer | None:
...
class APIMonitor:
    ...
    def record_request(
        self, endpoint: str, method: str, status_code: int, response_time_ms: float, client_ip: str
    ):
    ...
    def _check_alerts(self, metrics: RequestMetrics):
        ...
        if metrics.status_code >= 400:
            self._create_alert(
                "error_response", f"Error response: {metrics.status_code} for {metrics.endpoint}"
            )
