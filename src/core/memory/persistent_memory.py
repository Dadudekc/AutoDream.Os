...
class PersistentMemoryManager:
    ...
    def set(self, key: str, value: Any, compress: bool = True) -> bool:
        ...
        try:
            with self._lock:
                ...
                compressed_data = (
                    self._compress_data(value) if compress else json.dumps(value).encode("utf-8")
                )
                ...
                # Update cache
                self.cache[key] = {"value": value, "updated_at": now, "access_count": 0}
    ...
    def cleanup(self, max_age_days: int = 30) -> int:
        ...
        try:
            cutoff_date = (
                datetime.now(UTC).replace(day=datetime.now(UTC).day - max_age_days).isoformat()
            )
...
def list_memory_keys(pattern: str | None = None) -> list[str]:
    """List all keys in persistent memory."""
