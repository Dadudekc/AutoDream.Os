...
class AIDevWorkflow:
    ...
    def _get_ai_suggestions(self, step: WorkflowStep, output: str, **kwargs) -> List[str]:
    ...
    def _get_openai_suggestions(self, step: WorkflowStep, output: str, **kwargs) -> List[str]:
...
Format as a JSON list of strings."""
    ...
    def _get_anthropic_suggestions(self, step: WorkflowStep, output: str, **kwargs) -> List[str]:
...
Format as a JSON list of strings."""
        ...
        for py_file in self.project_path.rglob("*.py"):
            if not py_file.name.startswith("test_"):
                try:
                    ...
                    analysis_results.append(f"{py_file.name}: Complexity={analysis.complexity_score}, Maintainability={analysis.maintainability_index}")
                except Exception as e:
            ...
            if not py_file.name.startswith("test_"):
                try:
                    ...
                    if analysis.code_smells or analysis.complexity_score > 5:
                        suggestions.append(f"{py_file.name}: {', '.join(analysis.code_smells)}")
                except Exception as e:
                    logger.warning(f"Failed to analyze {py_file}: {e}")
        ...
        for py_file in self.project_path.rglob("*.py"):
            if not py_file.name.startswith("test_"):
                try:
                    ...
                    logger.warning(f"Failed to generate documentation for {py_file}: {e}")
    ...
    def analyze_project(self) -> ProjectAnalysis:
        ...
        test_files = len([f for f in self.project_path.rglob("*.py") if "test" in f.name.lower()])
        ...
        code_quality_score = sum(quality_scores) / len(quality_scores) if quality_scores else 0
        
        # Test coverage estimation
        test_coverage = (test_files / max(code_files, 1)) * 100 if code_files > 0 else 0
        
        # Documentation coverage
        documentation_coverage = (documentation_files / max(code_files, 1)) * 100 if code_files > 0 else 0
