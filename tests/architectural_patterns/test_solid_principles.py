        ...
        try:
            ...
            if total_methods > 0:
                messaging_methods = [
                    m
                    for m in methods
                    if "message" in m.lower() or "send" in m.lower() or "receive" in m.lower()
                ]
                messaging_ratio = len(messaging_methods) / total_methods
                assert messaging_ratio > 0.5, (
                    f"Service should be >50% messaging focused, got {messaging_ratio:.2f}"
                )
        ...
        try:
            ...
            class ExtendedMessagingService(ConsolidatedMessagingService):
                ...
                def send_priority_message(
                    self, agent_id: str, message: str, priority: str = "HIGH"
                ):
