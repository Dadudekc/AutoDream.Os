...
class AggressiveDocumentationCleanupSystem:
    ...
    def __init__(
        self, base_path: str = ".", strategy: CleanupStrategy = CleanupStrategy.AGGRESSIVE
    ):
    ...
    def create_aggressive_cleanup_plan(self) -> dict[str, Any]:
        ...
        # Calculate reduction
        total_files = len(self.cleanup_files)
        files_to_keep = len(self.cleanup_plan["keep_critical"]) + len(
            self.cleanup_plan["keep_recent"]
        )
        files_to_consolidate = len(self.cleanup_plan["consolidate"])
        files_to_delete = len(self.cleanup_plan["delete_aggressive"]) + len(
    ...
    def _perform_cleanup(self) -> dict[str, Any]:
        ...
        for file_path in (
            self.cleanup_plan["delete_aggressive"] + self.cleanup_plan["delete_nuclear"]
        ):
            try:
                os.remove(file_path)
                deleted_count += 1
...
def create_aggressive_cleanup_system(
    strategy: CleanupStrategy = CleanupStrategy.AGGRESSIVE,
) -> AggressiveDocumentationCleanupSystem:
    ...
    print(
        f"📋 Aggressive cleanup plan ready: {plan_results['reduction_percentage']}% reduction planned"
    )
