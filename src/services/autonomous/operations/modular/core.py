...
class AutonomousOperationsCore:
    ...
    async def run_autonomous_operations(self) -> dict[str, Any]:
        ...
        try:
            ...
            for operation in available_operations:
                ...
                try:
                    ...
                    if result.get("success", False):
                        ...
                        logger.info(
                            f"{self.agent_id}: Operation '{operation.get('name', 'Unknown')}' completed successfully"
                        )
                    else:
                        operations_results["operations_failed"] += 1
                        logger.warning(
                            f"{self.agent_id}: Operation '{operation.get('name', 'Unknown')}' failed: {result.get('error', 'Unknown error')}"
                        )

                    operations_results["details"].append(
                        {"operation": operation.get("name", "Unknown"), "result": result}
                    )

                except Exception as e:
                    operations_results["operations_failed"] += 1
                    logger.error(
                        f"{self.agent_id}: Operation '{operation.get('name', 'Unknown')}' failed with exception: {e}"
                    )
            ...
            await self.operations_manager.update_operations_with_results(
                available_operations, operations_results["details"]
            )

            logger.info(
                f"{self.agent_id}: Autonomous operations completed - {operations_results['operations_successful']}/{operations_results['operations_run']} successful"
            )
