...
class EngineMetrics:
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                (self.success_count / self.operation_count) if self.operation_count > 0 else 0
            ),
...
class EngineHealthMonitor:
    ...
    def perform_health_check(self) -> dict[str, Any]:
        ...
        critical_checks = [
            check for check in health_status["checks"].values() if check["status"] == "critical"
        ]
        warning_checks = [
            check for check in health_status["checks"].values() if check["status"] == "warning"
        ]
