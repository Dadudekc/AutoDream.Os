...
class APIKeyManager(BaseManager):
    ...
    def list_providers(self) -> List[str]:
        """List all available API key providers"""
        try:
            if not self.running:
                self.logger.warning("API Key Manager is not running")
                return []
            
            # Return list of services that have valid keys
            providers = []
            for service in self._keys:
                if self._keys.get(service) and self._validate_key(service, self._keys[service]):
