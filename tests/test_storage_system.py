...
from src.core.storage.storage_types import (
    StorageType, DataIntegrityLevel, BackupStrategy, 
    StorageMetadata, StorageConfig
)
...
class TestStorageBackupManager:
    ...
    def test_create_backup_duplicate_name(self, backup_manager, temp_data_dir):
        ...
        # Create first backup
        backup_name = backup_manager.create_backup(Path(temp_data_dir), "duplicate_name")
        assert backup_name == "duplicate_name"
        
        # Try to create second backup with same name
        second_backup_name = backup_manager.create_backup(Path(temp_data_dir), "duplicate_name")
