...
class OnboardingHandler:
    def can_handle(self, args) -> bool:
        ...
        return (
            ...
            and args.onboarding
            or hasattr(args, "onboard")
            and args.onboard
            or hasattr(args, "hard_onboarding")
    ...
    def handle_onboarding_commands(self, args) -> bool:
        if getattr(args, "hard_onboarding", False):
            ...
            if hasattr(args, "agent_subset") and args.agent_subset:
                agents = [a.strip() for a in args.agent_subset.split(",") if a.strip()] or None
    ...
    def _handle_hard_onboarding(
        ...
        for agent in target_agents:
            ...
            try:
                if use_ui:
                    ...
                    ok = ui_onboarder.perform(agent_id=agent, coords=coords, message=message)
                else:
                    if not dry_run:
                ...
                if ok:
                    ...
                    logger.info(f"❌ {agent}: {role} onboarding failed verification")
        ...
        if emit_proof:
            try:
                ...
                if dry_run:
                    logger.info("🧪 DRY-RUN: Skipping proof emission (simulated).")
                else:
        ...
        logger.info(f"📊 Role-based onboarding complete: {ok_count}/{total} agents ready")
        logger.info(
