...
class WorkspaceSecurityManager:
    ...
    def create_security_policy(
        ...
        permissions: Dict[str, List[Permission]] = None,
    ) -> bool:
        """Create a security policy for a workspace"""
        try:
            if workspace_name in self.security_policies:
                self.logger.warning(
                    f"Security policy for {workspace_name} already exists"
                )
                return False
        ...
        try:
            ...
            if agent_id not in policy.allowed_agents:
                self._log_access_attempt(
                    workspace_name, agent_id, f"check_{permission.value}", False
                )
                return False
            ...
            if agent_id in policy.permissions:
                if permission in policy.permissions[agent_id]:
                    self._log_access_attempt(
                        workspace_name, agent_id, f"check_{permission.value}", True
                    )
                    return True
    ...
    def _log_access_attempt(
        self, workspace_name: str, agent_id: str, action: str, success: bool
    ):
    ...
    def get_security_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_failures = len(
                [log for log in self.access_logs[-100:] if not log.success]
            )
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            has_permission = self.check_access_permission(
            ...
            # Test denied access
            no_permission = self.check_access_permission(
                test_workspace, "unauthorized_agent", Permission.READ
            )
            if no_permission:
                return False
...
def run_smoke_test():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test security policy creation
            success = manager.create_security_policy(
                "test_workspace", SecurityLevel.PRIVATE, ["Agent-1"]
            )
            assert success

            # Test access permission check
            has_permission = manager.check_access_permission(
                "test_workspace", "Agent-1", Permission.READ
            )
            assert has_permission

            # Test denied access
            no_permission = manager.check_access_permission(
                "test_workspace", "Agent-2", Permission.READ
            )
...
def main():
    ...
    parser.add_argument(
        "--create",
        nargs=3,
        metavar=("WORKSPACE", "LEVEL", "AGENTS"),
        help="Create security policy",
    )
    parser.add_argument(
        "--check",
        nargs=3,
        metavar=("WORKSPACE", "AGENT", "PERMISSION"),
        help="Check access permission",
    )
    ...
    if args.create:
        ...
        try:
            ...
            success = manager.create_security_policy(
                workspace, security_level, agent_list
            )
