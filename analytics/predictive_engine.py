...
class CapacityForecast:
    ...
    time_to_limit: Optional[timedelta]
...
class LoadForecastingModel:
    ...
    def _generate_load_recommendations(self, predicted_cpu: float, predicted_memory: float, trend: str) -> List[str]:
        ...
        if predicted_cpu > 90 or predicted_memory > 90:
            recommendations.append("URGENT: Predicted resource exhaustion - immediate scaling required")
...
class CapacityPlanningModel:
    ...
    def forecast_capacity(self, current_metrics: PerformanceMetrics, 
                         growth_rate: float = 0.1) -> List[CapacityForecast]:
        """Forecast capacity requirements."""
        forecasts = []
        
        # CPU forecast
        cpu_forecast = self._forecast_resource(
            "CPU", current_metrics.cpu_usage, growth_rate
        )
        ...
        disk_forecast = self._forecast_resource(
            "Disk", current_metrics.disk_io, growth_rate
        )
    ...
    def _forecast_resource(self, resource_type: str, current_usage: float, 
                          growth_rate: float) -> CapacityForecast:
...
class AnomalyDetectionModel:
    ...
    def detect_anomalies(self, current_metrics: PerformanceMetrics) -> List[PredictionResult]:
        ...
        for metric_name, current, baseline in metrics_to_check:
            ...
            if deviation > self.threshold:
                ...
                anomalies.append(PredictionResult(
    ...
    def _generate_anomaly_recommendations(self, metric_name: str, current: float, 
                                        baseline: float, deviation: float) -> List[str]:
...
class PredictiveAnalyticsEngine:
                ...
                "capacity_forecasts": [
                    {
                        ...
                        "time_to_limit": f.time_to_limit.total_seconds() / 3600 if f.time_to_limit else None,
    ...
    def _calculate_overall_health(self, metrics: PerformanceMetrics, 
                                 load_prediction: PredictionResult, 
                                 anomalies: List[PredictionResult]) -> Dict[str, Any]:
