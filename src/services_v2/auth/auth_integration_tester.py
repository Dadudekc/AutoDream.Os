...
class AuthIntegrationTester:
    ...
    def _default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "test_passwords": ["secure_password_123", "wrong_password", "empty_password"],
    ...
    def run_comprehensive_integration_tests(self) -> IntegrationTestReport:
        ...
        if not self.initialize_test_environment():
            return self._create_error_report("Environment initialization failed", start_time)
    ...
    def _test_core_authentication(self):
        ...
        test_cases = [
            ("Valid Admin Credentials", "admin", "secure_password_123", "127.0.0.1", True),
        ...
        for test_name, username, password, source_ip, expected_success in test_cases:
            ...
            try:
                ...
                if expected_success and result.status == AuthStatus.SUCCESS:
                    self._add_test_result(test_name, "PASS", duration, {
                    ...
                    self._add_test_result(test_name, "FAIL", duration, {
    ...
    def _test_security_features(self):
        ...
        for test_name, test_function in test_cases:
            ...
            try:
                test_function()
                duration = time.time() - start_time
                self._add_test_result(test_name, "PASS", duration, {"security_features": "validated"})
    ...
    def _test_security_context(self):
        ...
        result = self.auth_service.authenticate_user_v2(
            "admin", "secure_password_123", "127.0.0.1", "test_agent", suspicious_context
        )
        
        # Should still succeed but with security events
        if result.status != AuthStatus.SUCCESS:
            raise Exception("Authentication should succeed even with suspicious context")
    ...
    def _test_performance(self):
        ...
        self._add_test_result("Single Auth Performance", "PASS", single_auth_time, {
    ...
    def _test_concurrent_performance(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_concurrent) as executor:
            ...
            for i in range(num_concurrent):
                future = executor.submit(
                    ...
                    f"agent-{i}", "secure_password_123", f"192.168.1.{i}", "test_agent"
                )
        ...
        self._add_test_result("Concurrent Auth Performance", "PASS", total_time, {
    ...
    def _test_message_queue_integration(self):
        ...
        try:
            ...
            self.message_queue.register_agent("auth_test_agent", "Auth Test Agent", ["authentication"])
            ...
            if msg_id:
                ...
                if messages and len(messages) > 0:
                    duration = time.time() - start_time
                    self._add_test_result("Message Queue Integration", "PASS", duration, {
                        "message_id": msg_id,
            ...
            duration = time.time() - start_time
            self._add_test_result("Message Queue Integration", "ERROR", duration, {"error": str(e)})
    ...
    def _test_agent_coordinator_integration(self):
        ...
        try:
            ...
            if hasattr(self.agent_coordinator, 'get_system_status'):
                ...
                self._add_test_result("Agent Coordinator Integration", "PASS", duration, {
            ...
            duration = time.time() - start_time
            self._add_test_result("Agent Coordinator Integration", "ERROR", duration, {"error": str(e)})
    ...
    def _test_stress_scenarios(self):
        ...
        try:
            for i in range(50):  # Stress test with 50 rapid attempts
                result = self.auth_service.authenticate_user_v2(
                    f"stress_user_{i}", "wrong_password", f"192.168.1.{i}", "stress_test_agent"
                )
            ...
            duration = time.time() - start_time
            self._add_test_result("Stress Testing", "PASS", duration, {
                "stress_attempts": 50,
            ...
            duration = time.time() - start_time
            self._add_test_result("Stress Testing", "ERROR", duration, {"error": str(e)})
    ...
    def _add_test_result(self, test_name: str, status: str, duration: float, details: Dict[str, Any]):
    ...
    def _generate_test_report(self, start_time: datetime, end_time: datetime) -> IntegrationTestReport:
        ...
        integration_status = {
            ...
            "core_security_components": getattr(self.auth_service, 'auth_system', None) is not None if self.auth_service else False
        }
    ...
    def _create_error_report(self, error_message: str, start_time: datetime) -> IntegrationTestReport:
