...
class HealthMonitor:
    ...
    def _create_alert(
        self, component_id: str, metric_name: str, alert_level: AlertLevel, message: str
    ):
        ...
        with self.monitor_lock:
            for component_id, metrics in self.health_metrics.items():
                ...
                if len(self.health_history[component_id]) > 1000:
                    self.health_history[component_id] = self.health_history[
                        component_id
                    ][-1000:]
    ...
    def get_component_health(self, component_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.monitor_lock:
            ...
            return {
                ...
                "last_updated": max(metric.timestamp for metric in metrics.values())
                if metrics
                else 0,
            }
