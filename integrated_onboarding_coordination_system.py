...
class IntegratedOnboardingCoordinationSystem:
    ...
    def __init__(self):
        ...
        # Onboarding status
        self.onboarding_status = dict.fromkeys(self.swarm_agents, False)
        ...
        try:
            for agent_id in self.swarm_agents:
                ...
                if status_file.exists():
                    ...
                    if (
                        status_data.get("onboarding_status") == "COMPLETED"
                        or status_data.get("status") == "onboarded"
                        or status_data.get("status") == "OPERATIONAL"
                    ):
...
• Establish operational coordination with all agents""",
        ...
        return guidance_map.get(
            agent_id, "• Initialize general workspace and prepare for swarm coordination"
        )
...
"""
        ...
        if not is_onboarded:
            base_prompt += """🚨 ONBOARDING REQUIRED:
...
"""
    ...
    def run_enhanced_cycle(self) -> dict[str, bool]:
        ...
        logger.info(
            f"🔄 Starting enhanced cycle #{self.cycle_count + 1} at {cycle_start.strftime('%H:%M:%S')}"
        )
        ...
        logger.info(
            f"📊 Enhanced cycle #{self.cycle_count + 1} completed: {successful}/{total} agents in {cycle_duration:.1f}s"
        )
        self.cycle_count += 1
    ...
    def run_cli(self, args: list[str] | None = None) -> None:
        ...
        parser.add_argument(
            "--agent", "-a", help="Target specific agent for cycle message or onboarding"
        )
        parser.add_argument("--onboard-agent", help="Onboard specific agent with role")
        parser.add_argument(
            "--contract-status",
            "-c",
            action="store_true",
        ...
        parser.add_argument(
            "--fsm-status", "-f", action="store_true", help="Show FSM status for all agents"
        )
        parser.add_argument(
            "--onboarding-status", action="store_true", help="Show onboarding status for all agents"
        )
        parser.add_argument(
            "--create-contract",
        ...
        if parsed_args.onboarding_status:
            ...
            for agent_id in self.swarm_agents:
                status = (
                    "✅ ONBOARDED" if self.onboarding_status.get(agent_id, False) else "❌ PENDING"
                )
        ...
        if parsed_args.contract_status:
            ...
            if self.active_contracts:
                for agent_id, contract in self.active_contracts.items():
                    logger.info(
                        f"{agent_id}: {contract.contract_type.value} - {contract.description}"
                    )
                    logger.info(
                        f"  Progress: {contract.progress_percentage}% | Status: {contract.status} | Cycles: {contract.estimated_cycles}"
                    )
            else:
                ...
                if self.contract_history:
                    ...
                    for contract in self.contract_history[-5:]:  # Show last 5
                        logger.info(
                            f"  {contract.agent_id}: {contract.contract_type.value} - {contract.description}"
                        )
            return
        ...
        if parsed_args.fsm_status:
            ...
            for agent_id, fsm in self.agent_fsms.items():
                ...
                logger.info(
                    f"{agent_id}: {state_info['current_state']} (transitions: {state_info['transition_count']})"
                )
            return
        ...
        if parsed_args.create_contract:
            ...
            try:
                ...
                if len(parts) == 4:
                    ...
                    logger.error(
                        "❌ Invalid contract format. Use: agent_id:type:description:cycles"
                    )
            except Exception as e:
