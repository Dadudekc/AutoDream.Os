...
class ComplexityAnalyzer:
    ...
    def get_metric(self, component_name: str) -> ComplexityMetric | None:
    ...
    def check_compliance(self, component_name: str, rule_name: str) -> bool:
        ...
        if metric.method_count > rule.max_methods:
            violations.append(
                f"Method count {metric.method_count} exceeds limit {rule.max_methods}"
            )
...
class SimplicityEnforcer:
    ...
    def __init__(self):
        ...
        self._analyzer.add_rule(
            SimplicityRule(
                rule_name="v2_compliance", max_lines=400, max_enums=3, max_classes=5, max_methods=10
            )
        )
    ...
    def enforce_simplicity(
        ...
        result = {
            ...
            "message": "Component is simple and compliant"
            if compliant
            else "Component violates simplicity rules",
        }
        ...
        logger.info(
            f"Simplicity enforcement: {component_name} - {'COMPLIANT' if compliant else 'VIOLATION'}"
        )
        return result
    ...
    def generate_simplicity_recommendations(self, component_name: str) -> list[str]:
        ...
        if metric.line_count > 400:
            recommendations.append(
                f"Reduce {component_name} from {metric.line_count} to ≤400 lines"
            )

        if metric.enum_count > 3:
            recommendations.append(
                f"Reduce enums in {component_name} from {metric.enum_count} to ≤3"
            )

        if metric.class_count > 5:
            recommendations.append(
                f"Reduce classes in {component_name} from {metric.class_count} to ≤5"
            )

        if metric.method_count > 10:
            recommendations.append(
                f"Reduce methods in {component_name} from {metric.method_count} to ≤10"
            )
...
def enforce_simplicity(
    component_name: str, line_count: int, enum_count: int, class_count: int, method_count: int
) -> dict[str, Any]:
    """Enforce simplicity rules."""
