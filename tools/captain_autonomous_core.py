...
from .captain_autonomous_models import (
    Bottleneck,
    BottleneckType,
    Flaw,
    FlawSeverity,
...
class CaptainAutonomousCore:
    ...
    def check_stopping_conditions(self) -> list[tuple[StoppingCondition, str]]:
        ...
        if self._check_all_directives_complete():
            stopping_conditions.append(
                (StoppingCondition.ALL_DIRECTIVES_COMPLETE, "All directives completed")
            )

        # Check quality threshold breach
        if self._check_quality_threshold_breach():
            stopping_conditions.append(
                (StoppingCondition.QUALITY_THRESHOLD_BREACH, "Quality threshold breached")
            )

        # Check resource exhaustion
        if self._check_resource_exhaustion():
            stopping_conditions.append(
                (StoppingCondition.RESOURCE_EXHAUSTION, "Resources exhausted")
            )

        # Check critical flaw detected
        if self._check_critical_flaw_detected():
            stopping_conditions.append(
                (StoppingCondition.CRITICAL_FLAW_DETECTED, "Critical flaw detected")
            )

        # Check agent inactivity
        if self._check_agent_inactivity():
            stopping_conditions.append(
                (StoppingCondition.AGENT_INACTIVITY, "Agent inactivity detected")
            )

        # Check system failure
        if self._check_system_failure():
            stopping_conditions.append(
                (StoppingCondition.SYSTEM_FAILURE, "System failure detected")
            )
    ...
    def _get_agent_status(self, agent_id: str) -> dict | None:
        """Get agent status."""
        from .captain_autonomous_utility import CaptainAutonomousUtility

        return CaptainAutonomousUtility.get_agent_status(agent_id)

    def _find_next_task_for_agent(self, agent_id: str) -> str | None:
