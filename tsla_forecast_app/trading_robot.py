...
class MovingAverageStrategy(TradingStrategy):
    ...
    def analyze(self, data):
        ...
        if prev_short_ma <= prev_long_ma and short_ma > long_ma:
            return {'action': 'BUY', 'confidence': 0.8, 'reason': f'Golden Cross: MA{self.short_period} > MA{self.long_period}'}
        elif prev_short_ma >= prev_long_ma and short_ma < long_ma:
            return {'action': 'SELL', 'confidence': 0.8, 'reason': f'Death Cross: MA{self.short_period} < MA{self.long_period}'}
        
        return {'action': 'HOLD', 'confidence': 0.5, 'reason': f'MA{self.short_period}={short_ma:.2f}, MA{self.long_period}={long_ma:.2f}'}
...
class BollingerBandsStrategy(TradingStrategy):
    ...
    def __init__(self):
        super().__init__("Bollinger Bands", "Buy when price touches lower band, Sell when price touches upper band")
        self.price_history = []
        self.bb_period = 20
        self.bb_std = 2
    ...
    def analyze(self, data):
        ...
        if len(self.price_history) < self.bb_period:
            return {'action': 'HOLD', 'confidence': 0.0, 'reason': 'Insufficient data for Bollinger Bands'}
        
        upper, middle, lower = self.calculate_bollinger_bands(self.price_history, self.bb_period, self.bb_std)
        
        if current_price <= lower:
            return {'action': 'BUY', 'confidence': 0.85, 'reason': f'Price at lower band: ${current_price:.2f} <= ${lower:.2f}'}
        elif current_price >= upper:
            return {'action': 'SELL', 'confidence': 0.85, 'reason': f'Price at upper band: ${current_price:.2f} >= ${upper:.2f}'}
        
        return {'action': 'HOLD', 'confidence': 0.5, 'reason': f'Price within bands: ${lower:.2f} < ${current_price:.2f} < ${upper:.2f}'}
...
class TradingRobot(QMainWindow):
    ...
    def create_dashboard_tab(self):
        ...
        self.signals_text.setStyleSheet("background-color: #2a2a2a; color: #ffffff; border: 1px solid #444444; font-family: monospace;")
    ...
    def create_strategies_tab(self):
        ...
        self.performance_table.setHorizontalHeaderLabels(["Strategy", "Trades", "Win Rate", "Profit", "Status"])
        self.performance_table.setStyleSheet("QTableWidget { background-color: #2a2a2a; color: #ffffff; }")
    ...
    def create_portfolio_tab(self):
        ...
        self.portfolio_table.setHorizontalHeaderLabels(["Time", "Action", "Price", "Quantity", "Value", "Strategy"])
        self.portfolio_table.setStyleSheet("QTableWidget { background-color: #2a2a2a; color: #ffffff; }")
    ...
    def create_trading_tab(self):
        ...
        self.trading_log.setStyleSheet("background-color: #2a2a2a; color: #ffffff; border: 1px solid #444444; font-family: monospace;")
    ...
    def create_risk_tab(self):
        ...
        self.risk_text.setStyleSheet("background-color: #2a2a2a; color: #ffffff; border: 1px solid #444444; font-family: monospace;")
    ...
    def create_backtest_tab(self):
        ...
        self.backtest_results.setStyleSheet("background-color: #2a2a2a; color: #ffffff; border: 1px solid #444444; font-family: monospace;")
    ...
    def update_stock_data(self, data):
        ...
        self.statusBar().showMessage(f"Tesla: ${data['current_price']} ({data['change_percent']:+.2f}%) | Trading: {'ON' if self.trading_enabled else 'OFF'}")
    ...
    def run_trading_strategies(self, data):
        ...
        for strategy_name, strategy in self.strategies.items():
            if strategy.enabled:
                ...
                signals.append(f"{strategy.name}: {signal['action']} (Confidence: {signal['confidence']:.1f}) - {signal['reason']}")
    ...
    def execute_strategy_trade(self, strategy, signal, data):
        ...
        if quantity > 0:
            if signal['action'] == 'BUY' and self.portfolio['cash'] >= current_price * quantity:
                ...
                self.log_trade(f"🤖 {strategy.name}: BOUGHT {quantity} shares at ${current_price:.2f}")
    ...
    def update_portfolio_display(self):
        ...
        if self.stock_data:
            ...
            self.portfolio['total_value'] = self.portfolio['cash'] + (self.portfolio['shares'] * current_price)
    ...
    def manual_buy(self):
        ...
        if self.stock_data:
            ...
            if self.portfolio['cash'] >= total_cost:
                ...
                self.log_trade(f"❌ INSUFFICIENT CASH: Need ${total_cost:.2f}, have ${self.portfolio['cash']:.2f}")
    ...
    def manual_sell(self):
        ...
        if self.stock_data:
            ...
            if self.portfolio['shares'] >= quantity:
                ...
                self.log_trade(f"❌ INSUFFICIENT SHARES: Need {quantity}, have {self.portfolio['shares']}")
    ...
    def update_strategy_performance(self):
        ...
        for i, (name, strategy) in enumerate(self.strategies.items()):
            ...
            self.performance_table.setItem(i, 1, QTableWidgetItem(str(strategy.performance['total_trades'])))
            self.performance_table.setItem(i, 2, QTableWidgetItem(f"{strategy.performance['win_rate']:.1f}%"))
            self.performance_table.setItem(i, 3, QTableWidgetItem(f"${strategy.performance['total_profit']:.2f}"))
            self.performance_table.setItem(i, 4, QTableWidgetItem("🟢 Active" if strategy.enabled else "🔴 Inactive"))
    
    def run_backtest(self):
        """Run backtest simulation."""
        self.backtest_results.setText("🚀 Running backtest...\n\nThis feature will be implemented in the next version.\n\nBacktesting will include:\n- Historical data analysis\n- Strategy performance metrics\n- Risk-adjusted returns\n- Drawdown analysis\n- Sharpe ratio calculation")
