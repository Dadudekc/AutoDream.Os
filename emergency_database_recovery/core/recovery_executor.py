...
class RecoveryExecutor:
    ...
    def execute_recovery_plan(self, recovery_plan: RecoveryPlan) -> Dict[str, Any]:
        ...
        try:
            ...
            for action in sorted_actions:
                self.logger.info(f"Executing recovery action: {action.action_id} - {action.description}")
                ...
                if action_result["status"] == "failed" and action.priority == "CRITICAL":
                    self.logger.error(f"Critical recovery action failed: {action.action_id}")
            ...
            elif execution_result["actions_failed"] > 0 and execution_result["actions_successful"] > 0:
    ...
    def create_recovery_plan(self, issues: List[Dict[str, Any]], 
                           strategy: str = "comprehensive") -> RecoveryPlan:
        """Create a recovery plan based on identified issues"""
        self.logger.info(f"Creating recovery plan for {len(issues)} issues using {strategy} strategy")
    ...
    def validate_recovery_plan(self, recovery_plan: RecoveryPlan) -> Dict[str, Any]:
        ...
        for action in recovery_plan.actions:
            ...
            if not action_validation["valid"]:
                validation_result["valid"] = False
                validation_result["validation_errors"].extend(action_validation["errors"])
            
            if action_validation["warnings"]:
                validation_result["validation_warnings"].extend(action_validation["warnings"])
        
        # Assess risk level
        critical_actions = [a for a in recovery_plan.actions if a.priority == "CRITICAL"]
        if len(critical_actions) > 3:
            validation_result["risk_assessment"] = "HIGH"
            validation_result["recommendations"].append("Consider breaking plan into smaller phases")
        elif len(critical_actions) > 1:
            validation_result["risk_assessment"] = "MEDIUM"
            validation_result["recommendations"].append("Monitor critical actions closely")
        
        # Generate recommendations
        if validation_result["validation_errors"]:
            validation_result["recommendations"].append("Fix all validation errors before execution")
        
        if validation_result["validation_warnings"]:
            validation_result["recommendations"].append("Review warnings and address as needed")
    ...
    def _execute_recovery_action(self, action: RecoveryAction) -> Dict[str, Any]:
        ...
        try:
            self.logger.info(f"Starting recovery action: {action.action_id} - {action.description}")
            ...
            action_result["duration_seconds"] = (end_time - datetime.fromisoformat(action_result["start_time"])).total_seconds()
            
            self.logger.info(f"Recovery action completed successfully: {action.action_id}")
        ...
        for source_file in source_files:
            if source_file.exists():
                try:
                    ...
                    self.logger.warning(f"Could not read source file {source_file}: {e}")
    ...
    def _create_recovery_action_for_issue(self, issue: Dict[str, Any], strategy: str) -> Optional[RecoveryAction]:
    ...
    def _validate_recovery_action(self, action: RecoveryAction) -> Dict[str, Any]:
        ...
        if action.action_type not in self.recovery_strategies:
            validation_result["valid"] = False
            validation_result["errors"].append(f"Unknown action type: {action.action_type}")
        
        return validation_result
    
    def _apply_schema_template(self, current_data: Dict[str, Any], 
                              schema_template: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _notify_recovery_completion(self, execution_result: Dict[str, Any]):
        ...
        try:
            success_rate = (execution_result["actions_successful"] / execution_result["actions_executed"] * 100) if execution_result["actions_executed"] > 0 else 0
