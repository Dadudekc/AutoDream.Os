...
class EnhancedCoreAnalyzer:
    ...
    def filter_analysis_files(
        ...
        logger.info(
            f"✅ Filtered to {len(filtered)} relevant files from {len(analysis_results)} total"
        )
        return filtered
    ...
    def calculate_project_metrics(
        ...
        return {
            ...
            "languages": languages,
            "analysis_timestamp": os.path.getmtime(self.project_root / "project_analysis.json")
            if (self.project_root / "project_analysis.json").exists()
            else None,
        }

    def save_basic_analysis(
        self, analysis_results: dict[str, dict[str, Any]], metrics: dict[str, Any]
    ):
        """
