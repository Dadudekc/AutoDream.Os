"""
Tests for messaging health gate functionality.

Validates that the CI/CD health gate tools work correctly and return
...
from src.core.messaging.registry_loader import load_registry, SystemSpec
from src.core.messaging.health_check import check_imports, HealthCheckResult
...
class TestHealthCheckIntegration:
    ...
    def test_health_check_runs(self):
        ...
        specs = load_registry()
        assert len(results) == len(specs), f"Expected {len(specs)} results, got {len(results)}"
    ...
    def test_health_check_identifies_issues(self):
        ...
        # In a development environment, we expect some systems to be unhealthy
        # (missing implementations), so this is acceptable
...
def _run_script(module_name: str, *args) -> tuple[int, str, str]:
    """Helper function to run scripts (duplicated for test isolation)."""
