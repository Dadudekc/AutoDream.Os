...
class LogicPatternScanner:
    ...
    def scan_for_logic_patterns(self) -> Dict[str, List[LogicPattern]]:
        ...
        get_logger(__name__).info("🔍 Scanning for duplicate logic patterns...")
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    try:
                        ...
                        get_logger(__name__).warning(f"Could not analyze {file_path}: {e}")

        # Count duplicates
        for pattern_type, patterns in self.logic_patterns.items():
            if len(patterns) > 1:
                self.total_duplicates_found += len(patterns) - 1
                get_logger(__name__).info(f"Found {len(patterns)} {pattern_type} logic patterns")
    ...
    def _extract_logic_patterns(self, file_path: str) -> List[LogicPattern]:
        ...
        try:
            ...
            get_logger(__name__).warning(f"Error parsing {file_path}: {e}")
            return []
    ...
    def get_duplicate_logic(self) -> Dict[str, List[List[LogicPattern]]]:
        ...
        get_logger(__name__).info("🔍 Identifying duplicate logic patterns...")
        ...
        for pattern_type, patterns in self.logic_patterns.items():
            ...
            for pattern in patterns:
                ...
                normalized_sig = self._get_unified_utility().normalize_signature(pattern.signature)
            ...
            for signature, group in signature_groups.items():
                if len(group) > 1:
                    ...
                    get_logger(__name__).info(f"Found {len(group)} duplicate {pattern_type} patterns with signature: {signature}")

        return duplicate_logic

    def _get_unified_utility().normalize_signature(self, signature: str) -> str:
