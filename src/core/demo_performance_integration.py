...
class PerformanceIntegrationDemo:
    ...
    def _demo_agent_registration(self):
        ...
        agents = [
            ("monitoring_agent", "System Monitoring Agent", ["monitoring", "alerting", "metrics"]),
            ("data_processor", "Data Processing Agent", ["data_processing", "analytics", "reporting"]),
            ("task_coordinator", "Task Coordination Agent", ["coordination", "scheduling", "workflow"]),
            ("security_monitor", "Security Monitoring Agent", ["security", "threat_detection", "compliance"])
        ]
        
        for agent_id, name, capabilities in agents:
            self.agent_communication.register_agent(
                agent_id, name, capabilities, f"http://localhost:800{hash(agent_id) % 10}",
        ...
        message_id = self.agent_communication.send_message(
            "system", "monitoring_agent", MessageType.TASK_ASSIGNMENT,
            {"task": "monitor_system_health", "priority": "high"}, MessagePriority.HIGH
        )
    ...
    def _demo_service_registration(self):
        ...
        services = [
            ...
            ("notification_service", "Notification Service", "v1", "http://localhost:8004")
        ]
        
        for service_id, name, version, base_url in services:
            self.api_gateway.register_service(
                service_id, name, version, base_url,
        ...
        response = self.api_gateway.route_request(
            "GET", "/v1/users", {"Authorization": "Bearer demo-token"},
            {"page": "1", "limit": "10"}, None, "127.0.0.1", "demo-client"
        )
    ...
    def _demo_performance_metrics(self):
        ...
        for metric_type, value, agent_id in metric_scenarios:
            self.performance_tracker.record_metric(
                metric_type, value, agent_id=agent_id,
        ...
        # Test profiling
        with self.performance_profiler.profile("demo_metrics_collection", {"demo": True}):
    ...
    def _demo_cross_system_integration(self):
        ...
        for agent_id in ["monitoring_agent", "data_processor", "task_coordinator", "security_monitor"]:
        ...
        for service_id in ["user_service", "data_service", "analytics_service", "notification_service"]:
        ...
        print(f"    📊 Dashboard data collected: {len(dashboard_data.alerts)} alerts, {len(dashboard_data.system_metrics)} system metrics")
        
        # Add demo alert
        self.performance_dashboard.add_alert(
            AlertLevel.INFO, "Demo integration completed successfully", "demo_system",
    ...
    def _simulate_system_activity(self, iteration):
        ...
        agent_ids = ["monitoring_agent", "data_processor", "task_coordinator", "security_monitor"]
        ...
        for agent_id in agent_ids:
            ...
            self.performance_tracker.record_metric(
                MetricType.RESPONSE_TIME, base_response_time, agent_id=agent_id,
                context={"demo": True, "iteration": iteration}
            )
            
            self.performance_tracker.record_metric(
                MetricType.CPU_USAGE, base_cpu_usage, agent_id=agent_id,
                context={"demo": True, "iteration": iteration}
            )
        ...
        if iteration % 3 == 0:
            self.api_gateway.route_request(
                "GET", f"/v1/demo/{iteration}", {"X-Demo": "true"},
                {"iteration": str(iteration)}, None, "127.0.0.1", "demo-loop"
            )
        ...
        if iteration % 5 == 0:
            ...
            self.agent_communication.send_message(
                sender, recipient, MessageType.STATUS_UPDATE,
    ...
    def _show_demo_status(self, iteration):
        ...
        print(f"  📊 Performance Tracker: {metrics_count} metrics, {snapshots_count} snapshots")
        
        # Agent communication status
        agents_count = len(self.agent_communication.get_all_agents())
        messages_count = len(self.agent_communication.message_history)
        print(f"  📡 Agent Communication: {agents_count} agents, {messages_count} messages")
        ...
        print(f"  📋 Dashboard: {alerts_count} alerts, System Health: {system_health:.1f}/100")
    ...
    def export_demo_data(self, export_dir: str = None):
        ...
        summary = {
            ...
            'demo_summary': {
                ...
                'total_snapshots': len(self.performance_tracker.get_performance_snapshots()),
