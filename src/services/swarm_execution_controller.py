...
class SwarmExecutionController:
    ...
    def validate_task_execution(self, agent: str, task: str, result: str) -> dict[str, Any]:
        ...
        if has_measurable_outcome:
            validation_result.update({"valid_execution": True, "measurable_outcome": True})
            self.task_completions += 1
        else:
            validation_result.update(
                {
                    "missing_metrics": ["quantity", "action", "result"],
                    "suggested_format": "✅ [QUANTITY] [UNIT] [ACTION] - Example: '✅ 433 files deleted'",
                }
            )
    ...
    def _log_execution_validation(
        self, agent: str, task: str, result: str, validation: dict[str, Any]
    ):
...
if __name__ == "__main__":
    ...
    validation = controller.validate_task_execution(
        "Agent-7", "File cleanup", "✅ 433 files deleted from ML components"
    )
