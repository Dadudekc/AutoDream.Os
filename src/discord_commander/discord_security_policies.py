...
class DiscordSecurityManager:
    ...
    def _load_security_config(self):
        ...
        if config_path.exists():
            try:
                ...
                print(f"⚠️  Failed to load Discord security config: {e}")

    def is_channel_allowed(self, channel_id: int) -> bool:
        """Check if channel is allowed for bot commands."""
        if not self.allowed_channels:
            return True  # Allow all channels if none specified
    ...
    def check_security_policies(self, message) -> bool:
        ...
        return (
            self.is_guild_allowed(guild_id) and
            self.is_channel_allowed(message.channel.id) and
            self.is_user_allowed(message.author.id)
        )
    ...
    def _save_security_config(self):
        ...
        try:
            ...
            print(f"⚠️  Failed to save security config: {e}")
