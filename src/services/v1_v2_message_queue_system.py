...
class V1V2MessageQueueSystem:
        ...
        try:
            ...
            if hasattr(key, 'char') and key.char == '\r':  # Enter key
                if keyboard.is_pressed('ctrl'):
                    ...
                    if (current_time - self.last_ctrl_enter_time) < self.config["ctrl_enter_double_timeout"]:
    ...
    def _process_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"✅ Message sent successfully to {message.get('target_agent')}")
            else:
                self.message_history[-1]["status"] = "failed"
                logger.warning(f"⚠️ Message failed to send to {message.get('target_agent')}")
    ...
    def send_message(self, sender_agent: str, target_agent: str, content: str, priority: str = "normal", 
        ...
        try:
            ...
            if priority == "high" or priority == "urgent" or priority == "critical":
                ...
                priority_value = {"low": 3, "normal": 2, "high": 1, "urgent": 0, "critical": -1}.get(priority, 2)
    ...
    def send_message_to_all_agents(self, sender_agent: str, content: str, priority: str = "normal", 
                                  message_type: str = "text", metadata: Optional[Dict[str, Any]] = None) -> List[str]:
        """Send a message from sender_agent to all agents"""
        message_ids = []
        
        for agent_id in self.agent_registry.keys():
            message_id = self.send_message(sender_agent, agent_id, content, priority, message_type, metadata)
            if message_id:
    ...
    def send_high_priority_message(self, sender_agent: str, target_agent: str, content: str, 
                                 message_type: str = "text", metadata: Optional[Dict[str, Any]] = None) -> str:
        """Send a high priority message from sender_agent to target_agent (equivalent to Ctrl+Enter x2)"""
        return self.send_message(sender_agent, target_agent, content, "high", message_type, metadata)
    ...
    def get_all_agents_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_agents": len([a for a in self.agent_registry.values() if a["status"] == "available"])
        }
...
class MessageQueueManager:
            ...
            if hasattr(self.queue_system, 'agent_registry'):
                ...
                if default_coords:
                    ...
                    if self._validate_coordinates(default_coords):
                        ...
                        logger.info(f"✅ Agent {agent_id} coordinates validated and synchronized")
                    else:
                        logger.warning(f"⚠️ Agent {agent_id} has invalid coordinates: {default_coords}")
    ...
    def broadcast_message(self, source_agent: str, content: str, 
                        priority: str = "normal", metadata: Optional[Dict[str, Any]] = None) -> List[str]:
        ...
        try:
            ...
            for agent_id in self.agent_registry.keys():
                if agent_id != source_agent:  # Don't send to self
                    ...
                    if agent_id in self.queue_system.agent_registry:
                        agent_info = self.queue_system.agent_registry[agent_id]
                        if agent_info.get("coordinates") and agent_info.get("status") == "available":
                            message_id = self.send_message(source_agent, agent_id, content, priority, metadata)
                            if message_id:
                                message_ids.append(message_id)
                                successful_sends += 1
                            else:
                            ...
                            logger.warning(f"⚠️ Agent {agent_id} has no coordinates or is not available")
                            failed_sends += 1
                    else:
                        logger.warning(f"⚠️ Agent {agent_id} not found in queue system")
                        failed_sends += 1
            
            logger.info(f"📢 Broadcast message sent to {successful_sends} agents, {failed_sends} failed")
            return message_ids
    ...
    def send_message(self, source_agent: str, target_agent: str, 
                    ...
                    high_priority: bool = False, metadata: Optional[Dict[str, Any]] = None) -> str:
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Send a test message
        message_id = send_message("agent_7", "Hello from Multimedia & Content Specialist! Testing the message queue system.")
