...
from src.core.tracing.error_tracker import ErrorCategory, ErrorSeverity, ErrorTracker
from src.core.tracing.jaeger_tracer import TraceManager
...
class TracingValidator:
    ...
    def _generate_validation_report(self) -> None:
        ...
        self.validation_results["overall_status"] = (
            "success" if success_rate >= 80 else "partial" if success_rate >= 60 else "failed"
        )
        ...
        for name, status in components.items():
            status_icon = (
                "✅" if status.get("status") in ["deployed", "operational", "integrated"] else "❌"
            )
