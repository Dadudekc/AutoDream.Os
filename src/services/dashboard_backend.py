    ...
    async def get_metrics(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if metric_name:
                ...
                metric_names = self.performance_monitor.metrics_storage.get_all_metric_names()
...
class DashboardWebSocket:
    ...
    async def _handle_websocket_message(self, connection: WebSocketConnection, data: Dict[str, Any]):
        ...
        if message_type == 'subscribe':
            ...
            if not metric_names:
                metric_names = self.dashboard_backend.performance_monitor.metrics_storage.get_all_metric_names()
            
            metrics_data = {}
            for metric_name in metric_names:
                series = self.dashboard_backend.performance_monitor.get_metric_series(metric_name)
    ...
    async def broadcast_alert(self, alert):
        ...
        for connection_id, connection in self.connections.items():
            try:
                ...
                logger.error(f"Error broadcasting alert to connection {connection_id}: {e}")
                disconnected_connections.append(connection_id)
...
class DashboardBackend:
    ...
    def _register_default_endpoints(self):
        ...
        endpoints = [
            DashboardEndpoint("/api/metrics", "GET", self.api.get_metrics, "Get metrics data"),
            DashboardEndpoint("/api/health", "GET", self.api.get_health, "Get system health"),
            DashboardEndpoint("/api/status", "GET", self.api.get_status, "Get dashboard status"),
            DashboardEndpoint("/api/alerts", "GET", self.api.get_alerts, "Get active alerts"),
            DashboardEndpoint("/api/collectors", "GET", self.api.get_collectors, "Get collectors status"),
        ]
...
# Export all classes
__all__ = [
    'DashboardRoute', 'DashboardEndpoint', 'WebSocketConnection',
    'DashboardAPI', 'DashboardWebSocket', 'DashboardBackend'
]
