...
class AlertManager:
                ...
                if metric.metric_name in self.thresholds:
                    ...
                    if metric.value >= threshold.critical_threshold:
                        alert = await self._create_alert(
                            metric, AlertLevel.CRITICAL, threshold.critical_threshold
                        )
                        new_alerts.append(alert)
                    # Check warning threshold
                    elif metric.value >= threshold.warning_threshold:
                        alert = await self._create_alert(
                            metric, AlertLevel.WARNING, threshold.warning_threshold
                        )
                        new_alerts.append(alert)
    ...
    async def get_alerts_by_level(self, level: AlertLevel) -> list[Alert]:
        ...
        try:
            return [
                alert for alert in self.alerts if alert.alert_level == level and not alert.resolved
            ]
        except Exception as e:
    ...
    async def cleanup_old_alerts(self, hours: int = 24) -> int:
        ...
        try:
            cutoff_time = datetime.now(UTC).replace(hour=datetime.now().hour - hours)

            old_alerts = [
                alert for alert in self.alerts if alert.resolved and alert.timestamp < cutoff_time
            ]
