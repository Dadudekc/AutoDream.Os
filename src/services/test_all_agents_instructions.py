...
def test_all_agents_instructions():
    ...
    try:
        ...
        instructions = "[SYSTEM] Fallback message: Please check system_broadcast_message.txt file"
    ...
    recent_messages = mq.message_history[-8:] if len(mq.message_history) >= 8 else mq.message_history
    for i, msg_wrapper in enumerate(recent_messages):
        msg = msg_wrapper.get('message', {})
        print(f"  {i+1}. {msg.get('sender_agent', 'unknown')} → {msg.get('target_agent', 'unknown')}: {msg.get('content', '')[:50]}...")
