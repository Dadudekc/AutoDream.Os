...
class SwarmBrain:
    ...
    def upsert_document(
        ...
        *,
        ...
        if row:
            ...
            doc_id = row["id"]
            cur.execute(
                """UPDATE documents SET kind=?, project=?, agent_id=?, ts=?,
            ...
            # Insert new document
            cur.execute(
                """INSERT INTO documents(kind, ref_id, project, agent_id, ts, title,
    ...
    def mark_embedded(self, doc_id: int, backend: str, dim: int, norm: float) -> None:
        ...
        self.conn.execute(
            """INSERT OR REPLACE INTO embeddings(doc_id, backend, dim, norm, created_at)
                           VALUES(?,?,?,?,?)""",
            (doc_id, backend, dim, norm, int(time.time())),
        )
        self.conn.commit()

    def fetch_canonical_batch(
    ...
    def get(self, doc_id: int) -> sqlite3.Row | None:
    ...
    def get_agent_stats(self, agent_id: str) -> dict[str, Any]:
        ...
        result = self.conn.execute(
            "SELECT COUNT(*) as count FROM documents WHERE agent_id = ? AND kind = 'action'",
            (agent_id,),
        ).fetchone()
    ...
    def get_project_stats(self, project: str) -> dict[str, Any]:
        ...
        # Total documents
        result = self.conn.execute(
            "SELECT COUNT(*) as count FROM documents WHERE project = ?", (project,)
        ).fetchone()
    ...
    def cleanup_old_documents(self, max_age_days: int = 30) -> int:
        ...
        result = self.conn.execute(
            "SELECT COUNT(*) as count FROM documents WHERE ts < ?", (cutoff_time,)
        ).fetchone()
        count = result["count"]
