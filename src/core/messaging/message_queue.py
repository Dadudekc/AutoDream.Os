...
class PersistentMessageQueue(MessageQueue):
    ...
    def _load_messages(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {self._pending_queue.qsize()} messages for queue {self.name}")
        ...
        try:
            with self._lock:
                ...
                if message.is_expired():
                    logger.warning(f"Message {message.message_id} is expired, not enqueuing")
                    return False
    ...
    async def acknowledge(self, message_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to acknowledge message {message_id} in queue {self.name}: {e}")
            self._metrics['error_count'] += 1
            return False
    ...
    async def mark_failed(self, message_id: str) -> bool:
        ...
        try:
            with self._lock:
                ...
                logger.debug(f"Marked message {message_id} as failed in queue {self.name}")
                return True
                
        except Exception as e:
            logger.error(f"Failed to mark message {message_id} as failed in queue {self.name}: {e}")
            self._metrics['error_count'] += 1
            return False
        ...
        try:
            with self._lock:
                ...
                if old_ack or old_failed:
                    logger.info(f"Cleaned up {len(old_ack)} acknowledged and {len(old_failed)} failed messages in queue {self.name}")
