...
from .validation_rules import (
    ValidationRule, ValidationResult, Violation, ViolationType,
    ValidationSeverity, EnforcementAction, ValidationRuleManager
)
...
class ContractValidationEngine:
    ...
    def validate_contract(self, contract_data: Dict[str, Any]) -> List[ValidationResult]:
        """Validate a contract against all applicable rules"""
        results = []
        contract_id = contract_data.get("contract_id", "unknown")
        
        # Get enabled rules
        enabled_rules = [rule for rule in self.rule_manager.get_all_rules().values() if rule.enabled]
        ...
        for rule in enabled_rules:
            try:
                ...
                if result.passed:
                    self.logger.info(f"Rule {rule.rule_id} passed for contract {contract_id}")
                else:
                    self.logger.warning(f"Rule {rule.rule_id} failed for contract {contract_id}: {result.message}")
    ...
    def _validate_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> ValidationResult:
        """Validate a single rule against contract data"""
        # Get appropriate handler for rule type
        handler = self.validation_handlers.get(rule.rule_type, self._validate_generic_rule)
    ...
    def _validate_temporal_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
        ...
        if rule.rule_id == "deadline_check":
            ...
            if not deadline or not delivery_date:
                return False, "Missing deadline or delivery date", {"deadline": deadline, "delivery_date": delivery_date}
            ...
            details = {"deadline": deadline, "delivery_date": delivery_date, "on_time": passed}
            
            return passed, message, details
        
        return True, "Temporal rule passed", {}
    
    def _validate_quality_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
        ...
        if rule.rule_id == "quality_standard":
            ...
            details = {"quality_score": quality_score, "minimum_standard": minimum_standard, "passed": passed}
            
            return passed, message, details
        
        return True, "Quality rule passed", {}
    
    def _validate_resource_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
        ...
        if rule.rule_id == "resource_limit":
            ...
            details = {"resource_usage": resource_usage, "resource_limit": resource_limit, "within_limits": passed}
            
            return passed, message, details
        
        return True, "Resource rule passed", {}
    
    def _validate_dependency_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
        ...
        if rule.rule_id == "dependency_check":
            ...
            message = "All dependencies satisfied" if passed else "Dependencies not satisfied"
            details = {
    ...
    def _validate_schema_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
        ...
        message = "Schema validation passed" if passed else f"Missing required fields: {missing_fields}"
        details = {"required_fields": required_fields, "missing_fields": missing_fields, "valid": passed}
        
        return passed, message, details
    
    def _validate_generic_rule(self, rule: ValidationRule, contract_data: Dict[str, Any]) -> tuple:
    ...
    def create_violation(self, validation_result: ValidationResult) -> Violation:
        ...
        violation_type = self._map_rule_to_violation_type(rule) if rule else ViolationType.SCHEMA_VIOLATION
...
def main():
    ...
    parser.add_argument("--test", "-t", action="store_true", help="Test validation engine")
