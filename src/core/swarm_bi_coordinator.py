...
from .swarm_business_intelligence import (
    ConsolidationPhase,
    SwarmBusinessIntelligence,
    SwarmMetricType,
)
from .swarm_data_optimizer import DataType, OptimizationStrategy, SwarmDataOptimizer
from .swarm_performance_monitor import SwarmPerformanceMonitor
...
class SwarmBICoordinator:
    ...
    def _optimize_agent_data(self, agent_id: str, bi_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            if optimization_result.success:
                self.logger.debug(
                    f"Optimized data for agent {agent_id}: {optimization_result.compression_ratio:.1%} reduction"
                )
    ...
    def _record_coordination_activity(
        self, agent_id: str, activity_type: str, data: dict[str, Any]
    ) -> None:
        """Record coordination activity."""
        try:
            activity = {
                ...
                "data_summary": {"keys": list(data.keys()), "size": len(str(data))},
            }
        ...
        try:
            ...
            return {
                ...
                "active_agents": len(
                    [
                        agent
                        for agent, status in self.agent_bi_status.items()
                ...
                "optimization_summary": {
                    "total_optimizations": optimization_report.get("optimization_stats", {}).get(
                        "total_optimizations", 0
                    ),
                    "cache_hit_rate": optimization_report.get("cache_stats", {}).get(
                        "cache_hit_rate", 0
                    ),
                    "average_compression": optimization_report.get("optimization_stats", {}).get(
    ...
    def generate_coordination_report(self) -> SwarmBICoordinationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Generated BI coordination report - Active agents: {active_agents}/{len(self.agent_coordinates)}"
            )
            return report
    ...
    def _generate_coordination_recommendations(
        self, bi_health, performance_report, optimization_report
    ) -> list[str]:
        ...
        try:
            ...
            if len(performance_report.alerts) > 5:
                recommendations.append(
                    "Address performance alerts - multiple agents showing issues"
                )
