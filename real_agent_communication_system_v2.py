...
class ScreenRegionManager:
    ...
    async def execute_in_region(self, agent_id: str, operation: str, 
                               target_area: str = "input_box", **kwargs):
        ...
        async with lock:  # Only one operation per region at a time
            try:
                if operation == "type":
                    ...
                    return await self._type_in_region_with_line_breaks(region, target_area, **kwargs)
                ...
                self.logger.error(f"❌ Error executing {operation} in {agent_id} region: {e}")
                return False
    
    async def _type_in_region(self, region: ScreenRegion, target_area: str, 
                             message: str, method: str = "type") -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Typed in {region.agent_id} {target_area}: {message[:30]}...")
            return True
    ...
    async def _type_in_region_with_line_breaks(self, region: ScreenRegion, target_area: str, 
                                             message: str, method: str = "type") -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ Typed with line breaks in {region.agent_id} {target_area}: {message[:30]}...")
            return True
            
        except Exception as e:
            self.logger.error(f"❌ Failed to type with line breaks in {region.agent_id} region: {e}")
            return False
    
    async def _click_in_region(self, region: ScreenRegion, target_area: str, 
    ...
    async def _update_status_in_region(self, region: ScreenRegion, 
                                     status: str, color: str = "green") -> bool:
        ...
        try:
            ...
            self.logger.error(f"❌ Failed to update status in {region.agent_id} region: {e}")
            return False
...
class InputBufferSystem:
    ...
    async def execute_buffered_inputs(self) -> Dict[str, bool]:
        ...
        async with self.coordination_lock:
            self.logger.info(f"🚀 Executing {self.input_queue.qsize()} buffered inputs using regions...")
    ...
    async def execute_buffered_inputs_with_line_breaks(self) -> Dict[str, bool]:
        ...
        async with self.coordination_lock:
            self.logger.info(f"🚀 Executing {self.input_queue.qsize()} buffered inputs with line breaks using regions...")
            
            while not self.input_queue.empty():
                input_item = await self.input_queue.get()
                
                # Execute in agent's region with line break handling
                success = await self._execute_in_agent_region_with_line_breaks(input_item)
                results[input_item['buffer_id']] = success
    ...
    async def _execute_in_agent_region_with_line_breaks(self, input_item: Dict) -> bool:
        ...
        try:
            ...
            if success:
                self.buffer_stats['region_executions'] += 1
                line_break_info = " with line breaks" if line_breaks else ""
                self.logger.info(f"✅ Executed in {agent_id} region{line_break_info}: {message[:30]}...")
            else:
                self.logger.error(f"❌ Failed to execute in {agent_id} region")
...
class BroadcastSystem:
    ...
    async def region_based_broadcast(self, message: str, priority: str = "normal") -> Tuple[int, int]:
        """Broadcast message to all agents using region management"""
        self.logger.info(f"🚀 REGION-BASED BROADCAST TO ALL 8 AGENTS - Priority: {priority.upper()}")
        ...
        self.logger.info(f"📥 Buffered {len(buffer_ids)} inputs for region-based broadcast")
        ...
        self.logger.info(f"📊 REGION-BASED BROADCAST SUMMARY: {successful_sends} successful, {failed_sends} failed")
        
        # Add to broadcast history
        self.add_broadcast_history(message, priority, successful_sends, failed_sends, "region-based")
        
        return successful_sends, failed_sends
    
    async def region_based_broadcast_with_line_breaks(self, message: str, priority: str = "normal") -> Tuple[int, int]:
        """Broadcast message to all agents using region management with line break handling"""
        self.logger.info(f"🚀 REGION-BASED BROADCAST WITH LINE BREAKS TO ALL 8 AGENTS - Priority: {priority.upper()}")
        ...
        self.logger.info(f"📥 Buffered {len(buffer_ids)} inputs for region-based broadcast with line breaks")
        ...
        # Log broadcast summary
        self.logger.info(f"📊 REGION-BASED BROADCAST WITH LINE BREAKS SUMMARY: {successful_sends} successful, {failed_sends} failed")
        
        # Add to broadcast history
        self.add_broadcast_history(message, priority, successful_sends, failed_sends, "region-based-with-line-breaks")
        
        return successful_sends, failed_sends
    
    async def broadcast_to_all_agents(self, message: str, priority: str = "normal") -> Tuple[int, int]:
        """Legacy broadcast method - now uses region management"""
        return await self.region_based_broadcast(message, priority)
    
    def add_broadcast_history(self, message: str, priority: str, successful: int, 
                             failed: int, method: str = "region-based"):
...
class RealAgentCommunicationSystem:
    ...
    def load_real_coordinates(self):
        """Load real agent coordinates from cursor_agent_coords.json"""
        try:
            ...
            if config_path.exists():
                ...
                if layout:
                    ...
                    self.logger.info(f"✅ Loaded {len(self.agent_coordinates)} real agent coordinates")
                    self.logger.info(f"📍 Agent-1: ({self.agent_coordinates['Agent-1']['input_x']}, {self.agent_coordinates['Agent-1']['input_y']})")
                else:
    ...
    async def send_message_to_agent(self, agent_id: str, message: str, 
        ...
        try:
            self.logger.info(f"📤 Sending message to {agent_id} in {target_area}: {message}")
            ...
            if success:
                ...
                # Add to devlog
                self.add_devlog_update(f"Message sent to {agent_id} in {target_area}: {message}")
                
                self.logger.info(f"✅ Message sent successfully to {agent_id} in {target_area}")
                return True
            else:
    ...
    async def send_message_to_agent_with_line_breaks(self, agent_id: str, message: str, 
                                                    target_area: str = "input_box"):
        ...
        try:
            self.logger.info(f"📤 Sending message with line breaks to {agent_id} in {target_area}: {message}")
            ...
            if success:
                ...
                # Add to devlog
                self.add_devlog_update(f"Message with line breaks sent to {agent_id} in {target_area}: {message}")
                
                self.logger.info(f"✅ Message with line breaks sent successfully to {agent_id} in {target_area}")
                return True
            else:
                self.logger.error(f"❌ Failed to send message with line breaks to {agent_id}")
                return False
            
        except Exception as e:
            self.logger.error(f"❌ Failed to send message with line breaks to {agent_id}: {e}")
            return False
    
    async def send_message_to_all_agents(self, message: str, target_area: str = "input_box"):
        """Send message to all 8 agents using region management"""
        self.logger.info(f"🚀 Sending message to ALL agents using region management: {message}")
        
        # Use the region-based broadcast system
        successful, failed = await self.broadcast_system.region_based_broadcast(message)
        
        # Add to devlog
        self.add_devlog_update(f"Region-based multi-agent broadcast: {successful}/{len(self.agent_coordinates)} agents received message")
        
        return successful == len(self.agent_coordinates)
    
    async def send_message_to_all_agents_with_line_breaks(self, message: str, target_area: str = "input_box"):
        """Send message to all 8 agents with proper line break handling"""
        self.logger.info(f"🚀 Sending message with line breaks to ALL agents using region management: {message}")
        
        # Use the region-based broadcast system with line break handling
        successful, failed = await self.broadcast_system.region_based_broadcast_with_line_breaks(message)
        
        # Add to devlog
        self.add_devlog_update(f"Region-based multi-agent broadcast with line breaks: {successful}/{len(self.agent_coordinates)} agents received message")
        
        return successful == len(self.agent_coordinates)
    
    async def send_coordinated_messages(self, messages: Dict[str, str], 
                                      target_areas: Dict[str, str] = None):
        ...
        for agent_id, message in messages.items():
            if agent_id in self.agent_coordinates:
                ...
                # Add to devlog
                self.add_devlog_update(f"Buffered coordinated message to {agent_id} in {target_area}: {message}")
            else:
                self.logger.warning(f"⚠️ Agent {agent_id} not found for coordinated messaging")
        
        # Execute all buffered messages using region management
        results = await self.input_buffer.execute_buffered_inputs()
        
        success_count = sum(1 for r in results.values() if r)
        self.logger.info(f"✅ Coordinated messaging complete: {success_count}/{len(messages)} successful")
    ...
    def display_agent_status(self):
        ...
        print("\n" + "="*80)
        print("🎯 REAL AGENT COMMUNICATION SYSTEM V2 STATUS (WITH SCREEN REGION MANAGEMENT)")
        print("="*80)
        ...
        for agent_id, info in self.get_agent_status().items():
            ...
            if last_msg:
                msg_preview = last_msg['message'][:30] + "..." if len(last_msg['message']) > 30 else last_msg['message']
                print(f"{status_icon} {agent_id}: ({coords['x']}, {coords['y']}) - Last: {msg_preview}{region_status}")
            else:
                print(f"{status_icon} {agent_id}: ({coords['x']}, {coords['y']}) - No messages yet{region_status}")
...
class DiscordIntegration:
    ...
    async def broadcast_to_discord(self, message: str, channels: List[str] = None):
        ...
        success_count = sum(1 for r in results.values() if r)
        self.logger.info(f"✅ Discord broadcast complete: {success_count}/{len(channels)} channels")
...
Examples:
  ...
  python real_agent_communication_system_v2.py --show-layout
        ...
        parser.add_argument('--broadcast', type=str, metavar='MESSAGE',
        ...
        parser.add_argument('--area', choices=['input_box', 'workspace_box', 'status_box'], 
                          default='input_box', help='Target area in agent region (default: input_box)')
        ...
        parser.add_argument('--show-layout', action='store_true',
                          help='Display visual region layout')
    ...
    if args.broadcast:
        ...
        if broadcast_history:
            for entry in broadcast_history[-5:]:  # Show last 5
                print(f"  {entry['timestamp']}: {entry['message'][:50]}... ({entry.get('method', 'legacy')})")
        else:
        ...
        print("🚀 REAL AGENT COMMUNICATION SYSTEM V2 DEMO (PHASE 2: SCREEN REGION MANAGEMENT)")
        print("="*80)
        print("This system uses your REAL agent coordinates with advanced screen region management!")
        print("Messages will be sent to actual agent input locations in isolated workspaces.")
        ...
        results = await comm_system.send_coordinated_messages(coordinated_messages, target_areas)
