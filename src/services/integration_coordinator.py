...
try:
    ...
    from .integration_config_manager import IntegrationConfigManager, ConfigSource, ConfigScope
except ImportError:
...
class IntegrationCoordinator:
    ...
    def __init__(self, config_dir: Optional[str] = None):
        ...
        self.config_manager = IntegrationConfigManager(self.config_dir)
        ...
        # Event callbacks
        self.status_change_callbacks: List[Callable[[IntegrationStatus, IntegrationStatus], None]] = []
    ...
    def _register_default_endpoints(self):
        ...
        async def health_check_handler(request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
            return {
                'status_code': 200,
                'success': True,
                'data': {
        ...
        # Metrics endpoint
        async def metrics_handler(request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
            return {
                'status_code': 200,
                'success': True,
        ...
        async def config_handler(request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
            return {
                'status_code': 200,
                'success': True,
        ...
        # Services endpoint
        async def services_handler(request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
            return {
                'status_code': 200,
                'success': True,
    ...
    def _update_status(self, new_status: IntegrationStatus) -> None:
        ...
        if old_status != new_status:
            logger.info(f"Integration Coordinator status changed: {old_status.value} -> {new_status.value}")
    ...
    async def _metrics_collection_loop(self):
        ...
        while self.status == IntegrationStatus.RUNNING and not self.shutdown_event.is_set():
            try:
    ...
    def _update_metrics(self):
        ...
        if self.middleware_orchestrator.running:
            ...
            self.metrics.total_data_packets_processed = middleware_metrics.get('total_packets_processed', 0)
            self.metrics.middleware_chains_active = middleware_metrics.get('active_chains', 0)
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'middleware_orchestrator': {
                ...
                'performance_metrics': self.middleware_orchestrator.get_performance_metrics() if self.middleware_orchestrator.running else {}
            },
            'service_registry': {
                ...
                'registry_summary': self.service_registry.get_registry_summary() if self.service_registry.running else {}
            },
            'config_manager': {
    ...
    def add_status_change_callback(self, callback: Callable[[IntegrationStatus, IntegrationStatus], None]):
    ...
    async def process_data_packet(self, data_packet: DataPacket, 
                                chain_name: Optional[str] = None) -> DataPacket:
        ...
        try:
            ...
            result = await self.middleware_orchestrator.process_data_packet(data_packet, chain_name)
    ...
    def set_config_value(self, key: str, value: Any, source: ConfigSource = ConfigSource.API,
                        scope: ConfigScope = ConfigScope.GLOBAL, description: str = ""):
        """Set a configuration value."""
        self.config_manager.set_config_value(key, value, source, scope, description)
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        health_status['components']['config_manager'] = {
            ...
            'config_values': len(self.config_manager.config_values)
        }
        
        # Overall health depends on all critical components
        critical_components = ['api_manager', 'middleware_orchestrator', 'service_registry']
        all_healthy = all(health_status['components'][comp]['healthy'] for comp in critical_components)
...
async def main():
    ...
    def on_status_change(old_status: IntegrationStatus, new_status: IntegrationStatus):
        print(f"Integration Coordinator status changed: {old_status.value} -> {new_status.value}")
