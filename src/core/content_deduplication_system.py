...
class ContentDeduplicationSystem:
    ...
    def check_duplicate(self, content: str, file_path: str = "") -> Tuple[bool, Optional[str], float]:
        ...
        for hash_value, content_hash_obj in self.content_registry.items():
            try:
                with open(content_hash_obj.file_path, 'r', encoding='utf-8') as f:
        ...
        if content_hash in self.duplicate_groups:
            ...
            for existing_hash, existing_obj in self.content_registry.items():
                if existing_hash != content_hash and existing_obj.file_path != file_path:
                    try:
                        with open(existing_obj.file_path, 'r', encoding='utf-8') as f:
                            existing_content = f.read()
    ...
    def check_anti_slop_compliance(self, new_content: str, agent_id: str) -> Tuple[bool, str]:
        ...
        agent_files = [h.file_path for h in self.content_registry.values() 
                      if agent_id in h.file_path and 
                      h.created_at > (datetime.now() - timedelta(hours=24)).isoformat()]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        unique_size = sum(h.content_size for h in self.content_registry.values() 
