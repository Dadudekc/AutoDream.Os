...
class AutomationTask:
    ...
    def is_claimable(self) -> bool:
        ...
        return self.status == "available" or self.status == "pending" and not self.assigned_to

    def can_be_claimed_by(self, agent_id: str) -> bool:
        """Check if task can be claimed by specific agent."""
        return self.is_claimable() and (not self.assigned_to or self.assigned_to == agent_id)
...
class ConsolidatedAutomationSystem:
    ...
    def _extract_message_info(self, content: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Look for coordination patterns
                ...
                for keyword in ["COORDINATION", "COLLABORATION", "SUPPORT"]
            ):
    ...
    def _process_coordination_message(
        self, message: AutomationMessage, info: dict[str, Any]
    ) -> bool:
    ...
    def get_current_task(self) -> AutomationTask | None:
    ...
    def _select_best_task(self, available_tasks: list[AutomationTask]) -> AutomationTask | None:
        ...
        try:
            ...
            valid_tasks = [
                task for task in available_tasks if self._validate_task_dependencies(task)
            ]
    ...
    def claim_task(self, task_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not task.phases:
                task.phases = [
                    {
                        "phase": 1,
                        "description": f"Phase 1: {task.description}",
                        "status": "in_progress",
                    },
                    {
                        "phase": 2,
                        "description": f"Phase 2: {task.description}",
                        "status": "pending",
                    },
                    {
                        "phase": 3,
                        "description": f"Phase 3: {task.description}",
                        "status": "pending",
                    },
                ]
            ...
            pending_tasks = future_tasks.get("pending_tasks", [])
            future_tasks["pending_tasks"] = [t for t in pending_tasks if t.get("id") != task.id]
    ...
    def process_all_messages(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "timestamp": processing_start.isoformat()}
