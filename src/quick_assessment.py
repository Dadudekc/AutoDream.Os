...
def main():
    ...
    for agent_id in agents:
        ...
        web_files = list((repo_root / 'src' / 'web').glob('*.py')) if (repo_root / 'src' / 'web').exists() else []
        if web_files:
    ...
    print(f'- Completed: {sum(1 for a in results["agents"].values() if a["priority"] == "complete")}')
    print(f'- High Priority: {sum(1 for a in results["agents"].values() if a["priority"] == "high")}')
    print(f'- Medium Priority: {sum(1 for a in results["agents"].values() if a["priority"] == "medium")}')
