...
class AgentRegistrationManager:
    ...
    def _scan_for_new_agents(self):
        ...
        try:
            workspace_dir = Path(self.config_manager.get_config("agents", "workspace_dir", "agent_workspaces"))
    ...
    def _handle_new_agent_discovery(self, agent_id: str, agent_dir: Path):
        ...
        try:
            ...
            self.logger.error(f"Failed to handle new agent discovery for {agent_id}: {e}")
    ...
    def _should_auto_register(self, request: RegistrationRequest) -> bool:
        ...
        try:
            auto_register = self.config_manager.get_config("agents", "auto_register", True)
            if not auto_register:
                return False
            
            # Check if agent has minimum required capabilities
            min_capabilities = self.config_manager.get_config("agents", "min_capabilities", 1)
            ...
            self.logger.error(f"Failed to determine auto-registration for {request.agent_id}: {e}")
            return False
    
    def register_agent_manual(self, agent_id: str, agent_name: str, 
    ...
    def _process_registration_request(self, request_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self._perform_capability_assessment(request.agent_id, request.capabilities)
            ...
            self.logger.error(f"Failed to process registration request {request_id}: {e}")
            return False
    
    def _perform_capability_assessment(self, agent_id: str, capabilities: List[AgentCapability]):
        ...
        try:
            ...
            self.logger.error(f"Failed to perform capability assessment for {agent_id}: {e}")
    ...
    def get_pending_registrations(self) -> List[RegistrationRequest]:
        ...
        return [req for req in self.registration_requests.values() 
                if req.status == RegistrationStatus.DISCOVERED]
    
    def get_capability_assessment(self, agent_id: str) -> Optional[CapabilityAssessment]:
        """Get capability assessment for an agent"""
        return self.capability_assessments.get(agent_id)
    
    def update_capability_assessment(self, agent_id: str, capability: AgentCapability, 
                                   skill_level: float):
        ...
        try:
            if agent_id in self.capability_assessments:
                ...
                self.logger.info(f"Updated capability assessment for {agent_id}: {capability.value} = {skill_level}")
                
        except Exception as e:
            self.logger.error(f"Failed to update capability assessment for {agent_id}: {e}")
    ...
    def _cleanup_expired_registrations(self):
        ...
        try:
            ...
            if expired_requests:
                self.logger.info(f"Cleaned up {len(expired_requests)} expired registration requests")
    ...
    def get_registration_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "auto_register_enabled": self.config_manager.get_config("agents", "auto_register", True)
            }
        except Exception as e:
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            self.update_capability_assessment(test_agent_id, AgentCapability.TESTING, 0.8)
...
def main():
    ...
    parser.add_argument("--register", nargs=3, metavar=("ID", "NAME", "CAPABILITIES"), help="Manually register agent")
    parser.add_argument("--status", help="Get registration status by request ID")
    parser.add_argument("--pending", action="store_true", help="Show pending registrations")
    parser.add_argument("--assessment", help="Get capability assessment for agent")
    parser.add_argument("--summary", action="store_true", help="Show registration summary")
    ...
    if args.register:
        ...
        request_id = registration_manager.register_agent_manual(agent_id, name, capabilities)
