...
class CLIExecutor:
    ...
    def run_command(self, cmd_args: List[str], description: str = "", capture_output: bool = True) -> dict:
        """
    ...
    def run_python_module(self, module_path: str, args: List[str] = None, description: str = "") -> dict:
        """
        ...
        return self.run_command(cmd_args, description or f"Running Python module: {module_path}")

    def run_python_script(self, script_path: str, args: List[str] = None, description: str = "") -> dict:
        """
        ...
        return self.run_command(cmd_args, description or f"Running Python script: {script_path}")
...
def run_smoke_test():
    ...
    try:
        ...
        result = executor.run_python_module("src.utils.cli_utils", ["--test"], "Testing module execution")
...
def main():
    ...
    if args.command:
        result = executor.run_command(args.command, f"Executing: {' '.join(args.command)}")
        print(f"Result: {result}")
    elif args.module:
        result = executor.run_python_module(args.module, description=f"Running module: {args.module}")
        print(f"Result: {result}")
    elif args.script:
        result = executor.run_python_script(args.script, description=f"Running script: {args.script}")
        print(f"Result: {result}")
    else:
        parser.print_help()
