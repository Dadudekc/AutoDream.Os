...
class AgentAutonomousWorkflow:
        ...
        try:
            ...
            if task_status == "no_current_task":
                ...
                if claimed_task:
                    ...
                    cycle_results["actions_taken"].append(f"Claimed task: {claimed_task.get('title', 'Unknown')}")
            ...
            if task_status == "task_in_progress":
                ...
                cycle_results["actions_taken"].append(f"Autonomous operations: {operations_results['operations_successful']} successful")
    ...
    async def run_continuous_cycles(self, interval_seconds: int = 300) -> None:
        ...
        logger.info(f"{self.agent_id}: Starting continuous autonomous cycles (interval: {interval_seconds}s)")
        
        try:
            while True:
                cycle_results = await self.run_autonomous_cycle()
                logger.info(f"{self.agent_id}: Cycle completed - {cycle_results['messages_processed']} messages, {cycle_results['tasks_processed']} tasks")
