...
class TargetedDocumentationCleanupSystem:
    ...
    def create_targeted_cleanup_plan(self) -> dict[str, Any]:
        ...
        reduction_percentage = (
            round((total_to_delete / self.total_files_found) * 100, 1)
    ...
    def _perform_targeted_cleanup(self) -> dict[str, Any]:
            ...
            try:
                if action == "delete_entire_directory":
                    ...
                    if len(md_files) > 10:
                        ...
                        freed_size_mb += area_plan["size_to_free_mb"]
                        print(
                            f"🗑️ Kept 10 most recent devlogs, deleted {len(files_to_delete)} old ones"
                        )
        ...
        return {
            ...
            "reduction_achieved": round((deleted_count / self.total_files_found) * 100, 1)
            if self.total_files_found > 0
            else 0,
        }
