...
class AgentCommunicationProtocol:
    ...
    def register_agent(self, agent_id: str, name: str, capabilities: List[str], 
        ...
        with self.lock:
            ...
            logger.info(f"Agent {agent_id} ({name}) registered with capabilities: {capabilities}")
            return True
    ...
    def send_message(self, sender_id: str, recipient_id: str, message_type: MessageType,
        ...
        if self.performance_tracker:
            self.performance_tracker.record_metric(
                'message_sent', 1, 'count', sender_id,
                {'message_type': message_type.value, 'priority': priority.value}
            )
        
        logger.debug(f"Message queued: {message.message_id} from {sender_id} to {recipient_id}")
    ...
    def find_agents_by_capability(self, capability: str) -> List[AgentInfo]:
        ...
        with self.lock:
            ...
            return [self.registered_agents[aid] for aid in agent_ids 
        ...
        with self.lock:
            while self.message_queue:
                ...
                if not recipient_info or recipient_info.status == AgentStatus.OFFLINE:
                    logger.warning(f"Recipient {message.recipient_id} not available for message {message.message_id}")
                    continue
    ...
    def _route_message(self, message: Message):
        ...
        try:
            ...
            logger.debug(f"Routing message {message.message_id} to {message.recipient_id}")
            
            # Track message routing performance
            if self.performance_tracker:
                self.performance_tracker.record_metric(
                    'message_routed', 1, 'count', message.sender_id,
                    {'message_type': message.message_type.value, 'recipient': message.recipient_id}
                )
            ...
            for agent_id, agent_info in self.registered_agents.items():
                if agent_info.status != AgentStatus.OFFLINE:
                    ...
                    if time_since_last.total_seconds() > self.heartbeat_interval:
                        # Send heartbeat
                        self.send_message(
                            "system", agent_id, MessageType.HEALTH_CHECK,
            ...
            for agent_id, agent_info in list(self.registered_agents.items()):
                ...
                if time_since_last.total_seconds() > self.message_timeout:
                    if agent_info.status != AgentStatus.OFFLINE:
                        ...
                        self._notify_agent_status_callbacks(agent_id, AgentStatus.OFFLINE)
                        logger.warning(f"Agent {agent_id} marked as offline due to timeout")
