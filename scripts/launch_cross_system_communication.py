...
from services.cross_system_communication import (
    CrossSystemCommunicationManager, SystemEndpoint, CommunicationProtocol
)
from services.integration_testing_framework import (
    IntegrationTestRunner, IntegrationTestSuite,
    CrossSystemCommunicationTest, APIIntegrationTest, MiddlewareIntegrationTest
)
...
class CrossSystemCommunicationLauncher:
    ...
    def __init__(self, config_path: str = "config/cross_system_communication_config.json"):
        ...
        try:
            ...
            for system_id, system_config in systems_config.items():
                ...
                try:
                    ...
                    logger.warning(f"Invalid protocol '{protocol_str}' for system {system_id}, using HTTP")
                ...
                endpoint = SystemEndpoint(
                    ...
                    health_check_interval=system_config.get("health_check_interval", 60.0),
                ...
                logger.info(f"Added system endpoint: {system_id} ({protocol.value}://{endpoint.host}:{endpoint.port})")
        ...
        try:
            ...
            for suite_name, suite_config in test_suites_config.items():
                ...
                suite = IntegrationTestSuite(
                    name=suite_name,
                    description=suite_config.get("description", f"Test suite for {suite_name}")
                )
                
                # Configure suite options
                suite.parallel_execution = testing_config.get("parallel_execution", False)
                ...
                suite.retry_failed_tests = testing_config.get("retry_failed_tests", False)
                suite.max_retries = testing_config.get("max_retries", 3)
                
                # Add tests based on suite type
                if suite_name == "cross_system_communication":
                    suite.add_test(CrossSystemCommunicationTest("test_system_connections"))
                    suite.add_test(CrossSystemCommunicationTest("test_message_routing"))
                    suite.add_test(CrossSystemCommunicationTest("test_protocol_handling"))
                    ...
                    suite.add_test(MiddlewareIntegrationTest("test_data_transformation"))
    ...
    async def send_test_message(self, target_system: str, message_type: str = "request") -> bool:
        """Send a test message to a system."""
        if not self.communication_manager:
            return False
        
        try:
            from services.cross_system_communication import CrossSystemMessage, MessageType, MessagePriority
...
async def main():
    ...
    parser.add_argument(
        "action",
        choices=["start", "stop", "restart", "status", "test", "connect", "disconnect", "send-message"],
    ...
    parser.add_argument(
        "--suite", "-s",
        help="Test suite name (for test action)"
    )
    ...
    parser.add_argument(
        "--message-type", "-mt",
        default="request",
        choices=["request", "response", "event", "command", "query", "notification", "heartbeat"],
