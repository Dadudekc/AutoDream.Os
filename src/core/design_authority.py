...
class DesignAuthority:
    ...
    def review_code_complexity(
        ...
        critical_issues = [
            issue for issue in complexity_issues if issue["severity"] == DecisionSeverity.ERROR
        ]
                    ...
                    violations.append(
                        {
                            ...
                            "guideline": principle_data["guidelines"][0]
                            if principle_data["guidelines"]
                            else "",
                        }
                    )
    ...
    def _generate_recommendations(self, plan: str) -> list[str]:
        ...
        for complex_pattern, simple_alternative in self.knowledge_base[
            "preferred_alternatives"
        ].items():
            if complex_pattern in plan_lower:
                recommendations.append(
                    f"Consider {simple_alternative} instead of {complex_pattern}"
                )
    ...
    def _generate_feedback(
        self, violations: list[dict[str, Any]], recommendations: list[str]
    ) -> str:
    ...
    def get_knowledge_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "approval_rate": len([r for r in self.review_history if r.approved])
            / max(len(self.review_history), 1),
...
def review_component_plan(
    requester: str, component_name: str, plan: str, context: str = ""
) -> DesignReview:
