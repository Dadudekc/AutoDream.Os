...
class ProtocolComplianceChecker:
    ...
    def _generate_recommendations(self) -> None:
        ...
        for category in categories:
            ...
            critical_count = len([i for i in category_issues if i.level == ComplianceLevel.CRITICAL])
            
            if critical_count > 0:
                self.recommendations.append(f"🚨 CRITICAL: Fix {critical_count} critical issues in {category.value}")
            elif len(category_issues) > 0:
                self.recommendations.append(f"⚠️ Address {len(category_issues)} issues in {category.value}")
...
def main():
    ...
    parser.add_argument("--category", choices=["all", "git", "code", "docs", "agent", "testing"], 
                       default="all", help="Compliance category to check")
    parser.add_argument("--output", choices=["console", "json", "file"], 
                       default="console", help="Output format")
    parser.add_argument("--output-file", default="compliance_report.json", 
                       help="Output file for JSON/file output")
...
def format_compliance_report(report: ComplianceReport) -> str:
    ...
    if report.issues:
        ...
        for issue in report.issues:
            ...
            output.append(f"{level_emoji[issue.level]} [{issue.category.value.upper()}] {issue.message}")
            if issue.file_path:
