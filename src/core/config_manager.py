...
class ConfigValidator:
    ...
    def __init__(
        self, validation_level: ConfigValidationLevel = ConfigValidationLevel.STANDARD
    ):
        self.validation_level = validation_level
        self.logger = logging.getLogger(f"{__name__}.ConfigValidator")

    def validate_config(
        self, config_data: Dict[str, Any], schema: Dict[str, ConfigSection]
    ) -> ConfigValidationResult:
        ...
        try:
            ...
            if self.validation_level == ConfigValidationLevel.STRICT:
                strict_errors, strict_warnings = self._strict_validation(
                    config_data, schema
                )
    ...
    def _validate_section(
        self, section_data: Any, section_def: ConfigSection
        ...
        try:
            ...
            if expected_type and not isinstance(section_data, expected_type):
                errors.append(
                    f"Section '{section_def.name}' should be {expected_type.__name__}, got {type(section_data).__name__}"
                )
            ...
            if isinstance(section_data, (int, float)):
                ...
                if min_val is not None and section_data < min_val:
                    errors.append(
                        f"Section '{section_def.name}' value {section_data} is below minimum {min_val}"
                    )

                if max_val is not None and section_data > max_val:
                    errors.append(
                        f"Section '{section_def.name}' value {section_data} is above maximum {max_val}"
                    )
            ...
            if isinstance(section_data, str):
                ...
                if min_length is not None and len(section_data) < min_length:
                    errors.append(
                        f"Section '{section_def.name}' string length {len(section_data)} is below minimum {min_length}"
                    )

                if max_length is not None and len(section_data) > max_length:
                    errors.append(
                        f"Section '{section_def.name}' string length {len(section_data)} is above maximum {max_length}"
                    )

                if allowed_values and section_data not in allowed_values:
                    errors.append(
                        f"Section '{section_def.name}' value '{section_data}' is not in allowed values: {allowed_values}"
                    )
            ...
            if isinstance(section_data, list):
                ...
                if min_items is not None and len(section_data) < min_items:
                    errors.append(
                        f"Section '{section_def.name}' list has {len(section_data)} items, minimum required is {min_items}"
                    )

                if max_items is not None and len(section_data) > max_items:
                    errors.append(
                        f"Section '{section_def.name}' list has {len(section_data)} items, maximum allowed is {max_items}"
                    )
    ...
    def _strict_validation(
        ...
        for section_name, section_data in config_data.items():
            if section_data is None or (
                isinstance(section_data, str) and section_data.strip() == ""
            ):
...
class ConfigManager:
    ...
    def _initialize_default_schema(self):
        ...
        self.config_schema = {
            ...
            "logging": ConfigSection(
                ...
                validation_rules={
                    "type": dict,
                    "allowed_values": ["standard", "detailed", "minimal"],
                },
    ...
    def _start_file_watching(self):
        ...
        try:
            ...
            self.observer.schedule(
                self.file_watcher, str(self.config_dir), recursive=False
            )
            self.observer.start()
    ...
    def get_config_summary(self) -> Dict[str, Any]:
        ...
        try:
            core_summary = {
                ...
                "total_values": sum(
                    len(section.data) for section in self.configs.values()
                ),
            }
            ...
            handler_summary = {
                ...
                "total_handlers": sum(
                    len(handlers) for handlers in self.change_handlers.values()
                ),
            }

            return {"core": core_summary, "handlers": handler_summary}
    ...
    def register_change_handler(
        self, section: str, callback: Callable[[str, Any], None]
    ) -> bool:
    ...
    def unregister_change_handler(
        self, section: str, handler: ConfigChangeHandler
    ) -> bool:
        ...
        try:
            if section in self.change_handlers:
                if handler in self.change_handlers[section]:
                    ...
                    self.logger.info(
                        f"Unregistered change handler for section: {section}"
                    )
                    return True
    ...
    def _notify_change(self, section: str, new_value: Any):
        ...
        try:
            if section in self.change_handlers:
                ...
                self.logger.info(
                    f"Notified {len(self.change_handlers[section])} handlers of change in section: {section}"
                )
            else:
