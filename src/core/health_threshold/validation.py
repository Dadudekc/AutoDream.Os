...
class HealthThresholdValidation:
    ...
    def validate_threshold(self, threshold: HealthThreshold, value: float) -> str:
        ...
        try:
            ...
            self.logger.error(f"Failed to validate threshold for {threshold.metric_type}: {e}")
            return "unknown"

    def get_validation_history(self, metric_type: str = None) -> List[ValidationOperation]:
        """Get validation operation history, optionally filtered by metric type"""
        if metric_type:
            return [op for op in self.validation_operations if op.metric_type == metric_type]
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_validation": self.validation_operations[-1].timestamp if self.validation_operations else None
        }
