...
class EmailAlertChannel(AlertChannel):
    ...
    def __init__(self, name: str, recipients: List[str], smtp_server: str,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.webhook_url, json=payload) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"Slack alert sent via {self.name}: {alert.rule_name}")
                        return True
                    else:
...
class WebhookAlertChannel(AlertChannel):
    ...
    def __init__(self, name: str, webhook_url: str, method: str = "POST",
            ...
            async with aiohttp.ClientSession() as session:
                async with session.request(
                    ...
                    if 200 <= response.status < 300:
                        ...
                        logger.info(f"Webhook alert sent via {self.name}: {alert.rule_name}")
                        return True
                    else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.webhook_url, json=payload) as response:
                    if response.status == 204:  # Discord returns 204 for success
                        ...
                        logger.info(f"Discord alert sent via {self.name}: {alert.rule_name}")
                        return True
                    else:
...
class PagerDutyAlertChannel(AlertChannel):
    ...
    def __init__(self, name: str, integration_key: str, api_url: str = "https://events.pagerduty.com/v2/enqueue"):
        super().__init__(name)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(self.api_url, json=payload) as response:
                    if response.status == 202:  # PagerDuty returns 202 for accepted
                        ...
                        logger.info(f"PagerDuty alert sent via {self.name}: {alert.rule_name}")
                        return True
                    else:
...
class AlertManager:
    ...
    async def send_alert(self, alert: PerformanceAlert, channel_names: Optional[List[str]] = None) -> Dict[str, bool]:
        """Send alert through specified channels or all channels."""
        if channel_names is None:
            channels_to_use = list(self.channels.values())
        else:
            channels_to_use = [self.channels[name] for name in channel_names if name in self.channels]
...
# Export all classes
__all__ = [
    'AlertRule', 'AlertChannel', 'EmailAlertChannel', 'SlackAlertChannel',
