...
def demonstrate_swarm_integration():
    ...
    for agent_id, name, capabilities in test_agents:
        ...
        print(f"   {status_icon} {name} ({agent_id}): {'SUCCESS' if success else 'FAILED'}")
    ...
    for agent_id, success in results.items():
        status_icon = "✅" if success else "❌"
        print(f"     {status_icon} {agent_id}: {'COORDINATED' if success else 'FAILED'}")
...
def main():
    ...
    parser.add_argument("--integration", action="store_true", help="Run integration demo only")
