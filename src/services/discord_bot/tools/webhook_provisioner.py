...
class DiscordWebhookProvisioner:
    ...
    async def create(self, agent_id: str, channel: discord.TextChannel) -> Optional[str]:
        """
        ...
        try:
            ...
            webhook = await channel.create_webhook(
                name=name, 
                reason=f"Provision webhook for {agent_id}"
            )
            ...
            logger.info(
                "Webhook created: agent=%s channel=%s id=%s", 
                agent_id, channel.id, webhook.id
            )
            return webhook.url
    ...
    async def rotate(self, agent_id: str) -> Optional[str]:
        """
    ...
    async def test(self, agent_id: str) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                payload = {"content": f"TEST_EVENT|{agent_id}|webhook_provisioner_test|ts={agent_id}"}
                async with session.post(existing["url"], json=payload) as response:
                    ...
                    if success:
                        ...
                        logger.warning("Test message failed for %s: status %s", agent_id, response.status)
                    return success
        except Exception as e:
...
def channel_from_id(bot: discord.Client, channel_id: int) -> Optional[discord.TextChannel]:
    """
