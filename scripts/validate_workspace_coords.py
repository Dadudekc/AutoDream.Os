...
def list_workspace_agents(workspaces_dir: Path) -> set[str]:
    ...
    return {p.name for p in workspaces_dir.iterdir() if p.is_dir() and p.name.startswith("Agent-")}
...
def validate_workspaces(
    ...
    if missing or offenders:
        if missing:
            logger.info(f"Agents missing workspace directories: {sorted(missing)}")
        if offenders:
        ...
        return False
    if extra:
        logger.info(f"Extra workspace directories without SSOT entry: {sorted(extra)}")
    return True
