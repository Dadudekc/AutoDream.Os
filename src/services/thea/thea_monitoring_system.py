...
class TheaMonitoringSystem:
    ...
    def _cleanup_old_data(self):
        ...
        self.performance_metrics = [m for m in self.performance_metrics if m.timestamp > cutoff_str]

        # Cleanup system health history
        self.system_health_history = [
            h for h in self.system_health_history if h.timestamp > cutoff_str
        ]

        # Cleanup activity log
        self.activity_log = [a for a in self.activity_log if a.get("timestamp", "") > cutoff_str]
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_operation": self.performance_metrics[-1].timestamp
