...
from .validation_rules import get_default_onboarding_rules
...
class OnboardingValidator(BaseValidator):
    ...
    def __init__(self):
        ...
        self.onboarding_stages = ONBOARDING_STAGES.copy()
        self.verification_methods = VERIFICATION_METHODS.copy()

    def _setup_default_rules(self) -> None:
        """Setup default onboarding validation rules"""
        for rule in get_default_onboarding_rules():
    ...
    def _validate_verification(self, verification: Any) -> List[ValidationResult]:
        ...
        if "status" in verification:
            ...
            if isinstance(status, str):
                if status.lower() not in VERIFICATION_STATUSES:
                    result = self._create_result(
                        ...
                        actual_value=status,
                        expected_value=f"one of {VERIFICATION_STATUSES}",
                    )
                    results.append(result)
    ...
    def _validate_compliance(self, compliance: Any) -> List[ValidationResult]:
        ...
        if "status" in compliance:
            ...
            if isinstance(status, str):
                if status.lower() not in COMPLIANCE_STATUSES:
                    result = self._create_result(
                        ...
                        actual_value=status,
                        expected_value=f"one of {COMPLIANCE_STATUSES}",
                    )
                    results.append(result)
