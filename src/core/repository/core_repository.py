...
class IRepository(ABC):
    ...
    @abstractmethod
    async def read(self, entity_id: str) -> Any | None:
...
class CoreSystemRepository(IRepository):
    ...
    async def read(self, system_id: str) -> ICoreSystem | None:
...
class CoreSystemService:
    ...
    async def get_system(self, system_id: str) -> ICoreSystem | None:
    ...
    async def update_system(self, system_id: str, system: ICoreSystem) -> bool:
        ...
        if success:
            # Notify web interface
            self._notify_web_interface(
                "system_updated", {"system_id": system_id, "status": system.status.value}
            )
    ...
    async def unregister_system(self, system_id: str) -> bool:
        ...
        if success:
            # Notify web interface
            self._notify_web_interface("system_unregistered", {"system_id": system_id})
