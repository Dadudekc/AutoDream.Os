...
class DuplicationAnalyzer:
    ...
    def __init__(self):
        ...
        self.exclude_patterns = ['__pycache__', '.git', 'node_modules',
            '*.pyc', '.pytest_cache', 'build', 'dist', 'venv']
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.FunctionDef):
                            ...
                            functions[func_hash].append({'file': str(
                            ...
                            classes[class_hash].append({'file': str(py_file
                                .relative_to(self.project_root)), 'name':
                                node.name, 'line': node.lineno, 'content':
                                self._get_node_content(node, content)})
                except Exception as e:
                    logger.info(f'Warning: Could not process {py_file}: {e}')
        return {'functions': dict(functions), 'classes': dict(classes),
            'imports': dict(imports), 'summary': {'total_files': len(list(
            self.project_root.rglob('*.py'))), 'processed_files': len([f for
            f in self.project_root.rglob('*.py') if self.
            _should_include_file(f)]), 'function_groups': len(functions),
            'class_groups': len(classes), 'potential_duplicates': sum(len(v
            ) > 1 for v in functions.values()) + sum(len(v) > 1 for v in
            classes.values())}}
    ...
    def analyze_duplicates(self, scan_results: dict[str, Any]) ->dict[str, Any
        ...
        analysis = {'true_duplicates': [], 'similar_functions': [],
            'false_duplicates': [], 'consolidation_candidates': [],
            'risk_assessment': {}}
        for hash_key, instances in scan_results['functions'].items():
            if len(instances) > 1:
                category = self._categorize_function_duplicates(instances)
                analysis[category].append({'hash': hash_key, 'instances':
                    instances, 'similarity_score': self.
                    _calculate_similarity(instances)})
    ...
    def _are_contents_similar(self, contents: list[str]) ->bool:
        ...
        for i in range(len(contents)):
            for j in range(i + 1, len(contents)):
                similarity = difflib.SequenceMatcher(None, contents[i],
                    contents[j]).ratio()
                if similarity > 0.8:
                    return True
        return False
    ...
    def _generate_consolidation_plan(self, analysis: dict[str, Any]) ->dict[
        str, Any]:
        """Generate detailed consolidation plan."""
        plan = {'safe_consolidations': [], 'risky_consolidations': [],
            'manual_review_required': [], 'estimated_effort': {},
            'risk_assessment': {}}
        for duplicate in analysis['true_duplicates']:
            instances = duplicate['instances']
            if len(instances) >= 2:
                consolidation = {'type': 'exact_duplicate', 'target_file':
                    self._choose_target_file(instances), 'source_files': [
                    inst['file'] for inst in instances], 'function_class':
                    instances[0]['name'], 'risk_level': 'LOW', 'effort':
                    'SMALL'}
                plan['safe_consolidations'].append(consolidation)
        for similar in analysis['similar_functions']:
            instances = similar['instances']
            if len(instances) >= 2:
                consolidation = {'type': 'similar_function', 'target_file':
                    self._choose_target_file(instances), 'source_files': [
                    inst['file'] for inst in instances], 'function_class':
                    instances[0]['name'], 'similarity_score': similar[
                    'similarity_score'], 'risk_level': 'MEDIUM', 'effort':
                    'MEDIUM'}
                plan['risky_consolidations'].append(consolidation)
        for false_dup in analysis['false_duplicates']:
            instances = false_dup['instances']
            if len(instances) >= 2:
                review_item = {'type': 'potential_false_duplicate', 'files':
                    [inst['file'] for inst in instances], 'function_class':
                    instances[0]['name'], 'domains': list({self.
                    _get_file_domain(inst['file']) for inst in instances}),
                    'reason':
                    'Different domains or purposes despite similar code'}
    ...
    def generate_report(self, scan_results: dict[str, Any], analysis: dict[
        str, Any]) ->str:
        ...
        report.append(
            f"- Potential duplicates: {summary['potential_duplicates']}")
        report.append('')
        ...
        report.append(
            f"- **False Duplicates:** {len(analysis['false_duplicates'])} (DO NOT touch)"
            )
        report.append('')
        plan = analysis['consolidation_plan']
        report.append('## 🎯 CONSOLIDATION PLAN')
        report.append(
            f"- **Safe Consolidations:** {len(plan['safe_consolidations'])}")
        report.append(
            f"- **Risky Consolidations:** {len(plan['risky_consolidations'])}")
        report.append(
            f"- **Manual Review Required:** {len(plan['manual_review_required'])}"
            )
        report.append('')
        if plan['safe_consolidations']:
            report.append('### ✅ SAFE CONSOLIDATIONS (LOW RISK)')
            for i, consolidation in enumerate(plan['safe_consolidations'][:
                10], 1):
                report.append(f"{i}. **{consolidation['function_class']}**")
                report.append(f"   - Target: `{consolidation['target_file']}`")
                report.append(
                    f"   - Sources: {len(consolidation['source_files'])} files"
                    )
        ...
        if plan['risky_consolidations']:
            ...
            for i, consolidation in enumerate(plan['risky_consolidations'][
                :5], 1):
                report.append(f"{i}. **{consolidation['function_class']}**")
                report.append(
                    f"   - Similarity: {consolidation['similarity_score']:.1%}"
                    )
                report.append(
                    f"   - Files: {len(consolidation['source_files'])}")
                report.append(f"   - Risk: {consolidation['risk_level']}")
                report.append('')
...
def main():
    ...
    parser.add_argument('--find-true-duplicates', action='store_true', help
        ='Find exact duplicates')
    parser.add_argument('--generate-consolidation-plan', action=
        'store_true', help='Generate consolidation plan')
    ...
    if args.scan:
        ...
        logger.info(
            f"   Risky consolidations: {len(plan['risky_consolidations'])}")
        logger.info(
            f"   Manual reviews needed: {len(plan['manual_review_required'])}")
    else:
        logger.info('Usage:')
