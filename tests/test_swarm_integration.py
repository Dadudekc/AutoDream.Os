...
from core.swarm_coordination_system import (
    SwarmCoordinationSystem,
    SwarmIntegrationStatus,
)
...
class TestSwarmCoordinationSystem:
    ...
    def setup_method(self):
        ...
        self.coordination_system = SwarmCoordinationSystem(
            self.mock_agent_manager, self.mock_task_manager
        )
        ...
        for py_file in v2_src_path.glob("*.py"):
            if "swarm" in py_file.name.lower():
                with open(py_file, "r", encoding="utf-8") as f:
                    ...
                    assert (
                        len(code_lines) <= 200
                    ), f"{py_file.name} exceeds 200 LOC limit: {len(code_lines)}"
