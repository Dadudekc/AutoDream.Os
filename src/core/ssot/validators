...
component = Basic_Validator(config)
        ...
        try:
            ...
            for field in self.validation_rules["required_fields"]:
                if not hasattr(component, field) or getattr(component, field) is None:
                    ...
                    isinstance(getattr(component, field), str)
                    and not getattr(component, field).strip()
                ):
            ...
            for field, limit in self.validation_rules["field_length_limits"].items():
                if hasattr(component, field):
                    ...
                    if isinstance(value, str) and len(value) > limit:
                        issues.append(
                            f"Field '{field}' exceeds length limit ({len(value)} > {limit})"
                        )
