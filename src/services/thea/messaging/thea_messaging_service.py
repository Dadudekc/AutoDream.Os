...
class TheaMessagingService:
    ...
    def __init__(self, config: TheaConfig, browser_service:
        TheaBrowserService, response_service: TheaResponseService):
        self.config = config
    ...
    def _send_via_manual(self, message: str) ->MessageStatus:
        ...
        logger.info('-' * 30)
        try:
            if (not self.browser.driver or self.browser.mode == BrowserMode
                .MANUAL):
    ...
    def load_message_template(self) ->(str | None):
        ...
        try:
            if self.config.message_template_path.exists():
                with open(self.config.message_template_path, encoding='utf-8'
                    ) as f:
                    return f.read().strip()
        except Exception as e:
