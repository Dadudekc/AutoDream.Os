...
class DocumentationConsolidationSystem:
    ...
    def _classify_documentation_type(self, filename: str, content: str) -> DocumentationType:
        ...
        if "readme" in filename_lower:
            ...
            pattern in filename_lower for pattern in ["duplicate", "copy", "backup", "old", "temp"]
        ):
    ...
    def _determine_priority(
        self, file_path: Path, content: str, doc_type: DocumentationType
        ...
        if doc_type in [
            DocumentationType.DEVLOG,
            DocumentationType.DUPLICATE,
            DocumentationType.EMPTY,
        ]:
    ...
    def create_consolidation_plan(self) -> dict[str, Any]:
        ...
        consolidation_targets = {
            "README_FILES": [
                doc for doc in by_type["readme"] if doc.priority != DocumentationPriority.DELETE
            ],
            "API_DOCS": [
                doc for doc in by_type["api_docs"] if doc.priority != DocumentationPriority.DELETE
            ],
            "TUTORIALS": [
                doc for doc in by_type["tutorial"] if doc.priority != DocumentationPriority.DELETE
            ],
            "CONFIG_DOCS": [
                doc for doc in by_type["config"] if doc.priority != DocumentationPriority.DELETE
            ],
            "DEVLOGS": [
                doc for doc in by_type["devlog"] if doc.priority == DocumentationPriority.DELETE
            ],
            "DUPLICATES": [
                doc for doc in by_type["duplicate"] if doc.priority == DocumentationPriority.DELETE
            ],
            "EMPTY_FILES": [
                doc for doc in by_type["empty"] if doc.priority == DocumentationPriority.DELETE
            ],
        }
    ...
    def _calculate_reduction_percentage(self) -> float:
        ...
        files_to_consolidate = (
            len(self.consolidation_plan["consolidate_readme"])
            + len(self.consolidation_plan["consolidate_api"])
            + len(self.consolidation_plan["consolidate_tutorials"])
        )
    ...
    def _perform_consolidation(self) -> dict[str, Any]:
        ...
        for file_list in [
            self.consolidation_plan["delete_devlogs"],
        ...
        return {
            ...
            "priority_breakdown": {
                "critical": len(
                    [
                        d
                        for d in self.documentation_files
                        if d.priority == DocumentationPriority.CRITICAL
                    ]
                ),
                "high": len(
                    [
                        d
                        for d in self.documentation_files
                        if d.priority == DocumentationPriority.HIGH
                    ]
                ),
                "medium": len(
                    [
                        d
                        for d in self.documentation_files
                        if d.priority == DocumentationPriority.MEDIUM
                    ]
                ),
                "low": len(
                    [d for d in self.documentation_files if d.priority == DocumentationPriority.LOW]
                ),
                "delete": len(
                    [
                        d
                        for d in self.documentation_files
                        if d.priority == DocumentationPriority.DELETE
                    ]
                ),
            },
            "type_breakdown": {
                "readme": len(
                    [d for d in self.documentation_files if d.doc_type == DocumentationType.README]
                ),
                "devlog": len(
                    [d for d in self.documentation_files if d.doc_type == DocumentationType.DEVLOG]
                ),
                "duplicate": len(
                    [
                        d
                        for d in self.documentation_files
                        if d.doc_type == DocumentationType.DUPLICATE
                    ]
                ),
                "empty": len(
                    [d for d in self.documentation_files if d.doc_type == DocumentationType.EMPTY]
                ),
            },
