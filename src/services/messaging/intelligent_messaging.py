...
class IntelligentMessagingService(MessagingService):
    ...
    def send_message(
        ...
        logger.info(
            f"✅ Message sent: {success}, Intelligence gathered: {len(suggestions)} insights"
        )
    ...
    def _learn_from_message(
        self, agent_id: str, message: str, from_agent: str, priority: str, success: bool
    ):
    ...
    def _learn_from_broadcast(
        self, message: str, from_agent: str, priority: str, results: dict[str, bool]
    ):
    ...
    def _get_message_suggestions(
        self, agent_id: str, message: str, from_agent: str
    ) -> dict[str, Any]:
        ...
        try:
            ...
            similar_messages = self.retriever.search(
                f"successful messages to {agent_id}", kinds=["action"], k=5
            )
            ...
            suggestions = {
                ...
                "message_optimization": self._suggest_message_optimization(
    ...
    def _get_broadcast_insights(
        self, message: str, from_agent: str, results: dict[str, bool]
    ) -> dict[str, dict[str, Any]]:
    ...
    def _suggest_optimal_timing(self, agent_id: str) -> str:
        ...
        try:
            ...
            timing_patterns = self.retriever.search(
                f"successful messages to {agent_id}", kinds=["action"], k=10
            )
    ...
    def _suggest_message_optimization(self, message: str, similar_messages: list[dict]) -> str:
        ...
        try:
            if len(similar_messages) > 0:
                ...
                successful_patterns = [
                    msg for msg in similar_messages if msg.get("outcome") == "success"
                ]
    ...
    def _calculate_success_probability(self, agent_id: str, message: str) -> float:
        ...
        try:
            ...
            similar_messages = self.retriever.search(
                f"messages to {agent_id}", kinds=["action"], k=20
            )
    ...
    def _get_communication_history(self, agent_id: str, from_agent: str) -> dict[str, Any]:
        ...
        try:
            ...
            communication_patterns = self.retriever.search(
                f"communication between {from_agent} and {agent_id}", kinds=["action"], k=10
            )

            return {
                "total_interactions": len(communication_patterns),
                "successful_interactions": len(
                    [p for p in communication_patterns if p.get("outcome") == "success"]
                ),
    ...
    def _get_response_patterns(self, agent_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            response_patterns = self.retriever.search(
                f"responses from {agent_id}", kinds=["action"], k=10
            )
    ...
    def get_agent_communication_intelligence(self, agent_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            communication_patterns = self.retriever.search(
                f"communication with {agent_id}", kinds=["action"], k=20
            )

            # Get successful patterns
            successful_patterns = self.retriever.how_do_agents_do(
                f"successful communication with {agent_id}", k=10
            )
    ...
    def _calculate_communication_effectiveness(self, agent_id: str) -> float:
        ...
        try:
            ...
            communications = self.retriever.search(
                f"communication with {agent_id}", kinds=["action"], k=50
            )

            if len(communications) > 0:
                successful_communications = [
                    c for c in communications if c.get("outcome") == "success"
                ]
    ...
    def _get_optimal_strategies(self, agent_id: str) -> list[str]:
        ...
        try:
            ...
            strategies = self.retriever.search(
                f"successful communication strategies with {agent_id}", kinds=["action"], k=10
            )
