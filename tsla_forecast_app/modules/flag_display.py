...
from .trading_flags import AgentFlag, FlagType, TradingFlag, TradingFlagEngine
    ...
    def generate_flags(self, stock_data: dict[str, Any] = None):
        ...
        try:
            ...
            self.status_label.setText(
                f"Generated {len(flags)} flags at {datetime.now().strftime('%H:%M:%S')}"
            )
...
"""
            ...
            # Update status
            self.status_label.setText(
                f"Consensus generated at {datetime.now().strftime('%H:%M:%S')}"
            )
    ...
    def refresh_flags(self):
        ...
        self.status_label.setText(
            f"Refreshed {len(active_flags)} flags at {datetime.now().strftime('%H:%M:%S')}"
        )
        self.status_label.setStyleSheet("color: green;")
