...
def test_basic_functionality():
    ...
    try:
        ...
        print(f"✅ Status retrieved: {status['messages_queued']} queued, {status['messages_delivered']} delivered")
...
def test_priority_system():
    ...
    try:
        ...
        for priority in priorities:
            msg_id = mq_system.queue_message(
                "Agent-1", "Agent-3", f"Test {priority.name} priority message",
                priority=priority
            )
        ...
        # Check status
        status = mq_system.get_queue_status()
        print(f"✅ Priority test completed: {status['messages_delivered']} messages delivered")
...
def test_high_priority_flag():
    ...
    try:
        from v1_v2_message_queue_system import V1V2MessageQueueSystem, send_high_priority_message
        ...
        msg_id = send_high_priority_message(
            mq_system, "Agent-5", "Agent-3", "URGENT: Test high-priority message with Ctrl+Enter x2!"
        )
        ...
        # Check status
        status = mq_system.get_queue_status()
        print(f"✅ High-priority test completed: {status['messages_delivered']} messages delivered")
...
def main():
    ...
    if passed == total:
        print("🎉 All tests passed! The V1-V2 Message Queue System is working correctly.")
    else:
