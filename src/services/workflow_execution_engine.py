...
class WorkflowExecutionEngine:
    ...
    def _can_execute_step(
        self, step_data: WorkflowStepData, completed_steps: List[WorkflowStep]
    ) -> bool:
    ...
    def _execute_step(
        self, step_data: WorkflowStepData, workflow: WorkflowExecution
    ) -> bool:
        """Execute a single workflow step"""
        self.logger.info(
            f"Executing step {step_data.step_id} for workflow {workflow.workflow_id}"
        )
        ...
        for handler in handlers:
            try:
                ...
                if not result:
                    self.logger.error(
                        f"Handler {handler.__name__} failed for step {step_data.step_id}"
                    )
                    return False
            except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--test", "-t", action="store_true", help="Test workflow execution"
    )
