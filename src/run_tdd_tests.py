...
class TDDTestRunner:
    ...
    def run_tests(self, test_paths: List[Path], coverage: bool = True, 
                  parallel: bool = False, verbose: bool = True) -> Dict[str, Any]:
    ...
    def _save_test_results(self, results: Dict[str, Any]) -> None:
        ...
        for key, value in results.items():
            if key in ["stdout", "stderr"]:
                ...
                serializable_results[key] = str(value)[:1000] + "..." if len(str(value)) > 1000 else str(value)
            else:
    ...
    def run_performance_tests(self) -> Dict[str, Any]:
        ...
        performance_tests = [f for f in self.discover_tests() if "performance" in f.name.lower()]
    ...
    def generate_test_report(self) -> None:
        ...
        if len(smoke_tests) < 5:
            report["recommendations"].append("Consider adding more smoke tests for critical functionality")
        
        if len(unit_tests) < len(test_files) * 0.3:
            report["recommendations"].append("Increase unit test coverage for better code quality")
        
        if not (self.project_root / "tests" / "performance").exists():
            report["recommendations"].append("Consider adding performance testing directory")
    ...
    def validate_test_infrastructure(self) -> Dict[str, Any]:
        ...
        if self.test_dir.exists():
            ...
            if (self.test_dir / "test_utils.py").exists():
                ...
                validation["issues"].append("test_utils.py not found in tests directory")
        else:
...
def main():
    ...
    parser.add_argument(
        "--validate", 
        action="store_true",
        help="Validate test infrastructure only"
    )
