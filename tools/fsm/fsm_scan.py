...
from src.fsm.fsm_registry import AgentState, SwarmState, validate_agent_state, validate_swarm_state
...
def scan_swarm_state() -> tuple[str | None, list[str]]:
    ...
    if swarm_file.exists():
        try:
            with open(swarm_file, encoding="utf-8") as f:
                data = json.load(f)
...
def scan_all_states() -> tuple[dict[str, str], str | None, list[str]]:
...
def print_scan_results(agents: dict[str, str], swarm: str | None, issues: list[str]):
    ...
    print(
        f"  - Swarm state: {'Valid' if swarm and validate_swarm_state(swarm) else 'Invalid/Missing'}"
    )
