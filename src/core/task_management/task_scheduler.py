...
class TaskScheduler:
    ...
    def __init__(self, max_concurrent_tasks: int = 100):
        ...
        logger.info(f"TaskScheduler initialized with max {max_concurrent_tasks} concurrent tasks")
    ...
    def _calculate_priority_score(self, task: Task) -> float:
        ...
        if task.constraints.deadline:
            time_until_deadline = (task.constraints.deadline - datetime.now()).total_seconds()
            if time_until_deadline > 0:
                base_score += 10.0 / (time_until_deadline + 1)  # Higher score for urgent tasks
    ...
    def _can_agent_handle_task(self, agent_id: str) -> bool:
        ...
        # Check current load
        current_tasks = sum(1 for task in self._running_tasks.values() 
                          if task.assigned_agent == agent_id)
        
        # Simple limit - could be more sophisticated
        return current_tasks < 5
    
    def _get_next_task_from_priority(self, priority: TaskPriority, agent_id: str) -> Optional[Task]:
        ...
        while not queue.empty():
            ...
            if (task.status == TaskStatus.PENDING and 
                self._can_agent_handle_task(agent_id) and
                self._agent_can_handle_task(task, agent_id)):
    ...
    def _agent_can_handle_task(self, task: Task, agent_id: str) -> bool:
        ...
        if (task.constraints.required_agents and 
            agent_id not in task.constraints.required_agents):
            return False
