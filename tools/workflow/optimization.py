...
class PerformanceMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.steps_completed / (self.steps_completed + self.steps_failed) if (self.steps_completed + self.steps_failed) > 0 else 0
        }
...
class PerformanceMonitor:
    ...
    def get_average_metrics(self) -> Optional[Dict[str, float]]:
        ...
        return {
            "average_execution_time": sum(m.execution_time for m in self.metrics_history) / total_executions,
            "average_memory_usage": sum(m.memory_usage for m in self.metrics_history) / total_executions,
            "average_cpu_usage": sum(m.cpu_usage for m in self.metrics_history) / total_executions,
            "average_success_rate": sum(m.steps_completed / (m.steps_completed + m.steps_failed) for m in self.metrics_history if (m.steps_completed + m.steps_failed) > 0) / total_executions,
...
class WorkflowOptimizer:
    ...
    def optimize_workflow_execution(self, workflow_steps: List[Any], 
        ...
        while remaining_steps:
            ...
            for step in remaining_steps[:]:
                if not hasattr(step, 'dependencies') or not step.dependencies:
                    ...
                    completed_deps = all(
                        dep in [s.step_id for s in current_group] 
    ...
    def _execute_group_parallel(self, group: List[Any], execution_function: Callable) -> Dict[str, Any]:
    ...
    def _execute_group_sequential(self, group: List[Any], execution_function: Callable) -> Dict[str, Any]:
