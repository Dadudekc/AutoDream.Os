    ...
    def execute_step(self, step: WorkflowStep) -> bool:
        ...
        try:
            ...
            success = self.messaging_service.send_message(
                agent_id=step.agent_id, message=message, from_agent="WorkflowManager"
            )
    ...
    def run_workflow(self, max_concurrent: int = 2):
        ...
        while True:
            ...
            if not ready_steps:
                ...
                completed_steps = [
                    s for s in self.steps if s.status in ["completed", "failed", "timeout"]
                ]
...
def main():
    ...
    try:
        ...
        if args.command == "run":
            print(
                f"🐝 WE ARE SWARM - Starting workflow: {manager.workflow_data.get('name', 'Unnamed')}"
            )
            ...
            print("🐝 WE ARE SWARM - Workflow Status:")
