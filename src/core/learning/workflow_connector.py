...
from ..workflow.types.workflow_models import WorkflowDefinition, WorkflowExecution, WorkflowStep


class LearningWorkflowConnector:
    """Connects unified learning system with existing workflow engine"""
    
    def __init__(self, learning_engine: UnifiedLearningEngine, workflow_engine: WorkflowEngine):
    ...
    def _setup_learning_hooks(self) -> None:
        ...
        self.add_learning_hook("learning_workflow_started", self._on_learning_workflow_started)
        self.add_learning_hook("learning_workflow_step_completed", self._on_learning_workflow_step_completed)
        self.add_learning_hook("learning_workflow_completed", self._on_learning_workflow_completed)
        self.add_learning_hook("learning_workflow_failed", self._on_learning_workflow_failed)
    ...
    def create_learning_workflow(self, workflow_name: str, agent_id: str, 
                                learning_parameters: Optional[Dict[str, Any]] = None) -> Optional[str]:
        ...
        try:
            ...
            if not session_id:
                self.logger.error("Failed to create learning session using existing engine")
                return None
            ...
            self.workflow_engine.workflow_definitions[workflow_definition.workflow_id] = workflow_definition
            
            # Create workflow execution using existing workflow engine
            execution = self.workflow_engine.create_workflow_execution(workflow_definition.workflow_id)
            
            if not execution:
                self.logger.error("Failed to create workflow execution using existing engine")
                return None
            ...
            self.logger.info(f"Created learning workflow: {learning_workflow_id} using existing engines")
    ...
    def _create_learning_workflow_definition(self, workflow_name: str, agent_id: str, 
                                           learning_parameters: Optional[Dict[str, Any]] = None) -> WorkflowDefinition:
    ...
    def execute_learning_workflow(self, learning_workflow_id: str) -> bool:
        ...
        try:
            ...
            self._trigger_learning_hooks("learning_workflow_started", {
            ...
            self.logger.error(f"Failed to execute learning workflow: {learning_workflow_id} - {e}")
            return False
    ...
    def execute_learning_step(self, learning_workflow_id: str, step_id: str) -> bool:
        ...
        try:
            ...
            if not self.workflow_engine.execute_workflow_step(workflow_execution_id, step_id):
                return False
            
            # Execute learning operation using existing learning engine
            step = next((s for s in self.workflow_engine.active_workflows[workflow_execution_id].steps 
            ...
            if step:
                ...
                if operation_type == "session_initiation":
                    ...
                    self.logger.info(f"Learning session initiation completed for step: {step_id}")
                    ...
                    execution_result = self.learning_engine.execute_learning_operation(
                        ...
                        parameters={"step_id": step_id, "workflow_name": learning_session["workflow_name"]}
                    )
                    
                    if not execution_result:
                        self.logger.error(f"Learning execution failed for step: {step_id}")
                        return False
                        
                elif operation_type == "progress_evaluation":
                    # Use existing metrics system for evaluation
                    performance_summary = self.learning_engine.get_learning_performance_summary(
                        learning_session["agent_id"]
                    )
                    
                    if performance_summary and "error" not in performance_summary:
                        self.logger.info(f"Learning progress evaluation completed for step: {step_id}")
                    else:
                        self.logger.error(f"Learning progress evaluation failed for step: {step_id}")
                        return False
            
            # Trigger step completed hook
            self._trigger_learning_hooks("learning_workflow_step_completed", {
                "learning_workflow_id": learning_workflow_id,
                "step_id": step_id,
                "workflow_execution_id": workflow_execution_id,
    ...
    def get_learning_workflow_status(self, learning_workflow_id: str) -> Dict[str, Any]:
        ...
        workflow_status = self.workflow_engine.get_workflow_status(workflow_execution_id)
        ...
        return {
            ...
            "learning_session_status": learning_session_data.get("status", "unknown") if learning_session_data else "unknown",
    ...
    def list_learning_workflows(self, status_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        """List learning workflows with optional status filter"""
        workflows = []
        
        for learning_workflow_id, learning_session in self.learning_workflow_sessions.items():
            if status_filter is None or learning_session["status"] == status_filter:
                workflows.append(self.get_learning_workflow_status(learning_workflow_id))
    ...
    def _on_learning_workflow_step_completed(self, data: Dict[str, Any]) -> None:
        ...
        self.logger.info(f"Learning workflow step completed: {step_id} in workflow: {learning_workflow_id}")
        ...
        if learning_workflow_id in self.learning_workflow_sessions:
            ...
            if workflow_status == WorkflowStatus.COMPLETED:
                ...
                self._trigger_learning_hooks("learning_workflow_completed", {
                    "learning_workflow_id": learning_workflow_id,
                    "workflow_execution_id": workflow_execution_id,
    ...
    def cleanup_completed_workflows(self) -> None:
        ...
        for learning_workflow_id in completed_ids:
            ...
            self.logger.info(f"Cleaned up completed learning workflow: {learning_workflow_id}")
    ...
    def run_integration_test(self) -> bool:
        ...
        try:
            ...
            workflow_execution = self.workflow_engine.active_workflows.get(workflow_execution_id)
            if workflow_execution:
                for step in workflow_execution.steps:
                    if not self.execute_learning_step(learning_workflow_id, step.step_id):
                        return False
            
            # Verify completion
            status = self.get_learning_workflow_status(learning_workflow_id)
            if status.get("current_status") == "completed":
                self.logger.info(f"Learning workflow integration test passed: {learning_workflow_id}")
                return True
            else:
                self.logger.error(f"Learning workflow integration test failed: {learning_workflow_id}")
                return False
