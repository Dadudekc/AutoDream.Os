...
class PerpetualMotionContractService:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    log_dir / "perpetual_motion_contracts.log", encoding="utf-8"
                ),
    ...
    def _load_contract_templates(self) -> List[ContractTemplate]:
        ...
        try:
            ...
            if templates_file.exists():
                ...
                self.logger.info(
                    f"Loaded {len(templates)} contract templates from file"
                )
            else:
    ...
    def on_contract_completion(
        self, contract_id: str, agent_id: str, completion_data: Dict[str, Any]
    ):
        """Called when a contract is completed - triggers automatic generation of new contracts."""
        try:
            self.logger.info(
                f"Contract {contract_id} completed by {agent_id} - generating new contracts"
            )

            # Generate new contracts automatically
            new_contracts = self._generate_new_contracts(
                agent_id, self.contracts_per_completion
            )
            ...
            self.logger.info(
                f"🔄 PERPETUAL MOTION: Generated {len(new_contracts)} new contracts for {agent_id}"
            )
    ...
    def _auto_assign_contracts(self, agent_id: str, contracts: List[GeneratedContract]):
        ...
        try:
            ...
            self.logger.info(
                f"🔄 Auto-assigned {len(contracts)} contracts to {agent_id}"
            )
    ...
    def _create_task_assignment(self, contract: GeneratedContract):
        ...
        try:
            ...
            task_file = Path(
                f"agent_workspaces/{contract.assignee}/tasks/{contract.task_id}.json"
            )
    ...
    def _update_perpetual_motion_metrics(
        self, agent_id: str, completed_contract_id: str
    ):
    ...
    def detect_contract_completion(
        self, agent_id: str, trigger_type: str, data: Dict[str, Any]
    ):
        """🔴 CRITICAL: Detect contract completion from various triggers and auto-resume perpetual motion."""
        try:
            self.logger.info(
                f"🔄 PERPETUAL MOTION: Detected completion trigger '{trigger_type}' from {agent_id}"
            )
            ...
            if trigger_type in self.completion_triggers:
                ...
                if active_contracts:
                    ...
                    self.on_contract_completion(
                        contract_to_complete.contract_id, agent_id, data
                    )

                    self.logger.info(
                        f"🔄 PERPETUAL MOTION: Contract {contract_to_complete.contract_id} completed - new contracts generated!"
                    )
                    return True
                else:
                    # No active contracts - generate new ones anyway
                    self.logger.info(
                        f"🔄 PERPETUAL MOTION: No active contracts for {agent_id} - generating new ones"
                    )
                    new_contracts = self._generate_new_contracts(
                        agent_id, self.contracts_per_completion
                    )
        ...
        try:
            ...
            for contract_file in self.contracts_dir.glob("*.json"):
                try:
                    ...
                    if contract_data.get("assignee") == agent_id and contract_data.get(
                        "state"
                    ) in ["ready", "assigned", "in_progress"]:
                        ...
                        contract = GeneratedContract(
                            ...
                            template_source=contract_data.get(
                    ...
                    self.logger.error(
                        f"Error reading contract file {contract_file}: {e}"
                    )
    ...
    def _send_resume_message(
        self, agent_id: str, new_contracts: List[GeneratedContract]
    ):
...
🔄 **PERPETUAL MOTION ACTIVE** - Keep completing contracts to keep the cycle going!"""
    ...
    def start_monitoring(self):
        ...
        self.monitor_thread = threading.Thread(
            target=self._monitor_contracts, daemon=True
        )
    ...
    def test_perpetual_motion(self, agent_id: str = "Agent-1"):
        ...
        try:
            ...
            if result:
                ...
                self.logger.info(
                    f"📊 Check agent_workspaces/{agent_id}/tasks/ for new assignments"
                )
                self.logger.info(
                    f"📊 Check agent_workspaces/{agent_id}/inbox/ for resume messages"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--test-completion", type=str, help="Test contract completion for agent"
    )
    parser.add_argument(
        "--test-perpetual-motion",
        type=str,
        help="🧪 TEST: Test the perpetual motion system for agent",
    )
