...
class AITestRunner:
    ...
    def generate_tests_for_file(
        ...
        try:
            ...
            test_content = self._generate_test_content(
                source_path, source_code, test_framework
            )
    ...
    def _generate_test_content(
        self, source_path: Path, source_code: str, test_framework: str
    ) -> str:
        """Analyze a source file and generate skeletal tests.

        The implementation inspects the given ``source_code`` using ``ast`` to
        locate all public functions and classes. It then derives the import path
        for ``source_path`` so the resulting tests can import the module under
        test. For each discovered function and class a basic test is emitted that
        verifies the object exists and demonstrates simple error handling by
        calling it with invalid arguments when possible.
        """

        # Determine import path relative to the project ``src`` directory
        try:
            rel_path = source_path.relative_to(self.project_path / "src")
        except ValueError:
            # Fallback if the file is not under ``src``
            rel_path = source_path
        module_path = ".".join(rel_path.with_suffix("").parts)

        # Parse the source for public symbols
        import ast
        ...
        # Build import section and tests
        test_lines = [
            '"""',
            ...
            f"import {module_path} as module",
            "",
            "class TestGenerated:",
        ...
        for func in functions:
            test_lines.extend(
                [
                    "",
                    f"    def test_{func}_callable(self):",
                    f"        assert hasattr(module, '{func}')",
                    f"        assert callable(module.{func})",
                    "",
                    f"    def test_{func}_invalid_args(self):",
                    "        sig = inspect.signature(module.{0})".format(func),
                    "        if sig.parameters:",
                    "            with pytest.raises(Exception):",
                    "                module.{0}()".format(func),
                ]
            )
        ...
        for cls in classes:
            test_lines.extend(
                [
                    "",
                    f"    def test_{cls}_exists(self):",
                    f"        assert hasattr(module, '{cls}')",
                    f"        assert inspect.isclass(module.{cls})",
                    "",
                    f"    def test_{cls}_init_invalid(self):",
                    "        init_sig = inspect.signature(module.{0}.__init__)".format(cls),
                    "        params = [p for p in init_sig.parameters.values()",
        ...
        if not functions and not classes:
            # Fallback to keep file a valid test module
            test_lines.extend(
                [
                    "",
