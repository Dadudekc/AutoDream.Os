...
class DemoSuite:
    ...
    def run_basic_demo(self):
        ...
        try:
            ...
            print(f"System Status: {status.total_agents} agents, {status.online_agents} online")
    ...
    def run_messaging_demo(self):
        ...
        try:
            ...
            for i, agent_id in enumerate(self.demo_agents):
                ...
                if self.system.send_message(agent_id, next_agent, message, MsgTag.COORDINATION):
        ...
        try:
            ...
            if task_id:
                ...
                if self.coordination.start_coordination_task(task_id):
                    ...
                    # Complete task steps
                    for agent_id in self.demo_agents:
                        self.coordination.complete_task_step(task_id, f"step_{agent_id}", f"completed by {agent_id}")
                    
                    # Mark task as completed
                    task = self.coordination.tasks[task_id]
                    task.status = self.coordination.tasks[task_id].__class__.status.__class__.COMPLETED
...
def main():
    ...
    parser.add_argument("--coordination", action="store_true", help="Run coordination demo")
