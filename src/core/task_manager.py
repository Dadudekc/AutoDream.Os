...
class TaskManager:
    ...
    def create_task(self, title: str, description: str, assigned_to: str, 
                   created_by: str, priority: TaskPriority = TaskPriority.NORMAL,
                   due_date: Optional[str] = None, dependencies: Optional[List[str]] = None,
    ...
    def get_tasks(self, agent_id: str, status: Optional[TaskStatus] = None,
    ...
    def update_task_status(self, task_id: str, status: TaskStatus) -> bool:
        ...
        try:
            if task_id in self.tasks:
                ...
                # Update file
                assigned_workspace = self.workspace_manager.get_workspace_info(task.assigned_to)
                if assigned_workspace:
                    tasks_path = Path(assigned_workspace.tasks_path)
                    task_file = tasks_path / f"{task_id}.json"
                    ...
                    self.logger.info(f"Task status updated: {task_id} {old_status.value} -> {status.value}")
                    return True
            return False
        except Exception as e:
    ...
    def assign_task(self, task_id: str, new_agent: str) -> bool:
        ...
        try:
            if task_id in self.tasks:
                ...
                self.logger.info(f"Task reassigned: {task_id} {old_agent} -> {new_agent}")
                return True
            return False
        except Exception as e:
    ...
    def get_task_status(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "overdue_count": len([t for t in tasks if t.due_date and t.due_date < datetime.now().isoformat()])
            }
