...
class DualRoleCoordinationSystem:
    ...
    def get_dual_role_status(self) -> Dict[str, Any]:
        ...
        team_beta_role = next((role for role in self.roles if role.role_type == RoleType.TEAM_BETA), None)
        phase_3_role = next((role for role in self.roles if role.role_type == RoleType.PHASE_3_INTEGRATION), None)
    ...
    def get_coordination_plan(self) -> Dict[str, Any]:
        ...
        team_beta_tasks = [task for task in self.coordination_tasks if task.role_type == RoleType.TEAM_BETA]
        phase_3_tasks = [task for task in self.coordination_tasks if task.role_type == RoleType.PHASE_3_INTEGRATION]
    ...
    def get_task_priorities(self) -> List[CoordinationTask]:
        ...
        return sorted(self.coordination_tasks, key=lambda task: (
    ...
    def get_integration_support_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_capabilities": len(self.integration_support["integration_specialist_capabilities"]),
