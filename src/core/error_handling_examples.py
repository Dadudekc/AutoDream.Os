...
def example_basic_error_handling():
    ...
    error_scenarios = [{'error_type': ErrorType.VALIDATION_ERROR,
        'severity': ErrorSeverity.LOW, 'message':
        'Invalid email format provided', 'source':
        'user_registration_endpoint'}, {'error_type': ErrorType.
        NETWORK_ERROR, 'severity': ErrorSeverity.MEDIUM, 'message':
        'Database connection timeout', 'source': 'database_service'}, {
        'error_type': ErrorType.SYSTEM_ERROR, 'severity': ErrorSeverity.
        HIGH, 'message': 'Memory allocation failed', 'source':
        'image_processing_service'}]
    logger.info('\n🚀 Processing error scenarios:')
    for i, scenario in enumerate(error_scenarios, 1):
        error = ErrorInfo(error_id=f'web_api_error_{i:03d}', error_type=
            scenario['error_type'], severity=scenario['severity'], status=
            ErrorStatus.DETECTED, message=scenario['message'], source=
            scenario['source'])
        context = ErrorContext(context_id=f'web_context_{i:03d}', error_id=
            error.error_id, variables={'user_id': f'user_{i}', 'request_id':
            f'req_{i:06d}', 'timestamp': datetime.now().isoformat()})
        action = orchestrator.handle_error(error, context)
        if action:
            logger.info(
                f"  {i}. {scenario['error_type'].value}: {action.status.value}"
                )
            logger.info(f'     Result: {action.result}')
        else:
            logger.info(
                f"  {i}. {scenario['error_type'].value}: No handler found")
...
def example_circuit_breaker_pattern():
    ...
    for attempt in range(1, 8):
        error = ErrorInfo(error_id=f'payment_error_{attempt:03d}',
            error_type=ErrorType.NETWORK_ERROR, severity=ErrorSeverity.HIGH,
            status=ErrorStatus.DETECTED, message=
            f'Payment API call failed (attempt {attempt})', source=
            'payment_service')
        context = ErrorContext(context_id=f'payment_context_{attempt:03d}',
            error_id=error.error_id, variables={'payment_id':
            f'pay_{attempt:06d}', 'amount': 100.0, 'currency': 'USD'})
        action = cb_recovery.recover(error, context)
        logger.info(
            f'  Attempt {attempt}: State={cb_recovery.state}, Status={action.status.value}'
            )
        logger.info(f'    Result: {action.result}')
        time.sleep(0.1)
...
def example_fallback_recovery():
    ...
    data_processing_scenarios = [{'error_type': ErrorType.VALIDATION_ERROR,
        'message': 'Invalid date format in user profile', 'fallback_data':
        {'date': '2025-01-01', 'source': 'default'}}, {'error_type':
        ErrorType.DATA_ERROR, 'message':
        'Missing required field in order data', 'fallback_data': {'status':
        'pending', 'source': 'default'}}, {'error_type': ErrorType.
        VALIDATION_ERROR, 'message': 'Invalid email format in contact form',
    ...
    for i, scenario in enumerate(data_processing_scenarios, 1):
        error = ErrorInfo(error_id=f'data_error_{i:03d}', error_type=
            scenario['error_type'], severity=ErrorSeverity.LOW, status=
            ErrorStatus.DETECTED, message=scenario['message'], source=
            'data_processing_pipeline')
        context = ErrorContext(context_id=f'data_context_{i:03d}', error_id
            =error.error_id, variables=scenario['fallback_data'])
...
def example_error_metrics_monitoring():
    ...
    logger.info('=' * 55)
    orchestrator = create_error_orchestrator()
    handlers = [SystemErrorHandler('monitoring_system'),
        NetworkErrorHandler('monitoring_network'), ValidationErrorHandler(
        'monitoring_validation')]
    for handler in handlers:
        orchestrator.register_handler(handler)
    recoveries = [CircuitBreakerRecovery('monitoring_cb'), FallbackRecovery
        ('monitoring_fb')]
    for recovery in recoveries:
        orchestrator.register_recovery(recovery)
    logger.info('✅ Monitoring system initialized')
    logger.info('\n🚀 Simulating production error scenarios:')
    error_types = [ErrorType.SYSTEM_ERROR, ErrorType.NETWORK_ERROR,
        ErrorType.VALIDATION_ERROR, ErrorType.TIMEOUT_ERROR, ErrorType.
        DATA_ERROR]
    severities = [ErrorSeverity.LOW, ErrorSeverity.MEDIUM, ErrorSeverity.
    ...
    for i in range(20):
        ...
        error = ErrorInfo(error_id=f'prod_error_{i:03d}', error_type=
            error_type, severity=severity, status=ErrorStatus.DETECTED,
            message=f'Production error {i} - {error_type.value}', source=
            'production_system')
        context = ErrorContext(context_id=f'prod_context_{i:03d}', error_id
            =error.error_id, variables={'server': f'server_{i % 5}',
            'region': 'us-east-1', 'timestamp': datetime.now().isoformat()})
        action = orchestrator.handle_error(error, context)
        processed_errors += 1
...
async def example_async_error_handling():
    ...
    async def process_request(request_id: int, should_fail: bool=False):
        ...
        try:
            if should_fail:
                error = ErrorInfo(error_id=f'async_error_{request_id:03d}',
                    error_type=ErrorType.NETWORK_ERROR, severity=
                    ErrorSeverity.MEDIUM, status=ErrorStatus.DETECTED,
                    message=f'Async request {request_id} failed', source=
                    'async_processor')
                context = ErrorContext(context_id=
                    f'async_context_{request_id:03d}', error_id=error.
                    error_id, variables={'request_id': request_id, 'async':
                    True})
                action = orchestrator.handle_error(error, context)
    ...
    logger.info(
        f'   Average per Request: {processing_time / len(tasks):.3f} seconds')
...
class DatabaseErrorHandler(ErrorHandler):
    ...
    def __init__(self, handler_id: str=None):
        super().__init__(handler_id or 'database_handler',
    ...
    def can_handle(self, error: ErrorInfo) ->bool:
        ...
        return (error.error_type == ErrorType.RESOURCE_ERROR and 'database' in
            error.message.lower())

    def handle_error(self, error: ErrorInfo, context: ErrorContext
        ) ->RecoveryAction:
        """Handle database error with retry logic."""
        try:
            action = RecoveryAction(action_id=
                f'db_action_{self.retry_count}', error_id=error.error_id,
                strategy=RecoveryStrategy.RETRY, description=
                f'Database retry attempt {self.retry_count + 1}/{self.max_retries}'
            ...
            if self.retry_count < self.max_retries:
                ...
                action.result = (
                    f'Database connection retry {self.retry_count} successful')
            else:
...
def example_custom_error_handler():
    ...
    db_error_scenarios = ['Database connection pool exhausted',
        'Query timeout on user table', 'Database server unavailable',
    ...
    for i, error_message in enumerate(db_error_scenarios, 1):
        error = ErrorInfo(error_id=f'db_error_{i:03d}', error_type=
            ErrorType.RESOURCE_ERROR, severity=ErrorSeverity.HIGH, status=
            ErrorStatus.DETECTED, message=error_message, source=
            'ecommerce_database')
        context = ErrorContext(context_id=f'db_context_{i:03d}', error_id=
            error.error_id, variables={'table': 'users', 'query_type':
            'SELECT', 'connection_pool': 'main'})
        action = orchestrator.handle_error(error, context)
...
async def run_all_examples():
    ...
    try:
        ...
        logger.info(f'=' * 50)
        logger.info(f'✅ Basic Error Handling: Validated')
        logger.info(f'✅ Circuit Breaker Pattern: Functional')
        logger.info(f'✅ Fallback Recovery: Operational')
        logger.info(f'✅ Error Metrics: Collected')
        logger.info(f'✅ Custom Handlers: Implemented')
        logger.info(f'✅ Async Error Handling: Tested')
        logger.info(
            f"""
