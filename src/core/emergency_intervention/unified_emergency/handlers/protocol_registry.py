...
class ProtocolRegistry:
    ...
    def register_protocol(self, protocol: InterventionProtocol) -> bool:
        ...
        try:
            protocol_id = f"{protocol.emergency_type.value}_{protocol.severity_threshold.value}"
    ...
    def list_protocols(
        self, emergency_type: EmergencyType | None = None
    ...
    def remove_protocol(self, emergency_type: EmergencyType, severity: EmergencySeverity) -> bool:
        """Remove a protocol."""
        try:
            protocol_id = f"{emergency_type.value}_{severity.value}"
            if protocol_id in self.protocols:
            ...
            for protocol_id, data in protocol_data.items():
                try:
                    protocol = EmergencyInterventionModels.create_intervention_protocol(
                        ...
                        severity_threshold=EmergencySeverity(data["severity_threshold"]),
                        actions=[InterventionAction(action) for action in data["actions"]],
