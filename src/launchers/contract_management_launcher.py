...
try:
    from services.contract_lifecycle_service import ContractLifecycleService, ContractState, ContractType
    from services.contract_validation_service import ContractValidationService, ViolationType, ValidationSeverity
    IMPORT_SUCCESS = True
...
class ContractManagementLauncher:
    ...
    def create_comprehensive_contract(self, title: str, description: str, contract_type: str,
        ...
        try:
            ...
            critical_issues = len([r for r in validation_results 
            ...
            return {
                ...
                "metrics": {
                    ...
                    "violation_metrics": {
                        ...
                        "violation_rate": round(violation_rate, 2) if total_contracts > 0 else 0
                    }
                },
                "services": {
                ...
                "recommendations": self._get_system_recommendations(health_score, unresolved_violations),
                "timestamp": time.time()
            }
    ...
    def _get_system_recommendations(self, health_score: float, unresolved_violations: int) -> List[str]:
        """Get system improvement recommendations"""
        recommendations = []
        
        if health_score < 50:
            recommendations.append("URGENT: Review and resolve critical contract violations")
            recommendations.append("Consider system maintenance and cleanup")
        
        if unresolved_violations > 5:
            recommendations.append("High number of unresolved violations - prioritize resolution")
    ...
    def run_system_maintenance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if expired_contracts:
                maintenance_log.append(f"Processed {len(expired_contracts)} expired contracts")
            ...
            maintenance_log.append(f"Validated {len(active_contracts)} active contracts")
    ...
    def demonstrate_contract_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            parties = [
                {"party_id": "demo-agent-1", "party_type": "agent", "role": "contractor", "permissions": ["execute"]},
                {"party_id": "demo-agent-2", "party_type": "agent", "role": "client", "permissions": ["monitor"]}
            ]
            terms = {
                "deliverables": ["Complete demo task", "Provide status report"],
                "acceptance_criteria": ["Task completed successfully", "Quality standards met"],
            ...
            self.lifecycle_service.transition_contract_state(contract_id, "active", "Demo activation")
            
            # Get system health before and after
            health_report = self.get_system_health_report()
            demo_log.append(f"System health score: {health_report.get('health_score', 'N/A')}")
...
def main():
    ...
    parser.add_argument("--demo", action="store_true", help="Run contract system demonstration")
    ...
    if args.create:
        ...
        result = launcher.create_comprehensive_contract(title, description, contract_type, parties, terms)
