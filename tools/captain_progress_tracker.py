...
class CaptainProgressTracker:
    ...
    def update_agent_progress(
        self, agent_id: str, task: str, status: str, efficiency: float = 0.0, quality: float = 0.0
    ) -> None:
    ...
    def update_v3_contract(
        self, contract_id: str, agent_id: str, status: str, progress: float = 0.0
    ) -> None:
    ...
    def _update_swarm_status(self) -> None:
        ...
        completed_contracts = sum(
            1
            for contract in self.progress_data["v3_contracts"].values()
        ...
        if completed_contracts >= 3:
            self.progress_data["swarm_status"][
                "current_phase"
            ] = ContractPhase.PHASE_3_INTEGRATION.value
        elif completed_contracts >= 1:
            self.progress_data["swarm_status"][
                "current_phase"
            ] = ContractPhase.PHASE_2_EXPANSION.value
        else:
            self.progress_data["swarm_status"][
                "current_phase"
            ] = ContractPhase.PHASE_1_FOUNDATION.value
    ...
    def get_v3_contract_progress(self, contract_id: str) -> V3ContractProgress | None:
    ...
    def generate_progress_report(self) -> str:
        ...
        report.append(
            f"- **Completed Contracts**: {swarm.completed_contracts}/{swarm.total_contracts}"
        )
        report.append("")
        ...
        for phase_name, phase_data in self.progress_data["phase_progress"].items():
            ...
            report.append(
                f"- **Progress**: {phase_data['completed']}/{phase_data['total']} ({phase_data['progress_percentage']}%)"
            )
        ...
        for contract_id, contract_data in self.progress_data["v3_contracts"].items():
            ...
            report.append(
                f"- **Dependencies**: {', '.join(contract_data['dependencies']) or 'None'}"
            )
            report.append("")
...
def main():
    ...
    parser.add_argument(
        "--update-contract", help="Update contract progress (contract_id:agent_id:status:progress)"
    )
