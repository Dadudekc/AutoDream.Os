...
class SimpleLinearRegression:
    ...
    def fit(self, X: list[float], y: list[float]) -> None:
        ...
        sum_x = sum(X)
        sum_y = sum(y)
        sum_xy = sum(x * y for x, y in zip(X, y, strict=False))
        ...
        ss_res = sum(
            (y_val - (self.slope * x + self.intercept)) ** 2 for x, y_val in zip(X, y, strict=False)
        )
...
class TimeSeriesPredictor:
    ...
    def get_accuracy(self) -> float:
        ...
        mse = sum(
            (actual - pred) ** 2 for actual, pred in zip(y_values, predicted_values, strict=False)
        ) / len(y_values)
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    for i in range(20):
        ml_engine.add_time_series_data(
            "sales_data", now + timedelta(hours=i), 100 + i * 2 + i % 3 * 5
        )
    ...
    for value in [10, 12, 11, 13, 10, 11, 12, 50, 11, 12]:
        ...
        logger.info(f"Value {value}: Anomaly={is_anomaly}, Z-score={z_score:.2f}")
