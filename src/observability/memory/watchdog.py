...
class MemoryWatchdog:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.monitor_thread = threading.Thread(
                target=self._monitor_loop, daemon=True, daemon=True
            )
    ...
    def _handle_critical(self, service: str, snapshot, result: Dict) -> None:
        ...
        self.alerts.append(alert)
        logger.critical(
            f"Memory critical for {service}: {snapshot.current_mb:.2f}MB - Action: {action}"
        )
...
class IntegratedWatchdog:
    ...
    def __init__(
        self,
        config_path: str = "config/memory_policy.yaml",
        enforcement_mode: EnforcementMode = EnforcementMode.OBSERVE,
    ):
    ...
    def run_full_check(self) -> Dict[str, Any]:
        ...
        for service in ["messaging", "autonomous_workflow", "vector_database", "agent_workspace"]:
            budget_results[service] = self.policy_manager.check_service(
                service, snapshot.current_mb
            )
...
class WatchdogManager:
    ...
    def get_all_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "watchdogs": {name: watchdog.get_status() for name, watchdog in self.watchdogs.items()},
        }
