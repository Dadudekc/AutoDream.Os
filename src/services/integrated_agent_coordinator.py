...
try:
    ...
    async def ping_discord_status(agent, status, message):
        print(f"[DISCORD] {agent}: {status} - {message}")

    async def ping_discord_progress(agent, task, progress, message):
        print(f"[DISCORD] {agent}: {task} - {progress}% - {message}")

    async def ping_discord_completion(agent, task, message):
        print(f"[DISCORD] {agent}: {task} COMPLETE - {message}")

    async def ping_discord_coordination(agent, message):
        print(f"[DISCORD] {agent}: COORDINATION - {message}")

    async def ping_discord_error(agent, error, details):
        print(f"[DISCORD] {agent}: ERROR - {error} - {details}")

    async def ping_discord_system(agent, message):
        print(f"[DISCORD] {agent}: SYSTEM - {message}")
...
class IntegratedAgentCoordinator:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    log_dir / "integrated_coordinator.log", encoding="utf-8"
                ),
        ...
        try:
            ...
            for agent in agents:
                ...
                if success:
                    ...
                    self.logger.warning(
                        f"⚠️ Failed to register {agent['name']} in message queue"
                    )

            self.logger.info(
                f"📋 Message queue registration complete: {len(agents)} agents"
            )
        ...
        try:
            ...
            try:
                if self.integration_mode in ["hybrid", "message_queue"]:
                    ...
                    if message_id:
                        self.logger.info(
                            f"✅ Message sent via queue to {agent_id}: {message_id}"
                        )
                        success = True
            ...
            if not success and use_pyautogui:
                ...
                if pyautogui_agent:
                    success = self.pyautogui_coordinator.send_message_to_agent(
                        pyautogui_agent, message
                    )

                    if success:
                        self.logger.info(
                            f"✅ Message sent via PyAutoGUI to {pyautogui_agent}"
                        )
                        self.coordination_cycle += 1
                        self.last_coordination = datetime.now()
                    else:
                        self.logger.error(
                            f"❌ PyAutoGUI fallback failed for {pyautogui_agent}"
                        )
                else:
                    self.logger.error(
                        f"❌ Could not map agent ID {agent_id} to PyAutoGUI format"
                    )
            ...
            try:
                if self.integration_mode in ["hybrid", "message_queue"]:
                    message_ids = self.message_queue_manager.broadcast_message(
                        source_agent="agent_5", content=message, priority=priority
                    )

                    if message_ids:
                        self.logger.info(
                            f"✅ Broadcast sent via queue to {len(message_ids)} agents"
                        )
            ...
            if not any(results.values()) and use_pyautogui:
                ...
                for pyautogui_agent in self.agent_mapping.keys():
                    ...
                    if success:
                        ...
                        self.logger.error(
                            f"❌ PyAutoGUI broadcast failed to {pyautogui_agent}"
                        )
    ...
    async def coordinate_agent_workflow_hybrid(
        self, agent_id: str, workflow_type: str, use_pyautogui: bool = True
    ) -> bool:
        ...
        try:
            ...
            message = workflow_messages.get(
                workflow_type, f"Agent-5 CAPTAIN: Execute {workflow_type} workflow."
            )
            ...
            if success:
                self.logger.info(
                    f"✅ {agent_id} workflow '{workflow_type}' coordinated successfully"
                )

                # Report to Discord if available
                if DISCORD_AVAILABLE:
                    await ping_discord_progress(
                        "Agent-5",
                        f"Agent Coordination",
                        int(
                            (list(self.agent_mapping.values()).index(agent_id) + 1) * 20
                        ),
                        f"Successfully coordinated {agent_id}",
                    )
            else:
                self.logger.error(
                    f"❌ Failed to coordinate {agent_id} workflow '{workflow_type}'"
                )

                # Report error to Discord if available
                if DISCORD_AVAILABLE:
                    await ping_discord_error(
                        "Agent-5",
                        f"Coordination failed for {agent_id}",
    ...
    async def execute_coordination_cycle_hybrid(self) -> bool:
        ...
        try:
            self.coordination_cycle += 1
            self.logger.info(
                f"🚀 Starting hybrid coordination cycle {self.coordination_cycle}"
            )

            # Report cycle start to Discord
            if DISCORD_AVAILABLE:
                await ping_discord_status(
                    "Agent-5",
                    "Hybrid Coordinating",
                    f"Starting coordination cycle {self.coordination_cycle}",
                )
            ...
            for agent_id, role in self.pyautogui_coordinator.agent_roles.items():
                ...
                success = await self.coordinate_agent_workflow_hybrid(
                    queue_agent_id, workflow_type
                )
            ...
            if success_count == total_agents:
                if DISCORD_AVAILABLE:
                    await ping_discord_completion(
                        "Agent-5",
                        f"Hybrid Coordination Cycle {self.coordination_cycle}",
                        f"{success_count}/{total_agents} agents coordinated successfully",
                    )
                self.logger.info(
                    f"✅ Hybrid coordination cycle {self.coordination_cycle} completed successfully"
                )
            else:
                if DISCORD_AVAILABLE:
                    await ping_discord_error(
                        "Agent-5",
                        f"Hybrid coordination cycle {self.coordination_cycle} incomplete",
                        f"Only {success_count}/{total_agents} agents coordinated successfully",
                    )
                self.logger.warning(
                    f"⚠️ Hybrid coordination cycle {self.coordination_cycle} partially completed"
                )
            ...
            if DISCORD_AVAILABLE:
                await ping_discord_error(
                    "Agent-5", "Hybrid coordination cycle error", str(e)
                )
            return False

    async def run_continuous_coordination_hybrid(
        self, cycles: int = 5, interval: int = 120
    ):
        """Run continuous coordination cycles using hybrid approach"""
        try:
            self.logger.info(
                f"🚀 Starting continuous hybrid coordination: {cycles} cycles, {interval}s interval"
            )

            if DISCORD_AVAILABLE:
                await ping_discord_status(
                    "Agent-5",
                    "Continuous Hybrid Coordination",
                    f"Starting {cycles} coordination cycles",
                )
                ...
                if cycle < cycles:
                    ...
                    if DISCORD_AVAILABLE:
                        await ping_discord_status(
                            "Agent-5",
                            "Standby",
                            f"Waiting {interval}s before cycle {cycle + 1}",
                        )
            ...
            if DISCORD_AVAILABLE:
                await ping_discord_completion(
                    "Agent-5",
                    "Continuous Hybrid Coordination Complete",
            ...
            if DISCORD_AVAILABLE:
                await ping_discord_error(
                    "Agent-5", "Continuous hybrid coordination error", str(e)
                )
        ...
        try:
            ...
            if len(report) > max_length:
                ...
                for i, chunk in enumerate(chunks):
                    await ping_discord_system(
                        "Agent-5", f"Integrated Status Report Part {i+1}/{len(chunks)}"
                    )
                    await asyncio.sleep(1.0)  # Small delay between chunks
            else:
                await ping_discord_system(
                    "Agent-5", "Integrated Coordination Status Report"
                )
...
class Agent5PyAutoGUICoordinator:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler(
                    log_dir / "agent5_coordination.log", encoding="utf-8"
                ),
                logging.StreamHandler(),
            ],
        )
        ...
        try:
            if self.coord_file.exists():
                with open(self.coord_file, "r") as f:
                    coords = json.load(f)
                    self.logger.info(
                        f"Coordinates loaded: {len(coords)} agent configurations"
                    )
                    return coords
            else:
    ...
    def send_message_to_agent(
        self, agent_id: str, message: str, use_shift_enter: bool = True
    ) -> bool:
    ...
    def get_agent_coordinates(
        self, agent_id: str, mode: str = "5-agent"
        ...
        try:
            if mode in self.coordinates and agent_id in self.coordinates[mode]:
                ...
                self.logger.warning(
                    f"Coordinates not found for {agent_id} in {mode} mode"
                )
                return None
        except Exception as e:
...
async def main():
    ...
    coordinator = IntegratedAgentCoordinator(
        election_mode=election_mode, round_robin=round_robin
    )

    print(f"✅ Integrated coordinator initialized")
    print(f"🎯 Agent mapping: {len(coordinator.agent_mapping)} agents")
    print(
        f"📋 Message queue: {len(coordinator.message_queue_manager.agent_registry)} agents registered"
    )
