        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                return {
                    ...
                    'is_oop_compliant': has_classes and not has_functions_outside_classes
                }
        except Exception:
            return {
                'has_classes': False,
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                return {
                    ...
                    'is_cli_compliant': all([has_argparse, has_main, has_help, has_argument_parsing])
                }
        except Exception:
            return {
                'has_argparse': False,
...
# Test utilities
def assert_v2_standards_compliance(file_path: Path, 
                                 max_loc: int = TestConfig.MAX_LOC_CORE) -> None:
    """Assert V2 coding standards compliance for a file."""
    # Check line count
    with open(file_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
        code_lines = [line for line in lines if line.strip() and not line.strip().startswith('#')]
        assert len(code_lines) <= max_loc, f"File exceeds {max_loc} LOC limit: {len(code_lines)} lines"
    
    # Check OOP design
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
        assert 'class ' in content, "File must contain class definitions (OOP requirement)"
    
    # Check CLI interface
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
        assert 'argparse' in content, "File must have CLI interface (argparse usage)"
        assert 'def main(' in content or 'if __name__' in content, "File must have main function or entry point"


def create_test_component(component_type: str, 
                         component_name: str,
