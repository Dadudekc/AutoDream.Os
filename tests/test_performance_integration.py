...
from core.agent_communication import AgentCommunicationProtocol, MessageType, MessagePriority, AgentStatus
...
class TestPerformanceIntegration(unittest.TestCase):
    ...
    def test_01_performance_tracking_integration(self):
        ...
        agent1_metrics = self.performance_tracker.get_agent_performance_summary("test_agent_1")
        self.assertIn("response_time", agent1_metrics, "Agent 1 response time not recorded")
    ...
    def test_03_agent_communication_integration(self):
        ...
        self.agent_communication.register_agent(
            "test_agent_1", "Test Agent 1", ["task_execution", "monitoring"],
            "http://localhost:8001", {"version": "1.0"}
        )
        self.agent_communication.register_agent(
            "test_agent_2", "Test Agent 2", ["data_processing", "reporting"],
            "http://localhost:8002", {"version": "1.0"}
        )
        ...
        message_id = self.agent_communication.send_message(
            "test_agent_1", "test_agent_2", MessageType.TASK_ASSIGNMENT,
            {"task": "process_data", "priority": "high"}, MessagePriority.HIGH
        )
        self.assertIsNotNone(message_id, "Message sending failed")
        
        # Test capability-based agent discovery
        monitoring_agents = self.agent_communication.find_agents_by_capability("monitoring")
        ...
        broadcast_ids = self.agent_communication.broadcast_message(
            ...
            {"status": "maintenance_mode"}, MessagePriority.NORMAL
        )
    ...
    def test_04_api_gateway_integration(self):
        ...
        self.api_gateway.register_service(
            "user_service", "User Management Service", "v1",
            "http://localhost:8001", "http://localhost:8001/health",
            {"description": "User management and authentication"}
        )
        self.api_gateway.register_service(
            "data_service", "Data Processing Service", "v1",
            "http://localhost:8002", "http://localhost:8002/health",
        ...
        response = self.api_gateway.route_request(
            ...
            {"page": "1", "limit": "10"}, None, "127.0.0.1", "test-client"
        )
        ...
        for i in range(12):
            ...
            if i < 10:
                ...
                self.assertEqual(response.status_code, 429, f"Request {i} should be rate limited")
        ...
        self.assertIn(user_service.status.value, ["healthy", "degraded", "unhealthy", "unknown"])
    ...
    def test_05_performance_dashboard_integration(self):
        ...
        self.assertIsInstance(dashboard_data.timestamp, datetime, "Dashboard timestamp invalid")
        self.assertIsInstance(dashboard_data.system_metrics, dict, "System metrics not available")
        self.assertIsInstance(dashboard_data.alerts, list, "Alerts not available")
        self.assertIsInstance(dashboard_data.performance_summary, dict, "Performance summary not available")
        
        # Test dashboard views
        self.performance_dashboard.set_view(DashboardView.AGENT_PERFORMANCE)
        self.assertEqual(self.performance_dashboard.current_view, DashboardView.AGENT_PERFORMANCE)
        
        self.performance_dashboard.set_view(DashboardView.SYSTEM_OVERVIEW)
        self.assertEqual(self.performance_dashboard.current_view, DashboardView.SYSTEM_OVERVIEW)
        
        # Test alert system
        self.performance_dashboard.add_alert(
            AlertLevel.WARNING, "Test warning message", "test_system",
        ...
        if alerts:
            ...
            acknowledged_alerts = self.performance_dashboard.get_alerts(acknowledged=True)
            self.assertGreater(len(acknowledged_alerts), 0, "Alert acknowledgment failed")
    ...
    def test_06_cross_system_communication(self):
        ...
        self.agent_communication.register_agent(
            "integration_test_agent", "Integration Test Agent", ["testing"],
            "http://localhost:8003", {"test_mode": True}
        )
        ...
        message_id = self.agent_communication.send_message(
            "system", "integration_test_agent", MessageType.PERFORMANCE_METRIC,
            {"metric_type": "response_time", "value": 0.3}, MessagePriority.NORMAL
        )
        
        # Make an API request that should trigger performance tracking
        response = self.api_gateway.route_request(
            "POST", "/v1/test", {"Content-Type": "application/json"},
            {}, {"test_data": "integration_test"}, "127.0.0.1", "integration-test"
        )
    ...
    def test_07_data_export_and_persistence(self):
        ...
        for i in range(5):
            self.performance_tracker.record_metric(
                MetricType.RESPONSE_TIME, 0.1 + i * 0.1, agent_id=f"export_test_agent_{i}"
            )
        ...
        for export_file in export_files:
            self.assertTrue(os.path.exists(export_file), f"Export file not created: {export_file}")
            
            with open(export_file, 'r') as f:
                try:
                    export_data = json.load(f)
                    self.assertIsInstance(export_data, dict, f"Invalid JSON in {export_file}")
                    self.assertIn('export_timestamp', export_data, f"Missing timestamp in {export_file}")
    ...
    def test_08_error_handling_and_recovery(self):
        ...
        self.assertTrue(self.performance_dashboard.running, "Dashboard should still be running")
        self.assertTrue(self.agent_communication.communication_active, "Communication should still be active")
        self.assertTrue(self.api_gateway.gateway_active, "API gateway should still be active")
    ...
    def test_09_performance_under_load(self):
        ...
        for i in range(100):
            self.performance_tracker.record_metric(
                MetricType.RESPONSE_TIME, 0.1 + (i % 10) * 0.1, 
                agent_id=f"load_test_agent_{i % 5}"
            )
            self.performance_tracker.record_metric(
                MetricType.CPU_USAGE, 50.0 + (i % 30), 
                agent_id=f"load_test_agent_{i % 5}"
            )
        ...
        for i in range(50):
            self.agent_communication.send_message(
                f"load_test_agent_{i % 5}", f"load_test_agent_{(i + 1) % 5}",
                MessageType.STATUS_UPDATE, {"load_test": True, "iteration": i},
        ...
        for i in range(50):
            self.api_gateway.route_request(
                "GET", f"/v1/load_test/{i}", {}, {"test": "load"}, None,
                "127.0.0.1", "load-test-client"
            )
        ...
        self.assertTrue(self.performance_dashboard.running, "Dashboard failed under load")
        self.assertTrue(self.agent_communication.communication_active, "Communication failed under load")
        self.assertTrue(self.api_gateway.gateway_active, "API gateway failed under load")
        ...
        self.assertLess(total_time, 10.0, f"Performance under load too slow: {total_time:.2f}s")
        
        print(f"✅ Performance under load test PASSED ({total_time:.2f}s for 200 operations)")
    ...
    def test_10_system_integration_summary(self):
        ...
        self.assertTrue(self.performance_dashboard.running, "Performance dashboard not running")
        self.assertTrue(self.agent_communication.communication_active, "Agent communication not active")
        ...
        self.assertIn('system_health', performance_summary, "System health not calculated")
        system_health = performance_summary['system_health']
        self.assertGreaterEqual(system_health, 0, "System health should be non-negative")
        self.assertLessEqual(system_health, 100, "System health should be <= 100")
        
        print(f"✅ Overall system integration test PASSED (System Health: {system_health:.1f}/100)")
