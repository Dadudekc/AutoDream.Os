    ...
    def _check_documentation(self) -> QualityGate:
        ...
        try:
            ...
            return QualityGate(
                ...
                details={'documented_functions': documented_functions, 'total_functions': total_functions}
            )
    ...
    def _generate_gate_recommendations(self, suite: QualityGateSuite) -> list[str]:
        ...
        for gate in failed_gates:
            if gate.name == "Test Coverage":
                recommendations.append(f"📊 Increase test coverage from {gate.actual_value:.1f}% to {gate.threshold}%")
            elif gate.name == "V2 Compliance":
                recommendations.append(f"📏 Fix V2 compliance violations to achieve {gate.threshold}% compliance")
                ...
                recommendations.append(f"⚡ Improve performance score from {gate.actual_value:.1f} to {gate.threshold}")
            elif gate.name == "Documentation":
                recommendations.append(f"📚 Increase documentation coverage from {gate.actual_value:.1f}% to {gate.threshold}%")
            elif gate.name == "Complexity":
                recommendations.append(f"🧮 Reduce complexity from {gate.actual_value:.1f} to {gate.threshold}")
