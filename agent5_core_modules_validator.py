...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("agent5_core_modules_validator.log"), logging.StreamHandler()],
)
...
class Agent5CoreModulesValidator:
    ...
    def validate_messaging_system_consolidation(self) -> bool:
        ...
        try:
            ...
            target_file = (
                self.project_root / self.consolidation_targets["messaging_system"]["target_file"]
            )
    ...
    def validate_analytics_engine_consolidation(self) -> bool:
        ...
        try:
            ...
            # Check if target file exists
            target_file = (
                self.project_root / self.consolidation_targets["analytics_engine"]["target_file"]
            )
            if not target_file.exists():
            ...
            if total_files > target_files:
                logger.warning(
                    f"Analytics engine consolidation incomplete: {total_files} files remaining"
                )
    ...
    def validate_configuration_system_consolidation(self) -> bool:
        ...
        try:
            ...
            target_file = (
                self.project_root
                / self.consolidation_targets["configuration_system"]["target_file"]
            )
            if not target_file.exists():
            ...
            config_keywords = ["UnifiedConfig", "config_core", "env_loader", "configuration"]
    ...
    def validate_coordination_system_consolidation(self) -> bool:
        ...
        try:
            ...
            # Check if target file exists
            target_file = (
                self.project_root / self.consolidation_targets["coordination_system"]["target_file"]
            )
            if not target_file.exists():
            ...
            if len(existing_source_files) > target_files:
                logger.warning(
                    f"Coordination system consolidation incomplete: {len(existing_source_files)} files remaining"
                )
            ...
            coordination_keywords = ["UnifiedCoordination", "coordinator", "coordination", "swarm"]
    ...
    def validate_engine_system_consolidation(self) -> bool:
        ...
        try:
            ...
            # Check if target file exists
            target_file = (
                self.project_root / self.consolidation_targets["engine_system"]["target_file"]
            )
            if not target_file.exists():
            ...
            source_dir = (
                self.project_root / self.consolidation_targets["engine_system"]["directory"]
            )
            if source_dir.exists():
                ...
                if total_files > target_files:
                    logger.warning(
                        f"Engine system consolidation incomplete: {total_files} files remaining"
                    )
            ...
            engine_keywords = ["UnifiedEngines", "engine", "core_engine", "engines"]
    ...
    def generate_recommendations(self) -> list[str]:
            ...
            if status == "FAIL":
                if check == "messaging_system":
                    recommendations.append(
                        "Complete messaging system consolidation - merge all messaging files"
                    )
                elif check == "analytics_engine":
                    recommendations.append(
                        "Complete analytics engine consolidation - merge all analytics modules"
                    )
                elif check == "configuration_system":
                    recommendations.append(
                        "Complete configuration system consolidation - integrate all config files"
                    )
                elif check == "coordination_system":
                    recommendations.append(
                        "Complete coordination system consolidation - merge all coordination files"
                    )
                elif check == "engine_system":
                    recommendations.append(
                        "Complete engine system consolidation - merge all engine files"
                    )
        ...
        if not recommendations:
            recommendations.append(
                "Core modules consolidation validation passed - continue monitoring"
            )
