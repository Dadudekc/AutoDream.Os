...
class DataLogManager:
    ...
    def _optimize_databases(self) -> dict[str, Any]:
        ...
        for db_file in db_files:
            try:
                ...
                logger.info(f"Warning: Could not optimize database {db_file}: {e}")
        return db_results
    ...
    def print_optimization_summary(self, results: dict[str, Any]) -> None:
        ...
        logger.info(f"  Directories Created: {data_opt.get('directories_created', 0)}")
        ...
        logger.info(f"  Size Saved: {log_opt.get('total_size_saved', 0) / (1024 * 1024):.1f} MB")
        db_opt = results.get("database_optimization", {})
        logger.info("\nDatabase Optimization:")
        logger.info(f"  Databases Optimized: {db_opt.get('databases_optimized', 0)}")
        logger.info(f"  Databases Vacuumed: {db_opt.get('databases_vacuumed', 0)}")
        size_before = db_opt.get("total_size_before", 0)
        size_after = db_opt.get("total_size_after", 0)
        if size_before > 0:
            logger.info(f"  Size Reduction: {(size_before - size_after) / size_before * 100:.1f}%")
        cleanup = results.get("cleanup_results", {})
        logger.info("\nCleanup Results:")
        logger.info(f"  Temp Files Removed: {cleanup.get('temp_files_removed', 0)}")
        logger.info(f"  Cache Dirs Cleaned: {cleanup.get('cache_dirs_cleaned', 0)}")
        logger.info(f"  Space Freed: {cleanup.get('total_size_freed', 0) / (1024 * 1024):.1f} MB")
        logger.info("=" * 70)
