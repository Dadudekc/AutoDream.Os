...
class MessageValidator:
    ...
    def validate_response(response: Dict[str, Any]) -> bool:
        ...
        required_fields = ['response_id', 'original_message_id', 'timestamp', 'sender_id', 'recipient_id', 'success']
...
class CrossAgentCommunicator:
    ...
    def send_message(self, recipient_id: str, message_type: str, payload: Dict[str, Any] = None,
                    category: str = COMMAND_CATEGORIES["SYSTEM"], priority: MessagePriority = MessagePriority.NORMAL,
                    requires_response: bool = False, correlation_id: str = None) -> str:
    ...
    def broadcast_message(self, message_type: str, payload: Dict[str, Any] = None,
                        category: str = COMMAND_CATEGORIES["SYSTEM"], priority: MessagePriority = MessagePriority.NORMAL) -> str:
    ...
    def send_response(self, original_message: AgentMessage, success: bool, payload: Dict[str, Any] = None,
                     error_code: str = None, error_message: str = None, execution_time: float = None) -> str:
        ...
        self.logger.info(f"Sent response {response.response_id} to {original_message.sender_id}")
        return response.response_id
    
    def update_status(self, status: AgentStatus, current_task: str = None, load_percentage: float = 0.0,
                     memory_usage: float = 0.0, cpu_usage: float = 0.0, metadata: Dict[str, Any] = None):
    ...
    def _send_via_rabbitmq(self, message: Dict[str, Any], routing_key: str = None):
        ...
        if self.rabbitmq_connection:
            try:
                ...
                routing_key = routing_key or f"agent.{message.get('recipient_id', 'broadcast')}"
    ...
    def _broadcast_via_rabbitmq(self, message: Dict[str, Any], routing_key: str = "broadcast"):
                ...
                try:
                    ...
                    if message.requires_response:
                        if result is not None:
                            self.send_response(message, True, {'result': result}, execution_time=execution_time)
                        else:
                            self.send_response(message, True, execution_time=execution_time)
                    
                except Exception as e:
                    self.logger.error(f"Error in message handler: {e}")
                    if message.requires_response:
                        self.send_response(message, False, error_code="HANDLER_ERROR", error_message=str(e))
            else:
...
def create_flask_integration(communicator: CrossAgentCommunicator) -> Flask:
    ...
    app = Flask(__name__)
    app.config['SECRET_KEY'] = communicator.config.get('secret_key', 'default-secret-key')
...
# Utility functions
def create_agent_communicator(agent_id: str, config: Dict[str, Any] = None) -> CrossAgentCommunicator:
