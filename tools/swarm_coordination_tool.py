...
from services.swarm_intelligence_coordination import DecisionType, SwarmIntelligenceCoordination
...
async def propose_decision(agent_id: str, decision_type: str, title: str, description: str):
    ...
    try:
        ...
        print("✅ Decision proposed successfully!")
...
async def run_swarm_coordination_cycle():
    ...
    try:
        ...
        status = await swarm.get_swarm_status()
        print(
            f"📊 Swarm Status: {status['total_agents']} agents, {status['active_decisions']} active decisions"
        )
        ...
        if workload_balance["rebalancing_needed"]:
            print(
                f"⚖️ Workload rebalancing needed for {len(workload_balance['rebalancing_needed'])} agents"
            )
        else:
            print("✅ Workload balanced across all agents")
        ...
        print("✅ Swarm coordination cycle completed")
...
def main():
    ...
    vote_parser.add_argument(
        "--vote", required=True, choices=["yes", "no", "abstain"], help="Vote choice"
    )
