...
class CaptainContractInstructionService:
    ...
    def __init__(self):
        ...
        self.captain_inbox_file = self.project_root / "agent_workspaces" / "Captain-5" / "inbox" / "contract_instructions.json"
    ...
    def _create_captain_instruction(self, completion: ContractCompletion) -> CaptainInstruction:
        ...
        try:
            ...
            instruction_type = self._determine_instruction_type(completion.agent_id, completion.contract_id)
            
            # Determine priority based on quality score and agent
            priority = self._determine_priority(completion.quality_score, completion.agent_id)
    ...
    def _generate_suggested_contracts(self, completion: ContractCompletion) -> List[Dict[str, str]]:
    ...
    def _create_reasoning(self, completion: ContractCompletion, instruction_type: str) -> str:
...
def main():
    ...
    parser.add_argument("--instructions", action="store_true", help="Show pending Captain instructions")
    ...
    if args.complete:
        ...
        if success:
            print(f"✅ Contract completion processed and Captain instruction created for {agent_id}")
        else:
        ...
        if instructions:
            ...
            for instruction in instructions:
                ...
                print(f"   Suggestions: {len(instruction.suggested_contracts)} contracts")
        else:
