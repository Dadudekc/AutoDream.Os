...
Comprehensive unit tests for Discord embed generation and management.
...
class TestEmbedBuilder:
    """Test cases for EmbedBuilder static methods."""
...
class TestEmbedManager:
    """Test cases for EmbedManager functionality."""

    @pytest.fixture
    def embed_manager(self):
        """Create EmbedManager instance for testing."""
        return EmbedManager()
    ...
    def test_create_response_embed_ping(self, embed_manager):
        ...
        assert "42" in str(embed.fields[0].value) if embed.fields else True
    ...
    def test_create_response_embed_prompt(self, embed_manager):
        ...
        assert embed.title is not None
        assert "Agent-1" in str(embed.fields) if embed.fields else "Agent-1" in embed.title
    ...
    def test_create_response_embed_status(self, embed_manager):
        ...
        assert embed.title is not None
        assert "Agent-2" in str(embed.fields) if embed.fields else "Agent-2" in embed.title
...
class TestEmbedBuilderUpdateMethods:
    """Test cases for EmbedBuilder update methods."""

    def test_update_prompt_embed_success(self):
        """Test updating prompt embed for success."""
        base_embed = EmbedBuilder.create_base_embed("Test")
        updated_embed = EmbedBuilder.builder.update_prompt_embed_success(base_embed, "Agent-1")

        assert updated_embed is not None
        assert updated_embed.color.value == EmbedBuilder.COLORS["success"]

    def test_update_prompt_embed_error(self):
        """Test updating prompt embed for error."""
        base_embed = EmbedBuilder.create_base_embed("Test")
        updated_embed = EmbedBuilder.builder.update_prompt_embed_error(
            base_embed, "Agent-1", "Test error"
        )

        assert updated_embed is not None
        assert updated_embed.color.value == EmbedBuilder.COLORS["error"]
    ...
    def test_update_status_embed(self):
        ...
        assert any("active" in str(field.value).lower() for field in updated_embed.fields)


class TestEmbedErrorHandling:
    """Test cases for embed error handling and edge cases."""
    ...
    def test_embed_creation_memory_efficiency(self, embed_manager):
        ...
        assert all(embed is not None for embed in embeds)
...
class TestEmbedIntegration:
    ...
    def test_full_command_workflow_embeds(self, embed_manager):
        """Test embeds for a complete command workflow."""
        # Simulate a prompt command workflow
        author = Mock(display_name="TestUser", id=123456)

        # Initial prompt embed
        prompt_embed = embed_manager.create_response_embed(
            "prompt",
            agent_id="Agent-1",
            prompt="Test message",
            command_id="cmd_123",
        ...
        # Status embed
        status_embed = embed_manager.create_response_embed(
            "status",
            agent_id="Agent-1",
            author=author
        )
        ...
        # Error embed
        error_embed = embed_manager.create_response_embed(
            "error",
            title="Test Error",
        ...
        # All embeds should be created successfully
        assert prompt_embed is not None
        assert status_embed is not None
        assert error_embed is not None

        # Each should have appropriate colors
        assert prompt_embed.color.value == EmbedBuilder.COLORS["agent"]
        assert status_embed.color.value == EmbedBuilder.COLORS["info"]
