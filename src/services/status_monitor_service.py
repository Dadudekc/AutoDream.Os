...
class StatusMonitorService:
    ...
    def update_agent_status(self, agent_id: str, status: str, activity_type: str = "coordination"):
    ...
    def assess_system_health(self) -> SystemHealth:
        ...
        try:
            active_agents = sum(1 for metrics in self.agent_metrics.values() if metrics.status == "active")
            total_errors = sum(metrics.error_count for metrics in self.agent_metrics.values())
            total_warnings = sum(1 for metrics in self.agent_metrics.values() if metrics.success_rate < 90.0)
            ...
            self.logger.info(f"System health assessed: {system_status} (score: {health_score:.1f})")
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_metrics": {
                "total_coordinations": sum(m.coordination_count for m in self.agent_metrics.values()),
                ...
                "average_success_rate": sum(m.success_rate for m in self.agent_metrics.values()) / len(self.agent_metrics)
            }
        }
...
"""
            ...
            for agent_id, status in summary['agent_status'].items():
                status_emoji = "🟢" if status['status'] == "active" else "🟡" if status['status'] == "standby" else "🔴"
...
def main():
    ...
    parser.add_argument("--update", type=str, help="Update agent status (format: agent_id:status:activity_type)")
    parser.add_argument("--error", type=str, help="Record agent error (format: agent_id:error_message)")
    parser.add_argument("--response-time", type=str, help="Record response time (format: agent_id:time_seconds)")
    ...
    if args.status:
        ...
        print(f"  Total Coordinations: {summary['overall_metrics']['total_coordinations']}")
