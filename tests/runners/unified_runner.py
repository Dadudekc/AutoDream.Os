...
class UnifiedTestRunner(BaseTestRunner):
    ...
    def run_category(self, category: str, **kwargs) -> Dict[str, Any]:
        ...
        if not test_files:
            print(f"{Fore.YELLOW}⚠️ No test files found for {category}{Style.RESET_ALL}")
            return {
                "success": True,
                "category": category,
                "tests_run": 0,
    ...
    def run_all_categories(self, **kwargs) -> Dict[str, Any]:
        ...
        for category, config in self.test_categories.items():
            if critical_only and not config.get('critical', False):
                print(f"{Fore.YELLOW}⏭️ Skipping non-critical category: {category}{Style.RESET_ALL}")
                continue
    ...
    def run(self, mode: str = "all", **kwargs) -> Dict[str, Any]:
        ...
        if mode == "all":
            ...
            return {
                ...
                "available_modes": ["all", "critical", "files"] + list(self.test_categories.keys())
            }
...
Examples:
  ...
  python -m tests.runners.unified_runner --mode files test_file1.py test_file2.py
    ...
    parser.add_argument(
        "--mode", "-m",
        choices=["all", "critical", "smoke", "unit", "integration", "performance", "security", "api", "files"],
        default="all",
