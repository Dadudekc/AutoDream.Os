...
class V2MessageDeliveryService:
    ...
    def _initialize_agent_coordinates(self):
        ...
        try:
            ...
            try:
                ...
                v2_locations_path = os.path.join(
                    repo_root, "agent_complete_locations.json"
                )
                ...
                def _load_cursor_coords(path: str) -> bool:
                    if os.path.exists(path):
                        ...
                        if self.agent_coordinates:
                            logger.info(
                                f"✅ Loaded agent coordinates from cursor coords: {path}"
                            )
                            return True
                    return False
                ...
                def _load_v2_locations(path: str) -> bool:
                    if os.path.exists(path):
                        ...
                        if self.agent_coordinates:
                            logger.info(
                                f"✅ Loaded agent coordinates from V2 locations: {path}"
                            )
                            return True
                    return False

                loaded = (
                    _load_cursor_coords(cursor_repo_path)
                    or _load_cursor_coords(cursor_runtime_path)
                    or _load_v2_locations(v2_locations_path)
                )
                ...
                logger.warning(
                    f"⚠️ Error loading V2 coordinates: {e}, using placeholder coordinates"
                )
                self._initialize_placeholder_coordinates()
    ...
    def _initialize_agent_coordinates_fallback(self):
        """Fallback coordinate loading method for compatibility"""
        try:
            logger.info("🔄 Using fallback coordinate loading...")
            ...
            logger.warning(
                f"⚠️ Error loading V2 coordinates via fallback: {e}, using placeholder coordinates"
            )
            self._initialize_placeholder_coordinates()
    ...
    def _start_delivery_thread(self):
        ...
        try:
            ...
            self.delivery_thread = threading.Thread(
                target=self._message_delivery_loop, daemon=True
            )
    ...
    def _deliver_message(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(
                f"📤 Delivering {message_type} message to {agent_id} at ({agent_coords['input_x']}, {agent_coords['input_y']})"
            )
                ...
                self.delivery_status[agent_id].update(
                    {
                        ...
                        "delivery_count": self.delivery_status[agent_id][
                            "delivery_count"
                        ]
                        + 1,
                        "successful_deliveries": self.delivery_status[agent_id][
                ...
                self.delivery_status[agent_id].update(
                    {
                        ...
                        + 1,
                        "failed_deliveries": self.delivery_status[agent_id][
                            "failed_deliveries"
                        ]
                        + 1,
                    }
    ...
    def send_message_sync(
        ...
        try:
            ...
            logger.info(
                f"📤 Sending message synchronously to {agent_id}: {message_type}"
            )
            self._deliver_message(message_data)
            return True
...
def main():
    ...
    parser.add_argument(
        "--agents", action="store_true", help="List all available agents"
    )
    parser.add_argument(
        "--ping", nargs=1, metavar="AGENT", help="Send ping message to specific agent"
    )
    parser.add_argument(
        "--ping-all", action="store_true", help="Send ping to all agents"
    )
    parser.add_argument(
        "--coordinate",
        nargs=2,
    ...
    parser.add_argument(
        "--test-sync", action="store_true", help="Test synchronous message delivery"
    )
    ...
    try:
        ...
        if args.status:
            ...
            print(
                f"Success Rate: {(total_success/total_deliveries*100):.1f}%"
            ...
            print("-" * 40)
            print(
                f"PyAutoGUI Available: {'✅ Yes' if status['pyautogui_available'] else '❌ No'}"
            )
            ...
            for agent_id, agent_info in status["agent_coordinates"].items():
                ...
                print(
                    f"   Coordinates: ({agent_info['input_x']}, {agent_info['input_y']})"
                )
            ...
            success = delivery_service.send_message(
                agent_id, "ping", "Ping message - please acknowledge receipt"
            )
            print(
                f"{'✅' if success else '❌'} Ping {'sent' if success else 'failed'} to {agent_id}"
            )

        elif args.ping_all:
            print("📡 Pinging all agents...")
            results = delivery_service.broadcast_message(
            ...
            success_count = sum(1 for success in results.values() if success)
            print(
                f"📊 Coordination results: {success_count}/{len(target_agents)} agents received"
            )
            ...
            print(
                f"{'✅' if success else '❌'} Message {'sent' if success else 'failed'} to {agent_id}"
            )
        elif args.send_file:
            ...
            try:
                ...
                print(
                    f"{'✅' if success else '❌'} File message {'sent' if success else 'failed'} to {agent_id}"
                )
            except Exception as e:
            ...
            success = delivery_service.send_message_sync(
                agent_id, message_type, message
            )
            print(
                f"{'✅' if success else '❌'} Synchronous message {'sent' if success else 'failed'} to {agent_id}"
            )
            ...
            try:
                ...
                results = {}
                for agent_id in agents:
                    ok = delivery_service.send_message_sync(
                        agent_id, message_type, content
                    )
                    results[agent_id] = ok
                    time.sleep(0.2)
                success_count = sum(1 for success in results.values() if success)
                print(
                    f"📢 Broadcast (sync) sent to {success_count}/{len(results)} agents"
                )
                print(f"Results: {results}")
            except Exception as e:
                print(f"❌ Error reading file: {e}")
            ...
            success = delivery_service.update_agent_coordinates(
                agent_id, int(x), int(y)
            )
            print(
                f"{'✅' if success else '❌'} Coordinates {'updated' if success else 'failed'} for {agent_id}"
            )
            ...
            # Test broadcast
            delivery_service.broadcast_message(
                "test_broadcast", "This is a test broadcast"
            )
            time.sleep(1)
            ...
            delivery_service.send_message_sync(
            ...
            print(
                "  --send AGENT TYPE MESSAGE             Send message to specific agent"
            )
            print("  --send-file AGENT TYPE FILE           Send file content to agent")
            print("  --broadcast TYPE MESSAGE              Broadcast to all agents")
            print(
                "  --broadcast-file TYPE FILE            Broadcast file content to all agents"
            )
            print(
                "  --broadcast-sync TYPE MESSAGE         Broadcast synchronously to all agents"
            )
            print(
                "  --broadcast-file-sync TYPE FILE       Broadcast file content synchronously to all agents"
            )
            ...
            print(
                "  python v2_message_delivery_service.py --coordinate 'agent_1,agent_3' 'Coordinate multimedia integration'"
            )
            print(
                "  python v2_message_delivery_service.py --emergency 'System maintenance required'"
            )
