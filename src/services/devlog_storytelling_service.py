...
class DevlogStorytellingService:
    ...
    def _create_task_completion_story(self, content: Dict[str, Any]) -> DevlogStory:
        ...
        return DevlogStory(
            ...
            timeline=[
                ...
                StoryElement("progress", content.get('action', ''), {}, content.get('timestamp', '')),
                StoryElement("completion", "Task completed successfully", {}, content.get('timestamp', ''))
            ],
            key_achievements=[
    ...
    def _create_bug_fix_story(self, content: Dict[str, Any]) -> DevlogStory:
        ...
        return DevlogStory(
            ...
            summary=f"Identified and resolved critical bug affecting system stability",
            timeline=[
                StoryElement("discovery", "Bug discovered", {}, content.get('timestamp', '')),
                StoryElement("investigation", "Root cause analysis", {}, content.get('timestamp', '')),
                StoryElement("fix", content.get('action', 'Bug fix applied'), {}, content.get('timestamp', '')),
                StoryElement("verification", "Fix verified and tested", {}, content.get('timestamp', ''))
            ],
            key_achievements=[
    ...
    def _create_feature_story(self, content: Dict[str, Any]) -> DevlogStory:
        ...
        return DevlogStory(
            ...
            summary=f"Successfully implemented new feature to enhance system capabilities",
            timeline=[
                StoryElement("planning", "Feature requirements defined", {}, content.get('timestamp', '')),
                StoryElement("design", "Architecture designed", {}, content.get('timestamp', '')),
                StoryElement("implementation", content.get('action', 'Feature implemented'), {}, content.get('timestamp', '')),
                StoryElement("testing", "Feature tested and validated", {}, content.get('timestamp', '')),
                StoryElement("deployment", "Feature deployed successfully", {}, content.get('timestamp', ''))
            ],
            key_achievements=[
    ...
    def _create_optimization_story(self, content: Dict[str, Any]) -> DevlogStory:
        ...
        return DevlogStory(
            ...
            summary=f"Successfully optimized system performance and efficiency",
            timeline=[
                StoryElement("analysis", "Performance bottlenecks identified", {}, content.get('timestamp', '')),
                StoryElement("planning", "Optimization strategy developed", {}, content.get('timestamp', '')),
                StoryElement("implementation", content.get('action', 'Optimizations applied'), {}, content.get('timestamp', '')),
                StoryElement("measurement", "Performance improvements measured", {}, content.get('timestamp', ''))
            ],
            key_achievements=[
    ...
    def _create_generic_story(self, content: Dict[str, Any]) -> DevlogStory:
        ...
        return DevlogStory(
            ...
            timeline=[
                StoryElement("activity", content.get('action', 'Activity performed'), {}, content.get('timestamp', ''))
            ],
    ...
    def get_story_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'latest_story': max(story_files, key=lambda f: f.stat().st_mtime).name if story_files else None
        }
