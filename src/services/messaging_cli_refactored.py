...
class MessageCoordinator:
    ...
    @staticmethod
    def send_to_agent(agent: str, message: str, priority=UnifiedMessagePriority.REGULAR, use_pyautogui=False):
        try:
            return send_message_pyautogui(agent_id=agent, message=message, timeout=30) if use_pyautogui else send_message(
                content=message, sender="CAPTAIN", recipient=agent,
                message_type=UnifiedMessageType.CAPTAIN_TO_AGENT, priority=priority, tags=[UnifiedMessageTag.SYSTEM]
            )
        except: return False

    @staticmethod
    def broadcast_to_all(message: str, priority=UnifiedMessagePriority.REGULAR):
        return sum(1 for agent in SWARM_AGENTS if send_message(
            content=message, sender="CAPTAIN", recipient=agent,
            message_type=UnifiedMessageType.BROADCAST, priority=priority,
    ...
    def coordinate_survey():
        ...
        success_count = MessageCoordinator.broadcast_to_all(SURVEY_MESSAGE_TEMPLATE, UnifiedMessagePriority.URGENT)
        if success_count > 0:
    ...
    def coordinate_consolidation(batch: str, status: str):
        ...
        msg = CONSOLIDATION_MESSAGE_TEMPLATE.format(batch=batch, status=status, timestamp=datetime.now().isoformat())
        return sum(1 for agent in SWARM_AGENTS if send_message(
            content=msg, sender="CAPTAIN", recipient=agent,
            message_type=UnifiedMessageType.SYSTEM_TO_AGENT, priority=UnifiedMessagePriority.URGENT,
            tags=[UnifiedMessageTag.SYSTEM, UnifiedMessageTag.COORDINATION]
        ))
...
class MessagingCLI:
    ...
    def execute(self, args=None):
        if not MESSAGING_AVAILABLE: return 1
        parsed_args = self.parser.parse_args(args)
        try:
            if parsed_args.message or parsed_args.broadcast: return self._handle_message(parsed_args)
            elif parsed_args.survey_coordination: return self._handle_survey()
            elif parsed_args.consolidation_coordination: return self._handle_consolidation(parsed_args)
            elif parsed_args.coordinates: return self._handle_coordinates()
            else: self.parser.print_help(); return 0
        except: return 1

    def _handle_message(self, args):
        if not args.message and not args.broadcast: return 1
        ...
        if args.broadcast:
            ...
            logger.info(f"📢 Broadcast successful to {success}/8 agents" if success > 0 else "❌ Broadcast failed")
            return 0 if success > 0 else 1
        elif args.agent:
            success = MessageCoordinator.send_to_agent(args.agent, args.message, priority, args.pyautogui)
    ...
    def _handle_consolidation(self, args):
        if not (args.consolidation_batch and args.consolidation_status): return 1
        success = MessageCoordinator.coordinate_consolidation(args.consolidation_batch, args.consolidation_status)
        return 0 if success > 0 else 1
