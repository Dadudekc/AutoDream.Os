...
V2 Compliance: Optimized for quality gates.
"""
...
class MLPipelineCore:
    ...
    def create_training_data(self,
        ...
        try:
            ...
            # Split data
            return self._split_data(features, labels, num_samples)

        except Exception as e:
            logger.error(f"Error creating training data: {e}")
            raise

    def _split_data(self, features, labels, num_samples):
    ...
    def train_model(self,
        ...
        try:
            ...
            if hasattr(model, 'fit'):
                results = self._train_tf_model(model, training_data, epochs, batch_size)
            else:
    ...
    def _train_tf_model(self, model, training_data, epochs, batch_size):
        """Train TensorFlow model."""
        validation_data = self._get_val_data(training_data)
        ...
        return {
            ...
            "final_accuracy": self._get_accuracy(history),
            "model_type": "tensorflow"
        }

    def _get_val_data(self, training_data):
        """Get validation data tuple."""
        if training_data.validation_features is not None:
            return (training_data.validation_features,
                   training_data.validation_labels)
        return None

    def _get_accuracy(self, history):
    ...
    def _calculate_metrics(self, predictions, test_data):
        """Calculate model metrics."""
        if self.config.model_type == "classification":
            return self._calc_classification_metrics(predictions, test_data)
        else:
            return self._calc_regression_metrics(predictions, test_data)

    def _calc_classification_metrics(self, predictions, test_data):
    ...
    def _calc_regression_metrics(self, predictions, test_data):
