...
class InboxManager:
    ...
    def _save_message_to_inbox(self, message: Message):
        ...
        try:
            ...
            recipient_workspace = self.workspace_manager.get_workspace(message.recipient)
    ...
    def get_messages(self, recipient: str, status: Optional[MessageStatus] = None,
    ...
    def mark_as_failed(self, message_id: str, error_message: str = "") -> bool:
        ...
        try:
            if message_id in self.messages:
                ...
                self.logger.info(f"Marked message {message_id} as failed: {error_message}")
                return True
    ...
    def get_inbox_status(self, agent_id: str) -> Optional[InboxStatus]:
        ...
        try:
            ...
            for status in MessageStatus:
                status_counts[status.value] = len([m for m in agent_messages if m.status == status])
            
            # Count by priority
            priority_counts = {}
            for priority in MessagePriority:
                priority_counts[priority.value] = len([m for m in agent_messages if m.priority == priority])
            
            # Calculate unread and urgent counts
            unread_count = len([m for m in agent_messages if m.status == MessageStatus.PENDING])
            urgent_count = len([m for m in agent_messages if m.priority in [MessagePriority.HIGH, MessagePriority.URGENT]])
    ...
    def get_system_status(self) -> SystemStatus:
        ...
        try:
            ...
            urgent_messages = len([m for m in self.messages.values() 
    ...
    def search_messages(self, query: str, agent_id: Optional[str] = None) -> List[Message]:
        ...
        try:
            ...
            for message in self.messages.values():
                ...
                if (query_lower in message.subject.lower() or
                    ...
                    (message.metadata and any(query_lower in str(v).lower() 
    ...
    def get_message_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Count by status
            for status in MessageStatus:
                stats['by_status'][status.value] = len([m for m in self.messages.values() if m.status == status])
            
            # Count by priority
            for priority in MessagePriority:
                stats['by_priority'][priority.value] = len([m for m in self.messages.values() if m.priority == priority])
            
            # Count by sender and recipient
            for message in self.messages.values():
                stats['by_sender'][message.sender] = stats['by_sender'].get(message.sender, 0) + 1
                stats['by_recipient'][message.recipient] = stats['by_recipient'].get(message.recipient, 0) + 1
            ...
            for message in recent_messages:
                ...
                stats['recent_activity']['by_hour'][hour] = stats['recent_activity']['by_hour'].get(hour, 0) + 1
    ...
    def cleanup_old_messages(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            self.logger.info(f"Cleaned up {deleted_count} messages older than {days_old} days")
