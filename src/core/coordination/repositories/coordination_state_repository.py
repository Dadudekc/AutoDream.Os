...
Repository for coordination system state data access using Repository pattern.
...
class CoordinationStateRepository:
    """Repository for coordination system state data access."""
    ...
    def get_onboarding_status(self) -> Dict[str, bool]:
        ...
        return state.get('onboarding_status', {})
    ...
    def get_agent_fsm_states(self) -> Dict[str, str]:
        """Get FSM states for all agents."""
        state = self.load_state()
        return state.get('agent_fsm_states', {})
        
    def save_agent_fsm_states(self, fsm_states: Dict[str, str]) -> bool:
        """Save FSM states for all agents."""
        state = self.load_state()
        state['agent_fsm_states'] = fsm_states
        return self.save_state(state)
