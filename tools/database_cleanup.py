...
class DatabaseCleanupTool:
    ...
    def remove_duplicates(self, duplicate_groups: Dict[str, List[Dict]]) -> None:
        ...
        for group_name, docs_list in duplicate_groups.items():
            ...
            print(f"  📝 {group_name}: Keeping {keep_doc['id']}, removing {len(remove_docs)} duplicates")
        ...
        for doc in docs:
            try:
                ...
                if doc.get('kind') == 'action':
                    ...
                    self.vector_db.integrate_agent_status(doc.get('agent_id', ''), {
                        'content': content,
                        'metadata': metadata,
    ...
    def sync_databases(self) -> None:
        ...
        print(f"🔄 Cross-referencing {len(swarm_ids)} Swarm Brain docs with {len(vector_docs)} vector docs")
