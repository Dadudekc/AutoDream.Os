...
class AgentTaskManager:
    ...
    def list_tasks(self) -> Dict[str, Any]:
        ...
        result["task_summary"] = {
            ...
            "total_tasks": len(result["pending_tasks"]) + len(result["completed_tasks"]) + (1 if result["current_task"] else 0)
        }
        
        return result
    
    def create_task(self, description: str, priority: str = "normal", 
                   assigned_by: str = "system", dependencies: Optional[List[str]] = None) -> Dict[str, Any]:
        """Create a new task."""
        task = {
    ...
    def assign_task(self, target_agent: str, description: str, priority: str = "normal") -> Dict[str, Any]:
...
Examples:
  ...
  python tools/agent_task_manager.py --agent Agent-1 --report
    ...
    parser.add_argument(
        "--list-tasks",
        action="store_true",
        help="List all tasks for the agent"
    )
    ...
    parser.add_argument(
        "--assign-task",
        action="store_true",
        help="Assign a task to another agent"
    )
    
    parser.add_argument(
        "--complete-task",
        help="Complete a task by ID"
    )
    
    parser.add_argument(
        "--report",
        action="store_true",
        help="Generate a task report"
    )
    ...
    parser.add_argument(
        "--assigned-by",
        default="system",
        help="Who assigned the task (default: system)"
    )
