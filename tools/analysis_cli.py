...
def analyze_file(file_path: Path) ->dict[str, Any]:
    ...
    violations = {'file': str(file_path), 'syntax_errors': [],
        'loc_violations': [], 'line_length_violations': [],
        'print_statements': [], 'total_lines': 0, 'classes': [],
        'functions': []}
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                violations['classes'].append({'name': node.name, 'lines':
                    class_lines, 'line_number': node.lineno})
                if class_lines > MAX_CLASS_LOC:
                    violations['loc_violations'].append({'type': 'class',
                        'name': node.name, 'lines': class_lines,
                        'line_number': node.lineno, 'max_allowed':
                        MAX_CLASS_LOC})
            elif isinstance(node, ast.FunctionDef):
                func_lines = count_lines(node)
                violations['functions'].append({'name': node.name, 'lines':
                    func_lines, 'line_number': node.lineno})
                if func_lines > MAX_FUNCTION_LOC:
                    violations['loc_violations'].append({'type': 'function',
                        'name': node.name, 'lines': func_lines,
                        'line_number': node.lineno, 'max_allowed':
                        MAX_FUNCTION_LOC})
        if violations['total_lines'] > MAX_FILE_LOC:
            violations['loc_violations'].append({'type': 'file', 'name':
                file_path.name, 'lines': violations['total_lines'],
                'line_number': 1, 'max_allowed': MAX_FILE_LOC})
        for i, line in enumerate(content.splitlines(), 1):
            if len(line) > MAX_LINE_LENGTH:
                violations['line_length_violations'].append({'line_number':
                    i, 'length': len(line), 'max_allowed': MAX_LINE_LENGTH,
                    'content': line[:50] + '...' if len(line) > 50 else line})
            if 'logger.info(' in line and not line.strip().startswith('#'):
                violations['print_statements'].append({'line_number': i,
                    'content': line.strip()})
    except SyntaxError as e:
        violations['syntax_errors'].append({'line_number': e.lineno,
            'message': str(e), 'text': e.text})
    except Exception as e:
        violations['syntax_errors'].append({'line_number': 0, 'message':
            f'Error analyzing file: {e}', 'text': ''})
    return violations
...
def main():
    ...
    parser.add_argument('--n', type=int, default=1000, help=
...
def print_violations_report(violations: list[dict[str, Any]]) ->None:
    ...
    files_with_violations = sum(1 for v in violations if any([v[
        'syntax_errors'], v['loc_violations'], v['line_length_violations'],
        v['print_statements']]))
...
def print_summary_report(violations: list[dict[str, Any]]) ->None:
    ...
    line_length_violations = sum(len(v['line_length_violations']) for v in
        violations)
...
def run_ci_gate(violations: list[dict[str, Any]]) ->None:
    """Run CI gate validation."""
    total_violations = sum(len(v['syntax_errors']) + len(v['loc_violations'
        ]) + len(v['line_length_violations']) + len(v['print_statements']) for
        v in violations)
