...
class Task:
    ...
    def complete(self, success: bool = True) -> None:
        ...
        self._progress.percentage_complete = 100.0 if success else self._progress.percentage_complete
    ...
    def complete_milestone(self, milestone: str) -> None:
        ...
        if milestone in self._progress.milestones and milestone not in self._progress.completed_milestones:
    ...
    def is_overdue(self) -> bool:
        ...
        return datetime.now() > expected_completion and self._status not in [TaskStatus.COMPLETED, TaskStatus.CANCELLED]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'requirements': {
                ...
                'estimated_duration': self._requirements.estimated_duration.total_seconds() if self._requirements.estimated_duration else None,
            ...
            'progress': {
                ...
                'last_update': self._progress.last_update.isoformat() if self._progress.last_update else None,
                'notes': self._progress.notes
            },
            'metrics': {
                'time_created': self._metrics.time_created.isoformat(),
                'time_started': self._metrics.time_started.isoformat() if self._metrics.time_started else None,
                'time_completed': self._metrics.time_completed.isoformat() if self._metrics.time_completed else None,
                'total_duration': self._metrics.total_duration.total_seconds() if self._metrics.total_duration else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Task':
        ...
        if requirements_data.get('estimated_duration'):
            requirements_data['estimated_duration'] = timedelta(seconds=requirements_data['estimated_duration'])
        ...
        task._progress = TaskProgress(
            ...
            last_update=datetime.fromisoformat(progress_data['last_update']) if progress_data['last_update'] else None,
        ...
        task._metrics = TaskMetrics(
            ...
            time_started=datetime.fromisoformat(metrics_data['time_started']) if metrics_data['time_started'] else None,
            time_completed=datetime.fromisoformat(metrics_data['time_completed']) if metrics_data['time_completed'] else None,
            total_duration=timedelta(seconds=metrics_data['total_duration']) if metrics_data['total_duration'] else None,
