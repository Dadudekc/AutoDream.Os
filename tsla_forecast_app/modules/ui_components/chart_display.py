    ...
    def paintEvent(self, event):
        ...
        if len(self.data_points) > 1:
            for i in range(0, len(self.data_points), max(1, len(self.data_points) // 5)):
                x = margin + (chart_width * i / (len(self.data_points) - 1))
                painter.drawText(
                    x - 20, self.height() - 10, self.data_points[i]["time"].strftime("%H:%M")
                )

    def add_data_point(self, price, volume=0):
        """Add a new data point"""
        self.data_points.append({"price": price, "volume": volume, "time": datetime.now()})
...
class ChartDisplayWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.chart_type_combo.setStyleSheet(
            "padding: 5px; border: 1px solid #4169E1; border-radius: 3px;"
        )
