...
def test_performance_monitor_standalone():
    ...
    try:
        ...
        perf_monitor_module = load_module_from_file("performance_monitor", perf_monitor_path)
...
async def test_dashboard_backend_standalone():
    ...
    try:
        ...
        monitor = PerformanceMonitor()
        dashboard = DashboardBackend(monitor, host="localhost", port=0)  # Port 0 for testing
        ...
        if dashboard.websocket_handler:
            ...
            print(f"   - Active connections: {len(dashboard.websocket_handler.connections)}")
        else:
...
def test_dashboard_frontend_standalone():
    ...
    try:
        ...
        if "websocket" in js.lower() and "chart" in js.lower() and "cpu_usage_percent" in js:
    ...
    try:
        ...
        if len(alerting.alert_rules) == 3:
            ...
            for rule in alerting.alert_rules:
                print(f"   - {rule.name}: {rule.metric_name} {rule.condition.value} {rule.threshold}")
        else:
    ...
    try:
        ...
        if os.path.exists(config_file):
            ...
            if perf_config:
                ...
                print(f"   - Collection Interval: {perf_config.get('collection_interval', 'N/A')}s")
                print(f"   - Retention Period: {perf_config.get('retention_period', 'N/A')}s")
...
async def main():
    ...
    if failed == 0:
        ...
        print("\n🔗 Next Steps:")
        print("1. Start the system: python scripts/launch_performance_monitoring.py start")
