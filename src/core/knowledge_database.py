...
class KnowledgeDatabase:
    ...
    def store_knowledge(self, entry: KnowledgeEntry) -> bool:
        ...
        try:
            ...
            ''', (
                ...
                entry.created_at, entry.updated_at, entry.agent_id,
    ...
    def _row_to_entry(self, row: Tuple) -> KnowledgeEntry:
        ...
        return KnowledgeEntry(
            ...
            tags=json.loads(row[4]), source=row[5], confidence=row[6],
            created_at=row[7], updated_at=row[8], agent_id=row[9],
            related_entries=json.loads(row[10]), metadata=json.loads(row[11])
        )
...
class KnowledgeCLI:
    ...
    def display_statistics(self):
        ...
        stats_panel = Panel(
            ...
            "\n".join([f"  {agent}: {count}" for agent, count in stats.get('agents', {}).items()]),
...
# CLI Commands
@click.group()
@click.option('--db-path', default='knowledge_base.db', help='Path to knowledge database')
@click.pass_context
def cli(ctx, db_path):
...
def add(ctx, title, content, category, tags, source, confidence, agent_id, related):
    ...
    if cli_obj.db.store_knowledge(entry):
        cli_obj.console.print(f"[green]Knowledge entry added successfully: {entry_id}[/green]")
...
def show(ctx, entry_id):
    ...
    cli_obj.console.print(f"[yellow]Show command not yet implemented for ID: {entry_id}[/yellow]")
