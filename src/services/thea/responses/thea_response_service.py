...
class TheaResponseService:
    ...
    def wait_for_response(self, timeout: int = None) -> bool:
        ...
        if not self.browser.driver:
            print("⚠️  No browser driver available for response detection")
            return False

        print("🔍 Using automated DOM polling to detect when Thea is done...")

        result = self.response_detector.wait_until_complete(
            timeout=timeout,
        ...
        if result == ResponseWaitResult.COMPLETE:
            ...
            print("⚠️ Continue required but not auto-clicked; switching to manual confirmation.")
    ...
    def capture_response(self) -> Path | None:
        ...
        if screenshot_path:
            # Create comprehensive metadata and logs
    ...
    def _take_screenshot(self) -> Path | None:
        ...
        try:
            ...
            screenshot_path = self.config.responses_dir / f"thea_response_{timestamp}.png"
    ...
    def _create_response_metadata(self, screenshot_path: Path, extracted_response: str) -> None:
