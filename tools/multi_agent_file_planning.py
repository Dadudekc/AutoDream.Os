...
def main():
    ...
    print("  📋 Framework: MULTI_AGENT_FILE_PLANNING_FRAMEWORK.json")
    print(f"  👥 Agent plans: {len(agent_plans)} agent-specific plans")
    print(
        f"  🎯 Objective: Reduce from {framework['current_state']['total_files']:,} to {framework['current_state']['target_files']} files"
    )
    print(f"  📊 Reduction target: {framework['current_state']['reduction_percentage']}%")

    print("\n🎯 Next steps:")
    print("  1. Agent-5 leads multi-agent planning session")
    print("  2. All agents contribute to planning framework")
    print("  3. Use debate system for complex decisions")
    print("  4. Create final file list with agent consensus")
    print("  5. Execute cleanup based on agreed plan")
