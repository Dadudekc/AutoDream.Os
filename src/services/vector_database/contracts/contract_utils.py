...
def filter_contracts_by_status(contracts: List[V3Contract], status: ContractStatus) -> List[V3Contract]:
    """Filter contracts by status."""
    return [contract for contract in contracts if contract.status == status]


def filter_contracts_by_priority(contracts: List[V3Contract], priority: ContractPriority) -> List[V3Contract]:
    """Filter contracts by priority."""
    return [contract for contract in contracts if contract.priority == priority]


def get_contract_by_id(contracts: List[V3Contract], contract_id: str) -> Optional[V3Contract]:
    """Get contract by ID."""
...
def update_contract_status(contract: V3Contract, status: ContractStatus, agent_id: Optional[str] = None) -> None:
...
def calculate_contract_metrics(contracts: List[V3Contract]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'completion_rate': (status_counts['completed'] / total_contracts * 100) if total_contracts > 0 else 0
        }
