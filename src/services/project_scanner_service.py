...
class ProjectScannerService:
    ...
    def scan_project(self, project_path: Path, include_hidden: bool = False) -> Dict[str, Any]:
    ...
    def scan_directory(self, directory_path: Path, recursive: bool = True) -> List[Dict[str, Any]]:
    ...
    def _discover_project_files(self, project_path: Path, include_hidden: bool) -> List[Path]:
    ...
    def _filter_supported_files(self, files: List[Path]) -> List[Path]:
        ...
        supported_extensions = {
            '.py', '.js', '.ts', '.jsx', '.tsx', '.rs', '.java', '.cpp', '.c',
    ...
    def _generate_project_summary(self, project_path: Path, analysis_results: List[Dict[str, Any]], 
                                scan_time: float) -> Dict[str, Any]:
        """Generate comprehensive project summary"""
        try:
            # Filter successful analyses
            successful_analyses = [r for r in analysis_results if r.get('status') == 'analyzed']
            failed_analyses = [r for r in analysis_results if r.get('status') == 'failed']
            ...
            summary = {
                ...
                'summary': {
                    ...
                    'average_complexity': round(total_complexity / len(successful_analyses), 2) if successful_analyses else 0.0
                },
    ...
    def _calculate_language_distribution(self, analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _calculate_size_distribution(self, analyses: List[Dict[str, Any]]) -> Dict[str, Any]:
