...
class MLPipelineSystem:
    ...
    def create_training_data(self, 
        ...
        if data_type == "synthetic":
            ...
            labels = np.dot(features, true_weights) + np.random.randn(num_samples, self.config.output_size) * 0.1
        else:
        ...
        logger.info(f"✅ Created {data_type} training data: {num_samples} samples, {num_features} features")
    ...
    def _create_tensorflow_model(self):
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.Dense(self.config.hidden_size, activation='relu', input_shape=(self.config.input_size,)),
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_versions": {name: len(versions) for name, versions in self.model_versions.items()},
            "ml_directories": {
