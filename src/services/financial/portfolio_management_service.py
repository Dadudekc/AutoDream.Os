...
class PortfolioPosition:
    ...
    def update_pnl(self):
        ...
        if self.avg_price > 0:
            self.unrealized_pnl_pct = (self.unrealized_pnl / (self.quantity * self.avg_price)) * 100
...
class PortfolioManager:
    ...
    def __init__(self, portfolio_name: str = "default", data_dir: str = "portfolio_data"):
    ...
    def add_position(self, symbol: str, quantity: float, price: float, sector: str = "") -> bool:
        ...
        try:
            if not self.positions:
                self.metrics = PortfolioMetrics(
                    ...
                    daily_pnl=0, daily_pnl_pct=0, sharpe_ratio=0, max_drawdown=0,
                    volatility=0, beta=1.0
                )
                return self.metrics
    ...
    def calculate_volatility(self) -> float:
        ...
        weights = [pos.market_value / self.metrics.total_value for pos in self.positions.values()]
        return np.sqrt(np.sum(np.array(weights) ** 2)) * 0.15  # Assume 15% individual volatility
    ...
    def calculate_sharpe_ratio(self) -> float:
        ...
        if self.metrics and self.metrics.volatility > 0:
            return (self.metrics.total_pnl_pct - 2.0) / self.metrics.volatility  # Assume 2% risk-free rate
        return 0.0
