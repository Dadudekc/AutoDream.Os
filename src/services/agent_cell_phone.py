...
class AgentMessage:
    ...
    def __init__(self, from_agent: str, to_agent: str, content: str, 
                 tag: MsgTag = MsgTag.NORMAL, timestamp: Optional[float] = None):
...
class AgentCellPhone:
    ...
    def __init__(self, agent_id: str = "Agent-1", config_dir: str = "config", test: bool = False):
    ...
    def _emit_heartbeat(self):
        ...
        for agent_id in self.agent_manager.get_all_agents():
            if self.agent_manager.get_agent_status(agent_id) == AgentStatus.ONLINE:
                self.agent_manager.update_agent_metadata(agent_id, {
    ...
    def send(self, agent: str, message: str, tag: MsgTag = MsgTag.NORMAL, 
             new_chat: bool = False, nudge_stalled: bool = False) -> None:
