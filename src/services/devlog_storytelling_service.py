...
class DevlogStorytellingService:
    ...
    def process_devlog_batch(self, batch_size: int = 5) -> Optional[StoryChapter]:
    ...
    def _load_recent_devlogs(self, count: int) -> List[DevlogEntry]:
        ...
        try:
            ...
            with open(self.devlog_database_path, 'r') as f:
                devlog_data = json.load(f)
                
            # Get recent entries (assuming they're stored with timestamps)
            entries = devlog_data.get("entries", [])
            recent_entries = sorted(entries, key=lambda x: x.get("timestamp", ""), reverse=True)[:count]
    ...
    def _create_story_chapter(self, devlogs: List[DevlogEntry], analysis: Dict[str, Any]) -> StoryChapter:
    ...
    def _convert_devlog_to_narrative(self, devlog: DevlogEntry, sequence: int) -> str:
        ...
        agent_name = f"Agent-{devlog.agent_id.split('-')[-1]}" if '-' in devlog.agent_id else devlog.agent_id
        ...
        base_narrative = narrative_templates.get(narrative_type, narrative_templates["task_completion"])
    ...
    def _analyze_world_developments(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "system_improvements": ["Enhanced collaboration protocols", "Improved efficiency metrics"]
        }
