...
class PyAutoGUIMessageDelivery(PyAutoGUIDeliveryProvider):
                ...
                for agent_id, agent_info in agents_data.items():
                    if isinstance(agent_info, dict):
                        ...
                        if isinstance(coords, (list, tuple)) and len(coords) >= 2:
                            self.agent_coordinates[agent_id] = AgentCoordinates.from_tuple(
                                agent_id, coords
                            )
    ...
    def send_message(self, message: UnifiedMessage) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Message sent via PyAutoGUI to {message.recipient} at ({agent_coords.x}, {agent_coords.y})"
            )
            return True
    ...
    def _load_coordinates_from_json(self) -> dict[str, tuple]:
        ...
        try:
            if self.coordinates_file.exists():
                ...
                for agent_id, agent_info in agents_data.items():
                    if isinstance(agent_info, dict):
                        # Look for chat_input_coordinates
                        coords = agent_info.get("chat_input_coordinates")
                        if isinstance(coords, (list, tuple)) and len(coords) >= 2:
                            coords_dict[agent_id] = tuple(coords[:2])  # x, y
                    elif isinstance(agent_info, (list, tuple)) and len(agent_info) >= 2:
                        # Direct coordinate tuple
                        coords_dict[agent_id] = tuple(agent_info[:2])  # x, y
