...
class DiscordOnboardingIntegration:
    ...
    async def _handle_status_command(self, args: Dict[str, Any]) -> str:
        ...
        try:
            ...
            for agent_id, state in states.items():
                ...
                status_message += f"{status_emoji} **{agent_id}**: {state} (Contract: {contract_status})\n"
        ...
        try:
            ...
            for agent_id, contract in self.onboarding_service.contracts.items():
                if contract and contract.status in ["active", "pending"]:
                    active_contracts += 1
                    contracts_message += f"📄 **{agent_id}**: {contract.contract_type.value} ({contract.status})\n"
    ...
    async def _handle_reset_agent_command(self, args: Dict[str, Any]) -> str:
        ...
        try:
            # Reset agent state
            self.onboarding_service.agent_states[agent_id] = self.onboarding_service.AgentState.UNINITIALIZED
