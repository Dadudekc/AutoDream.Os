...
def execute_documentation_cleanup():
    ...
    if node_modules_path.exists():
        try:
            ...
            print(f"✅ Deleted node_modules: {len(md_files)} files, {size_before / (1024 * 1024):.1f} MB")
    ...
    if devlogs_path.exists():
        try:
            ...
            if len(md_files) > 10:
                ...
                freed_size_mb += size_before / (1024 * 1024)
                print(f"✅ Cleaned devlogs: {len(files_to_delete)} files deleted, {size_before / (1024 * 1024):.1f} MB freed")
            else:
    ...
    if agent_workspaces_path.exists():
        try:
            ...
            freed_size_mb += size_before / (1024 * 1024)
            print(f"✅ Cleaned agent workspaces: {len(all_workspace_files)} files deleted, {size_before / (1024 * 1024):.1f} MB freed")
        except Exception as e:
    ...
    if thea_responses_path.exists():
        try:
            ...
            freed_size_mb += size_before / (1024 * 1024)
            print(f"✅ Cleaned thea responses: {len(files_to_delete)} files deleted, {size_before / (1024 * 1024):.1f} MB freed")
        except Exception as e:
    ...
    if docs_path.exists():
        try:
            ...
            freed_size_mb += size_before / (1024 * 1024)
            print(f"✅ Consolidated docs: {len(files_to_delete)} files deleted, {size_before / (1024 * 1024):.1f} MB freed")
        except Exception as e:
