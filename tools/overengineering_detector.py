...
class OverengineeringIssue:
    ...
    def __init__(
        ...
        file_path: str,
        line_number: int,
        issue_type: OverengineeringType,
        severity: OverengineeringSeverity,
        description: str,
        recommendation: str,
    ):
...
class OverengineeringDetector:
            ...
            if len(content.splitlines()) > 400:
                issues.append(
                    OverengineeringIssue(
                        file_path,
                        0,
                        OverengineeringType.OVER_ARCHITECTURE,
                ...
                if len(node.bases) > 2:
                    issues.append(
                        OverengineeringIssue(
                            file_path,
                            node.lineno,
                            OverengineeringType.COMPLEX_ABSTRACTION,
                ...
                if red_flag.lower() in line_lower:
                    issues.append(
                        OverengineeringIssue(
                            file_path,
                            i,
                            OverengineeringType.UNNECESSARY_PATTERN,
                            OverengineeringSeverity.MEDIUM,
    ...
    def get_simplification_recommendations(self) -> list[str]:
        ...
        abstraction_issues = [
            i for i in self.issues if i.type == OverengineeringType.COMPLEX_ABSTRACTION
        ]
        if abstraction_issues:
            recommendations.append(
                "Consider simplifying abstractions - use direct method calls instead of complex patterns"
            )

        pattern_issues = [
            i for i in self.issues if i.type == OverengineeringType.UNNECESSARY_PATTERN
        ]
        if pattern_issues:
            recommendations.append(
                "Remove unnecessary design patterns - use simple, direct approaches"
            )

        architecture_issues = [
            i for i in self.issues if i.type == OverengineeringType.OVER_ARCHITECTURE
        ]
