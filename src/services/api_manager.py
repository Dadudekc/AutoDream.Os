...
class BaseMiddleware(ABC):
    ...
    @abstractmethod
    async def process(self, request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
...
class LoggingMiddleware(BaseMiddleware):
    ...
    async def process(self, request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"API Request [{request_id}]: {request.get('method', 'UNKNOWN')} {request.get('path', 'UNKNOWN')}")
...
class APIManager:
    ...
    def _setup_default_middleware(self):
        ...
        self.add_middleware(RateLimitMiddleware({'default_limit': 100, 'window_size': 60}), priority=300)
    ...
    def add_middleware(self, middleware: BaseMiddleware, priority: int = 0):
        ...
        logger.info(f"Added middleware: {middleware_config.name} (priority: {priority})")
    
    async def process_request(self, method: str, path: str, headers: Dict[str, str] = None, 
                            body: Any = None, client_id: str = None) -> Dict[str, Any]:
    ...
    async def _execute_handler(self, handler: Callable, request: Dict[str, Any], 
...
async def example_handler(request: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
...
def setup_example_endpoints():
    ...
    health_endpoint = APIEndpoint(
        path="/api/health",
        method=APIMethod.GET,
        handler=lambda req, ctx: {'status': 'healthy', 'services': service_registry.list_services()},
