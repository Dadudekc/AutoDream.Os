...
class DiscordServerManager:
    ...
    async def create_channel(self, name: str, channel_type: str = "text", category: str = None, topic: str = None) -> bool:
        ...
        try:
            ...
            if channel_type.lower() in ["text", "txt"]:
                overwrites = {
                    self.guild.default_role: discord.PermissionOverwrite(read_messages=True, send_messages=True)
                }
    ...
    async def manage_roles(self, action: str, role_name: str = None, permissions: str = None) -> bool:
        """Manage server roles."""
        ...
        try:
            if action == "list":
                ...
                if permissions:
                    perm_value = discord.Permissions(**{p.strip(): True for p in permissions.split(',')})
        ...
        try:
            if action == "list":
                ...
                for i, member in enumerate(self.guild.members[:20]):
                    ...
                    print(f"     ID: {member.id}, Joined: {member.joined_at.strftime('%Y-%m-%d') if member.joined_at else 'Unknown'}")
    ...
    async def server_audit(self) -> bool:
        ...
        if dangerous_roles:
            ...
            print(f"   ✅ No dangerous administrator roles found")
        
        # Channel audit
        print(f"\n📺 Channel Audit:")
        orphaned_channels = [c for c in self.guild.channels if c.category is None and not isinstance(c, discord.CategoryChannel)]
        if orphaned_channels:
            print(f"   ⚠️ Orphaned channels: {len(orphaned_channels)}")
            for channel in orphaned_channels:
                print(f"     - {channel.name} ({channel.id})")
        else:
            print(f"   ✅ All channels are properly categorized")
        
        # Member audit
        print(f"\n👥 Member Audit:")
        recent_joins = [m for m in self.guild.members if m.joined_at and (datetime.now(m.joined_at.tzinfo) - m.joined_at).days < 7]
        ...
        unused_roles = []
        for role in self.guild.roles:
            if role.name != "@everyone" and len([m for m in self.guild.members if role in m.roles]) == 0:
    ...
    def show_help(self) -> bool:
        ...
        print("   python discord_server_manager.py send-message --channel-id 123456789 --content 'Hello!'")
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Discord Server Manager - Complete Discord server control")
    ...
    create_parser.add_argument('--type', choices=['text', 'txt', 'voice', 'vc', 'category', 'cat'], 
                             default='text', help='Channel type')
    ...
    roles_parser.add_argument('--action', choices=['list', 'create', 'delete'], required=True, help='Action to perform')
    ...
    members_parser.add_argument('--action', choices=['list', 'kick', 'ban'], required=True, help='Action to perform')
