...
class TestMessagingSystemIntegration:
    ...
    def test_consolidated_service_integration(
        self, mock_coordinates_file, mock_pyautogui, mock_pyperclip
    ):
    ...
    def test_messaging_workflow_integration(
        self, mock_coordinates_file, mock_pyautogui, mock_pyperclip
    ):
    ...
    def test_status_monitoring_integration(
        self, mock_coordinates_file, mock_project_analysis, mock_fsm_status, mock_agent_workspace
    ):
    ...
    def test_error_handling_integration(self, mock_coordinates_file):
        ...
        with patch.object(
            service.messaging_service, "_is_agent_active", return_value=True
        ), patch.object(
            service.messaging_service, "_paste_to_coords", side_effect=Exception("Test error")
        ):
    ...
    def test_cookie_management_integration(self, mock_coordinates_file, mock_cookie_file):
        ...
        from thea_auth import TheaCookieManager, TheaLoginHandler
    ...
    def test_discord_integration(
        self, mock_coordinates_file, mock_discord_bot, mock_discord_context
    ):
        ...
        with patch(
            "src.services.discord_bot_with_devlog.json.load",
            return_value={"agents": {"Agent-1": {"active": True}}},
        ):
    ...
    def test_concurrent_operations_integration(self, mock_coordinates_file):
        ...
        for i in range(5):
            thread = threading.Thread(
                target=send_message, args=(f"Agent-{i+1}", f"Concurrent message {i}")
            )
