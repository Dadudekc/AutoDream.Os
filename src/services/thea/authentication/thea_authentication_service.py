    ...
    def save_cookies(self, cookies: list[dict[str, Any]]) ->bool:
        ...
        try:
            cookie_data = {'timestamp': datetime.now().isoformat(),
                'cookies': cookies, 'domain': 'chatgpt.com'}
    ...
    def load_cookies(self) ->tuple[list[dict[str, Any]], bool]:
        ...
        try:
            ...
            if isinstance(data, list):
                cookies = data
                logger.info(
                    f'✅ Cookies loaded (legacy format): {len(cookies)} cookies'
                    )
                return cookies, True
                ...
                if timestamp_str:
                    try:
                        ...
                        logger.info(
                            '⚠️  Invalid timestamp format in cookie file')
    ...
    def is_logged_in(self) ->AuthStatus:
        ...
        try:
            ...
            for indicator in login_indicators:
                try:
                    element = self.browser.driver.find_element(By.
                        CSS_SELECTOR, indicator)
                    if element.is_displayed():
                        logger.info(
                            '🔍 Detected login page - authentication required')
                        return AuthStatus.REQUIRES_LOGIN
                except Exception:
                    continue
            auth_indicators = ["[data-testid='profile-button']",
                "[data-testid='send-button']", '#prompt-textarea',
...
class TheaAuthenticationService:
    ...
    def ensure_authenticated(self, browser_service: 'TheaBrowserService'
        ) ->bool:
    ...
    def _try_cookie_authentication(self, browser_service: 'TheaBrowserService'
        ) ->bool:
    ...
    def save_session_cookies(self, browser_service: 'TheaBrowserService'
        ) ->None:
    ...
    def check_login_status(self, browser_service: 'TheaBrowserService'
        ) ->AuthStatus:
