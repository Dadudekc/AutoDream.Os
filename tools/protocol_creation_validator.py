...
class ProtocolCreationValidator:
    ...
    def validate_protocol_necessity(self, title: str, description: str, 
                                  problem_statement: str, proposed_solution: str) -> ValidationResult:
        ...
        if duplicate_check["found"]:
            reasons.append(f"Similar protocol already exists: {duplicate_check['similar_protocol']}")
        ...
        if kb_coverage["covered"]:
            reasons.append(f"Problem already covered in knowledge base: {kb_coverage['relevant_solutions']}")
        ...
        if tools_coverage["covered"]:
            reasons.append(f"Solution can be achieved with existing tools: {tools_coverage['existing_tools']}")
            alternatives.append("Use or extend existing tools instead")
        
        # Calculate confidence score
        confidence_score = self._calculate_confidence_score(reasons, alternatives, existing_protocols)
    ...
    def _check_knowledge_base_coverage(self, problem: str, solution: str) -> Dict[str, Any]:
        ...
        return {
            "covered": covered,
            "relevant_solutions": relevant_solutions[:3]  # Limit to top 3
        }
    ...
    def _check_existing_tools_coverage(self, solution: str) -> Dict[str, Any]:
        ...
        return {
            "covered": covered,
            "existing_tools": existing_tools[:3]  # Limit to top 3
        }
    ...
    def _calculate_confidence_score(self, reasons: List[str], alternatives: List[str], 
                                  existing_protocols: List[str]) -> float:
...
def main():
    ...
    parser.add_argument("--alternatives", action="store_true", help="Get alternatives to protocol creation")
    ...
    if args.validate:
        if not all([args.title, args.description, args.problem, args.solution]):
            print("❌ ERROR: All fields required for validation: --title, --description, --problem, --solution")
            return
        ...
        print("\nExample:")
        print("  python protocol_creation_validator.py --validate --title 'New Protocol' --description 'Description' --problem 'Problem statement' --solution 'Proposed solution'")
        print("  python protocol_creation_validator.py --alternatives --problem 'Need to standardize workflow'")
