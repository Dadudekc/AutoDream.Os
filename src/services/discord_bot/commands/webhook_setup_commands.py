...
def setup_webhook_setup_commands(bot: commands.Bot):
    ...
    @bot.tree.command(name="provision-webhook", description="Create per-agent webhook in a channel")
    @app_commands.describe(
        agent="Agent identifier (Agent-1 to Agent-8)", channel="Target channel for events"
    )
    async def provision_webhook(
        interaction: discord.Interaction, agent: str, channel: discord.TextChannel
    ):
        ...
        if not agent.startswith("Agent-") or agent not in [f"Agent-{i}" for i in range(1, 9)]:
            return await interaction.followup.send(
                "❌ Invalid agent ID. Use Agent-1 through Agent-8.", ephemeral=True
            )
        ...
        if not webhook_url:
            return await interaction.followup.send(
                "❌ Failed to create webhook. Check bot permissions in the channel.", ephemeral=True
            )
    ...
    @bot.tree.command(name="rotate-webhook", description="Rotate (recreate) an agent's webhook")
    ...
    @bot.tree.command(name="revoke-webhook", description="Delete an agent's webhook")
    @app_commands.describe(agent="Agent identifier (Agent-1 to Agent-8)")
    async def revoke_webhook(interaction: discord.Interaction, agent: str):
        ...
        if success:
            await interaction.followup.send(
                f"🧨 Webhook revoked and removed from storage for **{agent}**", ephemeral=True
            )
        else:
            await interaction.followup.send(
                f"⚠️ No webhook found to revoke for **{agent}**", ephemeral=True
            )

    @bot.tree.command(
        name="test-webhook", description="Send a test event through the stored webhook"
    )
    @app_commands.describe(agent="Agent identifier (Agent-1 to Agent-8)")
    async def test_webhook(interaction: discord.Interaction, agent: str):
        ...
        if success:
            await interaction.followup.send(
                f"✅ Test message delivered to **{agent}** webhook", ephemeral=True
            )
        else:
            await interaction.followup.send(
                f"❌ Test failed for **{agent}** (no webhook or network issue)", ephemeral=True
            )

    @bot.tree.command(name="list-webhooks", description="List all configured webhooks")
    async def list_webhooks(interaction: discord.Interaction):
        ...
        if not webhooks:
            return await interaction.followup.send("📝 No webhooks configured", ephemeral=True)
