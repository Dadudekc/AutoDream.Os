...
class CustomHealthCheck(HealthCheckStrategy):
    ...
    async def check_health(self, service: ServiceInfo) -> ServiceStatus:
        ...
        try:
            ...
            loop = asyncio.get_event_loop()
            with ThreadPoolExecutor() as executor:
                result = await loop.run_in_executor(executor, self.check_function, service)
...
class ServiceRegistry:
    ...
    def __init__(self):
        ...
        self.service_status_changed_callbacks: List[Callable[[ServiceInfo, ServiceStatus, ServiceStatus], None]] = []
    ...
    def register_health_check(self, service_type: ServiceType, strategy: HealthCheckStrategy):
        """Register a health check strategy for a service type."""
        self.health_checkers[service_type] = strategy
        logger.info(f"Registered health check strategy for {service_type.value}: {strategy.__class__.__name__}")
    ...
    def register_service(self, service: ServiceInfo) -> str:
        ...
        if service.name in self.service_names:
            ...
            logger.warning(f"Service name '{service.name}' already exists, updating existing service")
    ...
    def find_services(self, service_type: Optional[ServiceType] = None, 
                     ...
                     capabilities: Optional[Set[str]] = None) -> List[ServiceInfo]:
        ...
        for service in self.services.values():
            ...
            # Check capabilities
            if capabilities and not capabilities.issubset(service.metadata.capabilities):
                continue
    ...
    def update_service_status(self, service_id: str, status: ServiceStatus) -> bool:
        ...
        if old_status != status:
            ...
            logger.info(f"Service {service.name} status changed: {old_status.value} -> {status.value}")
    ...
    async def _perform_health_check(self, service: ServiceInfo) -> ServiceStatus:
        ...
        if not checker:
            logger.warning(f"No health checker for service type: {service.service_type.value}")
    ...
    def add_service_unregistered_callback(self, callback: Callable[[ServiceInfo], None]):
        """Add callback for when services are unregistered."""
        self.service_unregistered_callbacks.append(callback)
    
    def add_service_status_changed_callback(self, callback: Callable[[ServiceInfo, ServiceStatus, ServiceStatus], None]):
...
async def main():
    ...
    def on_service_status_changed(service: ServiceInfo, old_status: ServiceStatus, new_status: ServiceStatus):
        print(f"Service {service.name} status changed: {old_status.value} -> {new_status.value}")
