...
from swarm_brain.connectors import ingest_devlog, ingest_discord, ingest_performance, ingest_scan
...
def example_project_scanner_integration():
    ...
    scan_result = {
        "compliance": {
            "v2_pass": 97.6,
            "violations": [{"file": "large_file.py", "rule": "len>400"}],
        },
...
def example_devlog_integration():
    ...
    # Ingest devlog
    ingest_devlog(
        text=devlog_text, project="Agent_Cellphone_V2", agent_id="Agent-2", channel="devlog"
    )
...
def example_performance_integration():
    ...
    metrics = {"cpu_usage": 45.0, "memory_usage": 60.0, "response_time": 1.2, "active_agents": 8}

    anomalies = {"high_memory_usage": False, "slow_response": False}

    optimizations = {"cache_size": 1000, "parallel_processing": True}
...
def example_query_usage():
    ...
    for pattern in patterns[:3]:  # Show first 3
        logger.info(
            f"  - {pattern.get('title', 'No title')} by {pattern.get('agent_id', 'Unknown')}"
        )
...
def example_cli_usage():
    ...
    commands = [
        ...
        [
            ...
            '["test", "cli"]',
            "--payload",
            json.dumps({"tool": "test_tool", "outcome": "success", "context": {"test": True}}),
        ],
        # Query for patterns
        [
            "python",
