...
class ResponseCaptureService:
                ...
                for agent_dir in watch_root.iterdir():
                    if agent_dir.is_dir() and agent_dir.name.startswith("Agent-"):
                        ...
                        if response_file.exists():
                            self.watched_files[str(response_file)] = response_file.stat().st_mtime
                            self.logger.info(f"Monitoring response file: {response_file}")
            else:
    ...
    def _is_likely_ai_response(self, content: str) -> bool:
        ...
        ai_indicators = [
            "I'll help you", "Here's what I found", "Based on", "Let me",
            "I can", "I will", "Here's how", "The answer is"
        ]
    ...
    def _process_captured_response(self, response: CapturedResponse):
        ...
        try:
            self.logger.info(f"Captured response from {response.agent_id}: {response.content[:100]}...")
...
def main():
    ...
    parser.add_argument("--capture", nargs=3, help="Capture response (agent,text,source)")
