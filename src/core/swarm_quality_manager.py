...
class SwarmQualityManager:
    ...
    def perform_qc_check(self, standard: QCStandard, checked_by: str = "system") -> QCStandardResult:
        ...
        logger.info(f"QC check performed for {standard.value}: {'PASSED' if result.passed else 'FAILED'}")
        return result
    ...
    def get_qc_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "overall_pass_rate": (passed_standards / total_standards) * 100 if total_standards > 0 else 0,
            "last_checked": max(result.checked_at for result in self.qc_standards.values()) if self.qc_standards else None
        }
