...
from .status_types import UpdateFrequency, StatusEventType, StatusEvent, StatusMetrics, ActivitySummary
...
class LiveStatusSystem:
    ...
    def __init__(self, update_frequency: UpdateFrequency = UpdateFrequency.HIGH_FREQUENCY):
    ...
    def _set_update_interval(self):
        ...
        interval_map = {UpdateFrequency.REAL_TIME: 1.0, UpdateFrequency.HIGH_FREQUENCY: 5.0, 
                       UpdateFrequency.MEDIUM_FREQUENCY: 15.0, UpdateFrequency.LOW_FREQUENCY: 60.0}
    ...
    def start_live_updates(self):
        ...
        self.logger.info(f"Live status system started - updates every {self.update_interval} seconds")
    ...
    def _detect_status_changes(self, current_status: Dict[str, Any]):
        ...
        changes = [("total_agents", "Agent count", "high"), ("online_agents", "Online agents", "medium")]
        
        for key, description, priority in changes:
            old_val, new_val = old_status.get(key, 0), current_status.get(key, 0)
            if old_val != new_val:
                self._create_status_event(StatusEventType.STATUS_CHANGE, "system",
                    f"{description} changed from {old_val} to {new_val}",
                    {f"old_{key}": old_val, f"new_{key}": new_val}, priority)
    
    def _process_agent_status_update(self, agent_status: Dict[str, Any], timestamp: float):
        """Process individual agent status update"""
        agent_id, current_status = agent_status.get("agent_id"), agent_status.get("status")
        
        if agent_id in self.last_status_snapshot.get("agent_details", {}):
            old_status = self.last_status_snapshot["agent_details"][agent_id].get("status")
            if old_status != current_status:
                self._create_status_event(StatusEventType.STATUS_CHANGE, agent_id,
                    f"Status changed from {old_status} to {current_status}",
                    {"old_status": old_status, "new_status": current_status}, "medium")
        performance_score = agent_status.get("performance_score", 0.0)
        if performance_score < 0.3:
            self._create_status_event(StatusEventType.HEALTH_ALERT, agent_id,
                f"Low performance score: {performance_score:.2f}",
                {"performance_score": performance_score, "threshold": 0.3}, "high")
        current_task = agent_status.get("current_task")
        if current_task:
            self._create_status_event(StatusEventType.TASK_UPDATE, agent_id,
                f"Current task: {current_task}",
                {"task": current_task, "status": current_status}, "low")
    
    def _create_status_event(self, event_type: StatusEventType, agent_id: str, 
                           description: str, details: Dict[str, Any], priority: str):
        """Create a new status event"""
        event = StatusEvent(
            event_id=f"event_{event_type.value}_{int(time.time())}_{agent_id}",
            event_type=event_type, agent_id=agent_id, timestamp=time.time(),
            old_status=None, new_status=None, details=details, priority=priority)
        
        self.status_events.append(event)
        if len(self.status_events) > 1000:
            self.status_events = self.status_events[-1000:]
        self.logger.info(f"Status event created: {event_type.value} for {agent_id} - {description}")
    ...
    def _get_recent_events(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        return [{"event_id": event.event_id, "event_type": event.event_type.value,
                ...
                "priority": event.priority, "details": event.details}
    ...
    def _get_agent_activity_summary(self) -> Dict[str, Any]:
        ...
        activity_summary = {
            ...
            "status_distribution": {},
            "capability_summary": {},
            "performance_summary": {"average_score": 0.0, "high_performers": 0, "low_performers": 0}
        }
        
        if agent_statuses:
            for agent_info in agent_statuses.values():
                status = agent_info.status.value
                activity_summary["status_distribution"][status] = activity_summary["status_distribution"].get(status, 0) + 1
                for capability in agent_info.capabilities:
                    cap_value = capability.value
                    activity_summary["capability_summary"][cap_value] = activity_summary["capability_summary"].get(cap_value, 0) + 1
            
            scores = [agent_info.performance_score for agent_info in agent_statuses.values()]
            activity_summary["performance_summary"]["average_score"] = sum(scores) / len(scores)
            activity_summary["performance_summary"]["high_performers"] = sum(1 for score in scores if score >= 0.8)
            activity_summary["performance_summary"]["low_performers"] = sum(1 for score in scores if score < 0.3)
    ...
    def _get_performance_metrics(self) -> Dict[str, Any]:
        ...
        # Calculate metrics
        online_agents = sum(1 for a in agent_statuses.values() if a.status != AgentStatus.OFFLINE)
        total_agents = len(agent_statuses)
        online_ratio = online_agents / total_agents if total_agents > 0 else 0.0
        avg_performance = sum(a.performance_score for a in agent_statuses.values()) / total_agents if total_agents > 0 else 0.0
        ...
        for subscriber_id, callbacks in self.status_subscribers.items():
            for callback in callbacks:
                try:
                    ...
                    self.logger.error(f"Subscriber notification failed for {subscriber_id}: {e}")
    ...
    def unsubscribe_from_status_updates(self, subscriber_id: str, callback: Callable = None):
    ...
    def change_update_frequency(self, new_frequency: UpdateFrequency):
        ...
        self._set_update_interval()
        self.logger.info(f"Update frequency changed to {new_frequency.value} ({self.update_interval}s)")
        print(f"📊 Update frequency changed to {new_frequency.value} ({self.update_interval}s)")
    ...
    def get_status_events(self, event_type: Optional[StatusEventType] = None, 
