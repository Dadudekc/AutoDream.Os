...
try:
    ...
    logger.info(f'⚠️ Primary imports failed: {e}')
...
class DiscordAgentBot(commands.Bot):
    ...
    def __init__(self, command_prefix: str='!', intents=None):
        ...
        self.agent_handlers = AgentCommandHandlers(self.agent_engine, self.
            embed_manager)
        self.swarm_handlers = SwarmCommandHandlers(self.agent_engine, self.
            embed_manager)
    ...
    def _initialize_messaging_gateway(self):
        ...
        try:
            ...
            from integration.messaging_gateway import MessagingGateway
            coordinates_path = os.getenv('COORDINATES_PATH',
            ...
            logger.info(
                f'⚠️  MessagingGateway not available - PyAutoGUI integration disabled: {e}'
                )
            return None
        ...
        if config_path.exists():
            try:
                with open(config_path) as f:
                    ...
                    self.max_concurrent_commands = config.get(
                        'max_concurrent_commands', 10)
            except Exception as e:
                logger.info(f'⚠️  Failed to load Discord bot config: {e}')
    ...
    def _load_agent_coordinates(self) ->dict[str, dict[str, Any]]:
        ...
        try:
            ...
            return {agent: {'chat_input_coordinates': [0, 0],
                'onboarding_coordinates': [0, 0], 'description':
                f'Agent {agent}', 'active': True} for agent in [
    ...
    async def on_ready(self):
        ...
        logger.info(f'⏱️  Command timeout: {self.command_timeout}s')
        logger.info(
            f'🔢 Max concurrent commands: {self.max_concurrent_commands}')
        logger.info('=' * 60)
        logger.info('💡 Available commands:')
        logger.info('  !prompt @agent message  - Prompt specific agent')
        logger.info('  !status @agent         - Check agent status')
        logger.info('  !swarm message         - Send swarm-wide message')
        logger.info(
            '  !urgent message        - URGENT broadcast (high priority)')
        logger.info('  !agents                - List all agents')
        logger.info('  !help                  - Show help')
        logger.info('  !ping                  - Test bot responsiveness')
        logger.info(
            '  /agent                 - Slash command with autocomplete')
        logger.info('  !captain message       - Send to Captain (Agent-4)')
        logger.info('  !agent1 message        - Send to Agent-1')
        logger.info(
            '  ...                    - Dynamic aliases from config/agent_aliases.json'
            )
        logger.info('=' * 60)
        ...
        if self.messaging_gateway:
            try:
                ...
                logger.info('✅ Agent summary commands registered successfully')
            except Exception as e:
                logger.info(f'⚠️  Failed to setup agent summary commands: {e}')
    ...
    async def on_message(self, message):
        ...
        if not allow_guild(guild_id) or not allow_channel(message.channel.id
    ...
    def get_command_stats(self) ->dict[str, Any]:
        ...
        return {'active_agent_commands': self.agent_handlers.
            get_active_command_count(), 'active_swarm_broadcasts': self.
            swarm_handlers.get_active_broadcast_count(), 'total_active': 
            self.agent_handlers.get_active_command_count() + self.
            swarm_handlers.get_active_broadcast_count()}
    ...
    async def _register_dynamic_agent_commands(self):
        ...
        try:
            ...
            logger.info('✅ Dynamic agent commands registered (prefix + slash)')
        except Exception as e:
            logger.info(f'⚠️ Failed to register dynamic agent commands: {e}')
    ...
    def _register_basic_fallback_commands(self):
        ...
        async def simple_agent(ctx, agent_num: int, *, message: str):
            ...
            try:
                await self.agent_engine.send_to_agent_inbox(agent_id,
                    message, f'Discord:{ctx.author.id}')
                await ctx.reply(f'✅ Message sent to {agent_id}')
            except Exception as e:
                await ctx.reply(f'❌ Failed to send message: {e}')
        logger.info('✅ Basic fallback agent commands registered')
