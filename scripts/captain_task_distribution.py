#!/usr/bin/env python3
"""
Captain Task Distribution Script - Agent Cellphone V2
===================================================
...
try:
    ...
    from src.services.messaging.unified_pyautogui_messaging import UnifiedPyAutoGUIMessaging
...
class CaptainTaskDistributor:
    """Captain Agent-4 Task Distribution System"""
    
    def __init__(self):
        """Initialize the task distribution system"""
        self.coordinate_manager = CoordinateManager()
        self.messaging = UnifiedPyAutoGUIMessaging(self.coordinate_manager)
...
"""
    ...
    def distribute_all_tasks(self) -> bool:
        """Distribute all 13 remaining tasks to all agents"""
        logger.info("🚀 Starting comprehensive task distribution to all agents")
        ...
        for agent, contract_content, task_id in all_contracts:
            try:
                logger.info(f"📤 Distributing {task_id} to {agent}")
                
                # Send contract using existing messaging system
                success = self.messaging.send_message(
                    recipient=agent,
                    message_content=contract_content,
                ...
                if success:
                    success_count += 1
                    logger.info(f"✅ {task_id} successfully distributed to {agent}")
                    self.tasks_distributed += 1
                else:
                    logger.error(f"❌ Failed to distribute {task_id} to {agent}")
                
                # Brief pause between messages
                time.sleep(1)
                
            except Exception as e:
                logger.error(f"❌ Error distributing {task_id} to {agent}: {e}")
        
        logger.info(f"🎯 Task distribution complete: {success_count}/{len(all_contracts)} successful")
...
def main():
    ...
    try:
        ...
        success = distributor.distribute_all_tasks()
        
        if success:
            logger.info("🎉 All tasks successfully distributed with comprehensive standards!")
        else:
            logger.warning("⚠️ Some tasks failed to distribute - check logs")
