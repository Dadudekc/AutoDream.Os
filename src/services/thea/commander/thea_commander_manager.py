...
class CommanderTheaManager:
    ...
    def run_enhanced_consultation(self, message: str, context: dict[str, Any] = None) -> bool:
        ...
        consultation_path = (
            self.config.responses_dir / f"commander_thea_consultation_{timestamp}.md"
        )
        with open(consultation_path, "w", encoding="utf-8") as f:
            f.write("# Commander THEA Enhanced Consultation\n")
        ...
        try:
            ...
            self._create_enhanced_conversation_log(
                conversation_path, message, commander_analysis, response_result
            )
            ...
            logger.info(f"   • Response Quality: {response_result.quality_score:.1%}")
            logger.info(f"   • Key Insights: {len(response_result.key_insights)}")
            logger.info(f"   • Action Items: {len(response_result.action_items)}")
            logger.info(
    ...
    def run_quick_analysis(self, message: str, context: dict[str, Any] = None) -> str:
        ...
        logger.info("=" * 50)
        logger.info("🎯 Generating strategic analysis without browser interaction...")
        ...
        logger.info(f"📊 Success Probability: {assessment['success_probability']:.1%}")
        logger.info(f"🎯 Confidence Level: {assessment['overall_confidence'].value.upper()}")
    ...
    def _create_enhanced_conversation_log(
        self, log_path: Path, message: str, analysis: str, response_result
    ) -> None:
        ...
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            f.write("**Analysis Type:** Enhanced Commander THEA Consultation\n\n")
            ...
            f.write("- Response Processing: Enhanced Commander THEA Analysis\n")
            f.write("- Analysis Depth: Strategic\n")
            f.write("- Swarm Integration: Active\n")
    ...
    def _display_analysis_summary(self, response_result, commander_analysis: str) -> None:
        ...
        logger.info("📊 ANALYSIS SUMMARY:")
        logger.info(f"   • Response Quality Score: {response_result.quality_score:.1%}")
        logger.info(f"   • Key Insights Identified: {len(response_result.key_insights)}")
        logger.info(f"   • Actionable Items: {len(response_result.action_items)}")
        logger.info(f"   • Analysis Length: {len(commander_analysis)} characters")
        if response_result.key_insights:
            logger.info("\n💡 TOP INSIGHTS:")
            for i, insight in enumerate(response_result.key_insights[:3], 1):
                logger.info(f"   {i}. {insight[:80]}{'...' if len(insight) > 80 else ''}")
        if response_result.action_items:
            logger.info("\n🎯 IMMEDIATE ACTIONS:")
            for i, action in enumerate(response_result.action_items[:3], 1):
                logger.info(f"   {i}. {action[:80]}{'...' if len(action) > 80 else ''}")
