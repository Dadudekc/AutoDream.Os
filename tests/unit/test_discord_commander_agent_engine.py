...
Comprehensive unit tests for agent communication and inbox management.
...
try:
    from src.discord_commander.agent_communication_engine_core import AgentCommunicationEngineCore
    from src.discord_commander.discord_commander_models import CommandResult
except ImportError:
    # Fallback for direct execution
    from discord_commander.agent_communication_engine_core import AgentCommunicationEngineCore
    from discord_commander.discord_commander_models import CommandResult


class TestAgentCommunicationEngineCore:
    """Test cases for the core agent communication engine."""
    ...
    def agent_engine(self, temp_workspace):
        """Create AgentCommunicationEngineCore instance for testing."""
...
class TestAgentCommunicationEngineIntegration:
    """Integration tests for agent communication engine."""
    ...
    def test_multiple_messages_to_same_agent(self, temp_workspace):
        ...
        with patch('os.getcwd', return_value=str(temp_workspace)):
            ...
            # Check that all message files were created
            message_files = list(inbox_dir.glob("*.md"))
            assert len(message_files) == 3

            # Check that filenames are unique
            filenames = [f.name for f in message_files]
    ...
    def test_messages_to_multiple_agents(self, temp_workspace):
        ...
        with patch('os.getcwd', return_value=str(temp_workspace)):
            ...
            # Check that messages were delivered to correct agents
...
class TestAgentCommunicationEngineErrorHandling:
    """Test cases for error handling in agent communication engine."""
    ...
    def test_get_all_agent_names_consistency(self):
        ...
        assert len(names1) == 8
        assert all(name.startswith("Agent-") for name in names1)
        assert all(name.split("-")[1].isdigit() for name in names1)


class TestAgentCommunicationEngineLogging:
    """Test cases for logging in agent communication engine."""
