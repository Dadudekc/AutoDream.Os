...
class MasterV2TestRunner:
    ...
    def run_all_test_suites(self):
        ...
        for suite_name, test_suite in self.test_suites.items():
            if test_suite != Mock:  # Skip mock suites
                ...
                self.results[suite_name] = {"status": "mock", "tests_run": 0, "success_rate": 0.0}
    ...
    def print_summary(self, report):
        ...
        for suite_name, result in report['suite_results'].items():
            ...
            print(f"  {suite_name.upper()}: {status} ({tests_run} tests, {success_rate:.1f}% success)")
...
def main():
    ...
    parser.add_argument("--suite", choices=['comprehensive', 'core', 'advanced', 'enterprise_quality'], 
                       help="Run specific test suite")
    parser.add_argument("--generate-report", action="store_true", help="Generate master report")
    ...
    if args.run_all:
        ...
        if result:
            ...
            print(f"Tests: {result.get('total_tests', 0)}, Success Rate: {result.get('success_rate', 0.0):.1f}%")
