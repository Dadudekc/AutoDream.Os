    ...
    def get_average_metrics(self, time_window_minutes: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if "memory" in recent_metrics[0]:
                ...
                if memory_values:
                    avg_metrics["memory"] = {
                        "avg_percent": sum(m.get("percent", 0) for m in memory_values) / len(memory_values),
                        "avg_used_gb": sum(m.get("used_gb", 0) for m in memory_values) / len(memory_values)
                    }
            ...
            if "cpu" in recent_metrics[0]:
                ...
                if cpu_values:
                    avg_metrics["cpu"] = {
                        "avg_percent": sum(m.get("cpu_percent", 0) for m in cpu_values) / len(cpu_values)
                    }
...
def main():
    ...
    parser.add_argument("--metrics", action="store_true", help="Get current performance metrics")
    parser.add_argument("--history", type=int, help="Get metrics history (limit)")
    parser.add_argument("--average", type=int, default=5, help="Get average metrics over minutes")
