...
class AgentCellPhoneService:
    ...
    def initialize_agent(self, agent_id: str, layout_mode: str = "5-agent") -> bool:
        ...
        try:
            ...
            if agent_id not in self.coordinates[layout_mode]:
                self.logger.error(
                    f"Agent '{agent_id}' not found in layout '{layout_mode}'"
                )
                return False
    ...
    def send_message(
        self, from_agent: str, to_agent: str, message: str, tag: str = "TASK"
    ) -> bool:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Agent Cell Phone Service Testing Interface"
    )
    ...
    parser.add_argument(
        "--layout", type=str, default="5-agent", help="Layout mode for agent"
    )
    parser.add_argument(
        "--send",
        nargs=3,
        metavar=("FROM", "TO", "MESSAGE"),
        help="Send message between agents",
    )
    ...
    if args.agent:
        ...
        print(
            f"Agent '{args.agent}' initialization: {'✅ Success' if success else '❌ Failed'}"
        )
    ...
    if args.status:
        ...
        if service.agents:
            ...
            for agent_id, agent in service.agents.items():
                print(
                    f"  {agent_id}: {agent['status']} ({len(agent['messages'])} messages)"
                )
    ...
    if args.test:
        ...
        try:
            ...
            print(
                f"Agent initialization test: {'✅ Success' if success else '❌ Failed'}"
            )

            # Test message sending
            if success:
                success = service.send_message(
                    "Agent-1", "Agent-2", "Test message", "TEST"
                )
