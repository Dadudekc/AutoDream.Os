...
def send_system_broadcast(message_file: str, priority: str = "high", sender: str = "[SYSTEM]", agent_mode: str = "5"):
    """
    ...
    system_messages = [msg for msg in mq.message_history[-len(message_ids):] 
    ...
    print(f"📱 All {len(mq.agent_registry)} agents should now have received the [SYSTEM] message!")
...
End of System Message""".format(timestamp=time.strftime("%Y-%m-%d %H:%M:%S"))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        agent_mode = sys.argv[4] if len(sys.argv) > 4 else "5"  # Default to 5-agent mode
        ...
        print("=" * 40)
        print("Usage:")
        print("  python system_broadcast_utility.py <message_file> [priority] [sender] [agent_mode]")
        print("  Example: python system_broadcast_utility.py message.txt high [SYSTEM] 5")
        ...
        if os.path.exists(default_file):
            ...
            if create_sample == 'y':
                if create_sample_message_file():
                    send_system_broadcast("sample_system_message.txt", "high", "[SYSTEM]", agent_mode)
            else:
                message_file = input("Enter message file path: ").strip()
                if message_file:
                    send_system_broadcast(message_file, "high", "[SYSTEM]", agent_mode)
