...
class CoordinationValidator:
                ...
                issues.append(
                    ValidationIssue(
                        ...
                        details={
                            ...
                            "provided": message_data["message_type"],
                        },
                        timestamp=datetime.now(),
                        component="message_structure",
                    )
    ...
    def run_comprehensive_validation(
        self, target_system: str, validation_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        all_issues.extend(
            self.validate_coordination_system(validation_data.get("coordination", {}))
        )
    ...
    def get_validation_report(self, target_system: str = None) -> dict[str, Any]:
        ...
        if target_system:
            system_issues = [
                issue for issue in self.validation_history if issue.component == target_system
            ]
        else:
        ...
        return {
            "validation_summary": {
                ...
                "last_validation": (
                    self.validation_history[-1].timestamp if self.validation_history else None
                ),
            },
            "compliance_metrics": {
                "overall_compliance": self._calculate_compliance_score(self.validation_history),
                "system_compliance": (
                    self._calculate_compliance_score(system_issues) if system_issues else 100.0
                ),
            },
