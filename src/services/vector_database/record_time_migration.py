...
class RecordTimeMigration:
    ...
    def __init__(self, source_db_path: str, target_db_path: str, config: Optional[Dict[str, Any]] = None):
    ...
    def start_migration(self) -> None:
        ...
        self._running = True
        self._migration_stats['start_time'] = datetime.now(timezone.utc)
    ...
    def _migrate_new_vectors(self) -> None:
        ...
        try:
            ...
            self._migration_stats['last_migration'] = datetime.now(timezone.utc)
        ...
        try:
            ...
            for future in as_completed(future_to_vector):
                ...
                try:
                    ...
                    logger.error(f"Failed to migrate vector {vector.get('vector_id', 'unknown')}: {e}")
                    self._migration_stats['errors'] += 1
    ...
    def get_migration_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._migration_stats['start_time']:
                elapsed_time = (datetime.now(timezone.utc) - self._migration_stats['start_time']).total_seconds()
            ...
            status = {
                ...
                'estimated_completion': self._estimate_completion_time(source_count, target_count, migration_rate),
                'timestamp': datetime.now(timezone.utc).isoformat()
            }
    ...
    def _estimate_completion_time(self, source_count: int, target_count: int, migration_rate: float) -> Optional[str]:
        ...
        try:
            ...
            estimated_time = datetime.now(timezone.utc) + timezone.utc.localize(
