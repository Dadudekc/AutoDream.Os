...
class TradingIntelligenceService:
    ...
    def pairs_trading_strategy(self, symbol1: str, symbol2: str, historical_data1: pd.DataFrame, historical_data2: pd.DataFrame) -> Optional[TradingSignal]:
        ...
        try:
            ...
            if self.market_conditions:
                if self.market_conditions.volatility_regime == "HIGH":
                    recommendations["recommendations"].append("Consider mean reversion strategies in high volatility")
                elif self.market_conditions.volatility_regime == "LOW":
                    recommendations["recommendations"].append("Consider momentum strategies in low volatility")
                
                if self.market_conditions.trend_direction == "BULLISH":
                    recommendations["recommendations"].append("Favorable conditions for trend-following strategies")
                elif self.market_conditions.trend_direction == "BEARISH":
                    recommendations["recommendations"].append("Consider defensive strategies and risk management")
    ...
    def save_data(self):
        ...
        try:
            ...
            performance_data = {strategy.value: asdict(metrics) 
