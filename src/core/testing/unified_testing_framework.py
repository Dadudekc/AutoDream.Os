...
class UnifiedTestingFramework:
    ...
    def _discover_unittest_suites(self) -> Dict[str, Dict[str, Any]]:
        ...
        for test_file in unittest_files:
            if test_file.stem not in [f.stem for f in self.tests_dir.rglob("test_*.py")]:
                suite_name = test_file.stem
                suites[suite_name] = {
    ...
    def run_test_suite(self, suite_name: str, config: Optional[TestSuiteConfig] = None) -> TestSuiteResult:
    ...
    def run_all_suites(self, config: Optional[TestSuiteConfig] = None) -> List[TestSuiteResult]:
        ...
        for suite_name in self.registered_suites:
            try:
                ...
                self.output_formatter.print_error(f"Failed to run suite '{suite_name}': {e}")
    ...
    def _run_pytest_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.output_formatter.print_error(f"Test suite execution timed out: {suite_info['path']}")
            return {"total": 0, "passed": 0, "failed": 0, "errors": 1, "skipped": 0}
        except Exception as e:
            self.output_formatter.print_error(f"Pytest execution failed: {e}")
            return {"total": 0, "passed": 0, "failed": 0, "errors": 1, "skipped": 0}

    def _run_unittest_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            runner = unittest.TextTestRunner(verbosity=2 if config and config.verbose else 1)
    ...
    def _run_custom_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
    ...
    def _run_integration_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
        """Run an integration test suite"""
        # Integration tests often use pytest, so delegate to pytest handler
        return self._run_pytest_suite(suite_info, config)

    def _run_performance_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
        """Run a performance test suite"""
        # Performance tests often use pytest, so delegate to pytest handler
        return self._run_pytest_suite(suite_info, config)

    def _run_smoke_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
        """Run a smoke test suite"""
        # Smoke tests often use pytest, so delegate to pytest handler
        return self._run_pytest_suite(suite_info, config)

    def _run_security_suite(self, suite_info: Dict[str, Any], config: Optional[TestSuiteConfig] = None) -> Dict[str, Any]:
...
"""
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            self.output_formatter.print_success("Testing framework cleaned up successfully")
