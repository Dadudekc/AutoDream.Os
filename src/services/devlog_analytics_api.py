...
class DevlogAnalyticsAPI:
    ...
    def _generate_analytics(self) -> dict[str, Any]:
        ...
        try:
            ...
            for date in sorted_dates[-7:]:  # Last 7 days
                activity_trend.append({"date": date, "count": daily_counts[date]})
    ...
    def _get_export_data(self) -> list[dict[str, Any]]:
        ...
        try:
            devlogs = self.vector_db.query(
                query="devlog entry", namespace="agent_devlogs", limit=1000
            )
                ...
                export_data.append(
                    {
                        ...
                        "timestamp": metadata.get("timestamp", ""),
                        "content": devlog.get("content", "")[:200] + "..."
                        if len(devlog.get("content", "")) > 200
                        else devlog.get("content", ""),
                    }
                )
    ...
    def _get_agents_data(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Get unique agents
            agents = set()
            devlogs = self.vector_db.query(query="devlog", namespace="agent_devlogs", limit=1000)
    ...
    def _get_top_agents(self, devlogs: list[dict], limit: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for agent_id, count in sorted(agent_counts.items(), key=lambda x: x[1], reverse=True)[
                ...
                top_agents.append(
                    {"agent_id": agent_id, "count": count, "role": self._get_agent_role(agent_id)}
                )
