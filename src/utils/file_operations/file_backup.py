...
class FileBackupOperations:
    ...
    def create_backup(self, source: Path, backup_dir: Path = None, 
                     backup_name: str = None, incremental: bool = False) -> BackupResult:
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"Backup completed: {backup_path} ({files_backed_up} files, {total_size} bytes)")
            else:
        ...
        try:
            ...
            for item in source_dir.rglob('*'):
                if item.is_file():
                    ...
                    if not backup_item.exists() or item.stat().st_mtime > backup_item.stat().st_mtime:
