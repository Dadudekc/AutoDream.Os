...
class VectorDocument:
    ...
    def from_dict(cls, data: dict[str, Any]) ->'VectorDocument':
        ...
        return cls(id=data['id'], content=data['content'], embedding=data[
            'embedding'], metadata=data.get('metadata', {}), created_at=
            datetime.fromisoformat(data['created_at']), updated_at=datetime
            .fromisoformat(data['updated_at']))

    def to_dict(self) ->dict[str, Any]:
        """Convert to dictionary."""
        return {'id': self.id, 'content': self.content, 'embedding': self.
            embedding, 'metadata': self.metadata, 'created_at': self.
            created_at.isoformat(), 'updated_at': self.updated_at.isoformat()}
...
def create_vector_document(content: str, doc_type: DocumentType=
    DocumentType.TEXT) ->VectorDocument:
    """Create a new vector document."""
    now = datetime.now()
    return VectorDocument(id=f'doc_{now.timestamp()}', content=content,
        embedding=[], metadata={}, created_at=now, updated_at=now)
