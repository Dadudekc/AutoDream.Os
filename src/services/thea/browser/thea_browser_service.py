...
class TheaBrowserService:
    ...
    def __init__(self, config: TheaConfig):
        self.config = config
        self.driver = None
        self.mode = BrowserMode.SELENIUM if config.use_selenium else BrowserMode.MANUAL
    ...
    def initialize_driver(self) -> bool:
        ...
        if self.mode == BrowserMode.MANUAL or not self._selenium_available:
            print("🔄 USING MANUAL MODE - Browser initialization skipped")
            return True

        if not self.config.use_selenium:
            print("🔄 SELENIUM DISABLED - Using manual mode")
    ...
    def navigate_to_chatgpt(self) -> bool:
        ...
        try:
            print("🌐 Navigating to ChatGPT for verification...")
            self.driver.get(self.config.chatgpt_base_url)
            return True
        except Exception as e:
            print(f"❌ Failed to navigate to ChatGPT: {e}")
            return False
