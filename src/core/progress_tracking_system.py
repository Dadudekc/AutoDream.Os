...
from .progress_models import AgentProgress, ProgressMilestone, SuperiorityMetrics
            ...
            if "system_progress" in dashboard_data:
                ...
                if system_progress.total_milestones > 0:
                    metrics.consolidation_efficiency = (
                        system_progress.completed_milestones / system_progress.total_milestones
                    ) * 100
                ...
                metrics.integration_success = min(
                    100.0, agent_count * 15.0
                )  # Simplified calculation

                # Calculate blocker resolution (inverse of issues)
                metrics.blocker_resolution = max(
                    75.0, 100.0 - (100.0 - metrics.consolidation_efficiency) * 0.5
                )
