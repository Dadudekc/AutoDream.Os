...
class CoordinationFactory:
    ...
    def create_integrated_system(self):
        ...
        return IntegratedOnboardingCoordinationSystem(
            coordination_service, onboarding_coordinator, agent_instructions
        )
...
class IntegratedOnboardingCoordinationSystem:
    ...
    def validate_system(self) -> dict[str, bool]:
        ...
        try:
            ...
            validation_results["fsm_initialized"] = len(
                self.coordination_service.agent_fsms
            ) == len(self.swarm_agents)
        ...
        try:
            if args.onboard_all_agents:
                ...
                for agent_id, contract in status.items():
                    logger.info(
                        f"  {agent_id}: {contract['status']} ({contract['progress_percentage']}%)"
                    )
...
def main():
    ...
    parser.add_argument(
        "--start-enhanced-cycles", action="store_true", help="Start enhanced cycles"
    )
    ...
    parser.add_argument(
        "--validate-system", action="store_true", help="Validate system configuration"
    )
