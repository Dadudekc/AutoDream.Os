...
class ContentRegistryOperations:
    ...
    def manage_operations(self, action: str, **kwargs) -> Any:
        ...
        if action == "bulk_register":
            ...
            return self.migrate_content(
                kwargs["source_dir"], kwargs["target_dir"], kwargs.get("dry_run", False)
            )
    ...
    def bulk_register_content(
        self, directory: str, content_type: ContentType, agent_id: str, recursive: bool = False
    ) -> dict[str, Any]:
        ...
        results = {"success": True, "registered": 0, "failed": 0, "errors": []}
    ...
    def migrate_content(
        self, source_dir: str, target_dir: str, dry_run: bool = False
    ) -> dict[str, Any]:
    ...
    def restore_registry(self, backup_file: str) -> bool:
        ...
        try:
            ...
            current_backup = (
                f"content_registry_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def optimize_registry(self) -> dict[str, Any]:
        ...
        for file_path, metadata in self.registry.content_registry.items():
            try:
                ...
                if file_path_obj.exists():
                    ...
                    metadata.quality_score = self.registry.calculate_quality_score(
                        file_path, metadata.content_type
                    )
        ...
        if (
            optimization_report["removed_invalid"] > 0
            or optimization_report["updated_metadata"] > 0
        ):
            self.registry.manage_registry_operations("save")
