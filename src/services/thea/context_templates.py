...
CONTEXT: We need strategic guidance for system enhancement priorities and next development cycle direction."""
    ...
    def create_strategic_consultation(self, question: str, context_level: str = "standard") -> str:
        ...
        if context_level not in context_levels:
            raise ValueError(f"Invalid context level: {context_level}. Must be one of: {list(context_levels.keys())}")
        
        return context_levels[context_level](question)
    
    def create_multi_turn_context(self, question: str, previous_context: Optional[str] = None) -> str:
...
URGENT: We need immediate strategic guidance for crisis resolution."""
    ...
    def create_status_report(self, additional_status: Optional[Dict] = None) -> str:
...
CHALLENGES: {", ".join(self.project_status.current_challenges)}"""
    ...
    def optimize_for_limits(self, message: str, max_chars: int = 500) -> str:
        ...
        if "ACHIEVEMENTS:" in message:
            lines = message.split('\n')
            lines = [line for line in lines if not line.startswith(("ACHIEVEMENTS:", "CHALLENGES:"))]
            message = '\n'.join(lines)
        ...
        return self.create_essential_context(message.split("QUESTION: ")[-1] if "QUESTION: " in message else "Strategic guidance needed")
...
def create_quick_consultation(template_key: str, context_level: str = "standard") -> str:
    ...
    if template_key not in STRATEGIC_TEMPLATES:
        raise ValueError(f"Invalid template key: {template_key}. Available: {list(STRATEGIC_TEMPLATES.keys())}")
    
    context_manager = ProjectContextManager()
    return context_manager.create_strategic_consultation(
        STRATEGIC_TEMPLATES[template_key],
        context_level
    )
...
if __name__ == "__main__":
    ...
    print(f"Characters: {len(context_manager.create_essential_context('What should be our next priority?'))}")
    print()
    
    print("Standard Context:")
    print(context_manager.create_standard_context("What should be our next priority?"))
    print(f"Characters: {len(context_manager.create_standard_context('What should be our next priority?'))}")
    print()
    
    print("Detailed Context:")
    print(context_manager.create_detailed_context("What should be our next priority?"))
    print(f"Characters: {len(context_manager.create_detailed_context('What should be our next priority?'))}")
    print()
