...
class APIKeyManager:
    ...
    def _load_from_config(self, config_data: Dict[str, Any]) -> None:
        ...
        for service, config in api_keys_data.items():
            if (
                config.get("api_key")
                and config["api_key"] != "your_openai_api_key_here"
            ):
    ...
    def _load_from_environment(self) -> None:
        ...
        for service, env_vars in env_mappings.items():
            ...
            if api_key:
                config = APIKeyConfig(service_name=service, api_key=api_key)
    ...
    def _prompt_for_api_keys(self) -> None:
        ...
        for service in services:
            api_key = input(
                f"Enter {service} API key (or press Enter to skip): "
            ).strip()
            if api_key:
                self._api_keys[service] = APIKeyConfig(
                    service_name=service, api_key=api_key
                )
...
class TestAPIKeyManager:
    ...
    def test_validate_api_key_format_valid(self):
        ...
        # Valid Anthropic key
        assert (
            manager._validate_api_key_format("sk-ant-test-key-12345", "anthropic")
            is True
        )

        # Valid HuggingFace key
        assert (
            manager._validate_api_key_format("hf-test-key-12345", "huggingface") is True
        )
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            with patch.object(manager, "_load_from_environment") as mock_env_load:
                ...
                manager._api_keys["test_service"] = APIKeyConfig(
                    service_name="test_service", api_key="test-key-12345"
                )
