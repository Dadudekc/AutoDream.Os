...
class DiscordProviderService:
    ...
    def manage_provider_operations(self, action: str, **kwargs) -> Any:
        ...
        if action == "send_message":
            ...
            success = self.channel_manager.manage_channels(
                "register", agent_id=kwargs["agent_id"], channel_id=kwargs["channel_id"]
            )

            if success:
                # Also register with core provider
                self.core.manage_messaging(
                    "register_channel", agent_id=kwargs["agent_id"], channel=kwargs.get("channel")
                )
            ...
            return {**core_status, "channel_mappings": channel_status, "message_stats": log_stats}
    ...
    def cleanup_old_logs(self, days: int = 7) -> int:
        ...
        self.message_logger.message_log = [
            log
            for log in self.message_logger.message_log
