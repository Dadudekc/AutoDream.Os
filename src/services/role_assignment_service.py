...
from .role_definitions import (
    Capability, AgentRole, RoleAssignment, PermissionLevel,
    CapabilityType, RoleDefinitionManager
)
...
class RoleAssignmentService:
    ...
    def __init__(self):
        ...
        self.agent_capabilities: Dict[str, Set[str]] = {}  # agent_id -> set of capability_ids
        self.logger = logging.getLogger(f"{__name__}.RoleAssignmentService")
    
    def assign_role(self, agent_id: str, role_id: str, assigned_by: str = "system") -> bool:
    ...
    def revoke_role(self, agent_id: str, role_id: str, revoked_by: str = "system") -> bool:
        """Revoke a role from an agent"""
        assignment_id = f"{agent_id}_{role_id}"
        assignment = self.role_assignments.get(assignment_id)
        
        if not assignment:
            self.logger.warning(f"No role assignment found for agent {agent_id} and role {role_id}")
            return False
    ...
    def suspend_role(self, agent_id: str, role_id: str, suspended_by: str = "system") -> bool:
    ...
    def reactivate_role(self, agent_id: str, role_id: str, reactivated_by: str = "system") -> bool:
    ...
    def _get_agent_role_assignment(self, agent_id: str, role_id: str) -> Optional[RoleAssignment]:
    ...
    def has_capability(self, agent_id: str, capability_id: str) -> bool:
        ...
        return agent_id in self.agent_capabilities and capability_id in self.agent_capabilities[agent_id]
    
    def can_perform_action(self, agent_id: str, action: str, resource: str = None) -> bool:
...
def main():
    ...
    parser.add_argument("--assign", nargs=2, metavar=("AGENT", "ROLE"), help="Assign role to agent")
    parser.add_argument("--revoke", nargs=2, metavar=("AGENT", "ROLE"), help="Revoke role from agent")
    parser.add_argument("--suspend", nargs=2, metavar=("AGENT", "ROLE"), help="Suspend role for agent")
    parser.add_argument("--reactivate", nargs=2, metavar=("AGENT", "ROLE"), help="Reactivate role for agent")
    ...
    parser.add_argument("--check-capability", nargs=2, metavar=("AGENT", "CAPABILITY"), help="Check if agent has capability")
