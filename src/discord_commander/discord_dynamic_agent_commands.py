...
async def setup_dynamic_agent_commands(bot: commands.Bot):
    ...
    @app_commands.describe(
        agent="Agent or alias (autocomplete)", message="Urgent message to send immediately"
    )
    @app_commands.autocomplete(agent=_agent_autocomplete)
    async def urgent_cmd(interaction: discord.Interaction, agent: str, message: str):
...
*Delivered via Ctrl+Enter priority system*""",
    ...
    @app_commands.autocomplete(agent=_agent_autocomplete)
    async def agent_cmd(
        interaction: discord.Interaction, agent: str, message: str, summary: bool = True
    ):
        await interaction.response.defer(ephemeral=True, thinking=True)
        alias_to_agent, by_agent = _load_alias_map()
        ag = alias_to_agent.get(agent.lower(), agent)
