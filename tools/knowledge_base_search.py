...
class KnowledgeBaseSearch:
    ...
    def search(self, query: str, category: str | None = None) -> list[dict[str, str]]:
        ...
        sections_to_search = (
            [category] if category and category in self.sections else self.sections.keys()
        )
...
def print_search_results(results: list[dict[str, str]], query: str) -> None:
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            f"✅ Solution: {result['solution'][:200]}{'...' if len(result['solution']) > 200 else ''}"
        )
        print("-" * 40)
...
def print_quick_solutions(solutions: list[dict[str, str]]) -> None:
    ...
    for i, solution in enumerate(solutions, 1):
        print(f"\n{i}. {solution['title']}")
        print(
            f"   ❓ {solution['question'][:100]}{'...' if len(solution['question']) > 100 else ''}"
        )
        print(
            f"   ✅ {solution['solution'][:150]}{'...' if len(solution['solution']) > 150 else ''}"
        )
        print("-" * 40)
...
def main():
    ...
    parser.add_argument(
        "--quick-solutions", action="store_true", help="Show quick solutions for common problems"
    )
