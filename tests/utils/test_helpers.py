...
def create_mock_agent(agent_id: str = "test_agent", 
                     name: str = "Test Agent",
                     role: str = "testing") -> Mock:
    ...
    agent.execute_task = Mock(return_value={"success": True, "result": "Task completed"})
    agent.get_status = Mock(return_value="active")
    
    return agent


def create_test_task(task_id: str = None,
                    name: str = "Test Task", 
...
def assert_test_results(results: Dict[str, Any], 
                       expected_keys: List[str] = None,
                       min_tests: int = 1) -> None:
    ...
    if "tests_run" in results:
        assert results["tests_run"] >= min_tests, f"Expected at least {min_tests} tests to run"
...
def performance_test_wrapper(func, max_duration: float = 5.0):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            assert duration <= max_duration, f"Test took {duration:.2f}s, max allowed: {max_duration}s"
...
def create_mock_database():
    ...
    def mock_delete(table, criteria):
        ...
        db._data[table] = [
            item for item in db._data[table]
            if not all(
...
def create_mock_file_system():
    ...
    def mock_list_files(directory):
        return [
            path for path in fs._files.keys()
