...
class GitHubAgentController:
    ...
    def create_pull_request(self, owner: str, repo: str, title: str, 
        ...
        return GitHubPullRequest(
            number=pr_data["number"], title=pr_data["title"], body=pr_data["body"] or "",
            state=pr_data["state"], head_branch=pr_data["head"]["ref"], base_branch=pr_data["base"]["ref"],
            created_at=pr_data["created_at"], updated_at=pr_data["updated_at"],
            html_url=pr_data["html_url"], user=pr_data["user"]["login"],
    ...
    def merge_pull_request(self, owner: str, repo: str, pr_number: int, 
        ...
        try:
            ...
            response = self._make_request("PUT", f"/repos/{owner}/{repo}/pulls/{pr_number}/merge", json=data)
            return response.status_code == 200
        except requests.exceptions.RequestException:
            return False
    
    # File Operations
    def _file_operation(self, owner: str, repo: str, path: str, method: str, data: Dict[str, Any]) -> bool:
    ...
    def create_file(self, owner: str, repo: str, path: str, 
        ...
        data = {"message": message, "content": content.encode('utf-8').decode('base64'), "branch": branch}
    ...
    def update_file(self, owner: str, repo: str, path: str, 
        ...
        data = {"message": message, "content": content.encode('utf-8').decode('base64'), "sha": sha, "branch": branch}
        return self._file_operation(owner, repo, path, "PUT", data)
    
    def delete_file(self, owner: str, repo: str, path: str, 
                   message: str, sha: str, branch: str = None) -> bool:
    ...
    def search_repositories(self, query: str, sort: str = "stars") -> List[GitHubRepository]:
        ...
        return [self._create_repository_from_data(repo_data) for repo_data in response.json()["items"]]
