...
class TheaResponseService:
    ...
    def wait_for_response(self, timeout: int = None) -> bool:
        ...
        if not self.browser.driver:
            logger.info("⚠️  No browser driver available for response detection")
            return False
        logger.info("🔍 Using automated DOM polling to detect when Thea is done...")
        ...
        if result == ResponseWaitResult.COMPLETE:
            ...
            logger.info("⚠️ No assistant turn detected; switching to manual confirmation.")
            return self._wait_for_response_manual()
