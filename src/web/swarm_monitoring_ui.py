...
from .swarm_monitoring_core import SwarmMonitoringCore, AgentStatus, Alert, SystemMetrics
...
class SwarmMonitoringUI:
    ...
    def _setup_routes(self) -> None:
        ...
        async def update_agent_status(agent_id: str, status_data: Dict[str, Any]):
            ...
            if success:
                await self._broadcast_update("agent_status", {
                    "agent_id": agent_id,
                    "status": status_data
                })
                return {"success": True}
        ...
        async def get_metrics_history():
            """Get metrics history."""
            return {
                "metrics": [metric.dict() for metric in self.core.metrics_history[-100:]]
            }
