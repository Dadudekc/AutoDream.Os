...
from .fsm_registry import AgentState, read_agent_state, update_agent_state
    ...
    def _save_activity_data(self):
        ...
        try:
            ...
            self.activity_file.write_text(
                json.dumps(self.activity_data, indent=2), encoding="utf-8"
            )
    ...
    def check_messaging_inactivity(self, agent_id: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if time_since_activity > timedelta(minutes=MESSAGING_TIMEOUT_MINUTES):
                return (
                    True,
                    f"messaging_inactive_{time_since_activity.total_seconds() / 60:.1f}_minutes",
                )
        ...
        if workspace_path.exists():
            for agent_dir in workspace_path.iterdir():
                if agent_dir.is_dir() and agent_dir.name.startswith("Agent-"):
                    ...
                    is_messaging_inactive, messaging_reason = self.check_messaging_inactivity(
                        agent_id
                    )
                    current_state = read_agent_state(agent_id)
                    ...
                    summary[agent_id] = {
                        ...
                        "activity_count": self.activity_data.get(agent_id, {}).get(
                            "activity_count", 0
                        ),
                    }
