...
class ViolationDetector:
    ...
    def ci_gate_check(self, results: dict[str, Any]) -> tuple[bool, str]:
        ...
        if summary["error_files"] > 0:
            return False, f"CI gate failed: {summary['error_files']} files have error-level violations"
        
        # CI gate fails if compliance rate is below 90%
        if summary["compliance_rate"] < 90:
            return False, f"CI gate failed: compliance rate {summary['compliance_rate']:.1f}% below 90% threshold"
...
def format_violations_text(results: dict[str, Any]) -> str:
    ...
    for file_result in results["files"]:
        if file_result["violations"]:
            output.append(f"  {file_result['file']} ({file_result['lines']} lines) - {file_result['status']}")
            for violation in file_result["violations"]:
