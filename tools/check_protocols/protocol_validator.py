...
class ProtocolValidator:
    ...
    async def _validate_structural_layer(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.web_dir.exists():
                results["directories_validated"] = len(
                    [d for d in self.web_dir.rglob("*") if d.is_dir()]
                )
                results["files_validated"] = len(
                    [f for f in self.web_dir.rglob("*.py") if f.is_file()]
                )
    ...
    def _calculate_overall_score(
        self, layer_1: dict[str, Any], layer_2: dict[str, Any], layer_3: dict[str, Any]
    ) -> float:
        """Calculate overall validation score."""
        scores = [layer_1.get("score", 0.0), layer_2.get("score", 0.0), layer_3.get("score", 0.0)]
