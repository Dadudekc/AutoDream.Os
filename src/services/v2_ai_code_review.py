...
class V2AICodeReviewService:
    ...
    def review_project(self, project_path: str, focus_areas: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            valid_focus_areas = [area for area in focus_areas if area in self.focus_areas]
    ...
    def _create_review_task(self, file_path: str, focus_area: str) -> Optional[CodeReviewTask]:
    ...
    def _execute_review_workflow(self, focus_area: str, code_files: List[Path]) -> Optional[str]:
        ...
        try:
            ...
            for step in template["steps"]:
                ...
                if "{file_path}" in step["prompt_template"]:
                    for file_path in code_files:
                        customized_step["prompt_template"] = step["prompt_template"].replace(
            ...
            if workflow_id:
                ...
                if success:
                    self.logger.info(f"Executed {focus_area} review workflow: {workflow_id}")
                    return workflow_id
    ...
    def get_review_results(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if workflow_status["status"] != "completed":
                return {"status": "workflow_not_completed", "current_status": workflow_status["status"]}
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completed_reviews": len([t for t in self.review_tasks.values() if t.status == "completed"]),
            "pending_reviews": len([t for t in self.review_tasks.values() if t.status == "pending"]),
...
def main():
    ...
    parser.add_argument("--focus-areas", nargs="+", default=["code_quality"], 
