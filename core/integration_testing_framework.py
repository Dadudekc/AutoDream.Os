...
class BroadcastIntegrationTester:
    ...
    def test_coordinate_loading(self) -> bool:
        ...
        using_defaults = any(
            coords.get('x') in [400, 1200, 2000, 2800] and 
            coords.get('y') in [300, 900] 
    ...
    def test_message_queuing(self) -> bool:
        ...
        try:
            # Test single message
            msg_id = self.system_instance.send_message("system", "agent_1", "Test message", "normal")
            if not msg_id:
                return False
            
            # Test broadcast
            broadcast_ids = self.system_instance.send_message_to_all_agents("system", "Broadcast test", "normal")
            if not broadcast_ids or len(broadcast_ids) != 8:
                return False
            
            # Test high priority
            high_id = self.system_instance.send_high_priority_message("system", "agent_1", "Urgent test")
            if not high_id:
                return False
    ...
    def test_broadcast_functionality(self) -> bool:
        ...
        try:
            ...
            message_ids = self.system_instance.send_message_to_all_agents("system", test_content, "normal")
    ...
    def test_system_lifecycle(self) -> bool:
        ...
        try:
            ...
            # Coordinate reload
            reload_success = self.system_instance.reload_calibrated_coordinates()
            if not reload_success:
                print("   ❌ Coordinate reload failed")
                return False
            
            print("   ✅ Coordinate reload successful")
            return True
