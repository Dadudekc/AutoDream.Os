...
class UnifiedStatusTrackingSystem:
    ...
    def _update_system_status(self):
        ...
        active_count = sum(
            1 for status in self.agent_statuses.values() if status["status"] == "ACTIVE"
        )

        self.system_status.update(
            {"active_agents": active_count, "last_sync": datetime.now().isoformat()}
        )
                ...
                json.dump(
                    {
                        ...
                        "last_consolidation": self.consolidation_log[-1]
...
def main():
    ...
    # Test status update
    update_result = unified_status.update_agent_status(
        "Agent-8", "ACTIVE", {"mission": "system_consolidation"}
    )
