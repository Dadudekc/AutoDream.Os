...
class AntiSlopProtocol:
    ...
    def check_content_quality(self, content: str, agent_id: str) -> dict[str, Any]:
        ...
        if len(content) > self.max_content_per_file:
            result["reasons"].append(
                f"Content too large: {len(content)} chars (max: {self.max_content_per_file})"
            )
        ...
        if repetition_ratio > self.max_repetition_ratio:
            result["reasons"].append(
                f"Too much repetition: {repetition_ratio:.2%} (max: {self.max_repetition_ratio:.2%})"
            )
        ...
        if unique_ratio < self.unique_content_threshold:
            result["reasons"].append(
                f"Insufficient unique content: {unique_ratio:.2%} (min: {self.unique_content_threshold:.2%})"
            )
    ...
    def _register_content(self, content: str, agent_id: str, content_hash: str):
        ...
        if len(self.content_registry["generation_history"]) > 1000:
            self.content_registry["generation_history"] = self.content_registry[
                "generation_history"
            ][-1000:]
    ...
    def cleanup_old_content(self, days_old: int = 7):
        ...
        self.content_registry["generation_history"] = [
            entry
            for entry in self.content_registry["generation_history"]
...
def main():
    ...
    test_content = (
        "This is a test content for quality checking. It should be unique and meaningful."
    )
