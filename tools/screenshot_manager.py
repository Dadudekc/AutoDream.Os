...
class ScreenshotManager:
    ...
    def configure_trigger(self, trigger_type: str, cycle_interval: int, enabled: bool = True) -> bool:
    ...
    def _load_config(self):
        ...
        try:
            if self.config_file.exists():
                ...
                logger.info(f"Loaded config: cycle {self.current_cycle}, {len(self.triggers)} triggers")
            else:
...
def main():
    ...
    parser.add_argument("--check", nargs=2, metavar=("CYCLE_TYPE", "EVENT_TYPE"), 
                       help="Check if screenshot should be taken")
    parser.add_argument("--increment", action="store_true", help="Increment cycle counter")
    parser.add_argument("--configure", nargs=3, metavar=("TYPE", "INTERVAL", "ENABLED"),
