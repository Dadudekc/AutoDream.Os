...
class AgentMessagingView(discord.ui.View):
    ...
    def __init__(self, messaging_service: ConsolidatedMessagingService):
        ...
        self.agent_select = discord.ui.Select(
            placeholder="Select an agent to message...",
    ...
    async def _create_status_embed(self) -> discord.Embed:
        ...
        try:
            if hasattr(self.messaging_service, 'agent_data'):
                ...
                embed.add_field(
                    name="Summary",
                    value=f"Active: {active_count}/{total_count}",
                    inline=False
                )
        except Exception as e:
            embed.add_field(
                name="Error",
                value=f"Failed to load agent data: {str(e)}",
                inline=False
            )
...
class DiscordMessagingController:
    ...
    async def send_agent_message(self, agent_id: str, message: str, priority: str = "NORMAL") -> bool:
