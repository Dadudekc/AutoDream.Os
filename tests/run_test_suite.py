...
class TestSuiteRunner:
    ...
    def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            print(
                f"{Fore.RED}❌ pytest not available - install with: pip install -r requirements_testing.txt{Style.RESET_ALL}"
            )
            return False
    ...
    def save_results(self, output_file: Optional[Path] = None) -> Path:
        ...
        if output_file is None:
            ...
            output_file = (
                self.repo_root / "test_results" / f"test_suite_results_{timestamp}.json"
            )
