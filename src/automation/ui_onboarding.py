...
class UIOnboarder:
    ...
    def perform(self, agent_id: str, coords: tuple[int, int], message: str) -> bool:
        ...
        """
        if self.dry_run:
            logger.info(f"🧪 DRY-RUN: Would onboard {agent_id} at {coords} with message")
            return True
        try:
            x, y = coords
            ...
            for attempt in range(self.retries + 1):
                ...
                if (
                    ...
                    if PYPERCLIP_AVAILABLE and pyperclip:
                        ...
                        logger.info(f"⚠️ Pyperclip not available, typing message for {agent_id}")
                    ...
                    self.pyautogui.moveTo(x, y)
                    time.sleep(0.2)
            logger.info(f"❌ Failed to position mouse at coordinates for {agent_id}")
            return False
    ...
    def _validate_coordinates(self, x: int, y: int) -> bool:
        ...
        if x > 5000 or y > 5000:
            return False
        if x == 0 and y == 0:
            logger.info("⚠️  Using default coordinates (0,0) - may not be accurate")
            return True
        return True
    ...
    def _validate_message_format(self, agent_id: str, message: str) -> bool:
        ...
        if not message.startswith(f"YOU ARE {agent_id}"):
            ...
            return False
        if "[S2A]" in message:
            logger.info("❌ Message contains [S2A] format - should use 'YOU ARE AGENT X' format")
            return False
