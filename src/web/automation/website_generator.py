...
class WebsiteGenerator:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.logger.info(f"Website Generator initialized. Project root: {self.project_root}")
    ...
    def _setup_jinja(self) -> Environment:
        ...
        env.filters['slugify'] = lambda text: text.lower().replace(' ', '-').replace('_', '-')
        
        return env
    
    def generate_website(self, config: WebsiteConfig, pages: List[PageConfig], 
                        components: List[ComponentConfig] = None) -> Path:
    ...
    def _generate_components(self, website_dir: Path, components: List[ComponentConfig]):
        ...
        for component in components:
            try:
                ...
                self.logger.warning(f"Failed to generate component {component.name}: {e}")
    
    def _generate_pages(self, website_dir: Path, config: WebsiteConfig, 
                       pages: List[PageConfig], components: List[ComponentConfig] = None):
...
"""
    ...
    def _generate_config_files(self, website_dir: Path, config: WebsiteConfig, 
                             pages: List[PageConfig], components: List[ComponentConfig] = None):
    ...
    def _generate_sitemap(self, website_dir: Path, config: WebsiteConfig, pages: List[PageConfig]):
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    def generate_from_template(self, template_name: str, config: WebsiteConfig, 
                             custom_data: Dict[str, Any] = None) -> Path:
    ...
    def create_basic_website(self, name: str, title: str, description: str) -> Path:
        ...
        pages = [
            ...
            PageConfig(
                ...
                template="base/responsive_base.html",
                route="/about",
                content={"heading": "About Us", "description": "Learn more about our company"}
            ),
            PageConfig(
                name="contact",
                ...
                template="base/responsive_base.html",
                route="/contact",
                content={"heading": "Contact Us", "description": "Get in touch with us"}
            )
        ]
...
def generate_basic_website(name: str, title: str, description: str, 
