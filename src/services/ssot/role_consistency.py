...
class RoleConsistencyEnforcer:
    ...
    def enforce_all_roles(self) -> dict[str, Any]:
        ...
        doc_files = [self.project_root / "AGENT_ONBOARDING_CONTEXT_PACKAGE.md"]
        ...
        logger.info(
            f"Role enforcement complete. Made {len(results['role_corrections'])} corrections"
        )
        return results
            ...
            if "agents" in config:
                for agent_id, agent_config in config["agents"].items():
                    if isinstance(agent_config, dict) and "role" in agent_config:
                        ...
                        if expected_role and current_role != expected_role:
                            violations.append(
                                f"{agent_id}: Expected {expected_role}, got {current_role}"
                            )
...
def main():
    ...
    print("Role enforcement complete:")
