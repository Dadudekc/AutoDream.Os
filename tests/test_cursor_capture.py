...
def test_database_manager():
    ...
    try:
        ...
        assert recent_messages[0]["content"] == "Test assistant message", "Content should match"
...
def test_message_normalizer():
    ...
    for i, test_case in enumerate(test_cases):
        try:
            ...
            assert message.role in ["user", "assistant", "system", "tool"], f"Invalid role: {message.role}"
...
def test_cursor_capture_system():
    ...
    try:
        ...
        with patch('src.core.cursor_response_capture.CursorCDPBridge', MockCursorCDPBridge):
            ...
            assert stats["total_messages_captured"] > 0, "Should track captured messages"
...
def test_v2_integration():
    ...
    try:
        ...
        assert capture_system.performance_profiler.is_active, "Performance profiler should be active"
        assert capture_system.health_monitor.is_active, "Health monitor should be active"
        ...
        assert not capture_system.performance_profiler.is_active, "Performance profiler should be stopped"
        assert not capture_system.health_monitor.is_active, "Health monitor should be stopped"
        print("  ✅ V2 monitoring stop: PASSED")
        
        # Test performance profiling integration
        @capture_system.performance_profiler.profile_operation("test_operation", "cursor_capture")
        def test_operation():
            time.sleep(0.1)
            return "test result"
...
def main():
    ...
    if all_passed:
        print("🎉 ALL TESTS PASSED! Cursor Response Capture System is fully operational.")
        print("🚀 Ready for production deployment with V2 integration.")
        print("\n📋 NEXT STEPS:")
        print("   1. Install required dependencies: pip install websocket-client requests")
