...
class Retriever:
    ...
    def how_do_agents_do(
        self, topic: str, k: int = 10, *, project: str = None, agent_id: str = None
    ) -> list[dict[str, Any]]:
        """
        ...
        return self.search(
            query, k=k, project=project, kinds=["action", "protocol", "workflow"], agent_id=agent_id
        )

    def find_similar_problems(
        self, problem_description: str, k: int = 10, *, project: str = None
    ) -> list[dict[str, Any]]:
        """
    ...
    def get_agent_expertise(self, agent_id: str, k: int = 20) -> dict[str, Any]:
        ...
        try:
            ...
            successful_actions = self.search(
                f"successful actions by {agent_id}", k=k, agent_id=agent_id, kinds=["action"]
            )

            # Get agent's protocols
            protocols = self.search(
                f"protocols by {agent_id}", k=k, agent_id=agent_id, kinds=["protocol"]
            )

            # Get agent's workflows
            workflows = self.search(
                f"workflows by {agent_id}", k=k, agent_id=agent_id, kinds=["workflow"]
            )
    ...
    def get_project_patterns(self, project: str, k: int = 20) -> dict[str, Any]:
        ...
        try:
            ...
            successful_patterns = self.search(
                f"successful patterns in {project}",
                k=k,
                project=project,
            ...
            # Get protocols used
            protocols = self.search(
                f"protocols used in {project}", k=k, project=project, kinds=["protocol"]
            )
