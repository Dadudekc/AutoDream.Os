...
class ConfigurationBackupManager:
    ...
    def __init__(self, config_dir: str='config', backup_dir: str=
        'config/backup'):
    ...
    def create_backup(self) ->Dict[str, Any]:
        ...
        backup_info = {'timestamp': self.timestamp, 'backup_path': str(self
            .backup_path), 'files_backed_up': [], 'backup_size': 0,
            'status': 'success'}
        try:
            ...
            logger.info(f'✅ Configuration backup created: {self.backup_path}')
            logger.info(
                f"📁 Files backed up: {len(backup_info['files_backed_up'])}")
            logger.info(f"💾 Backup size: {backup_info['backup_size']} bytes")
    ...
    def restore_backup(self, backup_timestamp: str) ->Dict[str, Any]:
        ...
        restore_info = {'timestamp': backup_timestamp, 'restore_path': str(
            self.backup_dir / backup_timestamp), 'files_restored': [],
            'status': 'success'}
        try:
            ...
            for file_path in backup_path.iterdir():
                if file_path.is_file(
                    ) and file_path.name != 'backup_manifest.json':
                    target_path = self.config_dir / file_path.name
                    shutil.copy2(file_path, target_path)
            logger.info(
                f'✅ Configuration restored from backup: {backup_timestamp}')
            logger.info(
                f"📁 Files restored: {len(restore_info['files_restored'])}")
        except Exception as e:
    ...
    def list_backups(self) ->List[Dict[str, Any]]:
        ...
        for backup_path in self.backup_dir.iterdir():
            if backup_path.is_dir():
                manifest_path = backup_path / 'backup_manifest.json'
                backup_info = {'timestamp': backup_path.name, 'path': str(
                    backup_path), 'created': datetime.fromtimestamp(
                    backup_path.stat().st_mtime).isoformat()}
                if manifest_path.exists():
                    with open(manifest_path, 'r') as f:
                        manifest = json.load(f)
...
def main():
    ...
    parser.add_argument('--backup-dir', default='config/backup', help=
        'Backup directory')
    args = parser.parse_args()
    backup_manager = ConfigurationBackupManager(args.config_dir, args.
        backup_dir)
    if args.action == 'backup':
        ...
        if not args.timestamp:
            logger.info('❌ Timestamp required for restore')
            return
