...
class ErrorAnalyticsSystem:
    ...
    def __init__(
        ...
        self.prediction_confidence_threshold = self.config.get(
    ...
    def _analyze_error_trends(self):
        ...
        try:
            ...
            for hour_key, period_data in time_periods.items():
                recovery_rate = (
                    (
                        period_data["recovery_successes"]
                        / period_data["total_errors"]
                        * 100
                    )
            ...
            for i, service1 in enumerate(services):
                for service2 in services[i + 1 :]:
                    correlation = self._calculate_service_correlation(
                        service_errors[service1], service_errors[service2]
                    )
    ...
    def _calculate_service_correlation(
        self, errors1: List[ErrorInfo], errors2: List[ErrorInfo]
    ) -> float:
    ...
    def _analyze_error_impacts(self):
        ...
        try:
            ...
            self.impacts = sorted(
                new_impacts, key=lambda x: x.business_impact_score, reverse=True
            )
    ...
    def _generate_predictive_insights(self):
        ...
        try:
                    ...
                    if recent_avg > earlier_avg * 1.5:  # 50% increase
                        prediction = PredictiveInsight(
                            ...
                            evidence=[
                                ...
                                f"Earlier average: {earlier_avg:.1f}",
                            ],
                        )
            ...
            if self.correlations:
                high_correlation = max(
                    self.correlations, key=lambda x: x.correlation_strength
                )
                ...
                if high_correlation.correlation_strength > 0.8:
                    prediction = PredictiveInsight(
                        ...
                        evidence=[
                            f"Correlation strength: {high_correlation.correlation_strength:.2f}"
                        ],
                    )
                    new_predictions.append(prediction)
            ...
            if self.trends:
                ...
                recovery_rates = [
                    trend.recovery_success_rate for trend in recent_trends
                ]
                ...
                if len(recovery_rates) >= 2:
                    ...
                    if avg_recovery_rate < 70:  # Low recovery rate
                        prediction = PredictiveInsight(
                            ...
                            evidence=[
                                f"Average recovery rate: {avg_recovery_rate:.1f}%"
                            ],
                        )
                        new_predictions.append(prediction)
    ...
    def generate_analytics_report(
        self, time_range: str = "24h", format_type: ReportFormat = ReportFormat.MARKDOWN
    ) -> AnalyticsReport:
        ...
        try:
            ...
            summary = {
                ...
                "high_impact_services": len(
                    [i for i in self.impacts if i.business_impact_score > 50]
                ),
    ...
    def _calculate_system_health_score(self) -> float:
        ...
        try:
            ...
            high_impact_count = len(
                [i for i in self.impacts if i.business_impact_score > 70]
            )
        ...
        try:
            ...
            if error_stats["recovery_success_rate"] < 80:
                recommendations.append(
                    "Improve auto-recovery mechanisms and implement better fallback strategies"
                )
            ...
            if high_frequency_patterns:
                recommendations.append(
                    f"Investigate and resolve {len(high_frequency_patterns)} high-frequency error patterns"
                )

            # Service dependency recommendations
            if self.correlations:
                recommendations.append(
                    "Implement circuit breakers and service isolation to prevent cascading failures"
                )

            # Impact-based recommendations
            critical_services = [
                i for i in self.impacts if i.business_impact_score > 80
            ]
            if critical_services:
                recommendations.append(
                    f"Prioritize fixes for {len(critical_services)} critical services with high business impact"
                )

            # Predictive recommendations
            if self.predictions:
                recommendations.append(
                    "Act on predictive insights to prevent potential issues before they occur"
                )

            # General recommendations
            if not recommendations:
                recommendations.append(
                    "System is performing well - continue monitoring and preventive maintenance"
                )
    ...
    def _output_console_report(self, report: AnalyticsReport):
        ...
        if report.trends:
            ...
            for trend in report.trends[-6:]:
                print(
                    f"  {trend.time_period}: {trend.error_count} errors, "
        ...
        try:
            ...
            with open(report_path, "w") as f:
                ...
                if report.trends:
                    ...
                    for trend in report.trends[-6:]:
                        f.write(
                            f"| {trend.time_period} | {trend.error_count} | {trend.recovery_success_rate:.1f}% |\n"
                        )
                    f.write("\n")
                ...
                if report.patterns:
                    ...
                    for pattern in report.patterns[:10]:
                        ...
                        f.write(
                            f"- **Affected Services:** {', '.join(pattern.affected_services)}\n"
                        )
                        f.write(
                            f"- **Suggested Resolution:** {pattern.suggested_resolution}\n\n"
                        )
            ...
            with open(report_path, "w") as f:
                ...
                if report.patterns:
                    ...
                    for pattern in report.patterns:
                        f.write(
                            f"{pattern.error_signature},{pattern.occurrences},{pattern.first_seen},{pattern.last_seen}\n"
                        )
