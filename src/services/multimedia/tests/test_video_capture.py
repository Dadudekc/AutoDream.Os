...
class TestVideoCaptureService:
    ...
    def test_start_screen_capture_success(self, video_service):
        ...
        with patch('PIL.ImageGrab') as mock_pil, \
    ...
    def test_stop_capture_all_devices(self, video_service):
        ...
        mock_cap1 = Mock()
        mock_cap2 = Mock()
        video_service.capture_threads[0] = {'active': True, 'capture': mock_cap1, 'thread': Mock()}
        video_service.capture_threads[1] = {'active': True, 'capture': mock_cap2, 'thread': Mock()}
    ...
    def test_webcam_capture_loop_with_callback(self, video_service):
        ...
        mock_cap = Mock()
        mock_cap.read.side_effect = [(True, np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8))]
    ...
    def test_screen_capture_loop(self, video_service):
        ...
        with patch('PIL.ImageGrab.grab') as mock_grab, \
            ...
            mock_array.return_value = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
            mock_cvt.return_value = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
