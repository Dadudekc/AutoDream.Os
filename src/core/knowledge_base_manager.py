...
class KnowledgeBaseManager:
    ...
    def search_knowledge(self, query: str, search_type: str = "all") -> Dict[str, List[Any]]:
        ...
        if search_type in ["all", "principles"]:
            for principle in self.core.design_principles.values():
                if (query_lower in principle.name.lower() or
                    ...
                    query_lower in principle.rationale.lower()):
        ...
        if search_type in ["all", "anti_patterns"]:
            for anti_pattern in self.core.anti_patterns.values():
                if (query_lower in anti_pattern.name.lower() or
                    query_lower in anti_pattern.description.lower()):
    ...
    def filter_by_severity(self, severity: str) -> List[AntiPattern]:
        ...
        return [
            anti_pattern for anti_pattern in self.core.anti_patterns.values()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "patterns_by_complexity": {
                f"complexity_{i}": len([
                    p for p in self.core.code_patterns.values()
                    if p.complexity_score == i
                ])
                for i in range(1, 11)
            },
    ...
    def import_knowledge(self, import_data: Dict[str, Any]) -> Dict[str, int]:
        ...
        if "patterns" in import_data:
            patterns_data = [
                pattern_data for pattern_data in import_data["patterns"].values()
            ]
