...
class AudioProcessingService:
    ...
    def start_audio_capture(self, device_id: int = 0, callback: Optional[Callable] = None) -> bool:
        """
        ...
        try:
            ...
            self.audio_queues[device_id] = queue.Queue(maxsize=self.audio_config['buffer_size'])
    ...
    def _audio_capture_loop(self, stream, device_id: int, callback: Optional[Callable], audio):
        ...
        try:
            while self.is_processing and self.audio_threads[device_id]['active']:
                try:
                    ...
                    if len(self.audio_buffers[device_id]) > self.audio_config['buffer_size']:
    ...
    def _audio_playback_loop(self, stream, audio_data: np.ndarray, audio):
        ...
        try:
            ...
            chunk_size = self.audio_config['chunk_size'] * self.audio_config['channels'] * 2  # 2 bytes per sample
    ...
    def apply_audio_filter(self, audio_data: np.ndarray, filter_type: str, **kwargs) -> np.ndarray:
        """
        ...
        try:
            if filter_type == 'normalize':
                ...
                b, a = signal.butter(4, cutoff / (self.audio_config['sample_rate'] / 2), 'low')
                ...
                cutoff = kwargs.get('cutoff', 1000)
                from scipy import signal
                b, a = signal.butter(4, cutoff / (self.audio_config['sample_rate'] / 2), 'high')
                return signal.filtfilt(b, a, audio_data)
                
            elif filter_type == 'echo':
                # Simple echo effect
                delay_samples = kwargs.get('delay', int(self.audio_config['sample_rate'] * 0.1))
