...
class SessionPersistence:
    ...
    def get_session_json(self, session_id: str) -> Optional[ChatSession]:
    ...
    def get_session_sqlite(self, session_id: str) -> Optional[ChatSession]:
    ...
    def get_session_memory(self, session_id: str) -> Optional[ChatSession]:
    ...
    def save_message_sqlite(self, message: ChatMessage):
        ...
        self.conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (
            message.id, message.session_id, message.sender,
            message.recipient, message.content, message.timestamp
        ))
        self.conn.commit()
    ...
    def get_messages_sqlite(self, session_id: str, limit: int) -> List[ChatMessage]:
        ...
        for row in cursor.fetchall():
            messages.append(ChatMessage(
                id=row[0], session_id=row[1], sender=row[2],
                recipient=row[3], content=row[4], timestamp=row[5]
            ))
        return messages
    ...
    def cleanup_json_sessions(self, cutoff_time: float):
        ...
        # Remove old sessions
        old_sessions = [
            sid for sid, session in sessions_data.items()
