...
class ContractIntegration:
    ...
    def get_role_contract(self, role: str) -> Optional[RoleContract]:
    ...
    def validate_role_contract(self, role: str) -> bool:
        ...
        required_phases = ["CHECK_INBOX", "EVALUATE_TASKS", "EXECUTE_ROLE", "QUALITY_GATES", "CYCLE_DONE"]
...
def main():
    ...
    for role, contract in integration.role_contracts.items():
        ...
        print(f"  {role}: {summary['phases']} phases, {summary['requirements']} requirements, Valid: {summary['is_valid']}")
