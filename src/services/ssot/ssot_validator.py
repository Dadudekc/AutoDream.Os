            ...
            for agent_id in json_config["agents"]:
                if agent_id in yaml_config["agents"]:
                    ...
                    if json_role != yaml_role:
                        consistency_issues.append(f"Role mismatch for {agent_id}: JSON={json_role}, YAML={yaml_role}")
        ...
        for doc_file in docs_to_check:
            ...
            if doc_path.exists():
                ...
                if "Agent-6 (SSOT_MANAGER)" in content:
                    consistency_issues.append(f"{doc_file}: Agent-6 incorrectly listed as SSOT_MANAGER")
