...
class AgentStallPreventionService:
    ...
    def __init__(self):
        ...
        self.captain_status_file = self.project_root / "persistent_data" / "captain_status.json"
        ...
        self.response_timeout = self.config.get("response_timeout_minutes", 2)  # 2 minutes
        self.max_stall_duration = self.config.get("max_stall_duration_minutes", 10)  # 10 minutes
    ...
    def _check_captain_status(self):
        ...
        try:
            ...
            if time_since_activity > self.max_stall_duration:
                log.warning(f"⚠️ Captain inactive for {time_since_activity:.1f} minutes - potential stall detected")
                self._handle_captain_stall()
            else:
                log.info(f"✅ Captain active {time_since_activity:.1f} minutes ago - status normal")
...
CAPTAIN-5 - Please acknowledge this status check."""
            ...
            success = coordinator.send_message_to_agent(captain_id, status_message, high_priority=True)
    ...
    def _start_response_timer(self):
        ...
        def response_timer():
            ...
            if not self._captain_responded():
                log.warning(f"⚠️ Captain did not respond within {self.response_timeout} minutes")
                self._handle_captain_stall()
...
AUTOMATIC RECOVERY SYSTEM - EMERGENCY MODE"""
            ...
            results = coordinator.send_message_to_all_agents(recovery_message, high_priority=True)
            
            success_count = sum(results.values())
            log.info(f"✅ Automatic recovery messages sent: {success_count}/{len(results)} successful")
