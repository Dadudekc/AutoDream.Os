...
class VSCodeQualityGates:
    ...
    def check_file(self, file_path: str) -> VSCodeQualityResult:
        ...
        try:
            ...
            enums = len([node for node in ast.walk(tree) if isinstance(node, ast.ClassDef) and self._is_enum_class(node)])
            classes = len([node for node in ast.walk(tree) if isinstance(node, ast.ClassDef) and not self._is_enum_class(node)])
            ...
            # Calculate score
            score = self._calculate_score(line_count, enums, classes, functions, len(violations), len(vscode_issues))
    ...
    def _check_v2_violations(self, line_count: int, enums: int, classes: int, functions: int, tree: ast.AST) -> List[str]:
        """Check V2 compliance violations."""
        violations = []
        
        if line_count > self.v2_limits["max_lines"]:
            violations.append(f"File size violation: {line_count} lines (limit: {self.v2_limits['max_lines']})")
        ...
        if classes > self.v2_limits["max_classes"]:
            violations.append(f"Too many classes: {classes} (limit: {self.v2_limits['max_classes']})")
        
        if functions > self.v2_limits["max_functions"]:
            violations.append(f"Too many functions: {functions} (limit: {self.v2_limits['max_functions']})")
        
        # Check function complexity and parameters
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if len(node.args.args) > self.v2_limits["max_parameters"]:
                    violations.append(f"Too many parameters in {node.name}: {len(node.args.args)} (limit: {self.v2_limits['max_parameters']})")
                
                complexity = self._calculate_complexity(node)
                if complexity > self.v2_limits["max_complexity"]:
                    violations.append(f"Function {node.name} too complex: {complexity} (limit: {self.v2_limits['max_complexity']})")
    ...
    def _generate_recommendations(self, violations: List[str], vscode_issues: List[str]) -> List[str]:
        ...
        if any("Forbidden pattern" in i for i in vscode_issues):
            recommendations.append("Remove forbidden patterns and use VSCode-compatible alternatives")
    ...
    def _calculate_score(self, line_count: int, enums: int, classes: int, functions: int, violation_count: int, issue_count: int) -> int:
