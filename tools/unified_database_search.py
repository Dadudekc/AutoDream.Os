...
class UnifiedDatabaseSearch:
    ...
    def _load_project_analysis(self) -> dict:
        ...
        if self.project_analysis_file.exists():
            with open(self.project_analysis_file) as f:
                return json.load(f)
        return {}

    def search_all(self, query: str, limit: int = 20) -> dict[str, list[dict]]:
        """Search across all databases."""
        results = {"swarm_brain": [], "vector_database": [], "devlogs": [], "project_analysis": []}
        ...
        for vector in self.vector_db:
            ...
            if (
                ...
                or query_lower in metadata.get("agent_id", "").lower()
                or query_lower in metadata.get("kind", "").lower()
            ):
        ...
        for devlog in self.devlog_db:
            if (
                query_lower in devlog.get("action", "").lower()
                or query_lower in devlog.get("details", "").lower()
                or query_lower in devlog.get("agent_id", "").lower()
            ):
    ...
    def search_by_agent(self, agent_id: str, limit: int = 10) -> dict[str, list[dict]]:
        ...
        results = {"swarm_brain": [], "vector_database": [], "devlogs": []}
    ...
    def get_database_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "devlogs": {"total_entries": len(self.devlog_db), "agents": set(), "statuses": set()},
            "project_analysis": {
                "files_analyzed": self.project_data.get("analysis", {})
                .get("project_structure", {})
                .get("total_files", 0),
                "python_files": self.project_data.get("analysis", {})
                .get("project_structure", {})
                .get("python_files", 0),
                "v2_compliant": self.project_data.get("analysis", {})
                .get("python_analysis", {})
                .get("v2_compliant_files", 0),
            },
        }
        ...
        for db_name, entries in results.items():
            ...
            for entry in entries[:5]:  # Show first 5 results
                if db_name == "swarm_brain":
                    ...
                    print(
                        f"     Agent: {entry.get('agent_id', 'Unknown')} | Type: {entry.get('kind', 'Unknown')}"
                    )
                elif db_name == "vector_database":
                    metadata = entry.get("metadata", {})
                    print(f"  🧠 {entry.get('content', 'No content')[:60]}...")
                    print(
                        f"     Agent: {metadata.get('agent_id', 'Unknown')} | Source: {metadata.get('source', 'Unknown')}"
                    )
                elif db_name == "devlogs":
                    print(f"  📋 {entry.get('action', 'No action')[:60]}...")
                    print(
                        f"     Agent: {entry.get('agent_id', 'Unknown')} | Status: {entry.get('status', 'Unknown')}"
                    )
                elif db_name == "project_analysis":
                    print("  📊 Project Analysis Data Found")
                    print(
                        f"     Files: {entry.get('analysis', {}).get('project_structure', {}).get('total_files', 0)}"
                    )
