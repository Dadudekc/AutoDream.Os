...
class PerformanceAnalyzer:
    ...
    def calculate_statistics(
        ...
        if not get_unified_validator().validate_required(metrics):
            return {}
    ...
    def analyze_response_times(
        self, operation_name: str, time_window: timedelta = timedelta(hours=1)
    ) -> Dict[str, Any]:
    ...
    def analyze_throughput(
        ...
        for metric in metrics:
            interval_start = (
                int(metric.timestamp.timestamp() // interval_seconds) * interval_seconds
            )
        ...
        return {
            ...
            "average_throughput": statistics.mean(
                [t["throughput"] for t in throughput_per_interval]
            ),
    ...
    def generate_performance_report(
        ...
        for name, metrics in metrics_by_name.items():
            if "response_time" in name:
                ...
                analysis_results[name] = self.analyze_response_times(
