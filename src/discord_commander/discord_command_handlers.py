...
class DiscordCommandHandlers:
    ...
    def __init__(self, bot):
        ...
        self.agent_handlers = AgentCommandHandlers(bot.agent_engine, self.
            embed_manager)
        self.swarm_handlers = SwarmCommandHandlers(bot.agent_engine, self.
            embed_manager)
    ...
    async def process_command(self, message):
        ...
        try:
            ...
            cmd_type, args, remaining = self.command_router.parse_command(
                content)
            if cmd_type == 'unknown':
                return
            is_valid, error_msg = self.command_router.validate_command(cmd_type
                , args, content)
            if not is_valid:
                error_embed = self.embed_manager.create_response_embed('error',
                    title='❌ Invalid Command', description=error_msg)
            ...
            if total_active >= self.bot.max_concurrent_commands:
                ...
                return
            context = self.command_router.create_command_context(cmd_type,
                args, author, channel)
            response_data = await self._route_command(cmd_type, context,
                args, author, channel)
            if response_data:
                await self._handle_response(response_data, cmd_type, args,
                    author, channel)
        except Exception as e:
            logger.info(f'❌ Error processing command: {e}')
            error_embed = self.embed_manager.create_response_embed('error',
                title='❌ Command Processing Error', description=
    ...
    async def _route_command(self, cmd_type, context, args, author, channel):
        ...
        response_data = None
        if cmd_type == 'prompt':
            response_data = await self.agent_handlers.handle_prompt_command(
                context)
        elif cmd_type == 'status':
            response_data = await self.agent_handlers.handle_status_command(
                context)
        elif cmd_type == 'swarm':
            response_data = await self.swarm_handlers.handle_swarm_command(
                context)
        elif cmd_type == 'urgent':
            response_data = await self.swarm_handlers.handle_urgent_command(
                context)
        elif cmd_type == 'agents':
            agents = self.swarm_handlers.get_swarm_agent_list()
            embed = self.embed_manager.create_response_embed('agents',
                agents=agents, author=author)
            await channel.send(embed=embed)
            return None
        elif cmd_type == 'help':
            embed = self.embed_manager.create_response_embed('help', author
                =author)
            await channel.send(embed=embed)
            return None
        elif cmd_type == 'ping':
            ...
            embed = self.embed_manager.create_response_embed('ping',
                latency=latency, active_commands=active_commands)
            await channel.send(embed=embed)
            return None
        return response_data

    async def _handle_response(self, response_data, cmd_type, args, author,
        channel):
        ...
        if follow_up:
            ...
            if command_id and cmd_type == 'prompt':
                await self._handle_prompt_followup(command_id,
                    response_data, args, author, response_msg)
            elif command_id and cmd_type == 'swarm':
                await self._handle_swarm_followup(command_id, response_data,
                    author, response_msg)
            elif command_id and cmd_type == 'urgent':
                await self._handle_urgent_followup(command_id,
                    response_data, author, response_msg)
        else:
            await channel.send(embed=embed)

    async def _handle_prompt_followup(self, command_id, response_data, args,
        author, response_msg):
        """Handle prompt command followup."""
        try:
            result = await self.bot.agent_engine.send_to_agent_inbox(
                response_data['agent_id'], args[1],
            ...
            error_embed = self.embed_manager.create_response_embed('error',
                title='❌ Agent Communication Error', description=
                'Error communicating with agent.', error=str(e))
            await response_msg.edit(embed=error_embed)

    async def _handle_swarm_followup(self, command_id, response_data,
        author, response_msg):
        ...
        try:
            ...
            error_embed = self.embed_manager.create_response_embed('error',
                title='❌ Swarm Broadcast Error', description=
                'Error broadcasting to swarm.', error=str(e))
            await response_msg.edit(embed=error_embed)

    async def _handle_urgent_followup(self, command_id, response_data,
        author, response_msg):
        ...
        try:
            ...
            error_embed = self.embed_manager.create_response_embed('error',
                title='❌ Urgent Broadcast Error', description=
                'Error sending urgent broadcast.', error=str(e))
            await response_msg.edit(embed=error_embed)
