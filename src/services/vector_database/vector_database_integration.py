...
class VectorDatabaseIntegration:
    ...
    def __init__(self, db_path: str, config: Optional[Dict[str, Any]] = None):
        ...
        self.query_analytics = VectorDatabaseQueryAnalytics(
            self.core_integration.orchestrator,
            self.core_integration.status_indexer
        )
        
        logger.info("Vector Database Integration initialized")
    
    # Delegate to core integration
    async def integrate_agent_status(self, agent_id: str, status_data: Dict[str, Any], vector_type=None) -> str:
        """Integrate agent status data as vector."""
        return await self.core_integration.integrate_agent_status(agent_id, status_data, vector_type)
    ...
    def search_by_agent(self, agent_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
