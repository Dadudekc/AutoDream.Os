...
from strategic_consultation_templates import format_template_help, validate_template
...
def consult_command_handler(args: argparse.Namespace) -> int:
    ...
    try:
        result = consult_command(
            question=args.question, template=args.template, context=args.context
        )
    ...
    try:
        ...
        if history:
            for consultation in history:
                ...
                print(
                    f"   Question: {request['question'][:60]}{'...' if len(request['question']) > 60 else ''}"
                )
...
def emergency_command_handler(args: argparse.Namespace) -> int:
    ...
    try:
        result = consult_command(
            question=args.issue, template="crisis_response", context="emergency"
        )

        if result["success"]:
            print("✅ Emergency consultation completed!")
            print(f"📋 Consultation ID: {result['consultation_id']}")
...
def help_command_handler(args: argparse.Namespace) -> int:
    ...
    print(
        "   python strategic_consultation_cli.py consult --template priority_guidance --question 'What should be our next priority?'"
    )
    print("   python strategic_consultation_cli.py status-report")
    print(
        "   python strategic_consultation_cli.py emergency --issue 'System performance is degrading'"
    )
...
def main():
    ...
    status_parser = subparsers.add_parser(
        "status-report", help="Show consultation status and history"
    )

    # Emergency command
    emergency_parser = subparsers.add_parser(
        "emergency", help="Emergency consultation for critical issues"
    )
    emergency_parser.add_argument(
        "--issue", required=True, help="Description of the emergency issue"
    )
