...
class RoleAssignmentService:
        ...
        for protocol_file in protocols_dir.glob("*.json"):
            try:
                with open(protocol_file, 'r') as f:
                    ...
                    filtered_data = {k: v for k, v in role_data.items() 
                                   if k in RoleCapability.__annotations__}
    ...
    def assign_role(self, agent_id: str, role: str, task: str, duration: str, priority: str = "NORMAL") -> bool:
    ...
    def get_active_assignment(self, agent_id: str) -> Optional[RoleAssignment]:
...
def main():
    ...
    if args.list_roles:
        ...
        success = service.assign_role(args.agent, args.role, args.task, args.duration, args.priority)
        if success:
