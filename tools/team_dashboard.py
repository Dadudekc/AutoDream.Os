...
class TeamDashboard:
    ...
    def get_task_queue_summary(self) -> dict[str, Any]:
        ...
        task_summary = {"in_progress": [], "completed_today": [], "pending": [], "blocked": []}
    ...
    def get_communication_logs(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if devlogs_dir.exists():
            recent_files = sorted(
                devlogs_dir.glob("*.md"), key=lambda x: x.stat().st_mtime, reverse=True
            )
    ...
    def get_system_health(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "coordinates_valid": status_result.get("system_health", {}).get(
                    "coordinates_valid", False
                ),
                "pyautogui_available": status_result.get("system_health", {}).get(
                    "pyautogui_available", False
                ),
                "overall_status": status_result.get("system_health", {}).get(
...
def main():
    ...
    try:
        if args.display:
            ...
            data = dashboard.generate_dashboard_data()
            agents_data = data["agents_status"]
            print("🐝 WE ARE SWARM - Team Dashboard")
            print(f"Active Agents: {agents_data['active_agents']}/{agents_data['total_agents']}")
            print(
                f"Available: {agents_data['available_agents']} | Busy: {agents_data['busy_agents']}"
            )
            print(f"System: {data['system_health'].get('overall_status', 'Unknown')}")
            print("Use --display for real-time dashboard")
