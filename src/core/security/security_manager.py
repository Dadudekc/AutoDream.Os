...
from .oauth2_provider import OAuth2Client, OAuth2Provider
...
class SecurityManager:
    ...
    def hash_password(self, password: str, salt: str = None) -> tuple[str, str]:
        ...
        try:
            ...
            hasher = argon2.PasswordHasher(
                time_cost=3, memory_cost=65536, parallelism=4, hash_len=32, salt_len=16  # 64MB
            )
    ...
    def create_session(self, user_id: str, agent_id: str = None, roles: list[str] = None) -> str:
        ...
        token = self.jwt_manager.create_access_token(
            user_id=user_id, agent_id=agent_id, roles=roles or []
        )
    ...
    def create_oauth2_client(
        self, client_id: str, redirect_uris: list[str], scopes: list[str] = None
    ) -> OAuth2Client:
        """Create OAuth2 client."""
        return self.oauth2_provider.create_client(client_id, redirect_uris, scopes)

    def oauth2_authorize(self, client_id: str, user_id: str, redirect_uri: str, scope: str) -> str:
        """OAuth2 authorization flow."""
        return self.oauth2_provider.generate_authorization_code(
            client_id, user_id, redirect_uri, scope
        )

    def oauth2_token_exchange(self, code: str, client_id: str, client_secret: str) -> str | None:
        """OAuth2 token exchange."""
        access_token = self.oauth2_provider.exchange_code_for_token(code, client_id, client_secret)
        return access_token.token if access_token else None

    def validate_oauth2_token(self, token: str) -> dict[str, Any] | None:
        """Validate OAuth2 token."""
        return self.oauth2_provider.validate_token(token)

    def get_security_policy(self, policy_name: str) -> SecurityPolicy | None:
        """Get security policy by name."""
        return self.security_policies.get(policy_name)

    def update_security_policy(self, policy_name: str, policy: SecurityPolicy) -> None:
