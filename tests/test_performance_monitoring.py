...
try:
    from src.services.performance_monitor import (
        PerformanceMonitor, MetricType, MetricData, MetricSeries,
        PerformanceAlert, AlertSeverity, AlertCondition
    )
    ...
    from src.services.dashboard_backend import (
        DashboardBackend, DashboardAPI, DashboardWebSocket,
        DashboardEndpoint, DashboardRoute
    )
    from src.services.dashboard_frontend import (
        DashboardFrontend, DashboardWidget, ChartType,
        DashboardLayout, RealTimeUpdater
    )
    from src.services.performance_alerting import (
        AlertingSystem, AlertRule, AlertChannel, AlertManager,
        EmailAlertChannel, SlackAlertChannel, WebhookAlertChannel
    )
except ImportError:
...
class TestDashboardBackend:
    ...
    def sample_metrics_data(self):
        ...
        return [
            MetricData("cpu_usage", MetricType.GAUGE, 75.0, time.time(), {"host": "web1"}, "percent"),
            MetricData("memory_usage", MetricType.GAUGE, 60.0, time.time(), {"host": "web1"}, "percent"),
            MetricData("request_count", MetricType.COUNTER, 1500, time.time(), {"service": "api"}, "count")
        ]
    ...
    @pytest.mark.asyncio
    async def test_broadcast_metrics_update(self, dashboard_backend, sample_metrics_data):
    ...
    def test_get_metrics_api_endpoint(self, dashboard_backend, sample_metrics_data):
        ...
        mock_monitor.get_metric_series.return_value = MetricSeries(
            metric_name="cpu_usage",
            data_points=sample_metrics_data[:1]
        )
...
class TestDashboardFrontend:
    ...
    def test_generate_dashboard_html(self, dashboard_frontend):
        ...
        memory_widget = DashboardWidget("memory", "Memory Usage", ChartType.GAUGE, "memory_usage")
...
class TestPerformanceAndLoad:
    ...
    async def test_concurrent_dashboard_connections(self):
        ...
        test_data = [MetricData("test", MetricType.GAUGE, 100.0, time.time(), {}, "units")]
