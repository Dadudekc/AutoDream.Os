...
class DistributedTracingService:
    ...
    def manage_tracing_operations(self, action: str, **kwargs) -> Any:
        ...
        if action == "start_traced_operation":
            ...
            # Start span
            span = self.core.manage_tracing("start_span", name=span_name, attributes=kwargs.get("attributes", {}))
            
            # Register with span manager
            self.span_manager.manage_spans("register_span", span_id=span_id, name=span_name, attributes=kwargs.get("attributes", {}))
            ...
            # Set error status
            if span:
                self.core.manage_tracing("set_status", span=span, status="ERROR", description=error_message)
                self.core.manage_tracing("add_event", span=span, name="error", attributes={"error_type": error_type})
    ...
    def cleanup_old_data(self, days: int = 7) -> int:
        ...
        self.span_manager.span_history = [
            span for span in self.span_manager.span_history
