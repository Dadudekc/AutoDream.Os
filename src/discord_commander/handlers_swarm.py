...
class SwarmCommandHandlers:
    ...
    async def execute_swarm_broadcast(self, message: str, sender: str) -> CommandResult:
        ...
        try:
            ...
            if result.success:
                ...
                return CommandResult(
                    success=False,
                    message="Swarm broadcast failed",
                    data=result.data
                )
            ...
            return CommandResult(
                success=False,
                message=f"Swarm broadcast error: {str(e)}",
                data={'error': str(e)}
            )
    ...
    async def execute_urgent_broadcast(self, message: str, sender: str) -> CommandResult:
        ...
        """
        try:
            # Try to import the messaging system
            try:
                from ..core.messaging_pyautogui import broadcast_message_to_agents
            except ImportError:
                from core.messaging_pyautogui import broadcast_message_to_agents
            ...
            # Broadcast to all agents using the existing messaging infrastructure
            results = broadcast_message_to_agents(urgent_message, sender)
    ...
    async def coordinate_swarm_action(self, action: str, parameters: dict[str, Any]) -> CommandResult:
        ...
        try:
            if action == 'status_check':
                ...
                return CommandResult(
                    success=False,
                    message=f"Unknown swarm action: {action}"
                )

        except Exception as e:
            return CommandResult(
                success=False,
                message=f"Swarm coordination error: {str(e)}",
                data={'error': str(e)}
            )
    ...
    def get_broadcast_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            'oldest_broadcast': min(
                (b['timestamp'] for b in self.active_broadcasts.values()),
                default=None
            )
        }
...
def create_swarm_command_handlers(agent_engine, embed_manager: EmbedManager) -> SwarmCommandHandlers:
