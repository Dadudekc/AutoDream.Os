...
def demo_integrated_system_initialization():
    ...
    print(f"📋 Message queue agents: {len(coordinator.message_queue_manager.agent_registry)}")
    print(f"🤖 PyAutoGUI coordinator: {len(coordinator.pyautogui_coordinator.agent_roles)} agents")
...
def demo_system_status(coordinator):
    ...
    print(f"  Election Mode: {'ENABLED' if pyautogui_status['election_mode'] else 'DISABLED'}")
    print(f"  Round-Robin Mode: {'ENABLED' if pyautogui_status['round_robin'] else 'DISABLED'}")
...
async def main():
    ...
    try:
        ...
        print("  3. Run integrated coordinator: python src/services/integrated_agent_coordinator.py")
