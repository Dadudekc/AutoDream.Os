...
class SocialMediaView(View):
    ...
    async def social_status_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            await interaction.response.send_message("❌ Error getting social media status", ephemeral=True)
    ...
    async def analytics_button(self, interaction: discord.Interaction, button: Button):
        ...
        try:
            ...
            if analytics_result.get("success"):
                ...
                embed.add_field(name="Followers", value=str(analytics.get("followers", 0)), inline=True)
                embed.add_field(name="Engagement Rate", value=analytics.get("engagement_rate", "0%"), inline=True)
                embed.add_field(name="Posts Today", value=str(analytics.get("posts_today", 0)), inline=True)
                embed.add_field(name="Reach", value=str(analytics.get("reach", 0)), inline=True)
                embed.add_field(name="Impressions", value=str(analytics.get("impressions", 0)), inline=True)
                
                embed.set_footer(text="WE ARE SWARM - Social Media Analytics")
                await interaction.response.send_message(embed=embed, ephemeral=True)
            else:
                await interaction.response.send_message(f"❌ Error getting analytics: {analytics_result.get('error', 'Unknown error')}", ephemeral=True)
                
        except Exception as e:
            self.logger.error(f"Error in analytics button: {e}")
            await interaction.response.send_message("❌ Error getting social media analytics", ephemeral=True)
...
class MessageAgentModal(Modal, title="Send Message to Agent"):
    ...
    message = TextInput(label="Message", placeholder="Enter your message here...", style=discord.TextStyle.paragraph, required=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if success:
                ...
                embed.add_field(name="Message", value=self.message.value[:100] + "..." if len(self.message.value) > 100 else self.message.value, inline=False)
            else:
                embed = discord.Embed(
...
class BroadcastModal(Modal, title="Broadcast Message to All Agents"):
    ...
    message = TextInput(label="Broadcast Message", placeholder="Enter your broadcast message here...", style=discord.TextStyle.paragraph, required=True)
    
    async def on_submit(self, interaction: discord.Interaction):
        """Handle modal submission."""
        try:
            if not self.bot.connected_agents:
                await interaction.response.send_message("⚠️ No agents connected to broadcast to!", ephemeral=True)
                return
            ...
            embed.add_field(name="Message", value=self.message.value[:100] + "..." if len(self.message.value) > 100 else self.message.value, inline=False)
            embed.add_field(name="Recipients", value=", ".join(self.bot.connected_agents), inline=False)
            
            embed.set_footer(text="WE ARE SWARM - Discord Commander")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            
        except Exception as e:
            await interaction.response.send_message(f"❌ Error broadcasting message: {str(e)}", ephemeral=True)
...
class SocialMediaPostModal(Modal, title="Post to Social Media"):
    ...
    message = TextInput(label="Post Message", placeholder="Enter your social media post...", style=discord.TextStyle.paragraph, required=True)
    platform = TextInput(label="Platform", placeholder="all, twitter, facebook, instagram, linkedin", required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ Error posting to social media: {str(e)}", ephemeral=True)