...
class MediaProcessorService:
    ...
    def start_multimedia_pipeline(self, pipeline_name: str, config: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            if config.get('enable_video', False):
                ...
                video_started = self.video_service.start_webcam_capture(device_id, callback)
                
                if not video_started:
                    logger.error(f"Failed to start video capture for pipeline {pipeline_name}")
                    return False
            ...
            if config.get('enable_audio', False):
                ...
                audio_started = self.audio_service.start_audio_capture(device_id, callback)
                
                if not audio_started:
                    logger.error(f"Failed to start audio capture for pipeline {pipeline_name}")
                    return False
    ...
    def _create_video_callback(self, pipeline_name: str, config: Dict[str, Any]) -> Callable:
    ...
    def _create_audio_callback(self, pipeline_name: str, config: Dict[str, Any]) -> Callable:
        ...
        def audio_callback(audio_data):
            try:
                ...
                for effect in effects:
                    ...
                    if effect_type and hasattr(self.audio_service, 'apply_audio_filter'):
                ...
                # Update pipeline statistics
                if pipeline_name in self.processing_pipelines:
                    self.processing_pipelines[pipeline_name]['audio_chunks_processed'] += 1
    ...
    def create_effect_chain(self, chain_name: str, effects: List[Dict[str, Any]]) -> bool:
        """
    ...
    def apply_effect_chain(self, chain_name: str, media_data: Any, media_type: str) -> Any:
        """
        ...
        try:
            if pipeline_name:
                if pipeline_name in self.processing_pipelines:
                    ...
                    return {
                        'name': pipeline_name,
                        'status': 'active' if pipeline['video_active'] or pipeline['audio_active'] else 'inactive',
                ...
                return {
                    ...
                    'active_pipelines': sum(1 for s in statuses.values() if s['status'] == 'active'),
                    'pipelines': statuses
                }
