...
def main():
    ...
    parser.add_argument("--devlog", action="store_true", help="Eliminate devlog creation bottleneck")
    parser.add_argument("--compliance", action="store_true", help="Eliminate compliance checking bottleneck")
    parser.add_argument("--database", action="store_true", help="Eliminate database querying bottleneck")
    ...
    if args.eliminate_all:
        ...
        print(f"Inbox scanning bottleneck eliminated: {elimination.time_saved_per_cycle:.1f}s saved")
    
    elif args.tasks:
        elimination = eliminator.eliminate_manual_task_evaluation()
        print(f"Task evaluation bottleneck eliminated: {elimination.time_saved_per_cycle:.1f}s saved")
    
    elif args.devlog:
        elimination = eliminator.eliminate_manual_devlog_creation()
        print(f"Devlog creation bottleneck eliminated: {elimination.time_saved_per_cycle:.1f}s saved")
    
    elif args.compliance:
        elimination = eliminator.eliminate_manual_compliance_checking()
        print(f"Compliance checking bottleneck eliminated: {elimination.time_saved_per_cycle:.1f}s saved")
    
    elif args.database:
        elimination = eliminator.eliminate_manual_database_querying()
        print(f"Database querying bottleneck eliminated: {elimination.time_saved_per_cycle:.1f}s saved")
