    ...
    def process(self, data: AnalyticsData) -> AnalyticsData:
        try:
            ...
            self.compression_ratio = (
                (self.total_original - self.total_compressed) / self.total_original
...
class UnifiedAnalyticsEngine:
    ...
    def process_data(
        self, data: AnalyticsData, strategies: list[AnalyticsStrategy] = None
    ) -> AnalyticsData:
        ...
        try:
            ...
            for strategy_type in strategies:
                if strategy_type in self.strategies:
                    ...
                    self.logger.debug(
                        f"Processed data {data.id} with {strategy_type.value} strategy"
                    )
            ...
            self.logger.info(
                f"Processed data {data.id} in {processing_time:.3f}s using {len(strategies)} strategies"
            )
            return processed_data
        ...
        stats = {
            "processing": {
                ...
                "average_processing_time": (
                    self.total_processing_time / self.processed_data_count
        ...
        for strategy_type, strategy in self.strategies.items():
            if isinstance(strategy, CachingStrategy):
                stats["strategies"]["caching"] = {
                    ...
                    "hit_rate": (
                        strategy.cache_hits / (strategy.cache_hits + strategy.cache_misses) * 100
                        if (strategy.cache_hits + strategy.cache_misses) > 0
                        else 0
                    ),
                }
                ...
                stats["strategies"]["performance"] = {
                    ...
                    "average_response_time": (
                        sum(m.value for m in strategy.metrics) / len(strategy.metrics)
                        if strategy.metrics
                        else 0
                    ),
                }
    ...
    def get_strategy(self, strategy_type: AnalyticsStrategy) -> AnalyticsStrategyInterface | None:
