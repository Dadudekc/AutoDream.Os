...
class TestEndToEndIntegration:
    ...
    async def test_service_interaction(self, full_system):
        ...
        # Test service health
        healthy_services = sum(1 for s in status["services"].values() if s in ["active", "ready"])
...
class TestIntegrationErrorHandling:
    ...
    async def test_service_failure_recovery(self):
        ...
        with patch.object(
            manager, "_start_messaging_service", side_effect=Exception("Test failure")
        ):
    ...
    async def test_graceful_degradation(self):
        ...
        with patch.object(
            manager,
            "_start_social_media_service",
            side_effect=Exception("Social media unavailable"),
        ):
