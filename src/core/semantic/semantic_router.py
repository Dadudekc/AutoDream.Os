...
class SemanticRouter:
    ...
    def _determine_priority(self, task_text: str) -> str:
        ...
        if any(word in text_lower for word in ["urgent", "critical", "emergency", "fail", "error", "broken"]):
            return "URGENT"

        # Coordination priority indicators
        if any(word in text_lower for word in ["coordinate", "sync", "align", "consolidate", "integrate"]):
            return "COORDINATION"

        # Default priority
        return "NORMAL"

    def _get_agent_suggestions(self, task_text: str, similar_statuses: List) -> List[Dict[str, Any]]:
