...
class DiscordPerformanceMonitor:
    ...
    def calculate_average_metrics(self, minutes: int = 5) -> DiscordPerformanceMetrics:
        ...
        return DiscordPerformanceMetrics(
            message_processing_time=sum(m.message_processing_time for m in recent_metrics)
            / len(recent_metrics),
            command_execution_time=sum(m.command_execution_time for m in recent_metrics)
            / len(recent_metrics),
            event_handling_time=sum(m.event_handling_time for m in recent_metrics)
            / len(recent_metrics),
            memory_usage=sum(m.memory_usage for m in recent_metrics) / len(recent_metrics),
            messages_per_second=sum(m.messages_per_second for m in recent_metrics)
            / len(recent_metrics),
            commands_per_second=sum(m.commands_per_second for m in recent_metrics)
            / len(recent_metrics),
        ...
        for recommendation in recommendations:
            try:
                if "message processing pipeline" in recommendation.lower():
                    ...
                    results["optimizations_failed"].append(
                        f"Unknown optimization: {recommendation}"
                    )

            except Exception as e:
                results["optimizations_failed"].append(
                    f"Failed to apply {recommendation}: {str(e)}"
                )
