...
class EnhancedProjectScannerV2:
    ...
    def run_full_analysis(self, extensions: list = None) -> dict[str, dict]:
        ...
        logger.info(
            f"📊 {len(analysis_results)} files from cache, {len(files_to_analyze)} to analyze"
        )
        ...
        logger.info(
            f"📈 Project metrics: {project_metrics['total_files']} files, {project_metrics['total_lines']} lines"
        )
