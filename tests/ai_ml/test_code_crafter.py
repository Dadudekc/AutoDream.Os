...
class CodeCrafter:
    ...
    def generate_code(self, request: CodeGenerationRequest) -> CodeGenerationResult:
        ...
        try:
            ...
            # Extract dependencies before documentation so they can be referenced
            dependencies = self._extract_dependencies(code, request.language)
            documentation = (
                self._generate_documentation(request, dependencies)
...
const {{ main }} = require('./generated_code');
...
"""
    ...
    def _generate_documentation(self, request: CodeGenerationRequest, dependencies: Optional[List[str]] = None) -> str:
...
'''
    ...
    def test_generate_code_javascript(self):
        ...
        assert "- express" in result.documentation
        assert result.explanation is not None
        assert result.estimated_complexity > 0
        assert len(result.dependencies) >= 0
        assert len(result.usage_examples) > 0
