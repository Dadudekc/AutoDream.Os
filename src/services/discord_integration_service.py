...
class DiscordIntegrationService:
    ...
    def __init__(self, fsm_core: Optional[FSMCoreV2] = None, 
    ...
    def configure_discord(self, bot_token: str = None, webhook_url: str = None, 
                         guild_id: str = None):
    ...
    def send_message(self, sender: str, message_type: str, content: str, 
        ...
        try:
            ...
            if self.fsm_enabled and message_type in ["task_update", "state_change", "coordination"]:
                self._process_fsm_message(message)
            
            # Integrate with decision engine if available
            if self.decision_enabled and message_type in ["decision_request", "coordination"]:
    ...
    def register_agent(self, agent_id: str, name: str, capabilities: List[str] = None) -> bool:
        ...
        try:
            ...
            self.send_message("system", "agent_registration", 
    ...
    def update_agent_status(self, agent_id: str, status: str, details: str = None) -> bool:
        ...
        try:
            ...
            # Send Discord update
            self.send_message(agent_id, "status_update", 
    ...
    def create_discord_task(self, title: str, description: str, assigned_agent: str,
                           priority: str = "normal", channel: str = "tasks") -> str:
        ...
        try:
            ...
            if self.fsm_enabled:
                ...
                if task_id:
                    # Send Discord notification
                    self.send_message("system", "task_creation", 
                                    f"Task created: {title} assigned to {assigned_agent}",
                                    channel)
                    
                    self.logger.info(f"Discord task created with FSM integration: {task_id}")
                    return task_id
    ...
    def request_decision(self, context: str, options: List[str], 
                        requester: str, channel: str = "decisions") -> str:
        """Request a decision via Discord and integrate with decision engine"""
        try:
            if not self.decision_enabled:
                # Fallback: just send Discord message
                self.send_message(requester, "decision_request", 
            ...
            # Make decision
            result = self.decision_engine.make_autonomous_decision("agent_coordination", decision_context)
            
            # Send Discord notification
            self.send_message("decision_engine", "decision_result", 
    ...
    def _send_discord_webhook(self, message: Dict[str, Any]):
        ...
        self.logger.info(f"📱 Discord webhook: {message['channel']} - {message['content']}")
    ...
    def _process_decision_message(self, message: Dict[str, Any]):
        ...
        if message["type"] == "decision_request":
            ...
            self.logger.info(f"🧠 Decision engine integration: Processing decision request")
    
    def _notify_fsm_agent_registration(self, agent_id: str, name: str, capabilities: List[str]):
        """Notify FSM of agent registration"""
...
def main():
    ...
    parser.add_argument("--decision", action="store_true", help="Test decision engine integration")
    ...
    if args.test:
        ...
        # Test task creation
        service.create_discord_task("Test Task", "Integration testing", "agent-1", "high")
