...
class ProtocolCoordinator:
    ...
    def save_results(
        self, results: dict[str, Any], filename: str = "protocol_results.json"
    ) -> Path:
    ...
    def print_summary(self, results: dict[str, Any]) -> None:
        ...
        logger.info("=" * 60)
        logger.info(f"Coordination Status: {results.get('coordination_status', 'UNKNOWN')}")
        logger.info(f"Overall Score: {results.get('overall_score', 0.0):.1f}%")
        benchmark = results.get("benchmark_comparison", {})
        if benchmark:
            logger.info(f"Agent-2 Benchmark: {benchmark.get('agent_2_benchmark', 0.0)}%")
            logger.info(f"Exceeds Benchmark: {benchmark.get('exceeds_benchmark', False)}")
            logger.info(f"Achieves Domination: {benchmark.get('achieves_domination', False)}")
