...
def main():
    ...
    parser.add_argument(
        "--eliminate-redundant", action="store_true", help="Eliminate redundant documentation"
    )
    parser.add_argument(
        "--eliminate-duplicates", action="store_true", help="Eliminate duplicate content"
    )
    parser.add_argument(
        "--eliminate-all", action="store_true", help="Eliminate all documentation bloat"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Show what would be eliminated without actually doing it",
    )
    ...
    if args.eliminate_redundant:
        ...
        print(
            f"Redundant docs {'would be' if args.dry_run else ''} eliminated: {result['files_eliminated']}"
        )
        ...
        print(
            f"Duplicate files {'would be' if args.dry_run else ''} eliminated: {result['duplicates_eliminated']}"
        )
        print(f"Time saved: {result['time_saved_minutes']:.1f} minutes")
        ...
        total_eliminated = (
            redundant_result["files_eliminated"] + duplicate_result["duplicates_eliminated"]
        )
        total_time_saved = (
            redundant_result["time_saved_minutes"] + duplicate_result["time_saved_minutes"]
        )
