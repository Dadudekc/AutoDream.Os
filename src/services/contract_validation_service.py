...
class ContractValidationService:
    ...
    def validate_contract(self, contract_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = {
                ...
                "failed_rules": len(validation_results) - sum(1 for r in validation_results if r.passed),
                "validation_results": [
                    {
                        "rule_id": r.rule_id,
                        "passed": r.passed,
            ...
            self.logger.info(f"Contract {contract_data.get('contract_id')} validation completed: {response['passed_rules']}/{response['total_rules']} rules passed")
            return response
    ...
    def get_validation_rules(self, rule_type: Optional[str] = None, 
                           severity: Optional[str] = None) -> List[ValidationRule]:
    ...
    def get_violations(self, contract_id: Optional[str] = None, 
                      violation_type: Optional[str] = None) -> List[Violation]:
...
def main():
    ...
    parser.add_argument("--rules", "-r", action="store_true", help="Show validation rules")
    parser.add_argument("--violations", "-i", action="store_true", help="Show violations")
    ...
    if args.test:
        ...
        print(f"✅ Validation test completed: {result['passed_rules']}/{result['total_rules']} rules passed")
