    ...
    try:
        ...
        workflow_messages = [
            ...
            (
                "Agent-1",
                "Agent-2",
                "COORDINATION: Need resource estimates for strategic plan",
            ),
            (
                "Agent-2",
                "Agent-3",
                "COORDINATION: Resource allocation ready for technical planning",
            ),
            (
                "Agent-3",
                "Agent-4",
                "COORDINATION: Technical plan needs security validation",
            ),
            (
                "Agent-4",
                "Agent-1",
                "COORDINATION: Security protocols approved for strategic plan",
            ),
        ]
        ...
        if status["agent_status"]:
            ...
            for agent_id, agent_info in status["agent_status"].items():
                print(
                    f"   {agent_id}: {agent_info['messages_received']} messages received"
                )
    ...
    try:
        ...
        if mq_system.failed_messages:
            ...
            for msg_id, failure_info in mq_system.failed_messages.items():
                ...
                print(
                    f"   {msg_id}: {msg.sender} → {msg.recipient} - {failure_info['failure_reason']}"
                )
