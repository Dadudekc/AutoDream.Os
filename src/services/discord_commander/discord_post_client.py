...
class DiscordPostClient:
    ...
    def _load_config(self):
        ...
        logger.info(f"SSOT Config loaded - Bot: {'Yes' if self.bot_token else 'No'}, "
    ...
    async def post_devlog(self, agent_id: str, content: str) -> bool:
        ...
        try:
            ...
            if self.default_webhook_url:
                logger.warning(f"⚠️ Routing via default webhook (inaccurate routing) for {agent_id}: {self.default_webhook_url[:50]}...")
                result = await self._post_to_webhook(content, self.default_webhook_url, normalized_agent_id)
                if result:
                    logger.warning(f"⚠️ Devlog posted via default webhook for {agent_id} - check channel routing!")
                    return True
