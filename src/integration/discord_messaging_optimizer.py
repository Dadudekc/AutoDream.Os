    ...
    async def send_message_batch(self, messages: List[OptimizedMessage]) -> Dict[str, Any]:
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                ...
                results.append({"message_id": messages[i].message_id, "status": "failed", "error": str(result)})
            else:
...
async def main():
    ...
    print(f"Batch Processing: {test_result['batch_result']['successful']}/{test_result['batch_result']['batch_size']} successful")
