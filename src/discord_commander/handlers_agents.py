        ...
        if workspace_path.exists():
            ...
            if inbox_path.exists():
                ...
                recent_files = [
                    ...
                    if (datetime.utcnow() - datetime.fromtimestamp(f.stat().st_mtime)).seconds < 3600
                ]
    ...
    async def handle_direct_agent_followup(self, command_id: str, result: CommandResult) -> dict[str, Any]:
        ...
        if result.success:
            updated_embed = self.embed_manager.builder.update_direct_embed_success(
                self.embed_manager.builder.create_base_embed("✅ Message Sent to Agent"),
                agent_id
            )
        else:
...
def create_agent_command_handlers(agent_engine, embed_manager: EmbedManager) -> AgentCommandHandlers:
