...
class CoordinationTracker:
    ...
    def get_pending_requests(self, agent_id: str) -> List[CoordinationRequest]:
        ...
        with self._lock:
            ...
            for request in self.coordination_requests.values():
                if (request.to_agent == agent_id and 
                    not request.response_received and 
    ...
    def get_request_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            pending_requests = sum(1 for r in self.coordination_requests.values() 
                                 if r.status == "pending")
            completed_requests = sum(1 for r in self.coordination_requests.values() 
    ...
    def _cleanup_old_requests(self) -> int:
        ...
        for request_id, request in self.coordination_requests.items():
            if (request.response_received and 
                current_time - request.timestamp > self.cleanup_interval):
    ...
    def validate_coordination_protocol(self, from_agent: str, to_agent: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            for request in self.coordination_requests.values():
                if (request.from_agent == from_agent and 
                    request.to_agent == to_agent and 
                    current_time - request.timestamp < 60):  # Last minute
