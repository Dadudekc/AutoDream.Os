    ...
    def run_complete_migration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not validation_result['success']:
                migration_results['warnings'].append(f"Data validation issues: {validation_result['issues']}")
            ...
            if not schema_result['success']:
                migration_results['errors'].append(f"Schema creation failed: {schema_result['error']}")
                return migration_results
            ...
            if not migration_result['success']:
                migration_results['errors'].append(f"Data migration failed: {migration_result['error']}")
                return migration_results
            ...
            if not validation_result['success']:
                migration_results['errors'].append(f"Migration validation failed: {validation_result['error']}")
                return migration_results
            ...
            if not test_result['success']:
                migration_results['warnings'].append(f"Integration tests failed: {test_result['error']}")
