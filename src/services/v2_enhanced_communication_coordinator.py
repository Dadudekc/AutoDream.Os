...
class V2EnhancedCommunicationCoordinator:
    ...
    def _initialize_agent_coordination(self):
        ...
        try:
            ...
            logger.info(
                f"✅ V1-style agent coordination initialized for {len(self.agent_connections)} agents"
            )
    ...
    def _setup_communication_monitoring(self):
        ...
        try:
            ...
            self.communication_thread = threading.Thread(
                target=self._communication_monitoring_loop, daemon=True
            )
            ...
            self.communication_status.update(
                {
                    ...
                    "pending_decisions": len(
                        [
                            d
                            for d in self.presidential_decisions.values()
    ...
    def _process_agent_message(self, agent_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            if message_type == "task_update":
                ...
                logger.warning(
                    f"⚠️ Unknown message type from {agent_id}: {message_type}"
                )
        ...
        try:
            ...
            if task_id in self.tasks:
                ...
                if "progress" in message:
                    ...
                    logger.info(
                        f"📈 Task {task.title} progress: {old_progress}% → {task.progress_percentage}%"
                    )

                    # Notify stakeholders
                    self._notify_task_progress_update(
                        task, old_progress, task.progress_percentage
                    )

        except Exception as e:
            logger.error(f"❌ Error handling task update message: {e}")

    def _handle_collaboration_request_message(
    ...
    def _handle_presidential_decision_message(
        self, agent_id: str, message: Dict[str, Any]
    ):
        ...
        try:
            if agent_id == self.current_captain:
                ...
                logger.warning(
                    f"⚠️ Non-captain agent {agent_id} attempted to make presidential decision"
                )

        except Exception as e:
            logger.error(f"❌ Error handling presidential decision message: {e}")

    def _handle_multimedia_request_message(
        self, agent_id: str, message: Dict[str, Any]
    ):
    ...
    def _handle_content_generation_request(
        ...
        try:
            ...
            self._send_private_message(
                agent_id, {"type": "content_generation_error", "error": str(e)}
            )
    ...
    def _handle_streaming_setup_request(self, agent_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            if streaming_service.start_live_stream(stream_name, stream_config):
                ...
                self._send_private_message(
                    agent_id,
                    {"type": "streaming_error", "error": "Failed to start streaming"},
                )

        except Exception as e:
            logger.error(f"❌ Error handling streaming setup request: {e}")
            self._send_private_message(
                agent_id, {"type": "streaming_error", "error": str(e)}
            )
    ...
    def _handle_media_processing_request(self, agent_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            self._send_private_message(
                agent_id, {"type": "media_processing_error", "error": str(e)}
            )
            ...
            if agent_id in self.message_queues:
                ...
                if self.message_delivery_service:
                    ...
                    self.message_delivery_service.send_message(
                        agent_id, message_type, content, **message
                    )

                    logger.info(
                        f"📤 Message delivered to {agent_id} via V2 delivery service"
                    )
                else:
                    logger.warning(
                        f"⚠️ Message delivery service not available - message only queued for {agent_id}"
                    )
            ...
            if channel in self.broadcast_channels:
                ...
                if self.message_delivery_service:
                    ...
                    if message_type == "presidential_decision_proposed":
                        ...
                        content = (
                            f"Task Completed: {message.get('task_title', 'Unknown')}"
                        )
                    elif message_type == "multimedia_update":
                        content = message.get("message", "Multimedia update")
                    ...
                    results = self.message_delivery_service.broadcast_message(
                        message_type, content, target_agents, **message
                    )

                    logger.info(
                        f"📢 Broadcast delivered to {len(target_agents)} agents via V2 delivery service"
                    )
                    logger.info(f"📊 Broadcast results: {results}")
                else:
                    logger.warning(
                        f"⚠️ Message delivery service not available - broadcast only queued"
                    )
    ...
    def _notify_task_progress_update(
        self, task: CoordinationTask, old_progress: float, new_progress: float
    ):
            ...
            for agent_id in task.assigned_agents:
                if agent_id in self.agent_workloads:
                    ...
                    if task_id not in self.agent_workloads[agent_id]["completed_tasks"]:
                        self.agent_workloads[agent_id]["completed_tasks"].append(
                            task_id
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="V2 Enhanced Communication Coordinator"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show communication status"
    )
    parser.add_argument(
        "--create-task",
        nargs=4,
        metavar=("TITLE", "DESCRIPTION", "PRIORITY", "AGENTS"),
        help="Create a new task (title description priority agents)",
    )
    parser.add_argument(
        "--broadcast",
        nargs=2,
        metavar=("CHANNEL", "MESSAGE"),
