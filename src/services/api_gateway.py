...
class V2APIGateway:
    ...
    def register_route(self, path: str, method: RouteMethod, service_id: str, 
                      handler: Callable, requires_auth: bool = False, 
                      rate_limit: Optional[int] = None, timeout: float = 30.0) -> bool:
    ...
    def _check_rate_limit(self, request: GatewayRequest, route: RouteDefinition) -> bool:
        ...
        # Remove old requests
        self._rate_limit_store[rate_key] = [t for t in self._rate_limit_store[rate_key] if t > window_start]
    ...
    def add_auth_token(self, token: str, user_info: Dict[str, Any]):
        ...
        self.logger.info(f"Auth token added for user: {user_info.get('user_id', 'unknown')}")
...
def main():
    ...
    if args.test:
        ...
        success = gateway.register_route("/api/test", RouteMethod.GET, "test-service", test_handler)
