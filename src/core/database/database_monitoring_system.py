...
class DatabaseMonitoringSystem:
    ...
    def _get_overall_health_status(self) -> str:
        ...
        recent_checks = [
            h for h in self.health_checks if h.timestamp > datetime.now(UTC) - timedelta(minutes=5)
        ]
    ...
    def _get_health_summary(self) -> dict[str, Any]:
        """Get health summary."""
        recent_checks = [
            h for h in self.health_checks if h.timestamp > datetime.now(UTC) - timedelta(minutes=10)
        ]
    ...
    def _get_performance_summary(self) -> dict[str, Any]:
        ...
        recent_metrics = [
            m for m in self.metrics if m.timestamp > datetime.now(UTC) - timedelta(minutes=10)
        ]
