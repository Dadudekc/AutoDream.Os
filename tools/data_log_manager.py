...
class DataLogManager:
    ...
    def optimize_data_structure(self) ->dict[str, Any]:
        ...
        logger.info('🗂️  Optimizing data structure...')
        results = {'timestamp': datetime.now().isoformat(),
            'data_optimization': {}, 'log_optimization': {},
            'database_optimization': {}, 'cleanup_results': {}}
    ...
    def _organize_data_directory(self) ->dict[str, Any]:
        ...
        organization_results = {'files_moved': 0, 'directories_created': 0,
            'structure_created': {}}
        structure = {'databases': ['*.db', '*.sqlite', '*.sqlite3'],
            'vector_data': ['vector_*', 'embeddings_*'], 'semantic_data': [
            'semantic_*', 'knowledge_*'], 'cookies': ['*cookies*',
            '*session*'], 'backups': ['*backup*', '*archive*'], 'temp': [
            '*temp*', '*tmp*']}
        for category, patterns in structure.items():
            ...
            organization_results['structure_created'][category] = len(list(
                category_dir.iterdir()))
        return organization_results

    def _optimize_logs(self) ->dict[str, Any]:
        """Optimize log files."""
        log_results = {'logs_rotated': 0, 'logs_compressed': 0,
            'old_logs_removed': 0, 'total_size_saved': 0}
    ...
    def _optimize_databases(self) ->dict[str, Any]:
        ...
        db_results = {'databases_optimized': 0, 'databases_vacuumed': 0,
        ...
        for db_file in db_files:
            try:
                ...
                logger.info(
                    f'Warning: Could not optimize database {db_file}: {e}')
        return db_results

    def _cleanup_temp_files(self) ->dict[str, Any]:
        """Cleanup temporary files."""
        cleanup_results = {'temp_files_removed': 0, 'cache_dirs_cleaned': 0,
            'total_size_freed': 0}
        temp_patterns = ['*.tmp', '*.temp', '*.bak', '*.backup', '*~',
            '*.swp', '*.swo']
        cache_dirs = ['__pycache__', '.pytest_cache', '.coverage',
            'htmlcov', '.mypy_cache', '.tox', 'node_modules']
    ...
    def print_optimization_summary(self, results: dict[str, Any]) ->None:
        ...
        logger.info('\n' + '=' * 70)
        logger.info('📊 DATA & LOG OPTIMIZATION SUMMARY')
        logger.info('=' * 70)
        data_opt = results.get('data_optimization', {})
        logger.info('Data Organization:')
        logger.info(f"  Files Moved: {data_opt.get('files_moved', 0)}")
        logger.info(
            f"  Directories Created: {data_opt.get('directories_created', 0)}")
        log_opt = results.get('log_optimization', {})
        logger.info('\nLog Optimization:')
        logger.info(f"  Logs Rotated: {log_opt.get('logs_rotated', 0)}")
        logger.info(f"  Logs Compressed: {log_opt.get('logs_compressed', 0)}")
        logger.info(f"  Old Logs Removed: {log_opt.get('old_logs_removed', 0)}"
            )
        logger.info(
            f"  Size Saved: {log_opt.get('total_size_saved', 0) / (1024 * 1024):.1f} MB"
            )
        db_opt = results.get('database_optimization', {})
        logger.info('\nDatabase Optimization:')
        logger.info(
            f"  Databases Optimized: {db_opt.get('databases_optimized', 0)}")
        logger.info(
            f"  Databases Vacuumed: {db_opt.get('databases_vacuumed', 0)}")
        size_before = db_opt.get('total_size_before', 0)
        size_after = db_opt.get('total_size_after', 0)
        if size_before > 0:
            logger.info(
                f'  Size Reduction: {(size_before - size_after) / size_before * 100:.1f}%'
                )
        cleanup = results.get('cleanup_results', {})
        logger.info('\nCleanup Results:')
        logger.info(
            f"  Temp Files Removed: {cleanup.get('temp_files_removed', 0)}")
        logger.info(
            f"  Cache Dirs Cleaned: {cleanup.get('cache_dirs_cleaned', 0)}")
        logger.info(
            f"  Space Freed: {cleanup.get('total_size_freed', 0) / (1024 * 1024):.1f} MB"
            )
        logger.info('=' * 70)
...
def main():
    ...
    parser.add_argument('--optimize', action='store_true', help=
        'Optimize data and log management')
    parser.add_argument('--data-only', action='store_true', help=
        'Optimize data structure only')
    parser.add_argument('--logs-only', action='store_true', help=
        'Optimize logs only')
    args = parser.parse_args()
    project_root = Path('.')
    manager = DataLogManager(project_root)
    if args.optimize or args.data_only or args.logs_only:
        logger.info('🚀 Starting data and log optimization...')
