...
async def demo_unified_onboarding():
    ...
    for agent_id in onboarding_service.agent_roles.keys():
        ...
        print(
            f"  {agent_id}: {state.value if state else 'unknown'} (Contract: {contract.status if contract else 'none'})"
        )
    ...
    print(
        f"  - Active contracts: {len([c for c in onboarding_service.contracts.values() if c.status == 'active'])}"
    )
    print(
        f"  - Integration status: {'✅ Active' if discord_commander.onboarding_integration else '❌ Inactive'}"
    )
...
async def demo_contract_management():
    ...
    for agent_id, contract in onboarding_service.contracts.items():
        if contract:
            print(
                f"  {agent_id}: {contract.contract_type.value} - {contract.status} ({contract.progress_percentage}%)"
            )
