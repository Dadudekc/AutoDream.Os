...
class DeploymentDashboard:
    ...
    def display_dashboard(self):
        ...
        try:
            ...
            print(
                f"Workflow Cache Hit Rate: {workflow_stats.get('cache', {}).get('hit_rate', 0):.1%}"
            )
            print(
                f"Discord Cache Hit Rate: {discord_stats.get('cache', {}).get('hit_rate', 0):.1%}"
            )
            print(
                f"Workflow Active Tasks: {workflow_stats.get('parallel_executor', {}).get('active_tasks', 0)}"
            )
            print(
                f"Discord Active Tasks: {discord_stats.get('async_pool', {}).get('active_tasks', 0)}"
            )
