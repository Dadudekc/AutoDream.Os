...
from src.services.github_protocol_models import GitHubPermissionLevel, GitHubOperationStatus
...
class GitHubAgentCLI:
    ...
    def grant_permission(self, args):
        ...
        if success:
            print(f"✅ Permission granted: {args.agent_id} -> {args.repository} ({args.permission_level})")
        else:
    ...
    def export_data(self, args):
        ...
        output_file = args.output or f"github_protocol_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run(self):
        ...
        repo_parser.add_argument("-p", "--private", action="store_true", help="Make repository private")
        repo_parser.add_argument("-a", "--auto-init", action="store_true", help="Auto-initialize repository")
        ...
        perm_parser.add_argument("permission_level", choices=["read", "write", "admin", "owner"], help="Permission level")
        
        # List operations command
        ops_parser = subparsers.add_parser("list-operations", help="List operations")
        ops_parser.add_argument("-s", "--status", choices=["pending", "in_progress", "completed", "failed"], help="Filter by status")
