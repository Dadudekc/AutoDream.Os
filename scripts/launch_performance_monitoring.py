...
try:
    ...
    from services.metrics_collector import (
        ...
        NetworkMetricsCollector, CustomMetricsCollector
    )
    from services.dashboard_backend import DashboardBackend
    from services.dashboard_frontend import DashboardFrontend, DashboardWidget, ChartType, DashboardLayout
    from services.performance_alerting import (
        AlertingSystem, EmailAlertChannel, SlackAlertChannel,
        WebhookAlertChannel, DiscordAlertChannel, PagerDutyAlertChannel
    )
...
class PerformanceMonitoringLauncher:
        ...
        try:
            ...
            if collectors_config.get('system_metrics', {}).get('enabled', True):
                system_collector = SystemMetricsCollector(
                    collection_interval=collectors_config.get('system_metrics', {}).get('collection_interval', 30)
                )
                ...
                system_collector.collect_network = sys_config.get('collect_network', True)
            ...
            if collectors_config.get('application_metrics', {}).get('enabled', True):
                app_collector = ApplicationMetricsCollector(
                    collection_interval=collectors_config.get('application_metrics', {}).get('collection_interval', 60)
                )
            ...
            if collectors_config.get('network_metrics', {}).get('enabled', True):
                network_collector = NetworkMetricsCollector(
                    collection_interval=collectors_config.get('network_metrics', {}).get('collection_interval', 60)
                )
                
                # Add monitored ports
                monitored_ports = collectors_config.get('network_metrics', {}).get('monitored_ports', [])
            ...
            if collectors_config.get('custom_metrics', {}).get('enabled', True):
                custom_collector = CustomMetricsCollector(
                    collection_interval=collectors_config.get('custom_metrics', {}).get('collection_interval', 120)
                )
                
                # Add custom metrics (these would be implemented in the main application)
                custom_metrics = collectors_config.get('custom_metrics', {}).get('metrics', {})
    ...
    def setup_alerting_system(self) -> bool:
        ...
        try:
            alerting_config = self.config.get('performance_monitoring', {}).get('alerting', {})
            ...
            if email_config.get('enabled', False):
                ...
                email_channel.rate_limit_seconds = email_config.get('rate_limit_seconds', 300)
            ...
            if slack_config.get('enabled', False) and slack_config.get('webhook_url'):
                ...
                slack_channel.rate_limit_seconds = slack_config.get('rate_limit_seconds', 180)
            ...
            if discord_config.get('enabled', False) and discord_config.get('webhook_url'):
                ...
                discord_channel.rate_limit_seconds = discord_config.get('rate_limit_seconds', 120)
            ...
            if pagerduty_config.get('enabled', False) and pagerduty_config.get('integration_key'):
                pagerduty_channel = PagerDutyAlertChannel(
                    name="pagerduty",
                    integration_key=pagerduty_config.get('integration_key'),
                    api_url=pagerduty_config.get('api_url', 'https://events.pagerduty.com/v2/enqueue')
                )
                pagerduty_channel.min_severity = getattr(
                    type(self.performance_monitor).AlertSeverity,
                    pagerduty_config.get('min_severity', 'CRITICAL').upper()
                )
                pagerduty_channel.rate_limit_seconds = pagerduty_config.get('rate_limit_seconds', 60)
            ...
            if webhook_config.get('enabled', False) and webhook_config.get('webhook_url'):
                ...
                webhook_channel.rate_limit_seconds = webhook_config.get('rate_limit_seconds', 120)
            ...
            self.performance_monitor.alert_callbacks.append(self.alerting_system.process_alert)
    ...
    def setup_dashboard(self) -> bool:
        ...
        try:
            dashboard_config = self.config.get('performance_monitoring', {}).get('dashboard', {})
    ...
    def _print_startup_summary(self):
        ...
        if self.performance_monitor:
            ...
            print(f"   - Collection Interval: {self.performance_monitor.collection_interval}s")
        
        if self.dashboard_backend:
            print(f"✅ Dashboard Backend: Running")
            print(f"   - Host: {self.dashboard_backend.host}")
            print(f"   - Port: {self.dashboard_backend.port}")
            print(f"   - URL: http://{self.dashboard_backend.host}:{self.dashboard_backend.port}")
        ...
        print(f"\n🔗 Quick Links:")
        if self.dashboard_backend:
            print(f"   - Dashboard: http://{self.dashboard_backend.host}:{self.dashboard_backend.port}")
            print(f"   - Health Check: http://{self.dashboard_backend.host}:{self.dashboard_backend.port}/api/health")
            print(f"   - Metrics API: http://{self.dashboard_backend.host}:{self.dashboard_backend.port}/api/metrics")
    ...
    async def get_system_status(self) -> dict:
        ...
        if self.performance_monitor:
            status["components"]["performance_monitor"] = self.performance_monitor.get_system_status()
        ...
        if self.dashboard_backend:
            status["components"]["dashboard_backend"] = {
                ...
                "websocket_connections": len(self.dashboard_backend.websocket_handler.connections)
            }
    ...
    async def get_health_status(self) -> dict:
        ...
        if self.performance_monitor:
            health["checks"]["performance_monitor"] = {
                "status": "healthy" if self.performance_monitor.running else "unhealthy",
                "collectors": len(self.performance_monitor.collectors),
                "metrics_count": len(self.performance_monitor.metrics_storage.get_all_metric_names())
            }
        ...
        if self.dashboard_backend:
            health["checks"]["dashboard_backend"] = {
                ...
                "port": self.dashboard_backend.port,
                "connections": len(self.dashboard_backend.websocket_handler.connections)
            }
