...
def demo_basic_messaging():
    ...
    try:
        ...
        # Test message
        test_message = "🧪 CDP Demo: This is a test message from the CDP messenger system"
        target = targets[0]
        
        print(f"📤 Sending test message to: {target.get('title', 'Unknown')}")
        result = send_to_target(target["webSocketDebuggerUrl"], test_message, "DemoAgent")
...
def demo_agent_targeting():
    ...
    try:
        ...
        target = targets[0]
        test_message = "🎯 Agent-3: This is a targeted message specifically for you via CDP"
        
        print(f"📤 Sending targeted message to Agent-3 via: {target.get('title', 'Unknown')}")
...
def demo_broadcast_messaging():
    ...
    try:
        ...
        broadcast_message = "📢 BROADCAST: This is a demo broadcast message to all agents via CDP"
        ...
        for i, target in enumerate(targets):
            try:
                ...
                result = send_to_target(target["webSocketDebuggerUrl"], broadcast_message, "Broadcast")
    ...
    try:
        ...
        for priority, message in priority_messages:
            try:
                ...
                result = send_to_target(target["webSocketDebuggerUrl"], message, "PriorityDemo")
                
                if result.get("ok"):
                    success_count += 1
                    print(f"   ✅ {priority}: Success")
                else:
                    print(f"   ❌ {priority}: Failed - {result.get('reason', 'unknown error')}")
    ...
    try:
        ...
        try:
            ...
            msg_id = mq_system.queue_message(
                "Agent-5", "Agent-3", "🧪 Queue Integration Demo: Message from V1-V2 Queue System",
            ...
            print(f"📊 Queue Status: {status['messages_queued']} queued, {status['messages_delivered']} delivered")
...
def main():
    ...
    if passed == total:
        ...
        print("⚠️  Some demos failed. This is expected if Cursor isn't running with CDP.")
        print("\n🔧 To get started:")
    ...
    print("\n📚 Next Steps:")
    print("   1. Launch Cursor with CDP: .\\launch_cursor_with_cdp.ps1")
    print("   2. Test messaging: python cdp_send_message.py 'Hello Agent-3!' --target 'Agent-3'")
