...
class MailboxManager:
    ...
    async def _handle_task_assignment(self, message: dict[str, Any]) -> None:
        ...
        logger.info(
            f"{self.agent_id}: Received task assignment: {task_data.get('title', 'Unknown task')}"
        )
    ...
    async def _handle_coordination_message(self, message: dict[str, Any]) -> None:
        ...
        logger.info(
            f"{self.agent_id}: Received coordination message: {coordination_data.get('type', 'Unknown')}"
        )
    ...
    def deliver_message(self, message: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"{self.agent_id}: Message delivered to inbox - {message.get('id', 'unknown')}"
            )
            return True
