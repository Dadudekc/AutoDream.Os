...
def analyze_code_quality():
    ...
    for metric, count in v2_compliance.items():
        if metric != 'total_files':
            percentage = (count / v2_compliance['total_files'] * 100) if v2_compliance['total_files'] > 0 else 0
    ...
    quality_scores = {
        'type_hint_compliance': (v2_compliance['type_hints'] / total_files * 100) if total_files > 0 else 0,
        'documentation_compliance': (v2_compliance['docstrings'] / total_files * 100) if total_files > 0 else 0,
        'error_handling_compliance': (v2_compliance['error_handling'] / total_files * 100) if total_files > 0 else 0,
        'logging_compliance': (v2_compliance['logging_usage'] / total_files * 100) if total_files > 0 else 0,
        'anti_pattern_density': (sum(len(files) for files in anti_patterns.values()) / total_files * 100) if total_files > 0 else 0
    }
