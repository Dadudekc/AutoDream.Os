...
def simulate_real_world_scenarios():
    ...
    for i in range(3):
        try:
            ...
            error_handler.handle_error(e, db_context, ErrorSeverity.HIGH, ErrorCategory.DATABASE)
            time.sleep(0.5)
    ...
    for error in validation_errors:
        try:
            ...
            error_handler.handle_error(e, api_context, ErrorSeverity.MEDIUM, ErrorCategory.VALIDATION)
            time.sleep(0.3)
    ...
    for error in resource_errors:
        try:
            ...
            error_handler.handle_error(e, resource_context, ErrorSeverity.CRITICAL, ErrorCategory.RESOURCE)
            time.sleep(0.4)
    ...
    for i in range(4):
        try:
            ...
            error_handler.handle_error(e, network_context, ErrorSeverity.HIGH, ErrorCategory.NETWORK)
            time.sleep(0.6)
...
def demonstrate_error_analytics():
    ...
    try:
        ...
        # Show report summary
        print(f"\n📊 Report Summary:")
        print(f"   System Health Score: {report.summary.get('system_health_score', 'N/A')}")
        print(f"   Total Errors: {report.summary.get('total_errors', 'N/A')}")
        print(f"   Recovery Success Rate: {report.summary.get('recovery_success_rate', 'N/A')}%")
        print(f"   High Impact Services: {report.summary.get('high_impact_services', 'N/A')}")
...
def demonstrate_circuit_breakers_and_recovery():
    ...
    for i in range(8):  # More than circuit breaker threshold
        try:
            ...
            error_info = error_handler.handle_error(e, context, ErrorSeverity.HIGH, ErrorCategory.NETWORK)
    ...
    for config_name, config in recovery_configs.items():
        ...
        print(f"      Exponential Backoff: {'Yes' if config.exponential_backoff else 'No'}")
...
def demonstrate_decorators_and_integration():
    ...
    @handle_errors(severity=ErrorSeverity.HIGH, category=ErrorCategory.VALIDATION, service_name="decorator_demo")
    def risky_operation():
    ...
    def unreliable_service():
        ...
        if unreliable_service._attempts < 3:
            raise RuntimeError(f"Service unavailable (attempt {unreliable_service._attempts})")
