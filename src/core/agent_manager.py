...
class AgentManager:
    ...
    def __init__(self, agents_dir: str = "agent_workspaces"):
        ...
        self.capabilities: Dict[AgentCapability, Set[str]] = {cap: set() for cap in AgentCapability}
        ...
        try:
            ...
            if config_file.exists():
                ...
                agent_info = AgentInfo(
                    ...
                    capabilities=[AgentCapability(cap) for cap in config_data.get("capabilities", [])],
                    current_contract=config_data.get("current_contract"),
                    last_heartbeat=config_data.get("last_heartbeat", self._get_current_timestamp()),
    ...
    def register_agent(self, agent_id: str, name: str, capabilities: List[AgentCapability]) -> bool:
    ...
    def complete_contract(self, agent_id: str, contract_id: str, success: bool = True) -> bool:
        """Mark a contract as completed for an agent"""
        try:
            if agent_id in self.agents and self.agents[agent_id].current_contract == contract_id:
                ...
                self.logger.info(f"Completed contract {contract_id} for agent {agent_id}")
                return True
            return False
        except Exception as e:
    ...
    def _start_heartbeat_monitoring(self):
        ...
        self.running = True
        self.heartbeat_thread = threading.Thread(target=self._heartbeat_monitoring_loop, daemon=True)
        ...
        while self.running:
            try:
                ...
                for agent_id, agent in self.agents.items():
                    ...
                    if current_time - last_heartbeat > timedelta(seconds=self.heartbeat_interval * 2):
                        # Agent is offline
    ...
    def get_agent_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "agent_statuses": {status.value: len([a for a in self.agents.values() if a.status == status]) 
                                 for status in AgentStatus},
                "capability_distribution": {cap.value: len(agent_ids) for cap, agent_ids in self.capabilities.items()}
            }
        except Exception as e:
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            success = self.register_agent(test_agent_id, "Smoke Test Agent", [AgentCapability.TESTING])
            if not success:
                return False
...
def run_smoke_test():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test agent registration
            success = manager.register_agent("test_agent", "Test Agent", [AgentCapability.TESTING])
            assert success
            ...
            # Test capability search
            agents_with_testing = manager.get_agents_by_capability(AgentCapability.TESTING)
...
def main():
    ...
    parser.add_argument("--register", nargs=3, metavar=("ID", "NAME", "CAPABILITIES"), help="Register agent")
    parser.add_argument("--status", nargs=2, metavar=("ID", "STATUS"), help="Update agent status")
    parser.add_argument("--assign", nargs=2, metavar=("ID", "CONTRACT"), help="Assign contract")
    parser.add_argument("--complete", nargs=3, metavar=("ID", "CONTRACT", "SUCCESS"), help="Complete contract")
    ...
    if args.register:
        ...
        success = manager.complete_contract(agent_id, contract, success_str.lower() == "true")
