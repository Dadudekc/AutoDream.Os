...
class WorkspaceManager:
    ...
    def cleanup_workspace(self, agent_id: str) -> bool:
        ...
        try:
            ...
            for response_file in responses_path.glob("*.txt"):
                if response_file.stat().st_mtime < (datetime.now().timestamp() - 86400):  # 24 hours
            ...
            for task_file in tasks_path.glob("*.json"):
                if task_file.stat().st_mtime < (datetime.now().timestamp() - 86400):  # 24 hours
                    task_file.unlink()
    ...
    def get_workspace_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_workspaces = sum(1 for w in self.workspaces.values() 
                                  if Path(w.workspace_path).exists())
...
def main():
    ...
    parser.add_argument("--init", action="store_true", help="Initialize workspace manager")
    parser.add_argument("--create", metavar="AGENT_ID", help="Create workspace for agent")
    parser.add_argument("--status", action="store_true", help="Show workspace status")
    parser.add_argument("--cleanup", metavar="AGENT_ID", help="Cleanup workspace for agent")
    parser.add_argument("--test", action="store_true", help="Run workspace manager tests")
    ...
    if args.init or not any([args.init, args.create, args.status, args.cleanup, args.test]):
    ...
    if args.test:
        ...
        try:
            ...
            status = manager.get_workspace_status()
            print(f"Status retrieval test: {'✅ Success' if 'total_workspaces' in status else '❌ Failed'}")
