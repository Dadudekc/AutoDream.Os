...
class SystemEfficiencyMonitor:
    ...
    def start_efficiency_monitoring(self) -> Dict[str, Any]:
        ...
        if self.monitoring_active:
            return {"status": "ALREADY_ACTIVE", "message": "Efficiency monitoring already running"}

        self.monitoring_active = True

        # Start monitoring thread
        monitor_thread = threading.Thread(target=self._monitoring_loop, daemon=True, daemon=True)
    ...
    def _scan_for_duplicates(self) -> List[Dict[str, Any]]:
        ...
        messaging_systems = ["messaging_service", "pyautogui_messaging", "discord_messaging"]
    ...
    def _generate_recommendations(self, duplicates: List[Dict[str, Any]]) -> List[str]:
        ...
        for duplicate in duplicates:
            recommendations.append(
                f"Consolidate {duplicate['system_type']} systems to {duplicate['primary']}"
            )
            recommendations.append(
                f"Eliminate duplicate systems: {', '.join(duplicate['eliminate'])}"
            )
    ...
    def _update_efficiency_metrics(self):
        ...
        total_duplicates = sum(
            len(system["duplicates"]) for system in self.system_registry.values()
        )
        ...
        self.efficiency_metrics["efficiency_score"] = sum(efficiency_scores) / len(
            efficiency_scores
        )
