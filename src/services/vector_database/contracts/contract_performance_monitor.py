...
class ContractPerformanceMonitor:
    ...
    def update_execution_stats(self, contracts: list[V3Contract]) -> dict[str, Any]:
        ...
        try:
            ...
            self._execution_stats["last_update"] = datetime.now(UTC)

            # Initialize start time if not set
            if not self._execution_stats["start_time"]:
                self._execution_stats["start_time"] = datetime.now(UTC)

            # Store performance snapshot
            self._performance_history.append(
                {"timestamp": datetime.now(UTC), "metrics": self._execution_stats.copy()}
            )
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._execution_stats["start_time"]:
                uptime_seconds = (
                    current_time - self._execution_stats["start_time"]
                ).total_seconds()
                uptime = {
