...
class ComponentMetrics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data['success_rate'] = self.successful_requests / self.total_requests if self.total_requests > 0 else 0
        return data
    ...
    async def collect_system_metrics(self) -> PerformanceSnapshot:
        ...
        try:
            ...
            if self.workflow_optimizer and self.workflow_optimizer():
                ...
                workflow_executions = workflow_stats.get("performance", {}).get("total_executions", 0)
            ...
            if self.discord_optimizer and self.discord_optimizer():
                ...
                command_executions = discord_stats.get("performance", {}).get("commands_executed", 0)
            ...
            if self.component_metrics:
                ...
                response_time_ms = sum(avg_response_times) / len(avg_response_times) if avg_response_times else 0.0
    ...
    async def monitoring_loop(self):
        ...
        while self.is_monitoring:
            try:
                ...
                # Log current status
                logger.debug(f"Performance snapshot: CPU {snapshot.cpu_percent:.1f}%, "
        ...
        with self.lock:
            ...
            return {
                ...
                "recent_averages": {
                    ...
                    "response_time_ms": sum(s.response_time_ms for s in recent_snapshots) / len(recent_snapshots)
                },
                "component_summary": {
                    name: {
