...
class V2IntegrationMonitoring:
    ...
    def set_metric_threshold(self, metric_name: str, warning_threshold: float, 
    ...
    def _check_metric_thresholds(self, metric_name: str, value: float):
        ...
        if severity:
            ...
            self.logger.warning(f"Alert generated: {severity.value} - {metric_name} = {value}")
    ...
    def generate_monitoring_report(self) -> str:
        ...
        if active_alerts:
            ...
            for alert in active_alerts[:5]:  # Show first 5 alerts
                lines.append(f"  {alert.severity.value.upper()}: {alert.metric_name} = {alert.current_value}")
        else:
...
def main():
    ...
    if args.test:
        ...
        monitoring.set_metric_threshold("memory_usage", warning=80, error=90, critical=98)
