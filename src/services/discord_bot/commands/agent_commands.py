...
def setup_agent_commands(bot):
    ...
    async def list_agents(interaction: discord.Interaction):
        ...
        try:
            ...
            for i in range(1, 9):
                agent_id = f"Agent-{i}"
                # Check if agent_coordinates exists and has the agent
                if hasattr(bot, 'agent_coordinates') and bot.agent_coordinates:
                    status = (
                        "Active"
            ...
            await interaction.response.send_message(f"Error retrieving agent list: {e}", ephemeral=True)
    ...
    async def list_agent_channels(interaction: discord.Interaction):
        ...
        try:
            ...
            # Check if devlog_service exists and has agent_channels
            if hasattr(bot, 'devlog_service') and bot.devlog_service and hasattr(bot.devlog_service, 'agent_channels'):
                ...
                if not bot.devlog_service.agent_channels:
                    channels_text += "No agent-specific channels configured\n"
                ...
                channels_text += "Devlog service not available\n"
                channels_text += "Agent channels cannot be displayed"
            
            channels_text += "\n\nWE ARE SWARM - Agent channels ready!"
            await interaction.response.send_message(channels_text, ephemeral=True)
        except Exception as e:
            logger.error(f"Error in list_agent_channels command: {e}")
            await interaction.response.send_message(f"Error retrieving agent channels: {e}", ephemeral=True)
    ...
    async def swarm_message(interaction: discord.Interaction, message: str):
        ...
        try:
            ...
            if not hasattr(bot, 'messaging_service') or not bot.messaging_service:
                await interaction.response.send_message("Messaging service not available. Cannot send swarm message.", ephemeral=True)
                return
            ...
            logger.error(f"Error in swarm_message command: {e}")
            await interaction.response.send_message(f"Error sending swarm message: {e}", ephemeral=True)
