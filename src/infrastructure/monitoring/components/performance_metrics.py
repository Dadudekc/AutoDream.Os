...
class PerformanceMetricsCollector:
    ...
    def record_error(self, operation: str, error_type: str) -> None:
        ...
        self.error_history.append(
            {"operation": operation, "error_type": error_type, "timestamp": datetime.now()}
        )
    ...
    def get_average_response_time(self, operation: str | None = None) -> float:
    ...
    def get_error_rate(self, operation: str | None = None) -> float:
        ...
        if operation:
            ...
            errors = sum(
                count for key, count in self.error_counts.items() if key.startswith(f"{operation}:")
            )
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        return {
            "response_times": {
                ...
                "operations": {
                    op: {"average": self.get_average_response_time(op), "count": len(times)}
                    for op, times in self.operation_times.items()
                },
            },
            "error_rates": {
                "overall": self.get_error_rate(),
                "by_operation": {op: self.get_error_rate(op) for op in self.operation_times.keys()},
                "total_errors": sum(self.error_counts.values()),
            },
            "cache_performance": {
                cache: {
    ...
    def get_health_metrics(self) -> list[HealthMetric]:
        ...
        if avg_response_time > 0:
            metrics.append(
                HealthMetric(
                    name="average_response_time", value=round(avg_response_time, 2), unit="ms"
                )
            )
        ...
        if error_rate >= 0:
            metrics.append(HealthMetric(name="error_rate", value=round(error_rate, 2), unit="%"))
        ...
        for cache_name in self.cache_stats.keys():
            ...
            metrics.append(
                HealthMetric(
                    name=f"cache_hit_rate_{cache_name}", value=round(hit_rate, 2), unit="%"
                )
            )
        ...
        for resource in self.resource_usage.keys():
            ...
            metrics.append(
                HealthMetric(
                    name=f"resource_usage_{resource}", value=round(stats["current"], 2), unit="%"
                )
            )
