...
class MetricPoint:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {"timestamp": self.timestamp, "value": self.value, "tags": self.tags}
...
class MetricsCollector:
    ...
    def add_metric(self, name: str, value: float, tags: dict[str, str] = None):
        """Add a metric point."""
        metric = MetricPoint(
            timestamp=time.time(), value=value, tags={"metric": name, **(tags or {})}
        )
    ...
    def get_metrics(self, metric_name: str = None, limit: int = 100) -> list[dict[str, Any]]:
        ...
        if metric_name:
            filtered_metrics = [m for m in self.metrics if m.tags.get("metric") == metric_name]
...
class VectorDatabaseMonitoring:
    ...
    def record_operation_metric(self, operation_type: str, duration: float, success: bool):
        ...
        self.metrics_collector.add_metric(
            "operation_count", 1, {"operation_type": operation_type, "success": str(success)}
        )

    def record_connection_metric(self, active_connections: int):
        """Record connection metrics."""
        self.metrics_collector.add_metric("active_connections", float(active_connections))

    def record_query_metric(self, query_type: str, result_count: int, duration: float):
        """Record query metrics."""
        self.metrics_collector.add_metric(
            "query_result_count", float(result_count), {"query_type": query_type}
        )

        self.metrics_collector.add_metric("query_duration", duration, {"query_type": query_type})

    def add_health_check(self, name: str, status: HealthStatus):
        """Add a health check result."""
        health_data = {"name": name, "status": status.to_dict(), "timestamp": time.time()}
