    ...
    def send_message(self, from_agent, to_agent, message, priority="NORMAL", execution_mode=False):
        ...
        try:
            ...
            if execution_mode:
                ...
                formatted_message = self._format_a2a_message(from_agent, to_agent, message, priority)
...
def hard_onboard_handler(target_agent_id=None):
    ...
    try:
        ...
        pyautogui.click(onboard_coords[0], onboard_coords[1])
        time.sleep(1.0)
        ...
        print(f"✅ Hard onboarding successful! Agent(s) ready for interaction.")
...
def main():
    ...
    if len(sys.argv) < 2 or "--help" in sys.argv:
        ...
        print("=" * 60)
        print("Usage:")
        print("  python messaging_system.py <from_agent> <to_agent> '<message>' [priority] [--execution-mode]")
        ...
        print("  python messaging_system.py Agent-4 Agent-5 'Direct command' NORMAL --execution-mode")
    ...
    if len(sys.argv) == 3 and sys.argv[1] == "--soft-onboard":
        ...
        print("Sequence: 1) Click chat input 2) Send passdown 3) Ctrl+T 4) Move to onboard coords 5) Paste onboard message")
        ...
        try:
            ...
            send_result = messaging.send_message(result['current_agent'], target_agent_id, result['message'], "NORMAL")
            
            if not send_result["success"]:
                print(f"❌ Failed to send passdown message: {send_result.get('error', 'Unknown error')}")
                sys.exit(1)
            ...
            time.sleep(2.0)  # Wait for agent transition
    ...
    if len(sys.argv) < 4:
        ...
        print("Usage: python messaging_system.py <from_agent> <to_agent> '<message>' [priority] [--execution-mode]")
        sys.exit(1)
