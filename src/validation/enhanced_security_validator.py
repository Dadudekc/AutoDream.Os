...
class EnhancedSecurityValidator:
    ...
    def _load_false_positive_patterns(self) -> dict[str, list[str]]:
        ...
        return {
            ...
            "test_files": [r"test.*key", r"mock.*token", r"fake.*secret", r"example.*password"],
        }
    ...
    def _is_false_positive(self, finding: SecurityFinding) -> bool:
        ...
        if "test" in finding.file_path.lower() and any(
            word in line_content for word in ["mock", "fake", "example", "sample"]
        ):
            return True
    ...
    def _categorize_false_positives(self, false_positives: list[SecurityFinding]) -> dict[str, int]:
        ...
        for fp in false_positives:
            for category in self.false_positive_patterns.keys():
                if any(
                    re.search(pattern, fp.line_content.lower())
                    for pattern in self.false_positive_patterns[category]
                ):
