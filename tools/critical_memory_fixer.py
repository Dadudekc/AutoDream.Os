    ...
    def fix_performance_monitor(self) -> bool:
        ...
        try:
            ...
            if "threading.Thread(" in content and "daemon=True" not in content:
                content = content.replace("threading.Thread(", "threading.Thread(", daemon=True)
...
"""
            ...
            self.fixes_applied.append(
                "Fixed performance_monitor.py: Added thread cleanup, weakref cleanup, memory cleanup"
            )
        ...
        try:
            ...
            if "collections.deque(maxlen=1000)" in content:
                content = content.replace(
                    "collections.deque(maxlen=1000)", "collections.deque(maxlen=1000)"
                )
...
"""
            ...
            self.fixes_applied.append(
                "Fixed memory_aware_responses.py: Added memory limits and cleanup"
            )
...
"""
            ...
            self.fixes_applied.append(
                "Fixed integration_assessment_engine.py: Added cleanup and memory monitoring"
            )
...
"""
            ...
            self.fixes_applied.append(
                "Fixed complete_infrastructure_integration.py: Added thread and connection cleanup"
            )
...
"""
            ...
            self.fixes_applied.append(
                "Fixed thea_conversation_manager.py: Added conversation cleanup"
            )
    ...
    def apply_all_critical_fixes(self) -> dict[str, Any]:
        ...
        results = {"files_processed": 0, "files_fixed": 0, "fixes_applied": [], "errors": []}
        ...
        logger.info(
            f"Critical fixes complete: {results['files_fixed']}/{results['files_processed']} files fixed"
        )
        return results
