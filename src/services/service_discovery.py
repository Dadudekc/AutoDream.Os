...
class V2ServiceDiscovery:
    ...
    def update_service_health(
        self, service_id: str, endpoint_url: str, is_healthy: bool
    ):
    ...
    def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        active_services = len(
            [s for s in self._services.values() if s.state == ServiceState.ACTIVE]
        )
        failed_services = len(
            [s for s in self._services.values() if s.state == ServiceState.FAILED]
        )
        ...
        return {
            ...
            "health_check_active": self._health_check_thread
