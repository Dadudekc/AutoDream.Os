    ...
    def save_message(self, message: CursorMessage) -> bool:
        ...
        try:
            ...
            if not message.message_id or not message.thread_id or not message.content:
                self.logger.warning(f"Invalid message data: id={message.message_id}, thread={message.thread_id}, content={bool(message.content)}")
                return False
            ...
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (message.message_id, message.thread_id, message.role, 
                  message.content, message.created_at, message.meta_json))
...
class CursorMessageNormalizer:
    ...
    def normalize(payload: Dict[str, Any]) -> CursorMessage:
        ...
        # Extract timestamp
        created_at = int(payload.get("created_at") or 
...
class CursorResponseCapture:
    ...
    def _capture_loop(self):
        ...
        while self.is_capturing:
            try:
                ...
                self.error_counts["capture_loop"] = self.error_counts.get("capture_loop", 0) + 1
        ...
        try:
            ...
            for payload in test_messages:
                try:
                    ...
                    self.error_counts["message_processing"] = self.error_counts.get("message_processing", 0) + 1
    ...
    def _update_health_metrics(self, messages_captured: int, capture_time: float):
        ...
        try:
            ...
            if "cursor_capture" not in self.health_monitor.health_metrics:
                self.health_monitor.register_component("cursor_capture", self._get_health_metrics)
    ...
    def _get_health_metrics(self) -> Dict[str, float]:
        ...
        error_rate = sum(self.error_counts.values()) / max(self.total_messages_captured, 1)
        ...
        return {
            ...
            "throughput": self.total_messages_captured / max((time.time() - self.last_capture_time) / 3600, 1),  # messages/hour
    ...
    def get_capture_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_capture_time": sum(self.capture_times) / len(self.capture_times) if self.capture_times else 0,
...
def main():
    ...
    parser.add_argument("--cdp-port", type=int, default=9222, help="CDP port for cursor")
    ...
    parser.add_argument("--db-path", default="runtime/cursor_capture/cursor_threads.db", help="Database path")
        ...
        if args.once:
            ...
            try:
                while True:
                    ...
                    print(f"📈 Captured: {stats['total_messages_captured']} messages, "
