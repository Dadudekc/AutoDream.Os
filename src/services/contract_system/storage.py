...
class ContractStorage:
    ...
    def load_contract(self, contract_id: str) ->(Contract | None):
        ...
        try:
            ...
            logger.info(f'ERROR: Error loading contract: {e}')
            return None
    ...
    def load_all_contracts(self) ->dict[str, dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f'ERROR: Error loading contracts: {e}')
            return {}
    ...
    def load_agent_contracts(self, agent_id: str) ->dict[str, dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f'ERROR: Error loading agent contracts: {e}')
            return {}
    ...
    def get_agent_contracts(self, agent_id: str) ->list[Contract]:
        ...
        try:
            ...
            logger.info(f'ERROR: Error getting agent contracts: {e}')
            return []
        ...
        try:
            ...
            for contract_data in agent_contracts.values():
                ...
                for task in contract.tasks:
                    if (task.status == TaskStatus.PENDING and task.
            ...
            logger.info(f'ERROR: Error getting available tasks: {e}')
            return []
    ...
    def assign_task(self, task_id: str, agent_id: str) ->bool:
        ...
        try:
            ...
            logger.info(f'ERROR: Error assigning task: {e}')
            return False
    ...
    def complete_task(self, task_id: str, completion_notes: str='') ->bool:
        ...
        try:
            ...
            logger.info(f'ERROR: Error completing task: {e}')
            return False
    ...
    def get_agent_summary(self, agent_id: str) ->dict[str, Any]:
        ...
        try:
            agent_contracts = self.load_agent_contracts(agent_id)
            contracts = [Contract.from_dict(data) for data in
                agent_contracts.values()]
            total_contracts = len(contracts)
            active_contracts = len([c for c in contracts if c.status in [
                TaskStatus.ASSIGNED, TaskStatus.IN_PROGRESS]])
            completed_contracts = len([c for c in contracts if c.status ==
            ...
            for contract in contracts:
                for task in contract.tasks:
                    if task.status in [TaskStatus.ASSIGNED, TaskStatus.
                        IN_PROGRESS]:
                        current_tasks.append(task)
            return {'agent_id': agent_id, 'total_contracts':
                total_contracts, 'active_contracts': active_contracts,
                'completed_contracts': completed_contracts, 'total_points':
                total_points, 'completed_points': completed_points,
                'completion_rate': round(completion_rate, 2),
                'current_tasks': [task.to_dict() for task in current_tasks]}
        except Exception as e:
            logger.info(f'ERROR: Error getting agent summary: {e}')
            return {}
    ...
    def _read_json(self, file_path: Path) ->dict[str, Any]:
        ...
        try:
            ...
            logger.info(f'ERROR: Error reading JSON file {file_path}: {e}')
            return {}
    ...
    def _write_json(self, file_path: Path, data: dict[str, Any]) ->bool:
        ...
        try:
            ...
            logger.info(f'ERROR: Error writing JSON file {file_path}: {e}')
            return False
