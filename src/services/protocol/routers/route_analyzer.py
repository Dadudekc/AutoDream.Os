...
logger.info(f"Operation result: {result}")
    ...
    def _calculate_route_score(
        ...
        if (
            route == MessageRoute.BATCHED
            and ProtocolOptimizationStrategy.MESSAGE_BATCHING in strategies
        ):
            score += 3.0

        if (
            route == MessageRoute.LOAD_BALANCED
            and ProtocolOptimizationStrategy.LOAD_BALANCING in strategies
        ):
            score += 2.0
