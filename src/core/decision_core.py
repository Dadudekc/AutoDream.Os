    ...
    def _basic_decision(
        ...
        if decision_type == DecisionType.TASK_ASSIGNMENT.value:
            ...
            return {
                ...
                "reasoning": f"Basic assignment to {selected_agent}",
    ...
    def _intermediate_decision(
        ...
        if decision_type == DecisionType.TASK_ASSIGNMENT.value:
            ...
            for agent_id, capability in self.agent_capabilities.items():
                if capability.availability and all(
                    skill in capability.skills for skill in task_requirements
                ):
            ...
            if (
                decision_type_name in self.decision_patterns
                and self.decision_patterns[decision_type_name]
            ):
                pattern = max(
                    set(self.decision_patterns[decision_type_name]),
                    key=self.decision_patterns[decision_type_name].count,
                )
