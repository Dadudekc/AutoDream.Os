...
try:
    from .coordinator_types import (
        ...
        CoordinationMessage, AgentCapability
    )
except ImportError:
...
class ChannelManager:
    ...
    def send_message(self, channel_id: str, message: CoordinationMessage) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.error(f"Failed to send message through channel {channel_id}")
                return False
            ...
            self.logger.error(f"Error sending message through channel {channel_id}: {e}")
            return False

    def broadcast_message(self, message: CoordinationMessage, 
                        channel_types: List[str] = None) -> Dict[str, bool]:
    ...
    def get_all_channels_status(self) -> List[Dict[str, Any]]:
        ...
        return [self.get_channel_status(channel_id) 
...
class CommunicationChannel:
    ...
    def send_message(self, message: CoordinationMessage) -> bool:
        ...
        if not self._open:
            self.logger.error(f"Cannot send message: channel {self.channel_id} is closed")
            return False
