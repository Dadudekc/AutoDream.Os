...
class EnhancedLanguageAnalyzer:
    ...
    def _analyze_rust_enhanced(self, source_code: str) -> dict[str, Any]:
        ...
        def _traverse(node):
            if node.type == "function_item":
                ...
                if fn_name_node:
                    functions.append(
                        {"name": fn_name_node.text.decode("utf-8"), "line": node.start_point[0] + 1}
                    )
            elif node.type == "struct_item":
                struct_name_node = node.child_by_field_name("name")
                if struct_name_node:
                    struct_name = struct_name_node.text.decode("utf-8")
                    structs[struct_name] = {"line": node.start_point[0] + 1, "fields": []}
    ...
    def _analyze_javascript_enhanced(self, source_code: str) -> dict[str, Any]:
        ...
        def _traverse(node):
            if node.type == "function_declaration":
                ...
                if name_node:
                    functions.append(
                        {"name": get_node_text(name_node), "line": node.start_point[0] + 1}
                    )
            elif node.type == "class_declaration":
                name_node = node.child_by_field_name("name")
                if name_node:
                    cls_name = get_node_text(name_node)
                    classes[cls_name] = {"line": node.start_point[0] + 1, "methods": []}
    ...
    def _extract_js_route_info(self, node) -> dict[str, str] | None:
    ...
    def _extract_route_from_line(self, line: str) -> dict[str, str] | None:
        ...
        for file_path, analysis in self.analysis.items():
            if analysis.get("language") == ".py":
                for class_name, class_data in analysis.get("classes", {}).items():
                    ...
                    agent_data["maturity_distribution"][maturity] = (
                        agent_data["maturity_distribution"].get(maturity, 0) + 1
                    )
                    agent_data["agent_types"][agent_type] = (
                        agent_data["agent_types"].get(agent_type, 0) + 1
                    )
        ...
        architecture_data = {
            ...
            "quality_metrics": {
                ...
                "languages_supported": list(
                    set(a.get("language", "unknown") for a in self.analysis.values())
                ),
                "average_complexity": sum(a.get("complexity", 0) for a in self.analysis.values())
                / max(len(self.analysis), 1),
                "agent_distribution": self._calculate_agent_distribution(),
            },
            "v2_compliance": {
                "compliant_files": sum(
                    1 for a in self.analysis.values() if a.get("file_size", 0) <= 400
                ),
                "violation_files": sum(
                    1 for a in self.analysis.values() if a.get("file_size", 0) > 400
                ),
    ...
    def export_chatgpt_context(self):
        ...
        enhanced_context = {
            ...
            "swarm_intelligence": {
                ...
                "achievements": [
                    "8-agent debate coordination",
                    "Multi-monitor architecture",
                    "Physical swarm automation",
                ],
            },
