...
class AgentSummary(commands.Cog):
    ...
    async def _send_and_ack(
        ...
        try:
            ...
            print(f"✅ Summary request sent to {agent_key} via PyAutoGUI")
            ...
            print(f"❌ Failed to send summary request to {agent_key}: {e}")

    # Prefix commands for each agent
    @commands.command(name="summary1", help="Request status summary from Agent-1")
    ...
    @commands.command(name="summary2", help="Request status summary from Agent-2")
    ...
    @commands.command(name="summary3", help="Request status summary from Agent-3")
    ...
    @commands.command(name="summary_agent4", help="Request status summary from Agent-4")
    ...
    async def summary_general(self, ctx: commands.Context, agent_number: int, *, context: str = ""):
        ...
        if 1 <= agent_number <= 4:
            ...
            await ctx.reply("❌ Invalid agent number. Use 1-4.", mention_author=False)

    @commands.command(name="agentsummary", help="List available summary commands")
...
async def setup(bot: commands.Bot, gateway):
    ...
    print("✅ Agent Summary commands registered with Discord bot")
