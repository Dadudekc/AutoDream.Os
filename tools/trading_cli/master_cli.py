...
class MasterCLI:
    ...
    def run_comprehensive_analysis(self, symbol: str = "TSLA", output_dir: str = None):
        ...
        success = self.run_tool(
            "market", ["--symbol", symbol, "--output", f"{output_dir}/market_analysis.json"]
        )
        ...
        success = self.run_tool(
            "news", ["--symbol", symbol, "--output", f"{output_dir}/news_analysis.json"]
        )
        ...
        success = self.run_tool(
            "technical", ["--symbol", symbol, "--output", f"{output_dir}/technical_analysis.json"]
        )
        ...
        success = self.run_tool(
            "coordinator", ["--symbol", symbol, "--output", f"{output_dir}/agent_coordination.json"]
        )
