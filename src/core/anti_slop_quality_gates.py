...
from .progress_validation_system import ProgressValidationSystem, ValidationStatus, ResultType
from .content_deduplication_system import ContentDeduplicationSystem
from .centralized_content_registry import CentralizedContentRegistry, ContentType, ContentStatus
...
class AntiSlopQualityGates:
    ...
    def validate_content_uniqueness(self, content: str, agent_id: str) -> QualityGateResult:
        ...
        is_compliant, reason = self.deduplication_system.check_anti_slop_compliance(content, agent_id)
    ...
    def validate_measurable_results(self, agent_id: str, task_id: str, 
                                  before_value: Any, after_value: Any) -> QualityGateResult:
    ...
    def validate_real_work_requirement(self, agent_id: str, task_description: str) -> QualityGateResult:
        """Validate that task represents real work, not AI slop"""
        # Check for meta-work patterns
        meta_work_patterns = [
            'monitor', 'track', 'status', 'update', 'check', 'review',
            'analyze', 'report', 'document', 'log', 'audit'
        ]
        ...
        recent_tasks = [
            metadata.description for metadata in self.content_registry.get_content_by_agent(agent_id)
            if datetime.fromisoformat(metadata.created_at) > datetime.now() - timedelta(hours=24)
        ]
        
        similar_tasks = sum(1 for task in recent_tasks if task_lower in task.lower() or task.lower() in task_lower)
        
        # Check for measurable deliverables
        measurable_patterns = [
            'implement', 'fix', 'optimize', 'reduce', 'improve', 'create', 'build',
            'develop', 'integrate', 'deploy', 'configure', 'setup'
        ]
    ...
    def run_all_quality_gates(self, agent_id: str, file_path: str, content: str, 
                            task_description: str, before_value: Any = None, 
                            after_value: Any = None) -> List[QualityGateResult]:
        ...
        if before_value is not None and after_value is not None:
            results.append(self.validate_measurable_results(agent_id, f"{agent_id}_{int(time.time())}", before_value, after_value))
    ...
    def get_agent_compliance_report(self, agent_id: str) -> Dict[str, Any]:
        ...
        compliance_score = (passed_gates / (passed_gates + failed_gates)) * 100 if (passed_gates + failed_gates) > 0 else 100
    ...
    def get_system_compliance_report(self) -> Dict[str, Any]:
        ...
        total_compliance = sum(report['compliance_score'] for report in agent_reports.values()) / len(agent_reports) if agent_reports else 100
        
        return {
            'system_compliance_score': total_compliance,
            'total_agents': len(agent_reports),
            'compliant_agents': sum(1 for report in agent_reports.values() if report['status'] == 'compliant'),
