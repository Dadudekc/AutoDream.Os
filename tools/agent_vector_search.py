...
def search_similar_messages(query: str, agent_id: str = None, limit: int = 5) -> List[Dict[str, Any]]:
...
def print_experience_results(results: List[Dict[str, Any]], agent_id: str, query: str):
    ...
    for i, result in enumerate(results, 1):
        ...
        experience = result.get("experience", "")[:100] + "..." if len(result.get("experience", "")) > 100 else result.get("experience", "")
...
def main():
    ...
    parser.add_argument("--experience", action="store_true", help="Search agent experiences instead of messages")
    parser.add_argument("--knowledge-summary", action="store_true", help="Get agent knowledge summary")
