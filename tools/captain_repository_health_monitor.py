...
class CaptainRepositoryHealthMonitor:
            ...
            for py_file in python_files:
                try:
                    ...
                    if len(lines) > self.v2_standards["file_size_limit"]:
                        violations.append(
                            f"{py_file.name}: {len(lines)} lines (limit: {self.v2_standards['file_size_limit']})"
                        )
                    else:
                        score += 0.5
    ...
    def generate_health_report(self, health_results: list[RepositoryHealth]) -> str:
        ...
        # Summary statistics
        avg_score = (
            sum(h.overall_score for h in health_results) / len(health_results)
            if health_results
            else 0
        )
        critical_repos = [
            h for h in health_results if h.priority_tier == RepositoryPriority.TIER_1_CRITICAL.value
        ]
        ...
        report.append(
            f"- **Ready for Beta**: {len([h for h in health_results if h.beta_readiness == BetaReadiness.READY_FOR_BETA.value])}"
        )
        report.append("")
        ...
        for priority in RepositoryPriority:
            ...
            if count > 0:
                report.append(
                    f"- **{priority.value.replace('_', ' ').title()}**: {count} repositories"
                )
        report.append("")
