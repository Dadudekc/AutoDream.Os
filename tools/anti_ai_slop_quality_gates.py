                    ...
                    violations.append(
                        {
                            ...
                            "severity": "HIGH"
                            if pattern_name in ["repetitive_files", "captain_logs"]
                            else "MEDIUM",
                        }
                    )
...
class AntiAISlopPrevention:
    ...
    def validate_file_creation(self, proposed_files: list[str]) -> tuple[bool, list[str]]:
        ...
        if len(proposed_files) > self.detector.thresholds["max_files_per_cycle"]:
            violations.append(
                f"Too many files proposed: {len(proposed_files)} > {self.detector.thresholds['max_files_per_cycle']}"
            )
    ...
    def suggest_consolidation(self, files: list[str]) -> list[str]:
        ...
        for base_name, group_files in file_groups.items():
            if len(group_files) > 1:
                suggestions.append(
                    f"Consolidate {len(group_files)} files with base name '{base_name}': {', '.join(group_files)}"
                )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run cleanup (no actual deletion)"
    )
