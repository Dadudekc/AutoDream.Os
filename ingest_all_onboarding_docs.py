...
class CompleteOnboardingIngestor:
    ...
    def _create_agent_definitions(self, coords_data: dict) -> str:
        ...
        for agent_id, agent_data in coords_data.get("agents", {}).items():
            ...
            definitions += (
                f"- **Status**: {'Active' if agent_data.get('active', False) else 'Inactive'}\n"
            )
            definitions += (
                f"- **Chat Coordinates**: {agent_data.get('chat_input_coordinates', 'Not set')}\n"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Ingest all onboarding documentation into vector database"
    )
