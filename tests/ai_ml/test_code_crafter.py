...
class CodeCrafter:
    ...
    def generate_code(self, request: CodeGenerationRequest) -> CodeGenerationResult:
        ...
        try:
            ...
            documentation = self._generate_documentation(request) if request.include_docs else None
    ...
    def _validate_generation_request(self, request: CodeGenerationRequest) -> None:
        ...
        if request.framework and request.framework not in self.supported_frameworks.get(request.language, []):
            raise ValueError(f"Unsupported framework: {request.framework} for language: {request.language}")
