...
class WorkflowService:
    ...
    def create_workflow_task(self, workflow_type: str, agent_id: str, message: str) -> Optional[str]:
        ...
        try:
            ...
            if agent_id not in workflow.required_agents:
                self.logger.error(f"Agent {agent_id} not required for workflow {workflow_type}")
                return None
    ...
    def start_workflow_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            if task.status != "pending":
                self.logger.error(f"Task {task_id} is not pending (current: {task.status})")
                return False
    ...
    def complete_workflow_task(self, task_id: str, error: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if task.status != "running":
                self.logger.error(f"Task {task_id} is not running (current: {task.status})")
                return False
            ...
            self.logger.info(f"Completed workflow task {task_id} with status: {task.status}")
            return True
    ...
    def get_workflow_status(self, workflow_type: str) -> Dict[str, Any]:
        ...
        workflow_tasks = [task for task in self.active_tasks.values() if task.workflow_type == workflow_type]
...
def main():
    ...
    parser.add_argument("--create", type=str, help="Create workflow task (format: workflow_type:agent_id:message)")
    ...
    parser.add_argument("--error", type=str, help="Complete workflow task with error (format: task_id:error_message)")
    parser.add_argument("--status", type=str, help="Show workflow status for specific type")
    parser.add_argument("--agent", type=str, help="Show workflows for specific agent")
    parser.add_argument("--summary", action="store_true", help="Show workflow system summary")
    ...
    if args.create:
        try:
            ...
            task_id = workflow_service.create_workflow_task(workflow_type, agent_id, message)
            if task_id:
        ...
        try:
            ...
            print(f"Task completion with error: {'✅ Success' if success else '❌ Failed'}")
        except ValueError:
        ...
        if workflows:
            ...
            for workflow in workflows:
                print(f"  Task: {workflow['task_id']} - {workflow['workflow_type']} ({workflow['status']})")
        else:
