...
class ScalingMonitor:
    ...
    def _analyze_scaling_patterns(self):
        ...
        try:
            ...
            averages = {
                metric: round(
                    sum(getattr(m, metric) for m in recent_metrics)
                    / len(recent_metrics),
                    2,
                )
            ...
            trends = {
                metric.split("_")[0]: self._calculate_trend(
                    [getattr(m, metric) for m in recent_metrics]
                )
    ...
    def _analyze_decision_patterns(self):
        ...
        try:
            ...
            if len(recent_decisions) >= 2:
                time_span = (
                    recent_decisions[-1].timestamp - recent_decisions[0].timestamp
                )
            ...
            success_rate = (
                (successful_scales / len(recent_decisions)) * 100
                if recent_decisions
                else 0
            )
    ...
    def get_scaling_health_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.performance_alerts:
                critical_alerts = len(
                    [a for a in self.performance_alerts if a["severity"] == "critical"]
                )
                warning_alerts = len(
                    [a for a in self.performance_alerts if a["severity"] == "warning"]
                )
            ...
            if self.scaling_patterns.get("decision_analysis"):
                scaling_freq = self.scaling_patterns["decision_analysis"][
                    "scaling_frequency_per_hour"
                ]
        ...
        try:
            ...
            if current_cpu > 70:
                recommendations.append(
                    "Consider scaling up due to high CPU utilization"
                )
            elif current_cpu < 30:
                recommendations.append(
                    "Consider scaling down due to low CPU utilization"
                )
            ...
            if self.scaling_patterns.get("decision_analysis"):
                ...
                if scaling_freq > self.thresholds["scaling_frequency"]:
                    recommendations.append(
                        "Reduce scaling frequency to prevent thrashing"
                    )
    ...
    def export_metrics(self, output_path: Path) -> bool:
        ...
        try:
            export_data = {
                ...
                "decision_history": [vars(d) for d in self.decision_history],
