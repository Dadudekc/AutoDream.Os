...
from .scaling_types import (
    ScalingStrategy, ScalingStatus, ScalingConfig, 
    ScalingMetrics, ScalingDecision
)
...
class HorizontalScalingEngine:
    ...
    def _evaluate_scaling_needs(self, metrics: ScalingMetrics) -> Optional[ScalingDecision]:
        ...
        should_scale_up = (
            metrics.cpu_utilization > self.config.target_cpu_utilization or
            metrics.memory_utilization > self.config.target_memory_utilization or
            metrics.response_time > 200.0
        )
        
        should_scale_down = (
            metrics.cpu_utilization < self.config.target_cpu_utilization * 0.5 and
            metrics.memory_utilization < self.config.target_memory_utilization * 0.5 and
            self.current_instances > self.config.min_instances
        )
