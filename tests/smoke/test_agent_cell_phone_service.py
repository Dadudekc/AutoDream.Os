...
def test_configuration_loading():
    ...
    try:
        ...
        assert isinstance(service.coordinates, dict), "Coordinates should be a dictionary"
    ...
    try:
        ...
        if service.coordinates and "5-agent" in service.coordinates:
            ...
            if success:
                ...
                print("⚠️ Agent initialization failed (may be expected if config is missing)")
        else:
    ...
    try:
        ...
        if service.coordinates and "5-agent" in service.coordinates:
            ...
            if success:
                ...
                assert len(agent1_status["messages"]) > 0, "Agent-1 should have messages"
                assert len(agent2_status["messages"]) > 0, "Agent-2 should have messages"
                print("✅ Message retrieval successful")
            else:
                print("⚠️ Message sending failed (may be expected if config is missing)")
        else:
    ...
    try:
        ...
        if service.agents:
            for agent_id in service.agents:
                ...
                assert agent_status is not None, f"Agent {agent_id} status should not be None"
...
def test_service_shutdown():
    ...
    try:
        ...
        # Test status change
        assert service.status == "shutdown", f"Expected 'shutdown', got '{service.status}'"
...
def run_all_smoke_tests():
    ...
    if passed == total:
        print("🎉 All smoke tests passed! Agent Cell Phone Service is working correctly.")
        return True
    else:
