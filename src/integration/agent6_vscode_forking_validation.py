...
class Agent6VSCodeForkingValidation:
        ...
        try:
            ...
            if themes and len(themes) > 0:
                ...
                agent_themes = [
                    theme
                    for theme in themes
                    if hasattr(theme, "agent_optimized") and theme.agent_optimized
                ]
        ...
        try:
            ...
            if extensions and len(extensions) > 0:
                ...
                agent_extensions = [
                    ...
                    if hasattr(ext, "agent_specific") and ext.agent_specific
                ]
                ...
                repo_extensions = [
                    ext
                    for ext in extensions
                    if hasattr(ext, "repository_management") and ext.repository_management
                ]
        ...
        try:
            ...
            if layouts and len(layouts) > 0:
                ...
                agent_layouts = [
                    ...
                    if hasattr(layout, "agent_workflow_optimized")
                    and layout.agent_workflow_optimized
                ]
    ...
    def run_comprehensive_validation(self) -> dict[str, Any]:
        ...
        for validation_func in validations:
            ...
            print(
                f"  {result.status.value.upper()}: {result.component.value} ({result.score:.1%} score)"
            )

        # Calculate overall results
        validated_components = len(
            [r for r in self.validation_results if r.status == ValidationStatus.VALIDATED]
        )
        partial_components = len(
            [r for r in self.validation_results if r.status == ValidationStatus.PARTIAL]
        )
        total_components = len(self.validation_results)
        overall_score = (
            sum(r.score for r in self.validation_results) / len(self.validation_results)
...
if __name__ == "__main__":
    ...
    if results["recommendations"]:
        ...
        for rec in results["recommendations"]:
            print(
                f"  [{rec['priority']}] {rec['component']}: {rec['issues'][0] if rec['issues'] else 'No issues'}"
            )
