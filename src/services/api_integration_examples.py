...
class V2APIIntegrationExamples:
    ...
    def __init__(self):
        ...
        self.example_configs = {
            "rest_api": {
                ...
                "endpoints": [
                    {"name": "get_posts", "path": "/posts", "method": "GET", "auth": "none"},
                    {"name": "get_post", "path": "/posts/{id}", "method": "GET", "auth": "none"},
                    {"name": "create_post", "path": "/posts", "method": "POST", "auth": "none"},
                    {"name": "update_post", "path": "/posts/{id}", "method": "PUT", "auth": "none"},
                    {"name": "delete_post", "path": "/posts/{id}", "method": "DELETE", "auth": "none"}
                ]
            },
            "weather_api": {
                "base_url": "https://api.openweathermap.org/data/2.5",
                "endpoints": [
                    {"name": "current_weather", "path": "/weather", "method": "GET", "auth": "api_key"},
                    {"name": "forecast", "path": "/forecast", "method": "GET", "auth": "api_key"}
                ]
            },
            "github_api": {
                "base_url": "https://api.github.com",
                "endpoints": [
                    {"name": "get_user", "path": "/users/{username}", "method": "GET", "auth": "none"},
                    {"name": "get_repos", "path": "/users/{username}/repos", "method": "GET", "auth": "none"},
                    {"name": "create_repo", "path": "/user/repos", "method": "POST", "auth": "bearer"}
                ]
            }
        }
    ...
    def create_rest_api_example(self) -> V2APIIntegrationFramework:
        ...
        self.logger.info(f"REST API example created with {len(config['endpoints'])} endpoints")
        return framework
    ...
    def create_weather_api_example(self) -> V2APIIntegrationFramework:
        ...
        self.logger.info(f"Weather API example created with {len(config['endpoints'])} endpoints")
        return framework
    ...
    def create_github_api_example(self) -> V2APIIntegrationFramework:
        ...
        for endpoint_config in config["endpoints"]:
            method = HTTPMethod(endpoint_config["method"])
            auth_type = AuthType.NONE if "auth" not in endpoint_config else AuthType.BEARER_TOKEN
        ...
        framework.set_auth_credentials("create_repo", {"bearer_token": "demo_token_123"})
        
        self.logger.info(f"GitHub API example created with {len(config['endpoints'])} endpoints")
        return framework
    ...
    def run_weather_api_example(self) -> Dict[str, Any]:
        ...
        try:
            ...
            request = APIRequest(
                endpoint=endpoint,
                query_params={"q": "London", "appid": "demo_key_123", "units": "metric"}
            )
            response = framework.execute_request(request)
    ...
    def run_github_api_example(self) -> Dict[str, Any]:
        ...
        try:
            ...
            request = APIRequest(endpoint=endpoint, query_params={"username": "octocat"})
            response = framework.execute_request(request)
    ...
    def get_example_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "total_endpoints": sum(len(config["endpoints"]) for config in self.example_configs.values()),
...
def main():
    ...
    parser.add_argument("--run-weather", action="store_true", help="Run Weather API example")
    parser.add_argument("--run-github", action="store_true", help="Run GitHub API example")
