...
from .workspace_config import (
    WorkspaceType, WorkspaceStatus, WorkspaceConfig, 
    WorkspaceInfo, WorkspaceConfigManager
)
...
class WorkspaceArchitectureManager:
    ...
    def create_workspace(self, name: str, workspace_type: WorkspaceType, 
                        permissions: List[str] = None, isolation_level: str = "standard") -> bool:
        ...
        try:
            ...
            success = self.structure_manager.create_workspace_structure(workspace_path, workspace_type)
            if not success:
                return False
            ...
            self.logger.info(f"Created workspace: {name} with type {workspace_type.value}")
            return True
    ...
    def update_workspace_status(self, name: str, status: WorkspaceStatus) -> bool:
        ...
        try:
            if name in self.workspaces:
                ...
                self.workspaces[name].last_accessed = self.config_manager.get_current_timestamp()
    ...
    def get_architecture_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "workspace_types": {wt.value: len([w for w in self.workspaces.values() if w.workspace_type == wt]) 
                                  for wt in WorkspaceType},
                "isolation_levels": list(set(c.isolation_level for c in self.configs.values()))
            }
        except Exception as e:
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            ...
            # Test status update
            success = self.update_workspace_status(test_name, WorkspaceStatus.MAINTENANCE)
            if not success:
                return False
...
def run_smoke_test():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test status update
            success = manager.update_workspace_status("test_agent", WorkspaceStatus.MAINTENANCE)
            assert success
...
def main():
    ...
    parser.add_argument("--create", nargs=2, metavar=("NAME", "TYPE"), help="Create workspace")
    parser.add_argument("--list", action="store_true", help="List all workspaces")
    parser.add_argument("--info", help="Get workspace info")
    parser.add_argument("--status", nargs=2, metavar=("NAME", "STATUS"), help="Update workspace status")
