...
async def test_dashboard():
    ...
    try:
        ...
        print(f"🐦 Twitter Integration: {'✅ Enabled' if social_twitter == 'true' else '⚠️ Disabled'}")
        print(f"💬 Discord Integration: {'✅ Enabled' if social_discord == 'true' else '⚠️ Disabled'}")
        print(f"💼 Slack Integration: {'✅ Enabled' if social_slack == 'true' else '⚠️ Disabled'}")
        print(f"📱 Telegram Integration: {'✅ Enabled' if social_telegram == 'true' else '⚠️ Disabled'}")
                ...
                if response.status_code == 200:
                    ...
                    if 'social_media' in data:
                        ...
                        for platform, info in social_media.items():
                            print(f"   - {platform}: {'✅' if info['enabled'] else '⚠️'} ({info['status']})")
                    else:
