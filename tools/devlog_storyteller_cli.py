...
try:
    from src.services.devlog_storytelling import DevlogStorytellingService, create_story_from_devlogs
except ImportError:
    print("❌ Storytelling service not available. Please ensure all dependencies are installed.")
    sys.exit(1)
...
class DevlogStorytellerCLI:
    ...
    def __init__(self):
        """Initialize the CLI."""
        self.service = DevlogStorytellingService()
        self.setup_logging()
    ...
    def create_story(self, batch_size: int = 5, output_format: str = "markdown") -> bool:
        ...
        try:
            ...
            result = create_story_from_devlogs(batch_size)
            
            if result["status"] == "success":
                chapter = result["chapter"]
                print(f"✅ Story created successfully!")
                print(f"📖 Chapter: {chapter['title']}")
                print(f"📊 Devlog Range: {chapter['devlog_range']}")
                print(f"🎯 Conflicts Resolved: {len(chapter['conflicts_resolved'])}")
                print(f"🏆 Achievements Unlocked: {len(chapter['achievements_unlocked'])}")
                return True
    ...
    def auto_process(self, check_interval: int = 300) -> None:
        ...
        try:
            ...
            while True:
                ...
                if result["status"] == "success":
                    print(f"✅ Auto-processed story chapter")
                elif result["status"] == "insufficient_devlogs":
        ...
        try:
            ...
            for chapter_file in sorted(chapter_files):
                try:
                    ...
                    # Basic validation
                    required_fields = ["chapter_number", "title", "story_content", "character_progressions"]
    ...
    def update_characters(self) -> None:
        ...
        try:
            ...
            if character_registry_path.exists():
                with open(character_registry_path, 'r') as f:
    ...
    def update_world(self) -> None:
        ...
        try:
            ...
            if world_rulebook_path.exists():
                with open(world_rulebook_path, 'r') as f:
                    rulebook = json.load(f)
                
                discovery_count = len(rulebook.get("discoveries", []))
                timeline_entries = len(rulebook.get("timeline", []))
                print(f"✅ World rulebook updated with {discovery_count} discoveries, {timeline_entries} timeline entries")
            else:
    ...
    def show_tools(self) -> None:
        ...
        try:
            from src.services.devlog_storytelling import get_storytelling_tools
            tools = get_storytelling_tools()
...
def main():
    ...
    parser.add_argument("--create-story", action="store_true", help="Create story from recent devlogs")
    parser.add_argument("--batch-size", type=int, default=5, help="Number of devlogs per batch (default: 5)")
    parser.add_argument("--auto-process", action="store_true", help="Automatically process devlogs")
    parser.add_argument("--check-interval", type=int, default=300, help="Auto-process check interval in seconds")
    parser.add_argument("--validate-chapters", action="store_true", help="Validate existing story chapters")
    parser.add_argument("--update-characters", action="store_true", help="Update character progression tracking")
    parser.add_argument("--update-world", action="store_true", help="Update world building state")
    parser.add_argument("--show-tools", action="store_true", help="Show available storytelling tools")
