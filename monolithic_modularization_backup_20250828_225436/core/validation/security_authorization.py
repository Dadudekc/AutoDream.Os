...
class SecurityAuthorization(BaseValidator):
    ...
    def validate_authorization(self, auth_data: Dict[str, Any]) -> List[ValidationResult]:
        ...
        try:
            ...
            if "resources" in auth_data:
                resource_results = self._validate_resources(auth_data["resources"])
                ...
                if not isinstance(role_perms, list):
                    ...
                    for perm in role_perms:
                        if not isinstance(perm, str):
                            results.append(self._create_result(
                                rule_id="role_permission_item_type",
                                rule_name="Role Permission Item Type",
                                status=ValidationStatus.FAILED,
                                severity=ValidationSeverity.ERROR,
                                message=f"Role permission must be a string: {perm} in role: {role_name}"
                            ))
            
            # Validate role hierarchy
            if "parent_roles" in role_config:
                parent_roles = role_config["parent_roles"]
                if not isinstance(parent_roles, list):
                    results.append(self._create_result(
                        rule_id="parent_roles_type",
                        rule_name="Parent Roles Type",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Parent roles must be a list for role: {role_name}"
                    ))
                else:
                    for parent_role in parent_roles:
                        if not isinstance(parent_role, str):
                            results.append(self._create_result(
                                rule_id="parent_role_item_type",
                                rule_name="Parent Role Item Type",
                                status=ValidationStatus.FAILED,
                                severity=ValidationSeverity.ERROR,
                                message=f"Parent role must be a string: {parent_role} in role: {role_name}"
                            ))
    ...
    def _validate_permissions(self, permissions: Any) -> List[ValidationResult]:
        ...
        for perm_name, perm_config in permissions.items():
            ...
            if "resources" in perm_config:
                resources = perm_config["resources"]
                if not isinstance(resources, list):
                    results.append(self._create_result(
                        rule_id="permission_resources_type",
                        rule_name="Permission Resources Type",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Permission resources must be a list for permission: {perm_name}"
                    ))
                else:
                    for resource in resources:
                        if not isinstance(resource, str):
                            results.append(self._create_result(
                                rule_id="permission_resource_item_type",
                                rule_name="Permission Resource Item Type",
                                status=ValidationStatus.FAILED,
                                severity=ValidationSeverity.ERROR,
                                message=f"Permission resource must be a string: {resource} in permission: {perm_name}"
                            ))
        ...
        for i, policy in enumerate(policies):
            ...
            if "conditions" in policy:
                ...
                if not isinstance(conditions, dict):
                    results.append(self._create_result(
                        rule_id="policy_conditions_type",
                        rule_name="Policy Conditions Type",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Policy conditions must be a dictionary for policy {i}"
                    ))
            ...
            if "priority" in policy:
                ...
                if not isinstance(priority, int) or priority < 0:
                    results.append(self._create_result(
                        rule_id="policy_priority",
                        rule_name="Policy Priority",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Policy priority must be a non-negative integer for policy {i}"
                    ))
        ...
        for i, entry in enumerate(acl):
            ...
            if "action" in entry:
                ...
                if action not in self.valid_policy_actions:
                    results.append(self._create_result(
                        rule_id="acl_action",
                        rule_name="ACL Action",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Invalid ACL action: {action}. Valid actions: {', '.join(self.valid_policy_actions)}"
                    ))
        
        return results
    
    def _validate_resources(self, resources: Any) -> List[ValidationResult]:
        """Validate resource definitions."""
        results = []
        
        if not isinstance(resources, dict):
            results.append(self._create_result(
                rule_id="resources_type",
                rule_name="Resources Type",
                status=ValidationStatus.FAILED,
                severity=ValidationSeverity.ERROR,
        ...
        for resource_name, resource_config in resources.items():
            if not isinstance(resource_name, str):
                results.append(self._create_result(
                    rule_id="resource_name_type",
                    rule_name="Resource Name Type",
                    status=ValidationStatus.FAILED,
                    severity=ValidationSeverity.ERROR,
                    message=f"Resource name must be a string: {resource_name}"
                ))
                continue
            
            if not isinstance(resource_config, dict):
                results.append(self._create_result(
                    rule_id="resource_config_type",
                    rule_name="Resource Config Type",
                    status=ValidationStatus.FAILED,
                    severity=ValidationSeverity.ERROR,
                    message=f"Resource config must be a dictionary for resource: {resource_name}"
                ))
                continue
            ...
            if "type" in resource_config:
                ...
                if resource_type not in self.valid_resource_types:
                    results.append(self._create_result(
                        ...
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Invalid resource type: {resource_type}. Valid types: {', '.join(self.valid_resource_types)}"
                    ))
            
            # Validate resource permissions
            if "permissions" in resource_config:
                resource_perms = resource_config["permissions"]
                if not isinstance(resource_perms, list):
                    results.append(self._create_result(
                        rule_id="resource_permissions_type",
                        rule_name="Resource Permissions Type",
                        status=ValidationStatus.FAILED,
                        severity=ValidationSeverity.ERROR,
                        message=f"Resource permissions must be a list for resource: {resource_name}"
                    ))
