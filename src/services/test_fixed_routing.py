...
def test_fixed_routing():
    ...
    # Send test message
    msg_id = mq.send_message('agent_5', 'agent_3', 'TEST: Fixed routing - this should go to Agent-3!')
    ...
    if mq.message_history:
        ...
        if (message.get('sender_agent') == 'agent_5' and 
            message.get('target_agent') == 'agent_3' and
            'Fixed routing' in message.get('content', '')):
