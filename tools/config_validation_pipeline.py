...
class ConfigurationValidator:
    ...
    def _validate_yaml_file(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
    ...
    def _validate_json_file(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
    ...
    def _validate_config_structure(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
    ...
    def _validate_unified_config(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
    ...
    def _validate_coordinates_config(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
        ...
        try:
            ...
            if 'agents' not in config:
                ...
                for agent_id, agent_config in agents.items():
                    required_fields = ['chat_input_coordinates', 'onboarding_input_coords']
                    for field in required_fields:
                        if field not in agent_config:
                            errors.append(f"Agent {agent_id} missing field: {field}")
                        elif not isinstance(agent_config[field], list) or len(agent_config[field]) != 2:
                            errors.append(f"Agent {agent_id} {field} must be a list of 2 coordinates")
                            
        except Exception as e:
            errors.append(f"Coordinates config validation error: {str(e)}")
    
    def _validate_messaging_config(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
        ...
        try:
            ...
            # Check for common messaging configuration patterns
            messaging_keys = ['systems', 'providers', 'delivery', 'queue']
            found_keys = [key for key in messaging_keys if key in config]
            
            if not found_keys:
                warnings.append("No standard messaging configuration keys found")
                
        except Exception as e:
            errors.append(f"Messaging config validation error: {str(e)}")
    
    def _validate_discord_config(self, file_path: Path, errors: List[str], warnings: List[str]) -> None:
        ...
        try:
            ...
            # Check for common Discord configuration patterns
    ...
    def validate_against_schema(self, config_file: Path, schema_file: Path) -> Dict[str, Any]:
    ...
    def generate_validation_report(self, output_file: str = "runtime/reports/config_validation_report.json") -> str:
...
def main():
    ...
    parser.add_argument("--output", default="runtime/reports/config_validation_report.json", 
    ...
    if args.validate_file and args.schema:
        ...
        # Validate all configs and generate report
