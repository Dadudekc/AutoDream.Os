    ...
    try:
        ...
        if targets:
            ...
            for i, target in enumerate(targets):
                print(f"   {i+1}. {target.get('title', 'Unknown')} ({target.get('type', 'unknown')})")
            return True
        else:
...
def test_message_sending():
    ...
    try:
        ...
        target = targets[0]
        test_message = "🧪 CDP Test Message - This is a test from the CDP messenger system"
        
        print(f"📤 Sending test message to: {target.get('title', 'Unknown')}")
        
        result = send_to_target(target["webSocketDebuggerUrl"], test_message, "TestAgent")
...
def test_broadcast():
    ...
    try:
        ...
        broadcast_message = "📢 BROADCAST: This is a test broadcast message to all agents"
...
def main():
    ...
    if passed == total:
        ...
        print("   python cdp_send_message.py 'Agent-3: begin integration tests' --target 'Agent-3'")
    else:
