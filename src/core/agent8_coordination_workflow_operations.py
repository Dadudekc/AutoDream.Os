...
from src.core.agent8_coordination_workflow_core import (
    Agent8CoordinationWorkflowCore, CoordinationStatus, TaskPriority, CoordinationTask
)
...
class Agent8CoordinationWorkflowOperations:
    ...
    def manage_operations(self, action: str, **kwargs) -> Any:
        ...
        if action == "bulk_create_tasks":
            return self.bulk_create_tasks(
                kwargs["tasks_data"], kwargs.get("auto_assign", False)
            )
    ...
    def bulk_create_tasks(self, tasks_data: List[Dict[str, Any]], 
    ...
    def validate_workflow(self) -> Dict[str, Any]:
        ...
        for task_id, task in self.workflow.tasks.items():
            ...
            if task.agent_id not in self.workflow.agent_workloads:
                ...
                validation_report["issues"].append(f"Task {task_id} references non-existent agent {task.agent_id}")
            
            # Check dependencies
            for dep_id in task.dependencies:
                if dep_id not in self.workflow.tasks:
                    validation_report["statistics"]["orphaned_dependencies"] += 1
                    validation_report["issues"].append(f"Task {task_id} has orphaned dependency {dep_id}")
        ...
        for agent_id, workload in self.workflow.agent_workloads.items():
            ...
            if workload.performance_score < 0 or workload.performance_score > 2:
                validation_report["statistics"]["invalid_tasks"] += 1
                validation_report["issues"].append(f"Agent {agent_id} has invalid performance score")
    ...
    def analyze_workflow_performance(self, days: int = 30) -> Dict[str, Any]:
        ...
        if completion_times:
            analysis_report["average_completion_time"] = sum(completion_times) / len(completion_times)
        ...
        for agent_id, workload in self.workflow.agent_workloads.items():
            analysis_report["agent_performance"][agent_id] = {
                "completed_tasks": workload.completed_tasks,
                "failed_tasks": workload.failed_tasks,
                "success_rate": workload.completed_tasks / (workload.completed_tasks + workload.failed_tasks) if (workload.completed_tasks + workload.failed_tasks) > 0 else 0,
                "performance_score": workload.performance_score,
                "current_load": workload.active_tasks / workload.max_capacity if workload.max_capacity > 0 else 0
            }
    ...
    def optimize_workflow(self) -> Dict[str, Any]:
                    ...
                    if capacity_ratio > 0.8:
                        ...
                        if best_agent and best_agent != agent_id:
                            try:
                                ...
                                optimization_report["errors"].append(f"Error reassigning task {task_id}: {e}")
        ...
        if optimization_report["reassigned_tasks"] > 0 or optimization_report["balanced_agents"] > 0:
            self.workflow.manage_workflow_operations("save_state")
    ...
    def monitor_task_progress(self) -> Dict[str, Any]:
        ...
        for agent_id, workload in self.workflow.agent_workloads.items():
            utilization = workload.active_tasks / workload.max_capacity if workload.max_capacity > 0 else 0
        ...
        overloaded_agents = [
            agent_id for agent_id, util in monitor_report["agent_utilization"].items()
            if util["utilization"] > 0.9
        ]
        
        if overloaded_agents:
            monitor_report["recommendations"].append(f"Agents {', '.join(overloaded_agents)} are overloaded")
        ...
        if underutilized_agents:
            monitor_report["recommendations"].append(f"Agents {', '.join(underutilized_agents)} are underutilized")
