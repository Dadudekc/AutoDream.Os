...
class DatabaseAuditor:
    ...
    def audit_database_structure(self) -> Dict[str, Any]:
        ...
        audit_results.structure_validation = self._validate_structure(audit_results.file_analysis)
        
        # Check metadata consistency
        audit_results.metadata_consistency = self._check_metadata_consistency(audit_results.file_analysis)
        
        self.logger.info(f"Database structure audit completed. Found {len(audit_results.critical_issues)} critical issues.")
    ...
    def _validate_structure(self, file_analysis: Dict[str, FileAnalysis]) -> Dict[str, Any]:
        ...
        if structure_validation["total_files"] > 0:
            structure_validation["structure_score"] = (
                structure_validation["valid_json_files"] / structure_validation["total_files"]
            )
        
        return structure_validation
    
    def _check_metadata_consistency(self, file_analysis: Dict[str, FileAnalysis]) -> Dict[str, Any]:
        ...
        if existing_files:
            ...
            metadata_consistency["accessibility_consistency"] = all(f.readable for f in existing_files)
            
            # Check if all existing files have valid JSON
            metadata_consistency["json_validity_consistency"] = all(f.valid_json for f in existing_files)
            ...
            metadata_consistency["consistency_score"] = sum(consistency_factors) / len(consistency_factors)
