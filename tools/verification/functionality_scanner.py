...
class FunctionalityScanner:
            ...
            if self._should_include_file(py_file):
                try:
                    with open(py_file, "rb") as f:
                        content = f.read()
                        signatures[str(py_file.relative_to(self.project_root))] = hashlib.md5(
                            content
                        ).hexdigest()
                except Exception:
            ...
            if self._should_include_file(py_file):
                try:
                    with open(py_file, encoding="utf-8") as f:
                        ...
                        if functions:
                            function_signatures[str(py_file.relative_to(self.project_root))] = (
                                functions
                            )
                except Exception:
                    pass
    ...
    def _scan_test_signatures(self) -> dict[str, Any]:
        ...
        test_signatures = {"test_files": [], "test_functions": [], "test_coverage": {}}
