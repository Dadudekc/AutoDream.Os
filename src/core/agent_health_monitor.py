"""
Agent Health Monitoring System for Agent Cellphone V2

This module provides comprehensive agent health monitoring, including:
- Health metrics collection and analysis
- Issue detection and alerting
- Proactive health management
- Health scoring and recommendations
- Integration with core agent management systems
...
class AgentHealthMonitor:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        logger.info("AgentHealthMonitor initialized with default thresholds")
    ...
    def _evaluate_metric(self, agent_id: str, metric: HealthMetric, threshold: HealthThreshold):
    ...
    def _create_alert(self, agent_id: str, severity: AlertSeverity, metric: HealthMetric, threshold: HealthThreshold):
        ...
        for metric_type, metric in snapshot.metrics.items():
            if metric_type in self.thresholds:
                ...
                if metric.value <= threshold.warning_threshold:
                    ...
                    score = max(0.0, 75.0 - ((metric.value - threshold.critical_threshold) / threshold.critical_threshold) * 75.0)
    ...
    def _generate_health_recommendations(self, snapshot: HealthSnapshot) -> List[str]:
        ...
        for metric_type, metric in snapshot.metrics.items():
            if metric_type == HealthMetricType.RESPONSE_TIME and metric.value > 1000:
                recommendations.append("Consider optimizing response time by reviewing processing logic")
    ...
    def record_health_metric(self, agent_id: str, metric_type: HealthMetricType, 
                           value: float, unit: str, threshold: Optional[float] = None):
        ...
        try:
            ...
            logger.debug(f"Health metric recorded: {agent_id} - {metric_type.value}: {value}{unit}")
    ...
    def get_health_alerts(self, severity: Optional[AlertSeverity] = None, 
                         agent_id: Optional[str] = None) -> List[HealthAlert]:
    ...
    def set_health_threshold(self, metric_type: HealthMetricType, 
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        status_counts = {
            status.value: len([s for s in self.health_data.values() if s.overall_status == status])
            for status in HealthStatus
        }
        
        avg_health_score = sum(s.health_score for s in self.health_data.values()) / max(total_agents, 1)
    ...
    def run_smoke_test(self) -> bool:
        ...
        try:
            logger.info("Running AgentHealthMonitor smoke test...")
            ...
            self.record_health_metric("test_agent", HealthMetricType.RESPONSE_TIME, 500.0, "ms")
            ...
            test_config = {'health_check_interval': 1, 'alert_check_interval': 1}
            ...
            self.record_health_metric("test_agent", HealthMetricType.RESPONSE_TIME, 6000.0, "ms")
            ...
            logger.info("✅ AgentHealthMonitor smoke test PASSED")
            return True
            
        except Exception as e:
            logger.error(f"❌ AgentHealthMonitor smoke test FAILED: {e}")
            import traceback
    ...
    def shutdown(self):
        ...
        self.stop()
        logger.info("AgentHealthMonitor shutdown complete")
...
def main():
    ...
    if args.test:
        ...
        print("🚀 Starting Agent Health Monitor Demo...")
        monitor = AgentHealthMonitor()
        ...
        monitor.subscribe_to_health_updates(health_callback)
        ...
        try:
            ...
            monitor.record_health_metric("agent_001", HealthMetricType.RESPONSE_TIME, 1500.0, "ms")
            monitor.record_health_metric("agent_001", HealthMetricType.MEMORY_USAGE, 85.0, "%")
            monitor.record_health_metric("agent_002", HealthMetricType.CPU_USAGE, 90.0, "%")
            ...
            print("\n💡 Health Recommendations:")
            for agent_id in monitor.health_data:
                health = monitor.get_agent_health(agent_id)
                if health.recommendations:
                    print(f"  {agent_id}: {', '.join(health.recommendations)}")
