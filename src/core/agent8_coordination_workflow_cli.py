...
from src.core.agent8_coordination_workflow_core import (
    Agent8CoordinationWorkflowCore, TaskPriority, CoordinationStatus
)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    complete_parser.add_argument('--success', action='store_true', default=True,
                               help='Mark as successful')
    complete_parser.add_argument('--failure', action='store_true',
                               help='Mark as failed')
    
    # List tasks command
    list_parser = subparsers.add_parser('list', help='List tasks')
    list_parser.add_argument('--status', choices=[s.value for s in CoordinationStatus],
...
def handle_performance(args, workflow: Agent8CoordinationWorkflowCore) -> None:
    ...
    performance = workflow.manage_workflow_operations(
        "get_performance", 
        agent_id=args.agent
    )
    ...
    if args.agent:
        ...
        for agent_id, perf in performance.items():
            ...
            print(f"     Success rate: {perf['success_rate']:.2%}, Score: {perf['performance_score']:.2f}")
        print()
