...
class FSMCoreV2:
    ...
    def _handle_state_completion(
        self, workflow_id: str, state_name: str, result: Optional[StateExecutionResult]
    ) -> None:
        ...
        try:
            ...
            available_transitions = self.get_available_transitions(
                state_name, self._build_context(workflow)
            )
    ...
    def _handle_state_failure(
        self, workflow_id: str, state_name: str, result: StateExecutionResult
    ) -> None:
        ...
        try:
            ...
            if workflow.retry_count < state_def.retry_count:
                ...
                threading.Timer(
                    ...
                    lambda: self._execute_state(workflow_id, state_name),
                ).start()
            else:
