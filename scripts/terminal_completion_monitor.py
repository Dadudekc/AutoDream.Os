...
class TerminalCompletionMonitor:
    ...
    def __init__(
        ...
        verbose: bool = False,
        task_repo: CursorTaskRepository | None = None,
    ) -> None:
                ...
                for line in fh:
                    if COMPLETION_SIGNAL in line:
                        ...
                        event = CompletionEvent(
                            time.time(), str(source), line.strip(), task_id, task_found
                        )
                        self._emit(event)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Monitor logs for completion signals")
    parser.add_argument(
        "-s",
        "--source",
        action="append",
    ...
    parser.add_argument("-o", "--output", help="Path to output file for structured events.")
    parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose output.")
    parser.add_argument(
        "-i", "--interval", type=float, default=1.0, help="Polling interval in seconds."
    )
