...
class OnboardingOrchestrator:
    ...
    def onboard_agent(
        ...
        try:
            ...
            message = self.message_builder.create_comprehensive_message(
                agent_name, style
            )
            ...
            success = self.cli_executor.run_simple_command(
                cmd_args, f"Onboarding {agent_name} with {style} style"
            )
    ...
    def onboard_all_agents(
        self, style: str = "full", use_emojis: bool = True
    ) -> Dict[str, bool]:
        """
    ...
    def onboard_agent_list(
        self, agent_names: List[str], style: str = "full", use_emojis: bool = True
    ) -> Dict[str, bool]:
        """
    ...
    def print_onboarding_summary(
        self, results: Dict[str, bool], use_emojis: bool = True
    ):
        ...
        for agent_name in all_agents:
            ...
            if status_file.exists():
                try:
                    ...
                    agent_status["current_status"] = status_data.get(
                        "status", "unknown"
                    )
                    agent_status["last_updated"] = status_data.get(
...
def main():
    ...
    parser.add_argument(
        "--no-emojis", action="store_true", help="Disable emoji formatting"
    )
